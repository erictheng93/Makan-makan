 ...
 */
router.post("/:id/connect-cf", async (c) => {
  const tenantService = new TenantService(c.env);
  const tenantId = c.req.param("id");
  ...
  try {
    ...
    const result = await tenantService.connectCloudflareAccount(
      tenantId,
      apiToken,
      accountId,
    );
    ...
    if (!result.success) {
      return c.json(
        {
          success: false,
          error: result.error || "Failed to connect Cloudflare account",
          code: "CF_CONNECTION_FAILED",
        },
        400,
      );
    }
    ...
    console.error("[Tenants] Connect CF error:", error);
    return c.json(
      {
        success: false,
        error: "Failed to connect Cloudflare account",
        code: "CF_CONNECTION_FAILED",
      },
      500,
    );
