...
describe.skip("Offline Sync Integration Tests", () => {
  ...
  describe("Action Queuing and Caching", () => {
    it("should queue actions when offline", () => {
      ...
      const actionId = offlineService.queueAction("start_cooking", 1, {
        itemId: 1,
      });
    ...
    it("should handle batch operations offline", () => {
      ...
      expect(
        updatedOrder?.items.every((item) => item.status === "preparing"),
      ).toBe(true);
    });
  });
  ...
  describe("Offline Status Component Integration", () => {
    it("should display offline status correctly", async () => {
      ...
      expect(wrapper.find('[data-testid="offline-indicator"]').exists()).toBe(
        true,
      );
    ...
    it("should display conflict resolution UI", async () => {
      ...
      expect(wrapper.find('[data-testid="conflicts-modal"]').exists()).toBe(
        true,
      );
    });
  });
