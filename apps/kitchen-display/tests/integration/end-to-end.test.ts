...
const mockOrders: KitchenOrder[] = [
  ...
  },
  {
    ...
    estimatedTime: 5,
    priority: "normal",
    customer: { name: "張三", phone: "0965432187" },
    specialInstructions: "",
    assignedChef: "chef2",
  },
];
...
describe.skip("End-to-End Integration Tests", () => {
  ...
  describe("Workflow Automation Integration", () => {
    ...
    it("should handle smart scheduling and batching", async () => {
      const orders = [
        {
          ...mockOrders[0],
          items: [{ name: "炒飯", category: "rice", cookingTime: 8 }],
        },
        {
          ...mockOrders[1],
          items: [{ name: "炒麵", category: "noodles", cookingTime: 10 }],
        },
        {
          ...mockOrders[2],
          items: [{ name: "炒飯", category: "rice", cookingTime: 7 }],
        },
      ];
      ...
      const sortedByTime = riceOrders.sort(
        (a, b) => a.items[0].cookingTime - b.items[0].cookingTime,
      );
      expect(sortedByTime[0].items[0].cookingTime).toBeLessThan(
        sortedByTime[1].items[0].cookingTime,
      );
    });
  });
  ...
  describe("Multi-System Integration", () => {
    ...
    it("should handle keyboard shortcuts with full system integration", async () => {
      ...
      const shortcutMetric = metrics.find(
        (m) => m.name === "shortcut_execution_time",
      );
  ...
  describe("Error Recovery and Resilience", () => {
    it("should gracefully handle service failures", async () => {
      ...
      vi.spyOn(audioService, "play").mockRejectedValueOnce(
  ...
  describe("Data Consistency and Integrity", () => {
    it("should maintain data consistency across all systems", async () => {
      ...
      const metrics = performanceService.getMetrics();
      const statusChangeMetric = metrics.find((m) =>
        m.name.includes("status_change"),
      );
    ...
    it("should handle concurrent updates correctly", async () => {
      ...
      const queuedActions = offlineService.pendingActions.value.filter(
