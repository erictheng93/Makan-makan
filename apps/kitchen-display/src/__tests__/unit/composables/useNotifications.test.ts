...
function useNotifications() {
  ...
  const add = (notification: Omit<Notification, 'id' | 'timestamp' | 'read'>) => {
    ...
    const insertIndex = notifications.value.findIndex(
      (n) => priorityOrder[n.priority] > priorityOrder[newNotification.priority]
    );
  ...
  const updateSoundSettings = (type: Notification['type'], settings: Partial<NotificationSound>) => {
...
describe('useNotifications Composable', () => {
  ...
  describe('添加通知', () => {
    ...
    it('應該自動設置時間戳', () => {
      ...
      expect(composable.notifications.value[0].timestamp).toBeGreaterThanOrEqual(beforeTime);
      expect(composable.notifications.value[0].timestamp).toBeLessThanOrEqual(afterTime);
    });
  ...
  describe('刪除通知', () => {
    ...
    it('應該刪除單個通知', () => {
      ...
      expect(composable.notifications.value).toHaveLength(1);
      expect(composable.notifications.value.find((n) => n.id === notifId)).toBeUndefined();
    });
  ...
  describe('音效設置', () => {
    it('應該更新音效設置', () => {
      ...
      expect(composable.soundSettings.value.order.volume).not.toBe(originalVolume);
    });
  ...
  describe('邊界情況', () => {
    it('應該處理大量通知', () => {
      ...
      expect(composable.notifications.value).toHaveLength(1000);
      expect(composable.unreadCount.value).toBe(1000);
    });
