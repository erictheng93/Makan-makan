...
export const authApi = {
  ...
  async refreshToken(): Promise<ApiResponse<LoginResponse>> {
    try {
      ...
      const message = error.response?.data?.message || error.message || 'Token 刷新失敗'
      return {
        success: false,
        error: message,
        timestamp: new Date().toISOString()
  ...
  async validateToken(): Promise<ApiResponse<User>> {
    try {
      ...
      const message = error.response?.data?.message || error.message || 'Token 驗證失敗'
      return {
        success: false,
        error: message,
        timestamp: new Date().toISOString()
  ...
  async getCurrentUser(): Promise<ApiResponse<User>> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '獲取用戶資訊失敗'
      return {
        success: false,
        error: message,
        timestamp: new Date().toISOString()
  ...
  // 檢查廚師權限
  async checkKitchenAccess(restaurantId: number): Promise<ApiResponse<boolean>> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '權限檢查失敗'
      return {
        success: false,
        error: message,
        timestamp: new Date().toISOString()
