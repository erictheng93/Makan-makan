...
class KitchenStatisticsService {
  ...
  private computeOrderStatistics(orders: KitchenOrder[]): OrderStatistics {
    ...
    const avgCookingTime =
      cookingTimes.length > 0
        ? cookingTimes.reduce((sum, time) => sum + time, 0) /
          cookingTimes.length
        : 0;
  ...
  private computePerformanceMetrics(
    ...
    // Orders per hour
    const timeSpanHours =
      (this.timeRange.value.end.getTime() -
        this.timeRange.value.start.getTime()) /
      (1000 * 60 * 60);
  ...
  private computeChefPerformance(orders: KitchenOrder[]): ChefPerformance[] {
    ...
    return Array.from(chefStats.entries())
      .map(([chefId, stats]) => {
        const avgCookTime =
        ...
        const efficiency = this.calculateChefEfficiency(
          avgCookTime,
  ...
  private computeItemAnalytics(orders: KitchenOrder[]): ItemAnalytics[] {
    ...
    return Array.from(itemStats.entries())
      .map(([itemName, stats]) => ({
        ...
        successRate:
          Math.round((stats.successfulOrders / stats.count) * 10000) / 100,
  ...
  private computeCustomerSatisfaction(
    orders: KitchenOrder[],
  ...
  private computeRealTimeStats() {
    ...
    const avgWaitingTime =
      waitingOrders.length > 0
        ? waitingOrders.reduce(
            (sum: number, o: KitchenOrder) => sum + (o.elapsedTime || 0),
            0,
  ...
  private calculateEfficiency(
    ...
    const avgCompletionTime =
      completed.length > 0
        ? completed.reduce((sum, o) => sum + (o.elapsedTime || 0), 0) /
          completed.length
        : 0;
    ...
    return (
      Math.round((completionRate * 0.7 + timeEfficiency * 0.3) * 100) / 100
    );
  }

  private generateOrderTrends(
    orders: KitchenOrder[],
  ): Array<{ time: string; orders: number; completed: number }> {
    const trends = [];
    const now = new Date();
    ...
    for (let i = 23; i >= 0; i--) {
      ...
      const hourOrders = orders.filter((order) => {
        const orderTime = new Date(order.createdAt).getTime();
        return (
          orderTime >= hourStart.getTime() && orderTime < hourEnd.getTime()
        );
      });
  ...
  private analyzeItemCookingTimes(
    orders: KitchenOrder[],
  ): Record<string, { totalTime: number; count: number }> {
  ...
  private calculateChefEfficiency(
    avgCookTime: number,
    ordersCompleted: number,
  ): number {
    ...
    const timeEfficiency = Math.max(
      0,
      100 - ((avgCookTime - baselineTime) / baselineTime) * 100,
    );
  ...
  private generateSatisfactionTrends(): Array<{
    date: string;
    rating: number;
  }> {
    const trends = [];
