...
class FeatureToggleService {
  ...
  public isFeatureEnabled(featureKey: string): boolean {
    ...
    if (feature.restrictions) {
      ...
      if (
        feature.restrictions.environments &&
      ...
      if (
        feature.restrictions.userRoles &&
  ...
  public getFeatureUsageStats(): {
    ...
    return {
      ...
      featuresByCategory: features.reduce(
        (acc, feature) => {
  ...
  private setupWatchers(): void {
    ...
    watch(
      [this.features, this.configurations, this.profiles, this.activeProfile],
      () => {
  ...
  private saveToStorage(): void {
    try {
      localStorage.setItem(
        this.STORAGE_KEY,
        JSON.stringify(this.features.value),
      );
      localStorage.setItem(
        this.CONFIG_KEY,
        JSON.stringify(this.configurations.value),
      );
      localStorage.setItem(
        this.PROFILES_KEY,
        JSON.stringify({
