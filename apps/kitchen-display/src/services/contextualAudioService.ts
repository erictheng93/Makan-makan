...
const CONTEXT_PROFILES: Record<KitchenContext, ContextProfile> = {
  ...
  'preparation': {
    name: '準備時段',
    timeRanges: [{ start: 7, end: 10 }, { start: 14, end: 17 }],
    characteristics: {
      baseVolume: 0.6,
...
class ContextualAudioService {
    ...
    for (const [contextKey, profile] of Object.entries(CONTEXT_PROFILES)) {
      for (const timeRange of profile.timeRanges) {
        if (this.isTimeInRange(hour, timeRange)) {
          // Adjust for weekends
          if (isWeekend && (contextKey === 'lunch-rush' || contextKey === 'dinner-rush')) {
            // Weekends are generally less busy
            return contextKey === 'lunch-rush' ? 'preparation' : 'afternoon-lull'
          }
  ...
  private isTimeInRange(hour: number, range: { start: number; end: number }): boolean {
  ...
  async sendContextualNotification(
    ...
    const adaptations = this.generateAdaptations(type, urgency, context, options?.customAdaptations)
  ...
  private calculateUrgency(
    ...
    if (orderData) {
      // Wait time factor
      if (orderData.waitTime && orderData.waitTime > profile.triggers.waitTimeThreshold) {
        urgencyScore += Math.floor((orderData.waitTime - profile.triggers.waitTimeThreshold) / 5)
      }
    ...
    if (this.environmentalState.orderBacklog > profile.triggers.backlogThreshold) {
      urgencyScore += 1
    }
    
    if (this.environmentalState.recentOrderRate > profile.triggers.orderThreshold) {
      urgencyScore += 1
    }
  ...
  private generateAdaptations(
    ...
    volume *= urgencyMultipliers[urgency] * profile.characteristics.urgencyMultiplier
    
    // Adaptive learning adjustments
    const learningAdjustment = this.adaptiveLearning.volumeAdjustments.get(context) || 1.0
    ...
    switch (profile.characteristics.repeatBehavior) {
      case 'reduced':
        ...
        break
      case 'enhanced':
        repeat = urgency === 'critical' ? 4 : urgency === 'high' ? 3 : urgency === 'medium' ? 2 : 1
        break
      default:
    ...
    const useAlternateSound = profile.characteristics.soundVariation && 
  ...
  private shouldUseAlternateSound(type: SoundType, context: KitchenContext): boolean {
  ...
  private async executeNotification(notification: ContextualNotification): Promise<void> {
  ...
  private mapUrgencyToPriority(urgency: UrgencyLevel): 'low' | 'medium' | 'high' | 'urgent' {
    const mapping = {
      'low': 'low' as const,
  ...
  private recordNotification(notification: ContextualNotification): void {
    ...
    if (this.notificationHistory.value.length > 100) {
      this.notificationHistory.value = this.notificationHistory.value.slice(0, 100)
    }
  ...
  private trackNotificationEffectiveness(notification: ContextualNotification): void {
  ...
  updateEnvironmentalState(updates: Partial<typeof this.environmentalState>): void {
  ...
  async notifyUrgentAlert(message: string, orderId?: string): Promise<void> {
    await this.sendContextualNotification('urgent-alert', {
      id: orderId || 'system',
      priority: 'urgent',
      waitTime: 0
    }, {
  ...
  getContextAnalytics() {
    ...
    return {
      ...
      effectivenessMetrics: Object.fromEntries(this.adaptiveLearning.effectiveness),
      environmentalState: { ...this.environmentalState }
    }
  }

  private getContextDistribution(contexts: { context: KitchenContext; timestamp: number }[]) {
