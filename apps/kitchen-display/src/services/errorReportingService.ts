...
class ErrorReportingService {
  ...
  private errorCategories: Record<
    string,
    { severity: ErrorReport["severity"]; tags: string[] }
  > = {
  ...
  public getErrorStats(): ErrorStats {
    ...
    const errorsByType = reports.reduce(
      (acc, report) => {
  ...
  private processError(report: ErrorReport): void {
    ...
    if (this.errorReports.value.length > this.MAX_STORED_ERRORS) {
      this.errorReports.value = this.errorReports.value.slice(
        0,
  ...
  private saveErrors(): void {
    try {
      localStorage.setItem(
        ...
        JSON.stringify(this.errorReports.value),
      );
    } catch (error) {
