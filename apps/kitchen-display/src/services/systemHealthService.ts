...
class SystemHealthService {
  ...
  public addAlert(alertData: Omit<SystemAlert, 'id' | 'timestamp' | 'resolved'>): void {
    ...
    if (this.systemAlerts.value.length > this.MAX_STORED_ALERTS) {
      this.systemAlerts.value = this.systemAlerts.value.slice(0, this.MAX_STORED_ALERTS)
    }
  ...
  public getSystemStats(): {
    ...
    const resolvedAlerts = this.systemAlerts.value.filter(a => a.resolved).length
    const criticalAlerts = this.systemAlerts.value.filter(
      a => a.severity === 'critical' && !a.resolved
    ).length
    
    const responseTimeMetric = this.healthMetrics.value.find(m => m.id === 'response_time')
  ...
  private async collectSystemMetrics(): Promise<void> {
    ...
    newMetrics.push({
      ...
      unit: 'errors/hour',
      status: errorStats.errorRate > 10 ? 'critical' : errorStats.errorRate > 5 ? 'warning' : 'healthy',
    ...
    if ('storage' in navigator && 'estimate' in navigator.storage) {
      try {
        ...
        const storageUsage = estimate.usage && estimate.quota 
    ...
    newMetrics.push({
      ...
      unit: 'actions',
      status: offlineQueueSize > 50 ? 'critical' : offlineQueueSize > 20 ? 'warning' : 'healthy',
    ...
    if (this.healthMetrics.value.length > this.MAX_STORED_METRICS) {
      this.healthMetrics.value = this.healthMetrics.value.slice(0, this.MAX_STORED_METRICS)
    }
  }
        ...
        switch (component.id) {
          ...
          case 'offline-service':
            status = offlineService.isOnline.value 
              ? offlineService.pendingActions.value.length > 50 ? 'degraded' : 'online'
              : 'offline'
  ...
  private evaluateSystemAlerts(): void {
    ...
    // Memory alerts
    const memoryMetric = this.healthMetrics.value.find(m => m.id === 'memory_usage')
    if (memoryMetric && memoryMetric.status === 'critical') {
      this.addAlert({
        ...
        component: 'system',
        actions: ['Clear cache', 'Restart application', 'Check for memory leaks']
      })
    }

    // Response time alerts
    const responseMetric = this.healthMetrics.value.find(m => m.id === 'response_time')
    if (responseMetric && responseMetric.status === 'critical') {
      this.addAlert({
        ...
        component: 'network',
        actions: ['Check network connection', 'Optimize API calls', 'Enable caching']
      })
    }

    // Error rate alerts
    const errorMetric = this.healthMetrics.value.find(m => m.id === 'error_rate')
    if (errorMetric && errorMetric.status === 'critical') {
      this.addAlert({
        ...
        component: 'system',
        actions: ['Check error logs', 'Review recent changes', 'Monitor system stability']
      })
    }

    // Storage alerts
    const storageMetric = this.healthMetrics.value.find(m => m.id === 'storage_usage')
    if (storageMetric && storageMetric.status === 'critical') {
      this.addAlert({
        ...
        component: 'storage',
        actions: ['Clear old data', 'Optimize data storage', 'Increase storage quota']
      })
    }

    // Offline queue alerts
    const offlineMetric = this.healthMetrics.value.find(m => m.id === 'offline_queue')
    if (offlineMetric && offlineMetric.status === 'critical') {
      this.addAlert({
        ...
        actions: ['Check network connection', 'Force sync', 'Clear old actions']
      })
    }

    // Component health alerts
    const offlineComponents = this.systemComponents.value.filter(c => c.status === 'offline')
    if (offlineComponents.length > 0) {
      this.addAlert({
        ...
        component: 'system',
        actions: ['Check system status', 'Restart affected services', 'Review error logs']
      })
    }
  }
  ...
  private calculateOverallHealthScore(): number {
    const componentScores = this.systemComponents.value.map(c => c.healthScore)
  ...
  private updateComponentStatus(componentId: string, status: SystemComponent['status']): void {
    const component = this.systemComponents.value.find(c => c.id === componentId)
    if (component) {
      component.status = status
      component.lastCheck = Date.now()
      component.healthScore = status === 'online' ? 100 : status === 'degraded' ? 50 : 0
    }
  }
