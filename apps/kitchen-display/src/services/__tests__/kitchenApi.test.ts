...
describe("kitchenApi", () => {
  ...
  describe("getConnectionStatus", () => {
    it("should return connection data on success", async () => {
      const connectionData = { connected: true, clients: 3 };
      mockApi.get.mockResolvedValue({
        data: {
          data: connectionData,
          timestamp: "2026-01-01T00:00:00Z",
      ...
      const result = await kitchenApi.getConnectionStatus(7);

      expect(mockApi.get).toHaveBeenCalledWith("/kitchen/7/connections");
      expect(result).toEqual({
        success: true,
        data: connectionData,
        timestamp: "2026-01-01T00:00:00Z",
    ...
    it("should return error with error.message when no response data", async () => {
      mockApi.get.mockRejectedValue({ message: "Connection refused" });
    ...
    it("should return default Chinese message when no error details", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe("獲取連接狀態失敗");
    });
  });
  ...
  describe("broadcastTest", () => {
    it("should send POST request and return response data on success", async () => {
      const responseData = { sent: true, recipients: 5 };
      mockApi.post.mockResolvedValue({ data: responseData });

      const request = { type: "test", payload: { msg: "hello" } };
      const result = await kitchenApi.broadcastTest(3, request);

      expect(mockApi.post).toHaveBeenCalledWith(
        "/kitchen/3/broadcast-test",
        request,
      );
      expect(result.success).toBe(true);
      // broadcastTest uses response.data directly (not response.data.data)
    ...
    it("should handle empty request body", async () => {
      ...
      const result = await kitchenApi.broadcastTest(3, {});
    ...
    it("should return default Chinese message when no error details", async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe("廣播測試失敗");
    });
  });
