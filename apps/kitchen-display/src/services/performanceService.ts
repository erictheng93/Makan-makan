...
class PerformanceService {
  ...
  public recordMetric(
    ...
    if (this.metrics.value.length > this.config.value.maxMetrics) {
      this.metrics.value = this.metrics.value.slice(
        -this.config.value.maxMetrics,
      );
    }
  ...
  public endMeasure(
    name: string,
    _category: PerformanceMetric["category"] = "user",
  ) {
  ...
  private checkThresholds() {
    Object.entries(this.config.value.thresholds).forEach(
  ...
  private createAlert(
    ...
    const existingAlert = this.alerts.value.find(
      (a) => a.type === type && a.metric === metric && !a.resolved,
    );
    ...
    if (this.alerts.value.length > this.config.value.alerts.maxAlerts) {
      this.alerts.value = this.alerts.value.slice(
        -this.config.value.alerts.maxAlerts,
      );
    }
  ...
  public getMetricTrend(
    ...
    const relevantMetrics = this.metrics.value.filter(
      (m) => m.name === metricName && m.timestamp >= cutoff,
    );

    const bucketData = Array(buckets)
      .fill(null)
      .map((_, i) => ({
  ...
  public getErrorRate(timeWindow = 3600000): number {
    const cutoff = Date.now() - timeWindow;
    const recentMetrics = this.metrics.value.filter(
  ...
  public generateReport(): {
    ...
    return {
      ...
      alerts: {
        ...
        byType: this.alerts.value.reduce(
          (acc, alert) => {
  ...
  private loadStoredData() {
    try {
      ...
      if (data) {
        ...
        if (parsed.config)
          this.config.value = { ...this.config.value, ...parsed.config };
  ...
  private cleanupOldData() {
    ...
    this.metrics.value = this.metrics.value.filter(
      (m) => m.timestamp >= cutoff,
    );
