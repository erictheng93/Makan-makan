...
class EnhancedAudioNotificationService {
  ...
  private async loadAllSounds(): Promise<void> {
    const soundTypes: SoundType[] = [
      'new-order', 'order-ready', 'urgent-alert', 'order-complete',
      'warning', 'success', 'error', 'notification', 'bell', 'chime',
      'tick', 'whoosh'
    ]
  ...
  async playSound(
    ...
    try {
      if (this.settings.useWebAudio && this.audioContext && this.audioBuffers.has(type)) {
  ...
  async playNewOrderAlert(orderData?: { priority?: string; tableNumber?: string }): Promise<void> {
  ...
  private removeFromCurrentlyPlaying(type: SoundType): void {
    const index = this.currentlyPlaying.value.findIndex(event => event.type === type)
    if (index !== -1) {
