...
class AudioAccessibilityService {
  ...
  private culturalSoundPacks = reactive<Map<string, Record<SoundType, string>>>(new Map())
  ...
  async playAccessibleNotification(
    ...
    if (this.settings.showSubtitles && (options?.subtitle || message)) {
      this.addSubtitle(options?.subtitle || message || this.getDefaultMessage(soundType), soundType)
    }
  ...
  private async playAccessibleAudio(
    ...
    if (this.settings.personalizedSounds && this.personalSounds.has(soundType)) {
      ...
      const culturalPack = this.culturalSoundPacks.get(this.settings.languagePreference)
  ...
  private showVisualAlert(
    ...
    setTimeout(() => {
      const index = this.activeVisualAlerts.value.findIndex(a => a.id === alert.id)
      if (index !== -1) {
  ...
  private processVoiceCommand(transcript: string): void {
    ...
    const commands = {
      ...
      // Vietnamese  
      'hoàn thành đơn hàng': () => this.triggerKeyboardShortcut('quick_complete'),
      'đơn hàng tiếp theo': () => this.triggerKeyboardShortcut('toggle_order_status'),
  ...
  private getDefaultMessage(soundType: SoundType): string {
    ...
    const lang = this.settings.languagePreference.startsWith('zh') ? 'zh-TW' : 'en'
  ...
  private getSubtitleType(soundType: SoundType): 'notification' | 'system' | 'order' {
    const orderSounds: SoundType[] = ['new-order', 'order-ready', 'order-complete']
  ...
  private saveSettings(): void {
    localStorage.setItem('kitchen-accessibility-settings', JSON.stringify(this.settings))
  }
  ...
  removeVisualAlert(alertId: string): void {
    const index = this.activeVisualAlerts.value.findIndex(a => a.id === alertId)
    if (index !== -1) {
      this.activeVisualAlerts.value.splice(index, 1)
    }
  }
