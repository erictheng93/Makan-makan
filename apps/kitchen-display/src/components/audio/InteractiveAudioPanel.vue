<template>
  ...
  <div class="bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="flex-1">
            <input
              ...
              @input="setMasterVolume(($event.target as HTMLInputElement).value)"
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-3">音頻技術</label>
        <div class="flex items-center space-x-6">
          <label class="flex items-center">
            ...
            <span v-if="audioService.isWebAudioSupported" class="ml-1 text-green-500">✓</span>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-3">進階功能</label>
        <div class="space-y-3">
      ...
      <!-- Audio Effects -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-3">音效處理</label>
        <div class="space-y-3">
          <label class="flex items-center justify-between">
      ...
      <!-- Sound Testing -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-4">音效測試</label>
      ...
      <!-- Quick Actions -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-4">快速操作</label>
      ...
      <div v-if="audioService.soundHistory.value.length > 0">
        <label class="block text-sm font-medium text-gray-700 mb-3">最近播放</label>
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        ...
        <div class="space-y-2 text-xs">
          <div class="flex justify-between">
            <span>音頻系統:</span>
            <span :class="audioService.isInitialized ? 'text-green-600' : 'text-red-600'">
              {{ audioService.isInitialized ? '已初始化' : '未初始化' }}
            </span>
          </div>
          <div class="flex justify-between">
            <span>Web Audio 支援:</span>
            <span :class="audioService.isWebAudioSupported ? 'text-green-600' : 'text-red-600'">
              {{ audioService.isWebAudioSupported ? '支援' : '不支援' }}
            </span>
          </div>
          <div class="flex justify-between">
            <span>正在播放:</span>
            <span class="text-blue-600">{{ audioService.currentlyPlaying.value.length }} 個音效</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-3">
        ...
        <div class="text-xs text-blue-800 space-y-1">
          <div><kbd class="px-1 py-0.5 bg-white rounded text-xs">Ctrl+Shift+N</kbd> 測試新訂單</div>
          <div><kbd class="px-1 py-0.5 bg-white rounded text-xs">Ctrl+Shift+U</kbd> 測試緊急警報</div>
          <div><kbd class="px-1 py-0.5 bg-white rounded text-xs">Ctrl+Shift+R</kbd> 測試訂單完成</div>
        </div>
      </div>
    </div>
  </div>
...
const availableSounds = computed(() => [
  ...
  { type: 'urgent-alert' as SoundType, name: '緊急警報', icon: ExclamationTriangleIcon },
  ...
  { type: 'notification' as SoundType, name: '通知', icon: InformationCircleIcon },
...
const toggleEffect = (effect: 'reverb' | 'echo' | 'bass') => {
  toast.info(`${effect} 效果已${audioService.settings.effects[effect] ? '啟用' : '停用'}`)
}
