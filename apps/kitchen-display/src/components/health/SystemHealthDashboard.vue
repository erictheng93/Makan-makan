<template>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center space-x-3">
          ...
          <div class="flex items-center">
            <div :class="[
              ...
              systemHealth.isMonitoring ? 'bg-green-500 animate-pulse' : 'bg-red-500'
            ]"></div>
          ...
          >
            <CpuChipIcon :class="['w-4 h-4 mr-2', { 'animate-spin': runningDiagnostics }]" />
          ...
          <div class="flex items-center">
            ...
            <div class="ml-6 flex-1">
              ...
              <div class="mt-2 flex items-center">
                <div class="flex items-center">
                  <span class="text-3xl font-bold text-gray-900">{{ healthReport.overall.score }}</span>
                  <span class="text-lg text-gray-500 ml-1">/100</span>
                </div>
                <div class="ml-6">
                  <div class="text-sm text-gray-500 mb-1">運行時間</div>
                  <div class="text-lg font-semibold text-gray-900">{{ formatUptime(healthReport.uptime) }}</div>
                </div>
                <div class="ml-6">
                  <div class="text-sm text-gray-500 mb-1">最後更新</div>
                  <div class="text-lg font-semibold text-gray-900">{{ formatLastUpdate(healthReport.overall.lastUpdate) }}</div>
                </div>
              </div>
                ...
                <div class="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    :class="[
                      ...
                      healthReport.overall.score >= 80 ? 'bg-green-500' :
                      healthReport.overall.score >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                    ]"
                ...
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                      {{ alert.component }} • {{ formatRelativeTime(alert.timestamp) }}
                    </p>
                  </div>
                  <button
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
          ...
          >
              ...
              <div class="w-full bg-gray-200 rounded-full h-1.5">
                <div 
                  :class="[
                    ...
                    component.healthScore >= 80 ? 'bg-green-500' :
                    component.healthScore >= 50 ? 'bg-yellow-500' : 'bg-red-500'
                  ]"
            ...
            <div class="text-xs text-gray-500 space-y-1">
              ...
              <div>
                最後檢查: {{ formatRelativeTime(component.lastCheck) }}
              </div>
        ...
        <div class="p-6">
          <div class="grid grid-cols-2 gap-4">
            <div class="text-center">
              <div class="text-2xl font-bold text-gray-900">{{ systemStats.totalAlerts }}</div>
              <div class="text-sm text-gray-500">總警報</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-red-600">{{ systemStats.criticalAlerts }}</div>
              <div class="text-sm text-gray-500">嚴重警報</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-blue-600">{{ systemStats.avgResponseTime }}ms</div>
              <div class="text-sm text-gray-500">平均響應</div>
            </div>
            <div class="text-center">
              <div class="text-2xl font-bold text-yellow-600">{{ systemStats.errorRate }}</div>
    ...
    <!-- Diagnostics Modal -->
    <div v-if="showDiagnostics" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        ...
        <div class="space-y-3">
          ...
          >
            <span class="text-sm font-medium text-gray-700">{{ getDiagnosticName(test) }}</span>
            <div class="flex items-center">
              <div :class="[
                'w-3 h-3 rounded-full mr-2',
                result ? 'bg-green-500' : 'bg-red-500'
              ]"></div>
              <span class="text-sm text-gray-600">{{ result ? '正常' : '失敗' }}</span>
            </div>
          </div>
        </div>
      </div>
...
import type { SystemHealthReport, SystemAlert } from '@/services/systemHealthService'
