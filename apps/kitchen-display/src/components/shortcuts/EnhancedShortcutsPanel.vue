      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          <div class="flex items-center space-x-2 text-white">
            ...
            <span class="text-sm">{{ shortcuts.enabled ? '已啟用' : '已停用' }}</span>
          </div>
    ...
    <div class="p-6 space-y-6">
            ...
            <div class="flex items-center space-x-3">
              ...
              <div class="flex-1">
                <div class="flex items-center space-x-2">
                  <h4 class="text-sm font-medium text-gray-900">{{ shortcut.name }}</h4>
                  <span v-if="shortcut.global" class="px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full">
                    全域
                  </span>
                  <span v-if="shortcut.customizable" class="px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded-full">
                    可自訂
                  </span>
                </div>
                <p class="text-sm text-gray-600">{{ shortcut.description }}</p>
              </div>
            </div>
      ...
      <div v-if="shortcuts.stats.value.mostUsed?.length > 0">
        ...
        <div class="space-y-2">
          ...
          >
            <span class="text-gray-600">{{ getShortcutName(usage.shortcutId) }}</span>
            <div class="flex items-center space-x-2">
      ...
      <div class="border-t border-gray-200 pt-4">
        ...
        <div v-if="shortcuts.recordingMode" class="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
          <div class="flex items-center">
            <div class="w-3 h-3 bg-red-500 rounded-full animate-pulse mr-2"></div>
            <span class="text-red-800 text-sm font-medium">正在錄製快捷鍵組合...</span>
          </div>
...
const testShortcut = async (shortcut: KeyboardShortcut) => {
  try {
    const success = await shortcuts.executeShortcut(shortcut, new KeyboardEvent('keydown'))
    toast[success ? 'success' : 'error'](`快捷鍵測試${success ? '成功' : '失敗'}: ${shortcut.name}`)
  } catch (error) {
...
const updateShortcut = (shortcut: KeyboardShortcut) => {
  ...
  toast.success(`快捷鍵 ${shortcut.name} 已${shortcut.enabled ? '啟用' : '停用'}`)
}
...
kbd {
  ...
  font-weight: 600;
  font-size: 0.75rem;
  box-shadow: inset 0 -2px 0 0 #e5e7eb, inset 0 0 1px 1px #fff, 0 1px 2px 1px rgba(0, 0, 0, 0.1);
}
