    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <div class="bg-white shadow-lg rounded-lg p-6">
        ...
        <div v-if="isDevelopment && errorDetails" class="mb-6">
          ...
          <div class="bg-gray-100 rounded p-3 text-sm font-mono text-gray-800 overflow-auto max-h-40">
            {{ errorDetails }}
          </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-gray-200">
          <label class="flex items-center">
            ...
            />
            <span class="ml-2 text-sm text-gray-600">啟用安全模式（停用進階功能）</span>
          </label>
        </div>
      </div>
    </div>
...
function isCriticalError(err: Error): boolean {
  ...
  return criticalErrors.some(type => 
    err.name === type || err.message.includes(type)
  )
}
