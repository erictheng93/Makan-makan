<template>
  ...
  >
    ...
    >
        ...
        <div class="bg-gray-50 rounded-xl p-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <div class="font-medium">{{ order.tableName }}</div>
            </div>
            <div>
              <div class="text-sm text-gray-500">下單時間</div>
              <div class="font-medium">{{ formatDateTime(order.createdAt) }}</div>
            </div>
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  ...
                  <div class="space-y-2 text-sm">
                    ...
                    <div v-if="item.customizations && item.customizations.length" class="text-blue-600">
                      <span class="font-medium">客製:</span> {{ item.customizations.join(', ') }}
                    </div>
        ...
        <div v-if="order.notes">
          ...
          <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
            <div class="flex items-start space-x-2">
              <ChatBubbleLeftEllipsisIcon class="w-5 h-5 text-yellow-600 mt-0.5" />
          ...
          <div class="space-y-3">
            ...
            <div v-if="order.confirmedAt" class="flex items-center space-x-3">
              ...
              <div class="flex-1">
                ...
                <div class="text-sm text-gray-500">{{ formatDateTime(order.confirmedAt) }}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Modal Footer -->
      <div class="sticky bottom-0 bg-white rounded-b-2xl border-t border-gray-200 p-6">
...
const hasUncompletedItems = computed(() => {
  return props.order.items.some(item => item.status !== 'ready' && item.status !== 'completed')
})
