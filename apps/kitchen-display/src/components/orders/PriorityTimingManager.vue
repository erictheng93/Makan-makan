<template>
  <div class="priority-timing-manager bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
    <!-- Header -->
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
      <div class="bg-red-50 rounded-lg p-4 border border-red-100">
        ...
        <div class="text-2xl font-bold text-red-600 mt-1">{{ urgentCount }}</div>
      ...
      <div class="bg-orange-50 rounded-lg p-4 border border-orange-100">
        ...
        <div class="text-2xl font-bold text-orange-600 mt-1">{{ highPriorityCount }}</div>
        <div class="text-xs text-orange-600">{{ warningThreshold }}-{{ urgentThreshold }} 分鐘</div>
      </div>
      ...
      <div class="bg-green-50 rounded-lg p-4 border border-green-100">
        ...
        <div class="text-2xl font-bold text-green-600 mt-1">{{ normalCount }}</div>
        <div class="text-xs text-green-600">少於 {{ warningThreshold }} 分鐘</div>
      </div>
      ...
      <div class="bg-blue-50 rounded-lg p-4 border border-blue-100">
        ...
        <div class="text-2xl font-bold text-blue-600 mt-1">{{ averageWaitTime }}</div>
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          <div class="text-2xl font-bold text-yellow-600">{{ estimatedCompletionTimes.pending }}</div>
          <div class="text-sm text-yellow-600">待處理訂單</div>
          <div class="text-xs text-gray-600">平均完成時間</div>
        </div>
        
        <div class="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
          <div class="text-2xl font-bold text-blue-600">{{ estimatedCompletionTimes.preparing }}</div>
          <div class="text-sm text-blue-600">製作中訂單</div>
          <div class="text-xs text-gray-600">預計剩餘時間</div>
        </div>
        
        <div class="text-center p-4 bg-green-50 rounded-lg border border-green-200">
          <div class="text-2xl font-bold text-green-600">{{ workloadCapacity }}%</div>
...
const averageWaitTime = computed(() => {
  ...
  const totalTime = props.orders.reduce((sum, order) => sum + order.elapsedTime, 0)
  return Math.round(totalTime / props.orders.length)
})

const overdueOrders = computed(() =>
  props.orders.filter(order => order.elapsedTime >= urgentThreshold.value)
...
const estimatedCompletionTimes = computed(() => {
  ...
  const pendingAvg = pendingOrders.length > 0 
    ? Math.round(pendingOrders.reduce((sum, order) => sum + (order.estimatedTime || 15), 0) / pendingOrders.length)
    : 0
    
  const preparingAvg = preparingOrders.length > 0
    ? Math.round(preparingOrders.reduce((sum, order) => sum + Math.max(0, (order.estimatedTime || 15) - order.elapsedTime), 0) / preparingOrders.length)
    : 0
...
const saveThresholds = () => {
  ...
  emit('thresholds-updated', warningThresholdTemp.value, urgentThresholdTemp.value)
...
const generateTimingReport = () => {
  ...
  const reportData = props.orders.map(order => ({
    ...
    優先級: order.priority === 'urgent' ? '緊急' : order.priority === 'high' ? '重要' : '普通',
    預估時間: `${order.estimatedTime || '未設定'}分`,
    狀態: order.status === 1 ? '已確認' : order.status === 2 ? '製作中' : '準備完成',
    延遲狀況: order.elapsedTime >= urgentThreshold.value ? '嚴重延遲' : 
             order.elapsedTime >= warningThreshold.value ? '輕微延遲' : '正常'
  }))
  
  const csvContent = Object.keys(reportData[0] || {}).join(',') + '\n' +
