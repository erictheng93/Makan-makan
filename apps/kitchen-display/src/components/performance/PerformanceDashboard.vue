<template>
  <div class="performance-dashboard bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6">
    <!-- Header -->
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-gray-50 rounded-lg p-4">
          ...
          <div class="h-32 bg-white rounded border flex items-end justify-between p-2">
            <div
          ...
          <div class="space-y-3">
            <div>
              <div class="flex justify-between text-sm mb-1">
                ...
                <span class="font-medium">{{ formatMetricValue(memoryUsage.avg, '%') }}</span>
              </div>
            ...
            <div v-if="systemInfo.connection">
              <div class="flex justify-between text-sm mb-1">
                ...
                <span class="font-medium">{{ systemInfo.connection.rtt }}ms</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-2">
                <div 
                  :class="[
                    'h-2 rounded-full transition-all duration-300',
                    systemInfo.connection.rtt < 100 ? 'bg-green-500' :
                    systemInfo.connection.rtt < 300 ? 'bg-yellow-500' : 'bg-red-500'
                  ]"
                  :style="{ width: `${Math.min(100, (systemInfo.connection.rtt / 500) * 100)}%` }"
                ></div>
              </div>
            </div>
            ...
            <div>
              <div class="flex justify-between text-sm mb-1">
                ...
                <span class="font-medium">{{ formatMetricValue(frameRate.avg, 'fps') }}</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-2">
                <div 
                  :class="[
                    'h-2 rounded-full transition-all duration-300',
                    frameRate.avg >= 50 ? 'bg-green-500' :
                    frameRate.avg >= 30 ? 'bg-yellow-500' : 'bg-red-500'
                  ]"
                  :style="{ width: `${Math.min(100, (frameRate.avg / 60) * 100)}%` }"
                ></div>
              </div>
            </div>
          </div>
    ...
    <div class="mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div class="bg-gray-50 rounded-lg p-3">
          ...
          <div class="font-medium text-gray-900">{{ getBrowserInfo() }}</div>
        </div>
        
        <div class="bg-gray-50 rounded-lg p-3">
          <div class="text-sm text-gray-600">作業系統</div>
          <div class="font-medium text-gray-900">{{ systemInfo.platform }}</div>
        </div>
        
        <div class="bg-gray-50 rounded-lg p-3">
          <div class="text-sm text-gray-600">CPU 核心數</div>
          <div class="font-medium text-gray-900">{{ systemInfo.hardwareConcurrency }}</div>
        </div>
        ...
        <div v-if="systemInfo.connection" class="bg-gray-50 rounded-lg p-3">
          ...
          <div class="font-medium text-gray-900">{{ systemInfo.connection.effectiveType.toUpperCase() }}</div>
        </div>
        
        <div class="bg-gray-50 rounded-lg p-3">
          <div class="text-sm text-gray-600">運行時間</div>
          <div class="font-medium text-gray-900">{{ formatUptime(uptime) }}</div>
        </div>
      </div>
    </div>
          ...
          <div class="mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div class="text-center p-3 bg-blue-50 rounded-lg">
                <div class="text-xl font-bold text-blue-600">{{ performanceReport.summary.totalMetrics }}</div>
                <div class="text-sm text-blue-600">收集指標</div>
              </div>
              <div class="text-center p-3 bg-red-50 rounded-lg">
                <div class="text-xl font-bold text-red-600">{{ performanceReport.summary.activeAlerts }}</div>
                <div class="text-sm text-red-600">活動警告</div>
              </div>
              <div class="text-center p-3 bg-green-50 rounded-lg">
                <div class="text-xl font-bold text-green-600">{{ formatUptime(performanceReport.summary.uptime) }}</div>
                <div class="text-sm text-green-600">系統運行時間</div>
              </div>
              <div class="text-center p-3 bg-orange-50 rounded-lg">
                <div class="text-xl font-bold text-orange-600">{{ (performanceReport.summary.errorRate * 100).toFixed(2) }}%</div>
...
// Metric summaries
const pageLoadTime = computed(() => performanceService.getMetricSummary('page-load-time'))
const apiResponseTime = computed(() => performanceService.getMetricSummary('api-response-time'))
const memoryUsage = computed(() => performanceService.getMetricSummary('memory-usage'))
const frameRate = computed(() => performanceService.getMetricSummary('frame-rate'))
