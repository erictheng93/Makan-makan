...
export function useRealtimeKitchen() {
  ...
  const ordersArray = computed(() =>
    Array.from(kitchenOrders.value.values()).sort(
      (a, b) =>
  ...
  const playNewOrderSound = () => {
    if (audioEnabled.value) {
      ...
      audio
        .play()
        .catch((e) => console.log("Could not play kitchen sound:", e));
    }
  };
  ...
  const toggleAudio = () => {
    ...
    localStorage.setItem(
      "kitchen-audio-enabled",
      audioEnabled.value.toString(),
    );
  };
