...
class KitchenOfflineStorageManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("orderStatusUpdates")) {
          const statusStore = db.createObjectStore("orderStatusUpdates", {
            keyPath: "id",
          });
          statusStore.createIndex("order_id", "order_id", { unique: false });
          statusStore.createIndex("restaurant_id", "restaurant_id", {
            unique: false,
          });
        ...
        if (!db.objectStoreNames.contains("kitchenTimers")) {
          const timersStore = db.createObjectStore("kitchenTimers", {
            keyPath: "id",
          });
          timersStore.createIndex("order_id", "order_id", { unique: false });
          timersStore.createIndex("menu_item_id", "menu_item_id", {
            unique: false,
          });
        ...
        if (!db.objectStoreNames.contains("cachedOrders")) {
          const ordersStore = db.createObjectStore("cachedOrders", {
            keyPath: "id",
          });
        ...
        if (!db.objectStoreNames.contains("kitchenMetrics")) {
          const metricsStore = db.createObjectStore("kitchenMetrics", {
            keyPath: "id",
          });
          metricsStore.createIndex("restaurant_id", "restaurant_id", {
            unique: false,
          });
        ...
        if (!db.objectStoreNames.contains("offlineKitchenActions")) {
          const actionsStore = db.createObjectStore("offlineKitchenActions", {
            keyPath: "id",
          });
  ...
  async cleanupExpiredCache(expirationHours: number = 8): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      const request = timersStore.openCursor(timersRange);
      request.onsuccess = (event) => {
        const cursor = (event.target as IDBRequest).result;
        if (cursor) {
