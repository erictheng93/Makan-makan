...
class KitchenPushNotificationService {
  ...
  async subscribe(): Promise<NotificationSubscription | null> {
    ...
    try {
      ...
      console.error('Failed to subscribe to kitchen push notifications:', error)
      return null
    }
  }
  ...
  async sendSubscriptionToServer(subscription: NotificationSubscription): Promise<void> {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to register kitchen push subscription on server')
      }
  ...
  async showLocalNotification(options: KitchenPushNotificationOptions): Promise<void> {
    if (!this.isSupported || Notification.permission !== 'granted') {
      return
    }
    ...
    try {
      ...
      const notificationOptions: NotificationOptions = {
        ...
        requireInteraction: options.requireInteraction || (options.urgency === 'urgent'),
  ...
  async notifyNewOrder(orderId: string, tableNumber: string, itemCount: number, priority: 'normal' | 'high' | 'urgent' = 'normal'): Promise<void> {
    const urgencyEmoji = priority === 'urgent' ? '🚨' : priority === 'high' ? '⚡' : '🍽️'
  ...
  async notifyTimerAlert(orderId: string, itemName: string, timerName: string, isOverdue: boolean = false): Promise<void> {
  ...
  async notifyOrderModification(orderId: string, modificationType: 'updated' | 'cancelled' | 'rushed', details: string): Promise<void> {
    const typeEmojis = {
      'updated': '📝',
      'cancelled': '❌',
      'rushed': '🏃‍♂️'
    ...
    await this.showLocalNotification({
      ...
      urgency: modificationType === 'rushed' ? 'urgent' : modificationType === 'cancelled' ? 'high' : 'normal',
  ...
  async notifyKitchenAlert(alertType: 'equipment_failure' | 'temperature_warning' | 'inventory_low' | 'staff_needed', message: string): Promise<void> {
    const alertEmojis = {
      'equipment_failure': '⚠️',
  ...
  async notifyOrderReady(orderId: string, tableNumber: string, waitTime: number): Promise<void> {
    await this.showLocalNotification({
      title: '✅ Order Ready',
  ...
  async notifyShiftUpdate(updateType: 'shift_start' | 'shift_end' | 'break_time' | 'staff_change', details: string): Promise<void> {
    const updateEmojis = {
      'shift_start': '🌅',
      'shift_end': '🌙',
      'break_time': '☕',
  ...
  // Utility methods
  private getVibrationPattern(urgency: 'low' | 'normal' | 'high' | 'urgent'): number[] {
    switch (urgency) {
      case 'urgent':
  ...
  async testNotification(urgency: 'low' | 'normal' | 'high' | 'urgent' = 'normal'): Promise<void> {
    await this.showLocalNotification({
...
kitchenPushService.initialize().catch(error => {
  console.error('Failed to initialize kitchen push notification service:', error)
})
