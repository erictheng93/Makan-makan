...
export const useOrdersStore = defineStore('orders', () => {
  ...
  const fetchOrders = async (restaurantId: number) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log(`Loaded ${allOrders.length} orders for restaurant ${restaurantId}`)
      } else {
  ...
  const handleNewOrder = (event: KitchenSSEEvent) => {
    if (event.payload && event.payload.order) {
      ...
      // 檢查訂單是否已存在
      const existingOrderIndex = orders.value.findIndex(o => o.id === newOrder.id)
      ...
      if (orderIndex !== -1) {
        ...
        if (itemId) {
          ...
          if (itemIndex !== -1) {
            ...
            console.log(`Order ${orderId} item ${itemId} status updated to ${status}`)
          }
        }
  ...
  const updateOrderStatus = (order: KitchenOrder) => {
    ...
    if (itemStatuses.every(status => status === 'ready' || status === 'completed')) {
  ...
  const calculateAverageWaitingTime = (): number => {
    ...
    const totalWaitingTime = waitingOrders.reduce((sum, order) => 
      sum + order.elapsedTime, 0
    )
   ...
   * 開始製作訂單項目
   */
  const startCooking = async (restaurantId: number, orderId: number, itemId: number) => {
    try {
      const response = await kitchenApi.startCooking(restaurantId, orderId, itemId)
      
      if (response.success) {
        // 樂觀更新本地狀態
        const orderIndex = orders.value.findIndex(o => o.id === orderId)
        if (orderIndex !== -1) {
          const itemIndex = orders.value[orderIndex].items.findIndex(i => i.id === itemId)
          if (itemIndex !== -1) {
            orders.value[orderIndex].items[itemIndex].status = 'preparing'
            orders.value[orderIndex].items[itemIndex].startedAt = new Date().toISOString()
   ...
   * 標記項目完成
   */
  const markReady = async (restaurantId: number, orderId: number, itemId: number) => {
    try {
      const response = await kitchenApi.markItemReady(restaurantId, orderId, itemId)
      ...
      if (response.success) {
        ...
        if (orderIndex !== -1) {
          ...
          if (itemIndex !== -1) {
            ...
            orders.value[orderIndex].items[itemIndex].completedAt = new Date().toISOString()
            updateOrderStatus(orders.value[orderIndex])
            updateStats()
          }
        }
  ...
  const startAllItems = async (restaurantId: number, orderId: number) => {
    ...
    try {
      const response = await kitchenApi.startAllItems(restaurantId, orderId, pendingItemIds)
  ...
  const markAllReady = async (restaurantId: number, orderId: number) => {
    ...
    try {
      const response = await kitchenApi.markAllItemsReady(restaurantId, orderId, preparingItemIds)
