          ...
          <div class="flex bg-white border border-gray-300 rounded-lg">
            <button
              :class="[
                ...
                viewMode === 'dragdrop' ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-50'
              ]"
            ...
            <button
              :class="[
                ...
                viewMode === 'traditional' ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-50'
              ]"
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3 max-h-96 overflow-y-auto">
            ...
            <div v-if="filteredPendingOrders.length === 0" class="text-center py-8 text-gray-500">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3 max-h-96 overflow-y-auto">
            ...
            <div v-if="filteredPreparingOrders.length === 0" class="text-center py-8 text-gray-500">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3 max-h-96 overflow-y-auto">
            ...
            <div v-if="filteredReadyOrders.length === 0" class="text-center py-8 text-gray-500">
...
const handleOrderStatusChanged = async (orderId: number, newStatus: 'pending' | 'preparing' | 'ready') => {
  try {
...
const handleBatchPriorityUpdate = async (orderIds: number[], priority: string) => {
  try {
...
const handleBatchExport = async (orderIds: number[]) => {
  try {
    const selectedOrders = orders.value.filter(order => orderIds.includes(order.id))
    
    // Create CSV content
    const csvHeaders = ['訂單號', '桌號', '顧客姓名', '狀態', '優先級', '項目', '備註', '建立時間']
    const csvRows = selectedOrders.flatMap(order => 
      order.items.map(item => [
        order.orderNumber,
        order.tableName,
        order.customerName || '',
        order.status === 1 ? '已確認' : order.status === 2 ? '製作中' : '準備完成',
        order.priority === 'urgent' ? '緊急' : order.priority === 'high' ? '重要' : '普通',
