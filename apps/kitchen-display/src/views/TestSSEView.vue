  ...
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="flex space-x-4">
        ...
        <button class="btn-kitchen-secondary px-4 py-2" @click="reconnect">
          重新連接
        </button>
      </div>
    </div>
...
const {
  ...
  onOrderUpdate: (event) =>
    handleSSEEvent("ORDER_UPDATE", "訂單狀態更新", event),
  onOrderCancelled: (event) =>
    handleSSEEvent("ORDER_CANCELLED", "訂單已取消", event),
  onPriorityUpdate: (event) =>
    handleSSEEvent("PRIORITY_UPDATE", "優先級已更新", event),
...
const sendTestEvent = async (eventType: string) => {
  try {
    ...
    const response = await kitchenApi.broadcastTest(
      TEST_RESTAURANT_ID,
      payload,
    );
