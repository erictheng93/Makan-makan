...
describe("Token Refresh", () => {
  ...
  describe("Token Refresh Logic", () => {
    ...
    it("should update iat (issued at) timestamp", () => {
      ...
      delay.then(() => {
        const newToken = refreshToken(oldToken);
        expect(newToken.payload.iat).toBeGreaterThanOrEqual(
          oldToken.payload.iat,
        );
      });
    });
  });
  ...
  describe("Token Expiry Scenarios", () => {
    ...
    it("should handle rapid token refresh requests", () => {
      ...
      // Each token should have same or later expiry than previous
      // (may be equal if refreshed within the same millisecond)
      for (let i = 1; i < tokens.length; i++) {
        expect(tokens[i].expiresAt).toBeGreaterThanOrEqual(
          tokens[i - 1].expiresAt,
        );
      }
    });
    ...
    it("should handle token with maximum allowed lifetime", () => {
      ...
      expect(getTokenRemainingTime(maxLifetimeToken)).toBeGreaterThan(
        86000 * 1000,
      );
    });
  });
