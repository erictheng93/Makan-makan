...
describe("Role Validation", () => {
  ...
  describe("Auth Payload Validation", () => {
    ...
    it("should validate admin auth payload", () => {
      ...
      expect(hasAdminPrivileges(UserRole.Admin)).toBe(true);
      expect(canAccessRoomType(UserRole.Admin, authPayload.roomType)).toBe(
        true,
      );
    });
    ...
    it("should detect role mismatch with room type", () => {
      ...
      expect(
        canAccessRoomType(UserRole.Customer, invalidPayload.roomType),
      ).toBe(false);
    });
  });
  ...
  describe("Role Validation in WebSocket Context", () => {
    it("should validate role before WebSocket upgrade", () => {
      ...
      expect(canAccessRoomType(UserRole.Customer, authPayload.roomType)).toBe(
        true,
      );
    });
    ...
    it("should reject role-roomType mismatch", () => {
      ...
      const canAccess = canAccessRoomType(
        UserRole.Customer,
        authPayload.roomType,
      );
  ...
  describe("Role Hierarchy", () => {
    ...
    it("should check if role meets minimum privilege level", () => {
      ...
      expect(meetsMinimumPrivilege(UserRole.Customer, UserRole.Chef)).toBe(
        false,
      );
    });
  });
