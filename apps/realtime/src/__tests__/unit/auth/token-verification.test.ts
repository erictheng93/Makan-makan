...
describe("JWT Token Verification", () => {
  ...
  describe("verifyWebSocketToken()", () => {
    ...
    describe("失敗場景 - Token 格式錯誤", () => {
      ...
      it("應該拒絕錯誤簽名的 token", async () => {
        ...
        const token = sign(
          { roomType: "customer", roomId: "table-1" },
          wrongSecret,
        );
  ...
  describe("extractTokenFromUrl()", () => {
    it("應該從 URL 查詢參數中提取 token", () => {
      const url = new URL(
        "ws://localhost:8787/customer/table-123?token=my-test-token",
      );
    ...
    it("應該處理包含多個查詢參數的 URL", () => {
      const url = new URL(
        "ws://localhost:8787/customer/table-123?foo=bar&token=my-token&baz=qux",
      );
    ...
    it("應該處理包含特殊字符的 token", () => {
      ...
      const url = new URL(
        `ws://localhost:8787/customer/table-123?token=${encodeURIComponent(specialToken)}`,
      );
  ...
  describe("整合測試", () => {
    it("應該完整驗證從 URL 提取的 token", async () => {
      ...
      const url = new URL(
        `ws://localhost:8787/customer/table-123?token=${validToken}`,
      );
    ...
    it("應該拒絕從 URL 提取的無效 token", async () => {
      ...
      const url = new URL(
        `ws://localhost:8787/customer/table-123?token=${invalidToken}`,
      );
