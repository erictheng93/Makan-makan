...
describe('Room Management', () => {
  ...
  describe('Connection Management', () => {
    ...
    it('should handle multiple connections in same room', async () => {
      ...
      const roomConnections = Array.from(connections.values())
        .filter(conn => conn.roomId === 'table-001')
    ...
    it('should track connection metadata', async () => {
      ...
      expect(connection.metadata?.features).toHaveLength(3)
    })
  })
  ...
  describe('Room Access Control', () => {
    ...
    it('should enforce restaurant-level isolation', () => {
      ...
      expect(connection1.auth?.restaurantId).not.toBe(connection2.auth?.restaurantId)
    })
  })
  ...
  describe('Multi-Room Support', () => {
    it('should support multiple rooms simultaneously', () => {
      ...
      const connectionsByRoom = Array.from(connections.values())
        .reduce((acc, conn) => {
    ...
    it('should isolate messages between different rooms', () => {
      ...
      const connectionsInTargetRoom = targetRoom === 'table-001'
        ? room1Connections
        : room2Connections
