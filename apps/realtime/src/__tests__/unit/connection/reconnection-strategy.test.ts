...
function syncMissedEvents(
  ...
  const lastIndex = availableEvents.findIndex(e => e.eventId === state.lastEventId)
  const newEvents = lastIndex >= 0 ? availableEvents.slice(lastIndex + 1) : availableEvents
  ...
  return {
    ...
    state: {
      ...state,
      eventsSinceDisconnect: [],
      lastEventId: newEvents[newEvents.length - 1]?.eventId || state.lastEventId
    }
  }
}
