...
function shouldCleanupConnection(
  ...
  return isConnectionIdle(connection, config) || isHeartbeatOverdue(connection, config)
}
...
describe('Timeout Detection', () => {
  ...
  describe('Timeout Calculation', () => {
    it('should calculate time until idle timeout', () => {
      ...
      const expected = DEFAULT_TIMEOUT_CONFIG.idleTimeout - 60000
  ...
  describe('Grace Period Handling', () => {
    it('should allow reconnection within grace period', () => {
      ...
      const isWithinGracePeriod = timeSinceDisconnect < DEFAULT_TIMEOUT_CONFIG.reconnectGracePeriod
    ...
    it('should maintain connection state during grace period', () => {
      ...
      vi.advanceTimersByTime(DEFAULT_TIMEOUT_CONFIG.reconnectGracePeriod - 1000)
  ...
  describe('Multiple Connections Cleanup', () => {
    it('should identify which connections need cleanup', () => {
      ...
      // Add idle connection
      connections.set('conn-2', createMockConnection({
        id: 'conn-2',
        lastActivity: Date.now() - 400000 // 6.67 minutes ago
      }))

      // Add connection with stale heartbeat
      connections.set('conn-3', createMockConnection({
        id: 'conn-3',
        lastHeartbeat: Date.now() - 70000 // 70 seconds ago
      }))
    ...
    it('should cleanup connections in batch', () => {
      ...
      for (let i = 0; i < 10; i++) {
        const isIdle = i % 2 === 0
        connections.set(`conn-${i}`, createMockConnection({
          id: `conn-${i}`,
