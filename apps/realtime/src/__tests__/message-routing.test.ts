...
import type { RealtimeEvent, RealtimeAuthPayload } from '@makanmakan/shared-types'
...
describe('Message Routing Logic', () => {
  describe('餐廳 ID 隔離', () => {
    it('應該只發送給相同餐廳 ID 的連線', () => {
      ...
      expect(shouldSendEventToConnection(event, differentRestaurantConnection)).toBe(false)
    })
  })
  ...
  describe('NEW_ORDER 事件路由', () => {
    ...
    it('應該發送給所有角色', () => {
      ...
      expect(shouldSendEventToConnection(newOrderEvent, staffConnection)).toBe(true)
  ...
  describe('ORDER_STATUS_UPDATE 事件路由', () => {
    ...
    it('應該發送給顧客', () => {
      ...
      expect(shouldSendEventToConnection(statusUpdateEvent, customerConnection)).toBe(true)
    })
    ...
    it('應該發送給廚房員工', () => {
      ...
      expect(shouldSendEventToConnection(statusUpdateEvent, staffConnection)).toBe(true)
    })
    ...
    it('應該發送給管理員', () => {
      ...
      expect(shouldSendEventToConnection(statusUpdateEvent, adminConnection)).toBe(true)
    })
  })
  ...
  describe('KITCHEN_ITEM_STATUS 事件路由', () => {
    ...
    it('應該發送給廚房員工', () => {
      ...
      expect(shouldSendEventToConnection(kitchenEvent, staffConnection)).toBe(true)
    })
    ...
    it('應該發送給管理員', () => {
      ...
      expect(shouldSendEventToConnection(kitchenEvent, adminConnection)).toBe(true)
    })
    ...
    it('不應該發送給顧客', () => {
      ...
      expect(shouldSendEventToConnection(kitchenEvent, customerConnection)).toBe(false)
    })
  })
  ...
  describe('MENU_AVAILABILITY_UPDATE 事件路由', () => {
    ...
    it('應該發送給所有角色', () => {
      ...
      expect(shouldSendEventToConnection(menuEvent, customerConnection)).toBe(true)
      expect(shouldSendEventToConnection(menuEvent, staffConnection)).toBe(true)
      expect(shouldSendEventToConnection(menuEvent, adminConnection)).toBe(true)
    })
  })
  ...
  describe('系統事件路由', () => {
    ...
    it('應該發送給所有角色', () => {
      ...
      expect(shouldSendEventToConnection(systemEvent, customerConnection)).toBe(true)
      expect(shouldSendEventToConnection(systemEvent, staffConnection)).toBe(true)
      expect(shouldSendEventToConnection(systemEvent, adminConnection)).toBe(true)
    })
  })
  ...
  describe('內部事件過濾', () => {
    it('不應該通過 broadcast 發送 CONNECTION_ACK', () => {
      ...
      expect(shouldSendEventToConnection(connectionAckEvent, connection)).toBe(false)
    })
    ...
    it('不應該通過 broadcast 發送 HEARTBEAT', () => {
      ...
      expect(shouldSendEventToConnection(heartbeatEvent, connection)).toBe(false)
    })
  })
  ...
  describe('未知事件類型', () => {
    it('未知事件應該只發送給管理員', () => {
      ...
      expect(shouldSendEventToConnection(unknownEvent, customerConnection)).toBe(false)
      expect(shouldSendEventToConnection(unknownEvent, staffConnection)).toBe(false)
      expect(shouldSendEventToConnection(unknownEvent, adminConnection)).toBe(true)
    })
  })
})
