...
export function validateConfig(config: LocalPrintServiceConfig): ConfigValidationResult {
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      if (config.apiKey.includes('default') || config.apiKey.length < 32) {
        errors.push('Production environment requires a secure API key (min 32 characters)')
      }
    ...
    if (config.autoDiscovery && config.discoveryInterval < 10000) {
      errors.push('Auto discovery interval should be at least 10 seconds to avoid network spam')
    }
...
export function validateEnvironment(): { success: boolean; errors: string[] } {
  ...
  if (process.env.PRINT_AGENT_PORT && process.env.PRINT_AGENT_WS_PORT) {
    if (process.env.PRINT_AGENT_PORT === process.env.PRINT_AGENT_WS_PORT) {
      errors.push('PRINT_AGENT_PORT and PRINT_AGENT_WS_PORT cannot be the same')
    }
  }
