...
export class LocalPrintService {
  ...
  async start(): Promise<void> {
    try {
      console.log(`🖨️  Starting Local Print Service for Restaurant ${this.config.restaurantId}`)
  ...
  private setupApiRoutes(): void {
    ...
    router.delete('/print/:jobId', async (req, res) => {
      try {
        ...
        res.json({
          success: cancelled,
          message: cancelled ? 'Job cancelled' : 'Job not found or cannot be cancelled'
        })
      } catch (error) {
        res.status(500).json({
          success: false,
    ...
    router.post('/discover', async (req, res) => {
      try {
        const { connectionTypes } = req.body || { connectionTypes: ['usb', 'network'] }
        const devices = await this.driverFactory.scanForPrinters(connectionTypes)
  ...
  private async discoverPrinters(): Promise<void> {
    try {
      ...
      const devices = await this.driverFactory.scanForPrinters(['usb', 'network'])
