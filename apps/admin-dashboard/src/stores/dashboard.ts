...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  // Actions
  const fetchDashboardStats = async (dateRange?: { from: string; to: string }) => {
    ...
    try {
      ...
      const response = await api.get<DashboardStats>(`/analytics/dashboard?${params.toString()}`)
  ...
  const fetchRevenueAnalytics = async (period: 'daily' | 'weekly' | 'monthly') => {
  ...
  const fetchOrderAnalytics = async (period: 'daily' | 'weekly' | 'monthly') => {
    if (!authStore.restaurantId) return []
  ...
  const fetchTopMenuItems = async (limit: number = 10, period: 'today' | 'week' | 'month' = 'today') => {
    if (!authStore.restaurantId) return []
