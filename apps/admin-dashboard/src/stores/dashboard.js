...
export const useDashboardStore = defineStore('dashboard', () => {
    ...
    const fetchRevenueAnalytics = async (period) => {
        if (!authStore.restaurantId)
            return [];
        try {
            const response = await api.get(`/analytics/revenue?restaurant_id=${authStore.restaurantId}&period=${period}`);
    ...
    const fetchOrderAnalytics = async (period) => {
        if (!authStore.restaurantId)
            return [];
        try {
            const response = await api.get(`/analytics/orders?restaurant_id=${authStore.restaurantId}&period=${period}`);
            if (response.data.success) {
                return response.data.data;
            }
            return [];
    ...
    const fetchTopMenuItems = async (limit = 10, period = 'today') => {
        if (!authStore.restaurantId)
            return [];
        try {
            const response = await api.get(`/analytics/top-menu-items?restaurant_id=${authStore.restaurantId}&limit=${limit}&period=${period}`);
            if (response.data.success) {
                return response.data.data;
            }
            return [];
