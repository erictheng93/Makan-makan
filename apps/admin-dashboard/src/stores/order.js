...
export const useOrderStore = defineStore('order', () => {
    ...
    const preparingOrders = computed(() => orders.value.filter(order => order.status === OrderStatus.PREPARING));
    const readyOrders = computed(() => orders.value.filter(order => order.status === OrderStatus.READY));
    ...
    const activeOrdersCount = computed(() => orders.value.filter(order => [OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PREPARING, OrderStatus.READY].includes(order.status)).length);
    // Actions
