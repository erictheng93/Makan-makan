...
import { useThrottledRealtime, KITCHEN_THROTTLE_CONFIG } from '@/composables/useThrottledRealtime'
...
describe('useThrottledRealtime', () => {
  ...
  describe('Priority Handling', () => {
    it('should process updates by priority order', () => {
      ...
      const { throttledUpdate, flush } = useThrottledRealtime<number>(updateFn, {
        strategy: 'batch',
        interval: 100,
      })
  ...
  describe('Deduplication', () => {
    it('should deduplicate updates with same key', () => {
      const updateFn = vi.fn()
      const { throttledUpdate, flush } = useThrottledRealtime<{ id: number; value: string }>(
        updateFn,
        {
          strategy: 'batch',
          interval: 100,
  ...
  describe('Flush and Cancel', () => {
    ...
    it('should cancel all pending updates', () => {
      const updateFn = vi.fn()
      const { throttledUpdate, cancel, pending } = useThrottledRealtime<number>(updateFn, {
        strategy: 'batch',
        interval: 100,
      })
  ...
  describe('Stats Tracking', () => {
    it('should track update statistics', () => {
      const updateFn = vi.fn()
      const { throttledUpdate, stats, flush } = useThrottledRealtime<number>(updateFn, {
        strategy: 'batch',
        interval: 100,
      })
  ...
  describe('Performance - Kitchen Display Scenario', () => {
    it('should handle high-frequency order updates efficiently', () => {
      const updateFn = vi.fn()
      const { throttledUpdate } = useThrottledRealtime<{ orderId: number; status: string }>(
        updateFn,
      ...
      for (let i = 0; i < 20; i++) {
        throttledUpdate({ orderId: i, status: 'cooking' }, 'normal', `order-${i}`)
      ...
      expect(totalUpdatesProcessed).toBe(20)
    })
