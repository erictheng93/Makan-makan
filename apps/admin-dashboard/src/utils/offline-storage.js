...
class AdminOfflineStorageManager {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('offlineOrderUpdates')) {
                    const ordersStore = db.createObjectStore('offlineOrderUpdates', { keyPath: 'id' });
                ...
                if (!db.objectStoreNames.contains('cachedAnalytics')) {
                    const analyticsStore = db.createObjectStore('cachedAnalytics', { keyPath: 'id' });
                    analyticsStore.createIndex('restaurant_id', 'restaurant_id', { unique: false });
                    analyticsStore.createIndex('period', 'period', { unique: false });
                    analyticsStore.createIndex('cached_at', 'cached_at', { unique: false });
                }
                // Menu updates for offline editing
                if (!db.objectStoreNames.contains('offlineMenuUpdates')) {
                    const menuStore = db.createObjectStore('offlineMenuUpdates', { keyPath: 'id' });
                    menuStore.createIndex('restaurant_id', 'restaurant_id', { unique: false });
                ...
                if (!db.objectStoreNames.contains('cachedBackups')) {
                    const backupStore = db.createObjectStore('cachedBackups', { keyPath: 'id' });
                    backupStore.createIndex('restaurant_id', 'restaurant_id', { unique: false });
                    backupStore.createIndex('backup_type', 'backup_type', { unique: false });
                    backupStore.createIndex('cached_at', 'cached_at', { unique: false });
                    backupStore.createIndex('expires_at', 'expires_at', { unique: false });
                }
                // User actions for audit trail
                if (!db.objectStoreNames.contains('offlineUserActions')) {
                    const actionsStore = db.createObjectStore('offlineUserActions', { keyPath: 'id' });
                ...
                if (!db.objectStoreNames.contains('dashboardLayouts')) {
                    const layoutStore = db.createObjectStore('dashboardLayouts', { keyPath: 'id' });
                    layoutStore.createIndex('user_id', 'user_id', { unique: false });
                    layoutStore.createIndex('restaurant_id', 'restaurant_id', { unique: false });
                }
            };
        });
    }
    ...
    async saveOfflineOrderUpdate(update) {
    ...
    async markOrderUpdateAsSynced(updateId) {
        const store = this.getStore('offlineOrderUpdates', 'readwrite');
        return new Promise((resolve, reject) => {
    ...
    async getCachedAnalytics(restaurantId, period) {
    ...
    async saveOfflineMenuUpdate(update) {
    ...
    async markMenuUpdateAsSynced(updateId) {
        const store = this.getStore('offlineMenuUpdates', 'readwrite');
        return new Promise((resolve, reject) => {
            const getRequest = store.get(updateId);
            getRequest.onsuccess = () => {
    ...
    async getCachedBackups(restaurantId) {
    ...
    async saveOfflineUserAction(action) {
    ...
    async getDashboardLayout(userId, restaurantId) {
        const store = this.getStore('dashboardLayouts');
        const index = store.index('user_id');
        return new Promise((resolve, reject) => {
            const request = index.getAll(userId);
            request.onsuccess = () => {
                const results = request.result.filter(item => item.restaurant_id === restaurantId);
                resolve(results.length > 0 ? results[0] : null);
            };
            request.onerror = () => reject(request.error);
        });
    ...
    async cleanupExpiredCache(expirationHours = 12) {
