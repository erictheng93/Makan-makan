...
class AdminPushNotificationService {
  ...
  async subscribe(): Promise<NotificationSubscription | null> {
    ...
    try {
      ...
      const subscription = await registration.pushManager.subscribe({
        ...
        applicationServerKey: this.urlBase64ToUint8Array(this.vapidPublicKey) as BufferSource as BufferSource
      })
  ...
  async showLocalNotification(options: AdminPushNotificationOptions): Promise<void> {
    if (!this.isSupported || Notification.permission !== 'granted') {
      return
    }
  ...
  async notifyNewOrder(orderId: string, customerName: string, itemCount: number, totalAmount: number): Promise<void> {
  ...
  async notifySystemAlert(title: string, message: string, alertType: 'info' | 'warning' | 'error' | 'critical'): Promise<void> {
  ...
  async notifyBackupStatus(backupId: string, status: 'completed' | 'failed' | 'started', details?: string): Promise<void> {
  ...
  async notifyPerformanceAlert(metric: string, currentValue: number, threshold: number, trend: 'increasing' | 'decreasing'): Promise<void> {
  ...
  async notifyUserAction(userName: string, action: string, target: string): Promise<void> {
    await this.showLocalNotification({
      title: 'User Activity',
  ...
  async notifyOrderUpdate(orderId: string, oldStatus: string, newStatus: string, updatedBy: string): Promise<void> {
    await this.showLocalNotification({
  ...
  async notifyInventoryAlert(itemName: string, currentStock: number, minThreshold: number): Promise<void> {
    await this.showLocalNotification({
  ...
  async notifyRevenueUpdate(period: string, revenue: number, growth: number): Promise<void> {
