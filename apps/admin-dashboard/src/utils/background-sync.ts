...
import { adminOfflineStorage, type OfflineOrderUpdate, type OfflineMenuUpdate, type OfflineUserAction } from './offline-storage'
...
class AdminBackgroundSyncService {
  ...
  private async syncSingleOrderUpdate(update: OfflineOrderUpdate): Promise<void> {
    try {
  ...
  async queueOrderUpdateSync(update: OfflineOrderUpdate, reason: 'new' | 'retry' = 'new'): Promise<void> {
  ...
  async queueMenuUpdateSync(update: OfflineMenuUpdate, reason: 'new' | 'retry' = 'new'): Promise<void> {
    const syncEvent: AdminSyncEvent = {
  ...
  async queueAnalyticsSync(restaurantId: string, analyticsData: any): Promise<void> {
    const syncEvent: AdminSyncEvent = {
  ...
  private async syncAnalyticsData(data: any, restaurantId: string): Promise<void> {
    try {
  ...
  private addOrUpdateSyncEvent(event: AdminSyncEvent): void {
    ...
    this.syncQueue = this.syncQueue.filter(e =>
      !(e.type === event.type && e.data.id === event.data.id)
    )
