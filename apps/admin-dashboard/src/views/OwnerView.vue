<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="kpi in kpiMetrics" :key="kpi.key"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div class="space-y-3">
          <div v-for="order in realtimeOrders" :key="order.id"
               class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div>
              <p class="font-medium text-gray-900">桌號 {{ order.tableNumber }}</p>
            ...
            <div class="text-right">
              ...
              <p class="text-sm text-gray-500 mt-1">{{ order.time }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div class="space-y-3">
          <div v-for="staff in staffActivity" :key="staff.id"
            ...
            <div class="text-right">
              <p class="text-sm font-medium text-gray-900">{{ staff.performance }}%</p>
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div class="space-y-4">
          <div class="flex justify-between items-center">
            ...
            <span class="font-bold text-green-600">NT$ {{ todayRevenue.toLocaleString() }}</span>
          </div>
          <div class="flex justify-between items-center">
          ...
          <div class="flex justify-between items-center">
            ...
            <span class="font-medium text-gray-900">NT$ {{ Math.round(todayRevenue / todayOrders) }}</span>
          </div>
          <div class="pt-3 border-t border-gray-200">
            <div class="flex justify-between items-center">
              <span class="text-gray-600">預估月收入</span>
              <span class="font-bold text-purple-600">NT$ {{ (todayRevenue * 30).toLocaleString() }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <select v-model="revenueTimeRange" class="text-sm border border-gray-300 rounded-md px-3 py-1">
          ...
          <div v-for="item in popularItems" :key="item.id"
            ...
            <div class="flex items-center space-x-3">
              <div class="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center">
                <span class="text-sm font-bold text-orange-600">{{ item.rank }}</span>
              </div>
              <div>
            ...
            <div class="text-right">
              ...
              <p class="text-sm text-gray-600">NT$ {{ item.revenue.toLocaleString() }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div v-for="system in systemHealth" :key="system.name"
    ...
    <!-- 緊急處理面板 -->
    <div v-if="emergencyAlerts.length > 0" class="bg-red-50 border border-red-200 rounded-lg p-6">
      ...
      <div class="space-y-3">
        <div v-for="alert in emergencyAlerts" :key="alert.id"
          ...
          <div>
            ...
            <p class="text-xs text-red-600 mt-1">{{ alert.time }}</p>
          </div>
          <div class="flex space-x-2">
            <button @click="handleEmergencyAlert(alert.id, 'resolve')"
                    class="px-3 py-1 bg-green-600 text-white rounded-md text-sm hover:bg-green-700">
              處理完成
            </button>
            <button @click="handleEmergencyAlert(alert.id, 'escalate')"
