  ...
  <div class="cashier-view">
    ...
    <div class="flex justify-between items-center mb-8">
      ...
      <div class="flex items-center space-x-6">
        ...
        <div class="bg-blue-100 px-4 py-2 rounded-lg">
          <p class="text-sm text-blue-800 font-medium">班次: {{ currentShift.name }}</p>
          <p class="text-xs text-blue-600">{{ currentShift.startTime }} - {{ currentShift.endTime }}</p>
        </div>
        
        <!-- 今日業績 -->
        <div class="text-right">
          <p class="text-sm text-gray-500">今日業績</p>
          <p class="text-lg font-semibold text-green-600">RM{{ formatMoney(todayRevenue) }}</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow">
          ...
          <div class="divide-y divide-gray-200">
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <div class="ml-4">
                    <div class="flex items-center">
                      <h3 class="text-lg font-medium text-gray-900">{{ order.orderNumber }}</h3>
                      <span 
                    ...
                    <div class="flex items-center mt-1 text-sm text-gray-500">
                      ...
                      <span>{{ order.tableNumber ? `桌號 ${order.tableNumber}` : '外帶' }}</span>
                ...
                <div class="text-right">
                  <p class="text-xl font-bold text-gray-900">RM{{ formatMoney(order.totalAmount) }}</p>
                  <p class="text-sm text-gray-500">{{ order.items.length }} 項商品</p>
                </div>
              </div>
            </div>
            ...
            <div v-if="filteredOrders.length === 0" class="p-12 text-center">
              ...
              <h3 class="text-lg font-medium text-gray-900 mb-2">暫無待結帳訂單</h3>
        ...
        <div class="bg-white rounded-lg shadow">
          <div class="p-6">
            ...
            <div v-if="selectedOrder">
              ...
              <div class="mb-6">
                <div class="flex items-center justify-between mb-4">
                  <h4 class="font-medium text-gray-900">{{ selectedOrder.orderNumber }}</h4>
                  <span :class="getOrderStatusClass(selectedOrder.status)" class="px-2 py-1 text-xs font-medium rounded-full">
              ...
              <div class="border-t border-gray-200 pt-4 space-y-2">
                ...
                <div v-if="selectedOrder.discountAmount > 0" class="flex justify-between text-sm text-green-600">
                  <span>折扣:</span>
                  <span>-RM{{ formatMoney(selectedOrder.discountAmount) }}</span>
                </div>
            ...
            <div v-else class="text-center py-8">
              <CursorArrowRaysIcon class="mx-auto h-12 w-12 text-gray-400 mb-2" />
        ...
        <div v-if="selectedOrder" class="bg-white rounded-lg shadow">
          <div class="p-6">
            ...
            <div v-if="selectedPaymentMethod === 'cash'" class="mb-4">
              <label class="block text-sm font-medium text-gray-700 mb-2">客戶給付金額</label>
              <div class="relative">
              ...
              <div v-if="cashReceived > 0" class="mt-2 p-3 bg-gray-50 rounded-lg">
                <div class="flex justify-between text-sm">
                  <span>應收:</span>
                  <span class="font-medium">RM{{ formatMoney(selectedOrder.totalAmount) }}</span>
                </div>
                <div class="flex justify-between text-sm">
                  <span>實收:</span>
                  <span class="font-medium">RM{{ formatMoney(cashReceived) }}</span>
                </div>
                <div class="flex justify-between text-lg font-bold mt-1 pt-1 border-t border-gray-200">
                  <span>找零:</span>
    ...
    <div v-if="showShiftReport" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen px-4">
        <div class="fixed inset-0 bg-black opacity-30" @click="closeShiftReport"></div>
        <div class="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-semibold text-gray-900">班次報告</h3>
            <button @click="closeShiftReport" class="text-gray-400 hover:text-gray-600">
          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="bg-blue-50 p-4 rounded-lg">
                ...
                <div class="text-sm text-blue-800 space-y-1">
                  ...
                  <p>時間: {{ shiftReport.startTime }} - {{ shiftReport.endTime }}</p>
            ...
            <div>
              ...
              <div class="grid grid-cols-3 gap-4 text-center">
                <div class="bg-gray-50 p-3 rounded">
                  ...
                  <p class="text-2xl font-bold text-gray-900">{{ shiftReport.totalOrders }}</p>
                </div>
                <div class="bg-gray-50 p-3 rounded">
                  <p class="text-sm text-gray-600">平均客單價</p>
                  <p class="text-2xl font-bold text-gray-900">RM{{ formatMoney(shiftReport.avgOrderValue) }}</p>
                </div>
                <div class="bg-gray-50 p-3 rounded">
                  <p class="text-sm text-gray-600">退款次數</p>
                  <p class="text-2xl font-bold text-gray-900">{{ shiftReport.refundCount }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-yellow-50 p-4 rounded-lg">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-yellow-800 mb-1">系統記錄金額</label>
                  <div class="text-lg font-bold text-yellow-900">RM{{ formatMoney(shiftReport.systemCashAmount) }}</div>
                </div>
                <div>
                  <label class="block text-sm font-medium text-yellow-800 mb-1">實際盤點金額</label>
                  <input
              ...
              <div class="mt-3 p-2 rounded" :class="cashDifference === 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'">
                <p class="text-sm font-medium">
                  {{ cashDifference === 0 ? '盤點符合' : 
                     cashDifference > 0 ? `現金多 RM${formatMoney(Math.abs(cashDifference))}` : 
    ...
    <div v-if="showRefundDialog" class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen px-4">
        <div class="fixed inset-0 bg-black opacity-30" @click="closeRefundDialog"></div>
        <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-semibold text-gray-900">退款處理</h3>
            <button @click="closeRefundDialog" class="text-gray-400 hover:text-gray-600">
              <XMarkIcon class="w-6 h-6" />
            </button>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">退款金額</label>
              <div class="relative">
                <span class="absolute left-3 top-3 text-gray-500">RM</span>
                <input
...
const filteredOrders = computed(() => {
  let filtered = orders.value.filter(order => 
    ['ready', 'served'].includes(order.status) && order.paymentStatus === 'unpaid'
  )
...
const applyDiscount = () => {
  ...
  if (discountPercent && !isNaN(parseFloat(discountPercent))) {
    ...
    selectedOrder.value.totalAmount = Math.max(0, 
