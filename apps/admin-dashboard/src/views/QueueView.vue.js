    ...
    return availableTables
        ...
        if (selectedQueueItem.value?.tablePreference) {
            if (table.number.includes("1") || table.number.includes("2")) {
                // 假設靠窗
                if (selectedQueueItem.value.tablePreference === "window")
                    score += 15;
            }
        }
...
const callNextCustomer = () => {
    const nextCustomer = queueItems.value.find((item) => item.status === "waiting");
    if (nextCustomer) {
...
const submitAddToQueue = async () => {
    if (!canAddToQueue.value)
        return;
    try {
        const nextNumber = Math.max(...queueItems.value.map((item) => item.queueNumber)) + 1;
        const newItem = {
            id: `queue_${Date.now()}`,
...
const confirmSeatAssignment = async () => {
    ...
    try {
        ...
        // 更新桌位狀態
        const table = tables.value.find((t) => t.id === seatAssignment.value.tableId);
        if (table) {
...
if (__VLS_ctx.showAddDialog) {
    ...
    (__VLS_ctx.newQueueItem.phoneNumber);
    // @ts-ignore
    [newQueueItem,];
    __VLS_asFunctionalElement(__VLS_elements.div, __VLS_elements.div)({});
    __VLS_asFunctionalElement(__VLS_elements.label, __VLS_elements.label)({
    ...
    __VLS_asFunctionalElement(__VLS_elements.select, __VLS_elements.select)({
        value: (__VLS_ctx.newQueueItem.tablePreference),
        ...{ class: "w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" },
    });
    // @ts-ignore
    [newQueueItem,];
...
if (__VLS_ctx.showSeatDialog) {
    ...
    if (__VLS_ctx.selectedQueueItem) {
        ...
        // @ts-ignore
        [selectedQueueItem,];
        if (__VLS_ctx.selectedQueueItem.tablePreference) {
            // @ts-ignore
            [selectedQueueItem,];
            __VLS_asFunctionalElement(__VLS_elements.p, __VLS_elements.p)({});
            __VLS_asFunctionalElement(__VLS_elements.span, __VLS_elements.span)({
            ...
            (__VLS_ctx.selectedQueueItem.tablePreference);
            // @ts-ignore
            [selectedQueueItem,];
        }
        __VLS_asFunctionalElement(__VLS_elements.p, __VLS_elements.p)({});
