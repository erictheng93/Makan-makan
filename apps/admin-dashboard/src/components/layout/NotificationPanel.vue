<template>
  <div class="fixed right-0 top-16 w-80 max-w-sm bg-white shadow-2xl z-50 border border-gray-200 rounded-bl-lg overflow-hidden">
    <!-- 標題欄 -->
    ...
    <div class="px-4 py-2 bg-gray-50 border-b border-gray-200 flex items-center justify-between text-xs">
      ...
      <div class="flex items-center space-x-2">
        ...
        <button @click="refreshNotifications"
                class="p-1 rounded hover:bg-gray-200 transition-colors">
          <ArrowPathIcon class="w-4 h-4 text-gray-600" :class="{ 'animate-spin': isRefreshing }" />
        </button>
      </div>
    </div>
          ...
          <div class="flex items-start space-x-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div v-if="notification.data" class="mt-2 text-xs text-gray-500">
                <span v-if="notification.data.orderNumber" class="inline-flex items-center px-2 py-1 rounded-full bg-gray-100 text-gray-800">
                  訂單 #{{ notification.data.orderNumber }}
                </span>
                <span v-if="notification.data.tableNumber" class="inline-flex items-center px-2 py-1 rounded-full bg-blue-100 text-blue-800 ml-1">
                  桌號 {{ notification.data.tableNumber }}
                </span>
              </div>

              <!-- 操作按鈕 -->
              <div v-if="hasActionButtons(notification)" class="mt-3 flex space-x-2">
                <button
    ...
    <!-- 底部操作欄 -->
    <div v-if="notifications.length > 0" class="p-3 bg-gray-50 border-t border-gray-200">
