...
import type { RealtimeOverview, RoomStats, RoomType } from '@/types/realtime'
...
async function fetchOverview() {
  ...
  try {
    ...
    if (response.data.success && response.data.data) {
      ...
      throw new Error(response.data.error || 'Failed to fetch realtime stats')
    }
  } catch (error) {
...
function getRoomLabel(roomType: RoomType): string {
  switch (roomType) {
    case 'kitchen': return t('realtime.rooms.kitchen', '廚房')
    case 'admin': return t('realtime.rooms.admin', '管理後台')
    case 'customer': return t('realtime.rooms.customer', '顧客')
...
<template>
  <div class="realtime-monitor-panel bg-white rounded-lg shadow-sm border border-gray-200">
    <!-- Header -->
    <div class="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
      <div class="flex items-center space-x-2">
        <div class="w-2 h-2 rounded-full animate-pulse" :class="totalConnections > 0 ? 'bg-green-500' : 'bg-gray-400'" />
      ...
      <div class="grid grid-cols-3 gap-3">
        ...
        >
          ...
          <div class="text-xs text-gray-500 mt-1">
            {{ room.status === 'active' ? t('realtime.status.active', '活躍') : t('realtime.status.inactive', '閒置') }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="px-4 py-2 bg-gray-50 border-t border-gray-200 flex items-center justify-between text-xs text-gray-500">
      <span v-if="lastRefresh">
        {{ t('realtime.lastUpdate', '最後更新') }}: {{ formatTime(lastRefresh.toISOString()) }}
      </span>
      <span v-else>
        {{ t('realtime.loading', '載入中...') }}
      </span>
      <span>
        {{ t('realtime.autoRefresh', '自動刷新') }}: {{ props.refreshInterval }}s
      </span>
    </div>
