...
class StatisticsService {
  ...
  public async fetchDetailedPerformance(options: {
    restaurantId?: number
    dateFrom?: string
    dateTo?: string
    ...
    try {
      ...
      if (options.restaurantId) params.append('restaurantId', options.restaurantId.toString())
      if (options.dateFrom) params.append('dateFrom', options.dateFrom)
      if (options.dateTo) params.append('dateTo', options.dateTo)

      const response = await api.get(`/analytics/detailed-performance?${params}`)
  ...
  public get totalActiveOrders(): number {
    return this.dashboardData.realtime_stats.pending_orders + 
  ...
  public exportCSV(): string {
    const csvData = [
      ['指標', '數值', '單位'],
      ['總訂單數', this.dashboardData.realtime_stats.total_today.toString(), '筆'],
      ['已完成訂單', this.dashboardData.realtime_stats.completed_today.toString(), '筆'],
      ['完成率', this.dashboardData.kpis.completion_rate.toString(), '%'],
      ['平均製作時間', this.dashboardData.kpis.avg_preparation_time.toString(), '分鐘'],
      ['今日營收', this.dashboardData.realtime_stats.revenue_today.toString(), 'RM'],
      ['效率評分', this.dashboardData.kpis.efficiency_score.toString(), '分'],
      ['系統負載', this.dashboardData.system_load.load_percentage.toString(), '%'],
