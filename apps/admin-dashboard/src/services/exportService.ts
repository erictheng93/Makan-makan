...
export class ExportService {
  ...
  private generatePDFSummary(
    ...
    if (dataType === 'alerts' || dataType === 'all') {
      ...
      const critical = alerts.filter((a: any) => a.severity === 'critical' || a.severity === 'fatal')
   ...
   * 獲取表格列定義
   */
  private getTableColumns(dataType: ExportDataType): Array<{ key: string; label: string }> {
    const commonColumns = [
      { key: 'timestamp', label: '時間' },
      { key: 'component', label: '組件' },
    ]

    const typeColumns: Record<ExportDataType, Array<{ key: string; label: string }>> = {
      alerts: [
        ...commonColumns,
