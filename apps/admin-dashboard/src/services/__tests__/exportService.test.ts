...
import type { ExportOptions, ExportFormat, ExportDataType } from '@/types/monitoring-export'
...
describe('ExportService', () => {
  ...
  describe('exportData', () => {
    ...
    it('should generate unique filenames', async () => {
      ...
      const result1 = await exportService.exportData(mockData, options)
      const result2 = await exportService.exportData(mockData, options)
  ...
  describe('quickExport', () => {
    ...
    it('should support all formats', async () => {
      ...
      for (const format of formats) {
        const result = await exportService.quickExport(mockData, format, 'alerts')
        expect(result.success).toBe(true)
        expect(result.filename).toMatch(new RegExp(`\\.(${format === 'excel' ? 'xlsx' : format})$`))
      }
    })

    it('should support all data types', async () => {
      const dataTypes: ExportDataType[] = ['alerts', 'performance', 'errors', 'health', 'all']

      for (const dataType of dataTypes) {
        const result = await exportService.quickExport(mockData, 'csv', dataType)
        expect(result.success).toBe(true)
  ...
  describe('Data formatting', () => {
    it('should format dates correctly', async () => {
      ...
      const result = await exportService.quickExport(dataWithDates, 'csv', 'alerts')
      expect(result.success).toBe(true)
    })
    ...
    it('should handle nested objects', async () => {
      ...
      const result = await exportService.quickExport(dataWithObjects, 'csv', 'alerts')
      expect(result.success).toBe(true)
    })
    ...
    it('should handle null and undefined values', async () => {
      ...
      const result = await exportService.quickExport(dataWithNulls, 'csv', 'alerts')
      expect(result.success).toBe(true)
    })
    ...
    it('should handle special characters in text', async () => {
      ...
      const result = await exportService.quickExport(dataWithSpecialChars, 'csv', 'alerts')
      expect(result.success).toBe(true)
    })
  })
  ...
  describe('Excel Export', () => {
    it('should generate Excel file', async () => {
      const result = await exportService.quickExport(mockData, 'excel', 'performance')
    ...
    it('should handle large datasets', async () => {
      ...
      const result = await exportService.quickExport(largeData, 'excel', 'performance')
  ...
  describe('Performance', () => {
    it('should handle very large datasets', async () => {
      ...
      const result = await exportService.quickExport(hugeData, 'csv', 'performance')
  ...
  describe('Edge cases', () => {
    it('should handle data with missing fields', async () => {
      ...
      const result = await exportService.quickExport(incompleteData, 'csv', 'alerts')
      expect(result.success).toBe(true)
    })
    ...
    it('should handle data with extra fields', async () => {
      ...
      const result = await exportService.quickExport(extraFieldsData, 'csv', 'alerts')
      expect(result.success).toBe(true)
    })
    ...
    it('should handle circular references gracefully', async () => {
      ...
      try {
        const result = await exportService.quickExport([circularData], 'csv', 'alerts')
