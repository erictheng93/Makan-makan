...
export const posService = {
    ...
    async endShift(shiftId, data) {
        const response = await apiClient.post(`/api/v1/pos/shifts/${shiftId}/end`, data);
        return response.data.data || response.data;
    },
    async getCurrentShift(registerId) {
        try {
            const response = await apiClient.get(`/api/v1/pos/registers/${registerId}/current-shift`);
            return response.data.data || response.data;
        }
        catch (error) {
            return null;
    ...
    async getCashMovements(registerId, params) {
        const response = await apiClient.get(`/api/v1/pos/registers/${registerId}/cash-movements`, { params });
        return response.data.data || response.data;
    },
    // 收據管理
    ...
    async getReceipts(registerId, params) {
        const response = await apiClient.get(`/api/v1/pos/registers/${registerId}/receipts`, { params });
        return response.data.data || response.data;
    },
    // 退款處理
    ...
    async getDailyStats(registerId, date) {
        const response = await apiClient.get(`/api/v1/pos/registers/${registerId}/stats/daily`, {
            params: { date },
        });
        return response.data.data || response.data;
    },
    async getShiftReport(shiftId) {
        const response = await apiClient.get(`/api/v1/pos/shifts/${shiftId}/report`);
        return response.data.data || response.data;
    },
    // 快速收銀
