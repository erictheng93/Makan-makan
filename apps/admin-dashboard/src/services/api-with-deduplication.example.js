...
export async function getMenu(restaurantId) {
    ...
    const response = await api.get(`/restaurants/${restaurantId}/menu`, withDedupTTL(30000));
    return response.data;
}
...
export async function createOrder(restaurantId, orderData) {
    ...
    const response = await api.post(`/restaurants/${restaurantId}/orders`, orderData, skipDedup());
    return response.data;
}
// Example 4: Combine options
export async function getAnalytics(restaurantId, period) {
    const response = await api.get(`/restaurants/${restaurantId}/analytics`, combineConfigs(withDedupTTL(60000), // Cache for 1 minute
    {
        params: { period },
 ...
 */
export async function loadDashboard(restaurantId) {
 ...
 */
export async function refreshOrders(restaurantId) {
