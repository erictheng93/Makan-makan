...
export class MonitoringStorageService {
   ...
   * 保存篩選器
   */
  saveFilter(name: string, filter: MonitoringFilter, description?: string): SavedFilter {
   ...
   * 保存佈局
   */
  saveLayout(layout: Omit<DashboardLayout, 'id' | 'createdAt' | 'updatedAt'>): DashboardLayout {
