...
describe('monitoring-filters', () => {
  ...
  describe('FILTER_PRESETS', () => {
    ...
    it('critical-alerts preset should filter critical and fatal severity', () => {
      const criticalPreset = FILTER_PRESETS.find(p => p.id === 'critical-alerts')
  ...
  describe('serializeFilter / deserializeFilter', () => {
    ...
    it('should serialize and deserialize filter with custom date range', () => {
      ...
      expect(deserialized.customDateRange?.start.getTime()).toBe(startDate.getTime())
      expect(deserialized.customDateRange?.end.getTime()).toBe(endDate.getTime())
    })
