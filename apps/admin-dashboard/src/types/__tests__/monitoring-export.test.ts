...
describe('monitoring-export', () => {
  describe('REPORT_TEMPLATES', () => {
    ...
    it('should have valid template structures', () => {
      REPORT_TEMPLATES.forEach(template => {
        ...
        expect(['alerts', 'performance', 'errors', 'health', 'all']).toContain(template.dataType)
      })
    })
  ...
  describe('DEFAULT_EXPORT_OPTIONS', () => {
    ...
    it('should have CSV options', () => {
      ...
      expect(DEFAULT_EXPORT_OPTIONS.csvOptions?.dateFormat).toBe('YYYY-MM-DD HH:mm:ss')
    })
  })
  ...
  describe('generateExportFilename', () => {
    it('should generate filename with correct format', () => {
      ...
      expect(filename).toMatch(/^monitoring-alerts-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.csv$/)
    })
    ...
    it('should generate unique filenames for different calls', () => {
      ...
      expect(filename1).toMatch(/^monitoring-alerts-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.csv$/)
      expect(filename2).toMatch(/^monitoring-alerts-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.csv$/)
    })
  })
  ...
  describe('Template compatibility', () => {
    it('should have templates for each data type', () => {
      ...
      dataTypes.forEach(dataType => {
        const templatesForType = REPORT_TEMPLATES.filter(t => t.dataType === dataType)
    ...
    it('should have templates for each format', () => {
      ...
      formats.forEach(format => {
        const templatesForFormat = REPORT_TEMPLATES.filter(t => t.format === format)
