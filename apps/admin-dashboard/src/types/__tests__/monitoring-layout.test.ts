...
describe('monitoring-layout', () => {
  describe('WIDGET_TYPES', () => {
    ...
    it('should have valid widget metadata', () => {
      WIDGET_TYPES.forEach(widget => {
        ...
        expect(['overview', 'performance', 'alerts', 'metrics', 'charts']).toContain(widget.category)
        expect(['small', 'medium', 'large', 'xlarge']).toContain(widget.defaultSize)
      })
    })

    it('should categorize widgets correctly', () => {
      const overview = WIDGET_TYPES.filter(w => w.category === 'overview')
      const performance = WIDGET_TYPES.filter(w => w.category === 'performance')
  ...
  describe('LAYOUT_PRESETS', () => {
    it('should have 4 layout presets', () => {
      ...
      const expectedIds = ['default-overview', 'performance-focused', 'alerts-monitoring', 'minimal']
    ...
    it('default-overview should be marked as default', () => {
      const defaultPreset = LAYOUT_PRESETS.find(p => p.id === 'default-overview')
    ...
    it('minimal preset should have fewest widgets', () => {
      ...
      otherPresets.forEach(preset => {
        expect(minimalPreset!.layout.widgets.length).toBeLessThanOrEqual(preset.layout.widgets.length)
      })
    })
  })
  ...
  describe('DEFAULT_LAYOUT', () => {
    it('should be based on first preset', () => {
      ...
      expect(DEFAULT_LAYOUT.widgets.length).toBe(firstPreset.layout.widgets.length)
    })
  ...
  describe('checkWidgetOverlap', () => {
    ...
    it('should be symmetric', () => {
      ...
      expect(checkWidgetOverlap(widget1, widget2)).toBe(checkWidgetOverlap(widget2, widget1))
    })
  })

  describe('findNextAvailablePosition', () => {
    it('should return (0, 0) for empty grid', () => {
      const position = findNextAvailablePosition([], { width: 3, height: 2 }, 12)
    ...
    it('should find position after existing widget', () => {
      ...
      const position = findNextAvailablePosition(existingWidgets, { width: 3, height: 2 }, 12)
    ...
    it('should respect grid column limit', () => {
      ...
      const position = findNextAvailablePosition([], { width: widgetWidth, height: 2 }, gridColumns)
    ...
    it('should find position in next row when current row is full', () => {
      ...
      const position = findNextAvailablePosition(existingWidgets, { width: 4, height: 2 }, 12)

      expect(position.y).toBeGreaterThan(0)
    })

    it('should handle large widgets', () => {
      const position = findNextAvailablePosition([], { width: 12, height: 5 }, 12)
  ...
  describe('Edge cases', () => {
    it('should handle widget with zero dimensions', () => {
      ...
      const position = findNextAvailablePosition([widget], { width: 3, height: 2 }, 12)
      expect(position).toBeDefined()
    })

    it('should handle very large grid', () => {
      const position = findNextAvailablePosition([], { width: 1, height: 1 }, 24)
      expect(position.x).toBe(0)
      expect(position.y).toBe(0)
    })
  })
