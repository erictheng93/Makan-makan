...
export function usePagination(fetchFn, initialParams = {}) {
     ...
     */
    const changeSort = async (sortBy, sortOrder = 'desc') => {
 ...
 * )
 */
export function useInfiniteScroll(fetchFn, initialParams = {}) {
    const state = ref({
        items: [],
        isLoading: false,
        hasMore: true,
    ...
    const loadMore = async () => {
        ...
        try {
            ...
            const paginatedResult = await fetchFn(params.value)(state.value.items).push(...paginatedResult.data);
    ...
    const isEmpty = computed(() => state.value.items.length === 0 && !state.value.isLoading);
    return {
        // State
 ...
 * )
 */
export function useCursorPagination(fetchFn, limit = 20) {
    const data = ref([]);
    const isLoading = ref(false);
    const error = ref(null);
