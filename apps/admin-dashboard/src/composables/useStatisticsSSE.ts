...
export function useStatisticsSSE(options: {
  ...
  const lastMessage = ref<StatisticsSSEEvent | null>(null)
  ...
  const connect = () => {
    ...
    try {
      ...
      eventSource.onmessage = (event) => {
        try {
          const message: StatisticsSSEEvent = JSON.parse(event.data)
  ...
  const scheduleReconnect = () => {
    ...
    console.log(`Scheduling SSE reconnect attempt ${connectionAttempts.value}/${retryAttempts} in ${delay}ms`)
  ...
  // Message handlers
  const handleMessage = (message: StatisticsSSEEvent) => {
    switch (message.type) {
      case 'order_created':
      case 'order_updated':
  ...
  const handleOrderEvent = (message: StatisticsSSEEvent) => {
    // 訂單事件發生時，觸發統計數據刷新
  ...
  const handleStatisticsUpdate = (message: StatisticsSSEEvent) => {
    // 直接更新統計服務的數據
    if (message.data) {
      try {
        // 更新實時統計數據
        if (message.data.realtime_stats) {
          Object.assign(statisticsService.dashboardData.realtime_stats, message.data.realtime_stats)
        }
        
        // 更新KPI指標
        if (message.data.kpis) {
          Object.assign(statisticsService.dashboardData.kpis, message.data.kpis)
        }
        
        // 更新系統負載
        if (message.data.system_load) {
          Object.assign(statisticsService.dashboardData.system_load, message.data.system_load)
        }
        
        // 更新活躍訂單
        if (message.data.active_orders) {
          statisticsService.dashboardData.active_orders = message.data.active_orders
        }

        // 更新每小時完成率
        if (message.data.hourly_completion_rate) {
          statisticsService.dashboardData.hourly_completion_rate = message.data.hourly_completion_rate
        }

        // 更新分類平均時間
        if (message.data.avg_time_by_category) {
          statisticsService.dashboardData.avg_time_by_category = message.data.avg_time_by_category
        }

        // 更新績效趨勢
        if (message.data.performance_trend) {
          statisticsService.dashboardData.performance_trend = message.data.performance_trend
        }
