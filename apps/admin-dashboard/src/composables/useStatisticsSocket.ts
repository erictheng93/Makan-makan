...
export function useStatisticsSocket(options: {
  ...
  const lastMessage = ref<StatisticsSocketEvent | null>(null)
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        send({
          type: 'subscribe',
          data: {
            events: ['order_created', 'order_updated', 'order_completed', 'order_cancelled', 'statistics_update']
          }
        })
      }
      ...
      ws.onmessage = (event) => {
        try {
          const message: StatisticsSocketEvent = JSON.parse(event.data)
      ...
      ws.onclose = (event) => {
        console.log('Statistics WebSocket disconnected:', event.code, event.reason)
  ...
  const scheduleReconnect = () => {
    ...
    console.log(`Scheduling reconnect attempt ${connectionAttempts.value}/${retryAttempts} in ${delay}ms`)
  ...
  // Message handlers
  const handleMessage = (message: StatisticsSocketEvent) => {
    switch (message.type) {
      case 'order_created':
      case 'order_updated':
  ...
  const handleStatisticsUpdate = (message: StatisticsSocketEvent) => {
    // 直接更新統計服務的數據
    if (message.data) {
      try {
        // 更新實時統計數據
        ...
        console.error('Failed to update statistics from WebSocket message:', error)
      }
    }
  }
