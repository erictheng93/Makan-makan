...
export function useDateFormatter() {
   ...
   * rÖ~„ûo
   */
  const getWeekdayName = (date: Date, format: 'short' | 'long' = 'short'): string => {
   ...
   */
  const formatDate = (date: Date | string, includeWeekday: boolean = false): string => {
    ...
    switch (locale.value) {
      case 'zh-TW':
      case 'zh-CN':
        formatted = `${year}t${parseInt(month)}${parseInt(day)}å`
        break
      case 'ja-JP':
        formatted = `${year}t${parseInt(month)}${parseInt(day)}å`
        break
   ...
   * rÖý1
   */
  const getMonthName = (monthIndex: number, format: 'short' | 'long' = 'long'): string => {
    ...
    return new Intl.DateTimeFormat(localeMap[locale.value] || 'zh-TW', options).format(date)
  }
