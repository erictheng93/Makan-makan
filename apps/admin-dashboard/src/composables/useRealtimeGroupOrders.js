...
export function useRealtimeGroupOrders() {
    ...
    const activeGroupOrders = ref(new Map());
    ...
    const myMember = computed(() => {
        if (!currentGroupOrder.value || !myMemberId.value)
            return null;
        return (currentGroupOrder.value.members.find((m) => m.id === myMemberId.value) ||
            null);
    });
    const canEditCart = computed(() => {
        if (!currentGroupOrder.value)
            return false;
        return (currentGroupOrder.value.status === "active" ||
            currentGroupOrder.value.status === "ordering");
    });
    const canInitiateSplit = computed(() => {
        if (!myMember.value || !currentGroupOrder.value)
            return false;
        return ((myMember.value.role === "creator" || myMember.value.role === "admin") &&
    ...
    const allMembersPaid = computed(() => {
        if (!currentGroupOrder.value)
            return false;
        return currentGroupOrder.value.members.every((m) => m.paymentStatus === "paid");
    });
    /**
     * WebSocket 連接管理
     */
    const connectWebSocket = async (groupOrderId) => {
     ...
     * 群組訂單事件處理
     */
    const handleGroupOrderEvent = (message) => {
    ...
    const handleGroupOrderJoined = (message) => {
     ...
     * SSE 事件處理
     */
    const handleSSEGroupOrderUpdate = (message) => {
        const update = {
     ...
     * 群組訂單操作方法
     */
    const joinGroupOrder = async (shareCode, memberName, phone) => {
        try {
            // 首先建立WebSocket連接
    ...
    const initiateSplitBill = async (splitType, customSplits) => {
        if (!currentGroupOrder.value || !canInitiateSplit.value)
            return false;
        try {
            sendWebSocketMessage({
    ...
    const processPayment = async (paymentMethod, amount, transactionId) => {
        if (!currentGroupOrder.value || !myMemberId.value)
            return false;
        try {
            sendWebSocketMessage({
     ...
     * 通知管理
     */
    const createNotification = (event, _groupOrder) => {
        let message = "";
        switch (event.type) {
            case "member_joined":
     ...
     * 實用工具方法
     */
    const getGroupOrderById = (groupOrderId) => {
        return activeGroupOrders.value.get(groupOrderId) || null;
    };
    const getRecentEvents = (limit = 20) => {
    ...
    const getMemberById = (memberId) => {
        if (!currentGroupOrder.value)
            return null;
        return (currentGroupOrder.value.members.find((m) => m.id === memberId) || null);
    };
    const getCartItemsByMember = (memberId) => {
        if (!currentGroupOrder.value)
            return [];
        return currentGroupOrder.value.cart.filter((item) => item.memberId === memberId);
    };
    ...
    const getMySplitBill = () => {
        if (!currentGroupOrder.value || !myMemberId.value)
            return null;
        return (currentGroupOrder.value.splitBills.find((bill) => bill.memberId === myMemberId.value) || null);
    };
    /**
     * 開始監聽群組訂單事件
     */
    const startListening = () => {
        if (!authStore.user?.restaurantId) {
            console.warn("No restaurant ID found, cannot start group order listening");
            return;
        }
        // SSE訂閱群組訂單事件
        ...
        const sseSubId = subscribe(groupOrderEventTypes, handleSSEGroupOrderUpdate, authStore.user.restaurantId.toString());
