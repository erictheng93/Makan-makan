...
export function useThrottledRealtime<T>(
  ...
  const {
    strategy = 'throttle',
    interval = strategy === 'throttle' ? 33 : strategy === 'debounce' ? 150 : 100,
    maxWait = 1000,
    batchSize = 10,
    leading = true,
    trailing = true,
  ...
  const executeUpdate = (updates: UpdateItem<T>[]) => {
    ...
    const finalUpdates = [...Array.from(uniqueUpdates.values()), ...noKeyUpdates]
    ...
    stats.value.averageBatchSize =
      (stats.value.averageBatchSize * (stats.value.processedBatches - 1) + updates.length) /
      stats.value.processedBatches
  ...
  const throttleUpdate = (item: UpdateItem<T>) => {
    ...
    if (leading && !state.value.isThrottled && state.value.lastUpdateTime === 0) {
      executeUpdate([item])
      state.value.isThrottled = true
      return
    }
