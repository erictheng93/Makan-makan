...
export function useRealtimeOrders() {
    ...
    // 團體訂單更新處理函數
    const handleGroupOrderUpdate = (message) => {
        const update = {
    ...
    const startListening = () => {
        if (!authStore.user?.restaurantId) {
            console.warn("No restaurant ID found, cannot start realtime orders listening");
            return;
        }
        // 訂閱訂單相關事件
        ...
        const orderSubId = subscribe(orderEventTypes, handleOrderUpdate, authStore.user.restaurantId.toString());
        // 訂閱團體訂單相關事件
        ...
        const groupOrderSubId = subscribe(groupOrderEventTypes, handleGroupOrderUpdate, authStore.user.restaurantId.toString());
    ...
    // 檢查特定訂單是否有更新
    const hasOrderUpdate = (orderId, since) => {
    ...
    // 檢查特定團體訂單是否有更新
    const hasGroupOrderUpdate = (groupOrderId, since) => {
