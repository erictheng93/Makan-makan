...
export function useRealtimeQueue() {
    ...
    const handleQueueUpdate = (message) => {
        ...
        if (update.type === "joined") {
            showQueueNotification(`新顧客加入排隊: ${update.customerName || `排號 ${update.queueNumber}`} (${update.partySize}人)`, "info");
        }
        else if (update.type === "called") {
            showQueueNotification(`叫號: ${update.customerName || `排號 ${update.queueNumber}`}`, "success");
        }
        else if (update.type === "seated") {
            showQueueNotification(`入座完成: ${update.customerName || `排號 ${update.queueNumber}`}`, "success");
        }
        else if (update.type === "no_show") {
            showQueueNotification(`未到場: ${update.customerName || `排號 ${update.queueNumber}`}`, "warning");
        }
        else if (update.type === "cancelled") {
            showQueueNotification(`已取消: ${update.customerName || `排號 ${update.queueNumber}`}`, "info");
        }
    };
    // 桌位更新處理函數
    ...
    const updateQueueStats = (update) => {
        switch (update.type) {
            case "joined":
                ...
                break;
            case "seated":
                queueStats.value.currentWaiting = Math.max(0, queueStats.value.currentWaiting - 1);
                ...
                if (update.waitTime) {
                    ...
                    queueStats.value.averageWaitTime = Math.round((currentAvg * (totalServed - 1) + update.waitTime) / totalServed);
                    // 更新最長等待時間
    ...
    const startListening = () => {
        if (!authStore.user?.restaurantId) {
            console.warn("No restaurant ID found, cannot start realtime queue listening");
            return;
        }
        ...
        const queueSubId = subscribe(queueEventTypes, handleQueueUpdate, authStore.user.restaurantId.toString());
        // 訂閱桌位相關事件
        ...
        const tableSubId = subscribe(tableEventTypes, handleTableUpdate, authStore.user.restaurantId.toString());
    ...
    const getTableUpdatesByNumber = (tableNumber) => {
        return tableUpdates.value.filter((update) => update.tableNumber === tableNumber);
    };
    // 檢查是否有待處理的叫號 - 包含 notified 狀態
    const hasPendingCalls = () => {
        return queueUpdates.value.some((update) => (update.type === "called" || update.type === "notified") &&
            new Date().getTime() - new Date(update.timestamp).getTime() < 300000);
    };
    // 獲取特定狀態的候位更新數量
    const getUpdateCountByStatus = (status) => {
        return queueUpdates.value.filter((update) => update.status === status).length;
    };
    // 獲取平均等待時間
    const getAverageWaitTime = () => {
        const seatedUpdates = queueUpdates.value.filter((update) => update.type === "seated" && update.actualWaitMinutes);
        if (seatedUpdates.length === 0)
            return 0;
        const totalWait = seatedUpdates.reduce((sum, update) => sum + (update.actualWaitMinutes || 0), 0);
    ...
    const getAvailableTablesCount = () => {
        ...
        const availableUpdates = recentTableUpdates.filter((update) => update.type === "available");
