...
export function useRealtimePOS() {
    ...
    const handleTransactionUpdate = (message) => {
        ...
        // 顯示通知
        showPOSNotification(`新交易: ${transaction.type} - RM${Math.abs(transaction.amount).toFixed(2)}`, transaction.amount >= 0 ? "success" : "info");
    };
    // 現金異動處理函數
    const handleCashMovement = (message) => {
        ...
        // 顯示通知
        showPOSNotification(`現金異動: ${getCashMovementTypeText(movement.type)} - RM${Math.abs(movement.amount).toFixed(2)}`, "info");
    };
    // 班次事件處理函數
    const handleShiftEvent = (message) => {
        ...
        if (shiftEvent.type === "started") {
            ...
            posStats.value.currentShifts = Math.max(0, posStats.value.currentShifts - 1);
        }
        // 限制歷史記錄長度
        ...
        // 顯示通知
        showPOSNotification(`班次${shiftEvent.type === "started" ? "開始" : "結束"}: 現金櫃 ${shiftEvent.registerId}`, "info");
    };
    // 現金櫃狀態處理函數
    const handleRegisterStatus = (message) => {
        ...
        // 更新活躍現金櫃統計
        const activeCount = Array.from(registerStatuses.value.values()).filter((reg) => reg.status === "active").length;
        posStats.value.activeRegisters = activeCount;
        console.log("Register status received:", status);
        // 顯示狀態變更通知
        showPOSNotification(`現金櫃 ${status.registerId} 狀態: ${getRegisterStatusText(status.status)}`, status.status === "active" ? "success" : "warning");
    };
    // 更新 POS 統計
    ...
    const startListening = () => {
        if (!authStore.user?.restaurantId) {
            console.warn("No restaurant ID found, cannot start realtime POS listening");
            return;
        }
        // 訂閱 POS 交易事件
        const transactionSubId = subscribe([REALTIME_EVENTS.POS_TRANSACTION], handleTransactionUpdate, authStore.user.restaurantId.toString());
        // 訂閱現金異動事件
        const cashMovementSubId = subscribe([REALTIME_EVENTS.CASH_MOVEMENT], handleCashMovement, authStore.user.restaurantId.toString());
        // 訂閱班次事件
        const shiftEventTypes = [
            REALTIME_EVENTS.SHIFT_STARTED,
            REALTIME_EVENTS.SHIFT_ENDED,
        ];
        const shiftSubId = subscribe(shiftEventTypes, handleShiftEvent, authStore.user.restaurantId.toString());
        // 訂閱現金櫃狀態事件
        const registerSubId = subscribe([REALTIME_EVENTS.REGISTER_STATUS_CHANGED], handleRegisterStatus, authStore.user.restaurantId.toString());
