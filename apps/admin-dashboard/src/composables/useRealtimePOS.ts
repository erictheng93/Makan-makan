...
export function useRealtimePOS() {
  ...
  const transactions = ref<RealtimePOSTransaction[]>([]);
  const cashMovements = ref<RealtimeCashMovement[]>([]);
  const shiftEvents = ref<RealtimeShiftEvent[]>([]);
  const registerStatuses = ref<Map<string, RealtimeRegisterStatus>>(new Map());
  ...
  // 更新 POS 統計
  const updatePOSStats = (transaction: RealtimePOSTransaction) => {
  ...
  // 獲取現金異動類型文字
  const getCashMovementTypeText = (type: string) => {
  ...
  // 按現金櫃篩選交易
  const getTransactionsByRegister = (registerId: string) => {
  ...
  // 獲取現金櫃狀態
  const getRegisterStatus = (registerId: string) => {
