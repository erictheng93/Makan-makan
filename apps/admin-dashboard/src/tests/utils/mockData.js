 ...
 */
// 創建模擬群組訂單
export function createMockGroupOrder(id, options = {}) {
    const now = Date.now();
    return {
...
// 創建模擬群組成員
export function createMockMember(id, name, role = "member", options = {}) {
    const now = Date.now();
    return {
        id: id || `member-${Date.now()}`,
        sessionId: options.sessionId || `session-${Math.random().toString(36).substr(2, 8)}`,
...
// 創建模擬購物車項目
export function createMockCartItem(id, memberId, options = {}) {
    const menuItems = [
        "招牌牛肉麵",
        "蒸餃",
        "炸醬麵",
    ...
    return {
        ...
        customizations: options.customizations || {
            spicy: ["不辣", "微辣", "小辣", "中辣", "大辣"][Math.floor(Math.random() * 5)],
...
// 創建模擬分帳記錄
export function createMockSplitBill(id, memberId, options = {}) {
    const subtotal = options.subtotal || Math.floor(Math.random() * 80) + 30;
    const taxRate = 0.06;
    const serviceChargeRate = 0.1;
    const taxAmount = options.taxAmount || Math.round(subtotal * taxRate * 100) / 100;
    const serviceCharge = options.serviceCharge ||
...
// 創建模擬訂單操作
export function createMockOperation(type = "add", entity = "cart_item", options = {}) {
    const entityIds = {
        member: "member-1",
        cart_item: "item-1",
        split_bill: "split-1",
...
// 根據實體類型生成模擬數據
function getMockDataForEntity(entity, operation) {
    switch (entity) {
        case "member":
            if (operation === "add") {
...
// 創建模擬協作動作
export function createMockCollaborativeAction(type = "editing", entityType = "cart_item", options = {}) {
    return {
        id: crypto.randomUUID(),
        type,
        entityType,
...
// 創建模擬錯誤
export function createMockError(type = "connection", severity = "medium", options = {}) {
    return {
        id: crypto.randomUUID(),
        type,
        severity,
...
// 創建模擬衝突警報
export function createMockConflictAlert(type = "edit_conflict", options = {}) {
    return {
        id: crypto.randomUUID(),
        type,
...
// 創建批量測試數據
export function createBulkMockData(count, type) {
    const results = [];
...
// 創建具有特定狀態的群組訂單
export function createMockGroupOrderWithStatus(status) {
    ...
    switch (status) {
        case "checkout":
            // 添加分帳記錄
            groupOrder.splitBills = groupOrder.members.map((member) => createMockSplitBill(undefined, member.id));
            break;
        case "completed":
            // 所有成員已付款
...
export const mockScenarios = {
    // 小型群組
    smallGroup: () => createMockGroupOrder("small-group", {
        members: createBulkMockData(3, "members"),
        cart: createBulkMockData(5, "cartItems"),
    }),
    // 大型群組
    largeGroup: () => createMockGroupOrder("large-group", {
        members: createBulkMockData(10, "members"),
        cart: createBulkMockData(25, "cartItems"),
    }),
    // 準備分帳的群組
