...
import { createMockGroupOrder, createMockMember, createMockCartItem, } from "../utils/mockData";
...
describe("實時群組訂單集成測試", () => {
    ...
    beforeAll(() => {
        ...
        global.WebSocket = mockWebSocket.constructor;
        // Mock 瀏覽器 API
        global.navigator = {
            ...global.navigator,
            onLine: true,
    ...
    describe("基礎實時同步功能", () => {
        ...
        it("應該正確處理成員加入事件", async () => {
            const { joinGroupOrder, currentGroupOrder, myMemberId } = useRealtimeGroupOrders();
            // 模擬加入群組成功響應
            ...
            const success = await joinGroupOrder("PARTY-ABC123", "測試用戶", "1234567890");
        ...
        it("應該正確廣播購物車變更", async () => {
            ...
            const addItemMessage = sentMessages.find((msg) => msg.type === "add_cart_item");
    ...
    describe("多用戶協作功能", () => {
        it("應該正確處理多用戶同時加入", async () => {
            ...
            // 初始化協作環境
            await collaborativeOrderService.initializeCollaboration(groupOrderId, testUsers[0].id, testUsers[0].name);
            await collaborativeOrderService.initializeCollaboration(groupOrderId, testUsers[1].id, testUsers[1].name);
            // 檢查在線用戶
            const onlineUsers = collaborativeOrderService.getOnlineUsers(groupOrderId);
            expect(onlineUsers).toHaveLength(2);
            expect(onlineUsers.map((u) => u.userId)).toEqual(expect.arrayContaining([testUsers[0].id, testUsers[1].id]));
        });
        it("應該正確處理編輯鎖定", async () => {
            const user1 = testUsers[0];
            const user2 = testUsers[1];
            // 用戶1請求鎖定
            const lock1Result = await collaborativeOrderService.requestEditLock(groupOrderId, "cart_item", "item-1", user1.id, user1.name, "write");
            expect(lock1Result.success).toBe(true);
            expect(lock1Result.lock).toBeDefined();
            // 用戶2嘗試請求相同鎖定（應該失敗）
            const lock2Result = await collaborativeOrderService.requestEditLock(groupOrderId, "cart_item", "item-1", user2.id, user2.name, "write");
        ...
        it("應該正確處理權限檢查", async () => {
            ...
            // 初始化協作環境
            await collaborativeOrderService.initializeCollaboration(groupOrderId, user1.id, user1.name);
            // 普通成員嘗試發起分帳（應該失敗）
            const user1Permission = await collaborativeOrderService.checkPermission(groupOrderId, "initiate_split", "split_bill", user1.id);
            expect(user1Permission.allowed).toBe(false);
            // 創建者發起分帳（應該成功）
            const adminPermission = await collaborativeOrderService.checkPermission(groupOrderId, "initiate_split", "split_bill", admin.id, { user: admin });
    ...
    describe("衝突解決機制", () => {
        ...
        it("應該處理併發編輯衝突", async () => {
            ...
            const user2 = testUsers[1];
            // 同時編輯相同項目
            const edit1Promise = collaborativeOrderService.handleRealtimeEdit(groupOrderId, "cart_item", "item-1", user1.id, { quantity: 3, customizations: { spicy: "hot" } });
            const edit2Promise = collaborativeOrderService.handleRealtimeEdit(groupOrderId, "cart_item", "item-1", user2.id, { quantity: 5, specialInstructions: "無蔥" });
    ...
    describe("錯誤處理和網絡彈性", () => {
        ...
        it("應該記錄和恢復錯誤", async () => {
            ...
            // 檢查恢復狀態
            const unrecoveredErrors = realtimeResilienceService.getUnrecoveredErrors();
    ...
    describe("複雜場景測試", () => {
        it("應該處理大型群組訂單場景", async () => {
            ...
            // 初始化協作環境
            for (let i = 0; i < largeGroupSize; i++) {
                await collaborativeOrderService.initializeCollaboration(groupOrderId, `user-${i}`, `用戶${i}`);
            }
            // 檢查系統性能
            ...
            const operations = [];
            for (let i = 0; i < largeGroupSize; i++) {
                operations.push(groupOrderBroadcastService.broadcastOperation(groupOrderId, {
                    type: "update",
                    entity: "cart_item",
                    entityId: `item-${i}-0`,
        ...
        it("應該處理網絡中斷和恢復場景", async () => {
            const { connectWebSocket, addCartItem, myMemberId, currentGroupOrder } = useRealtimeGroupOrders();
            // 建立連接並設置初始狀態
            await connectWebSocket(groupOrderId);
            ...
            // 驗證操作已同步
            const sentMessages = mockWebSocket.getSentMessages();
            const syncMessage = sentMessages.find((msg) => msg.type === "add_cart_item" && msg.data.menuItemName === "離線商品");
