...
describe("Queue Management Integration Tests", () => {
  ...
  describe("Queue Filtering and Search", () => {
    it("should filter queue by status", async () => {
      ...
      expect(filtered.every((item: any) => item.status === "waiting")).toBe(
        true,
      );
    });
    ...
    it("should sort queue by priority and time", () => {
      ...
      for (let i = 0; i < queue.length - 1; i++) {
        ...
        if (current.priority === next.priority) {
          expect(new Date(current.joinedAt).getTime()).toBeLessThanOrEqual(
            new Date(next.joinedAt).getTime(),
          );
        } else {
  ...
  describe("Table Management", () => {
    ...
    it("should filter tables by status", async () => {
      ...
      expect(filtered.every((table: any) => table.status === "available")).toBe(
        true,
      );
    });
  });
  ...
  describe("Error Handling", () => {
    it("should handle service errors gracefully", async () => {
      mockQueueService.getQueue.mockRejectedValue(
