...
try {
  ...
  // 驗證
  console.log('\n驗證結果:');
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_new' ORDER BY name").all();
  ...
  if (userCount.count > 0) {
    const samples = db.prepare("SELECT id, username, restaurant_id FROM users LIMIT 3").all();
    console.log('範例數據:');
    samples.forEach(u => {
      console.log(`  - id: ${u.id}, username: ${u.username}, restaurant_id: ${u.restaurant_id || 'NULL'} (type: ${typeof u.restaurant_id})`);
    });
  }
