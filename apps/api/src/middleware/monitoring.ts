 ...
 */
export type MonitoringServiceFactory = (
  kv: unknown,
...
export function errorMonitoringMiddleware(
  deps: MonitoringMiddlewareDependencies = {},
) {
  const createMonitoringService =
    deps.createMonitoringService ?? defaultCreateMonitoringService;
    ...
    if (c.env.NODE_ENV === "development") {
      try {
        ...
        c.header(
          ...
          metrics.apiMetrics.totalRequests.toString(),
        );
        c.header(
          "X-Monitoring-Errors",
          metrics.errorMetrics.totalErrors.toString(),
        );
        c.header(
          "X-Monitoring-Avg-Response",
          metrics.apiMetrics.averageResponseTime.toFixed(2),
        );
        c.header(
          "X-Monitoring-Cache-Hit-Rate",
          (metrics.cacheMetrics.hitRate * 100).toFixed(2),
        );
      } catch (error) {
