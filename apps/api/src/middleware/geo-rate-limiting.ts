...
const HIGH_RISK_COUNTRIES = new Set([
  'CN', 'RU', 'KP', 'IR', 'SY', 'AF', 'MM', 'BY', 'CU', 'VE'
])

// Known bot/hosting ASNs
const BOT_ASNS = new Set([
  '13335', '15169', '16509', '14061', '32934', '62240', '44273', '49505'
])
...
export class GeoIntelligentRateLimiter {
  ...
  calculateDynamicRateLimit(
    ...
    if (endpoint.includes('/auth/')) {
      baseConfig = { requests: 5, windowSeconds: 60, burstMultiplier: 1.2, blockDuration: 300 }
    } else if (endpoint.includes('/admin/') || endpoint.includes('/system/')) {
      baseConfig = { requests: 20, windowSeconds: 60, burstMultiplier: 1.5, blockDuration: 600 }
    } else if (endpoint.includes('/api/v1/orders') && request.method === 'POST') {
      baseConfig = { requests: 10, windowSeconds: 60, burstMultiplier: 2.0, blockDuration: 120 }
    } else if (endpoint.includes('/api/v1/menu')) {
      baseConfig = { requests: 100, windowSeconds: 60, burstMultiplier: 3.0, blockDuration: 60 }
    } else {
      baseConfig = { requests: 60, windowSeconds: 60, burstMultiplier: 2.0, blockDuration: 120 }
    }
    ...
    const finalRequests = Math.max(1, Math.floor(baseConfig.requests * riskMultiplier))
    
    return {
      requests: finalRequests,
      windowSeconds: baseConfig.windowSeconds,
      burstMultiplier: baseConfig.burstMultiplier,
      blockDuration: this.calculateBlockDuration(threatIntel, baseConfig.blockDuration)
    }
  }
  ...
  async applyRateLimit(
    ...
    const windowKeys = this.generateWindowKeys(identifier, rateLimit.windowSeconds, now)
    
    try {
      // Get request counts for the entire sliding window
      const counts = await Promise.all(
        windowKeys.map(key => this.rateLimitKV.get(key).then(val => parseInt(val || '0')))
      )
      
      const totalRequests = counts.reduce((sum, count) => sum + count, 0)
      const burstLimit = Math.ceil(rateLimit.requests * rateLimit.burstMultiplier)
      
      // Check if over limit
      if (totalRequests >= burstLimit) {
        // Record rate limit violation
        this.recordRateLimitViolation(request, identifier, totalRequests, burstLimit)
      ...
      await this.rateLimitKV.put(
        requestKey,
        (parseInt(await this.rateLimitKV.get(requestKey) || '0') + 1).toString(),
  ...
  async blockIdentifier(
    ...
    try {
      ...
      const currentLevel = parseInt(await this.rateLimitKV.get(escalationKey) || '0')
      const newLevel = currentLevel + 1
      
      // Escalating block durations
      const escalatedDuration = durationSeconds * Math.pow(2, Math.min(newLevel - 1, 5)) // Max 32x
      ...
      if (newLevel >= 3) {
        this.context.waitUntil(this.triggerSecurityAlert(identifier, blockData, threatIntel))
      }
  ...
  private extractThreatIntelligence(request: Request): ThreatIntelligence {
    ...
    if (threatScore > 80 || (threatScore > 60 && HIGH_RISK_COUNTRIES.has(country))) {
   ...
   */
  private assessGeographicRisk(threatIntel: ThreatIntelligence): GeoRiskProfile {
    ...
    if (HIGH_RISK_COUNTRIES.has(country) || threatScore > 70) {
      return {
        ...
        additionalChecks: ['captcha', 'device_fingerprint', 'behavioral_analysis']
      }
   ...
   */
  private assessEndpointRisk(endpoint: string): { multiplier: number; additionalChecks: string[] } {
    if (endpoint.includes('/auth/login') || endpoint.includes('/auth/register')) {
      return { multiplier: 0.5, additionalChecks: ['captcha', 'device_fingerprint'] }
    } else if (endpoint.includes('/admin/') || endpoint.includes('/system/')) {
   ...
   */
  private generateWindowKeys(identifier: string, windowSeconds: number, now: number): string[] {
    const keys: string[] = []
   ...
   */
  private calculateBlockDuration(threatIntel: ThreatIntelligence, baseDuration: number): number {
    let multiplier = 1.0
    ...
    if (this.analyticsEngine) {
      try {
        this.analyticsEngine.writeDataPoint({
          ...
          indexes: [blockData.escalationLevel.toString(), threatIntel.threatScore.toString()]
        })
      } catch (error) {
        console.error('Analytics error:', error)
      }
          ...
          body: JSON.stringify({
            ...
            attachments: [{
              ...
              fields: [
                ...
                { title: 'Threat Score', value: threatIntel.threatScore.toString(), short: true },
                { title: 'Escalation Level', value: blockData.escalationLevel.toString(), short: true },
                { title: 'Reason', value: blockData.reason, short: false },
                { title: 'Risk Factors', value: threatIntel.riskFactors.join(', '), short: false }
              ]
            }]
          })
        })
 ...
 */
export function geoIntelligentRateLimitMiddleware(options: {
  skipPaths?: string[]
  ...
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    const rateLimit = rateLimiter.calculateDynamicRateLimit(c.req.raw, path, user?.role)
    ...
    // Check rate limit
    const result = await rateLimiter.applyRateLimit(c.req.raw, rateLimit, identifier)
    
    if (!result.allowed) {
      // Extract threat intelligence for blocking decision
      const threatIntel = (rateLimiter as any).extractThreatIntelligence(c.req.raw)
      
      // Block high-threat sources
      if (threatIntel.threatScore > 70 || threatIntel.actionRecommendation === 'block') {
