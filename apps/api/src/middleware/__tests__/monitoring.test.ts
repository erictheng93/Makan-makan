...
const createMockMonitoringService = (): MonitoringServiceInterface => ({
  ...
  getHealthStatus: vi.fn().mockResolvedValue({ overall: 'healthy', components: {} }),
...
describe('Metrics Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app.use('*', metricsMiddleware({ createMonitoringService: mockServiceFactory }))
  ...
  it('should record slow request warning', async () => {
    ...
    slowApp.use('*', metricsMiddleware({ createMonitoringService: mockServiceFactory }))
...
describe('Error Monitoring Middleware', () => {
  ...
  it('should record database errors when using onError handler', async () => {
    ...
    app.onError(async (err, c) => {
      ...
      const errorType = err.message.toLowerCase().includes('database') ? 'database_error' : 'unknown_error'
      await service.recordError(errorType, err.message, 'critical')
      return c.text(err.message, 500)
    })

    app.use('*', errorMonitoringMiddleware({ createMonitoringService: mockServiceFactory }))
    app.get('/db-error', () => {
  ...
  it('should record auth errors when using onError handler', async () => {
    ...
    app.onError(async (err, c) => {
      const service = mockServiceFactory(c.env.CACHE_KV)
      const errorType = err.message.toLowerCase().includes('unauthorized') ? 'auth_error' : 'unknown_error'
  ...
  it('should record timeout errors when using onError handler', async () => {
    ...
    app.onError(async (err, c) => {
      const service = mockServiceFactory(c.env.CACHE_KV)
      const errorType = err.message.toLowerCase().includes('timeout') ? 'timeout_error' : 'unknown_error'
      await service.recordError(errorType, err.message, 'critical')
      return c.text(err.message, 500)
    })
  ...
  it('should record validation errors when using onError handler', async () => {
    ...
    app.onError(async (err, c) => {
      const service = mockServiceFactory(c.env.CACHE_KV)
      const errorType = err.message.toLowerCase().includes('validation') ? 'validation_error' : 'unknown_error'
      await service.recordError(errorType, err.message, 'warning')
      return c.text(err.message, 500)
    })
...
describe('Health Check Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app = new Hono<{ Bindings: typeof mockEnv }>()
    app.use('*', healthCheckMiddleware({ createMonitoringService: mockServiceFactory }))
    app.get('/health', (c) => {
  ...
  it('should handle health check errors with onError', async () => {
    ;(mockService.getHealthStatus as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('Health check failed'))
    ...
    testApp.use('*', healthCheckMiddleware({ createMonitoringService: mockServiceFactory }))
...
describe('Cache Monitoring Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app = new Hono<{ Bindings: typeof mockEnv }>()
    app.use('*', cacheMonitoringMiddleware({ createMonitoringService: mockServiceFactory }))
  ...
  it('should track cache misses', async () => {
    ...
    missApp.use('*', cacheMonitoringMiddleware({ createMonitoringService: mockServiceFactory }))
...
describe('Monitoring Stats Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app = new Hono<{ Bindings: typeof mockEnv }>()
    app.use('*', monitoringStatsMiddleware({ createMonitoringService: mockServiceFactory }))
    app.get('/test', (c) => c.json({ success: true }))
  })
  ...
  it('should not add headers in production', async () => {
    ...
    prodApp.use('*', monitoringStatsMiddleware({ createMonitoringService: mockServiceFactory }))
...
describe('Performance Monitoring Middleware', () => {
  ...
  beforeEach(() => {
    ...
    app = new Hono<{ Bindings: typeof mockEnv }>()
    app.use('*', performanceMonitoringMiddleware({ createMonitoringService: mockServiceFactory }))
    app.get('/test', (c) => c.json({ success: true }))
  })
