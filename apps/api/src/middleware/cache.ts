...
import {
  ...
  CACHE_STRATEGIES,
  CacheKeys,
} from "../features/cache/services/CacheService";
  ...
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    if (
      ...
      try {
        ...
        // 記錄失效操作
        console.log(
          `Cache invalidated: ${invalidatedCount} keys for tags: ${tags.join(", ")}`,
        );
  ...
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    try {
      ...
      if (warmupData.length > 0) {
        ...
        console.log(
          `Cache warmup completed: ${successCount}/${warmupData.length} keys`,
        );
  ...
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    try {
      ...
      if (stats.averageHitRate < 0.5) {
        console.warn(
          `Low cache hit rate: ${(stats.averageHitRate * 100).toFixed(2)}%`,
        );
      }
...
): Promise<Array<{ key: string; value: unknown; config: CacheConfig }>> {
  const warmupData: Array<{
    key: string;
    value: unknown;
    config: CacheConfig;
  }> = [];
 ...
 * 餐廳失效裝飾器
 */
export const invalidateRestaurantCache = cacheInvalidationMiddleware([
  "restaurant",
]);
 ...
 * 分析數據失效裝飾器
 */
export const invalidateAnalyticsCache = cacheInvalidationMiddleware([
  "analytics",
]);
