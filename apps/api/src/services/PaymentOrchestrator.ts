...
export class PaymentOrchestrator {
  ...
  async processPayment(request: PaymentRequest): Promise<PaymentResult> {
    ...
    try {
      ...
      if (!countryConfig) {
        throw new Error(`No payment configuration found for country: ${request.country}`)
      }

      // 驗證金額範圍
      if (request.amount < countryConfig.minimumAmount || request.amount > countryConfig.maximumAmount) {
      ...
      // 嘗試主要提供商
      try {
        const result = await this.executePaymentWithTimeout(primaryProvider, request)
        
        // 記錄成功的支付
        await this.logPaymentAttempt(request, primaryProvider.name, result, Date.now() - startTime)
      ...
      // 如果啟用了容錯機制，嘗試備用提供商
      if (this.config.fallbackEnabled && countryConfig.fallbackProviders.length > 0) {
        for (const fallbackProviderName of countryConfig.fallbackProviders) {
          ...
          // 檢查該提供商是否支援此支付方式
          if (!fallbackProvider.supportedMethods.includes(request.method)) continue

          try {
            const result = await this.executePaymentWithTimeout(fallbackProvider, request)
            
            // 記錄成功的備用支付
            await this.logPaymentAttempt(request, fallbackProvider.name, result, Date.now() - startTime, true)
      ...
      await this.logPaymentAttempt(request, 'none', failureResult, Date.now() - startTime)
      ...
      await this.logPaymentAttempt(request, 'none', errorResult, Date.now() - startTime)
      return errorResult
    }
  }

  async getPaymentStatus(transactionId: string, providerId?: string): Promise<PaymentStatus> {
    if (providerId) {
  ...
  async refundPayment(request: RefundRequest, providerId: string): Promise<RefundResult> {
    const provider = this.providers.get(providerId)
    if (!provider) {
      throw new Error(`Provider not found: ${providerId}`)
    }
  ...
  private selectProvider(request: PaymentRequest, config: CountryPaymentConfig): PaymentProvider {
    // 首先檢查是否有針對特定支付方式的偏好
    const methodSpecificProvider = this.getMethodSpecificProvider(request.method, request.country)
    ...
    if (!primaryProvider.supportedMethods.includes(request.method)) {
      throw new Error(`Primary provider ${config.primaryProvider} does not support method: ${request.method}`)
    }

    return primaryProvider
  }

  private getMethodSpecificProvider(method: PaymentMethod, country: CountryCode): PaymentProvider | null {
    // 針對特定支付方式的提供商映射
  ...
  private async executePaymentWithTimeout(
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(new Error(`Payment timeout after ${this.config.defaultTimeout}ms`))
  ...
  private async logPaymentAttempt(
    ...
    console.log(`Payment attempt - Provider: ${providerId}, Success: ${result.success}, Duration: ${duration}ms, Fallback: ${isFallback}`)
  }
  ...
  private async logRefundAttempt(
    ...
    // TODO: 實作退款日誌記錄
    console.log(`Refund attempt - Provider: ${providerId}, Success: ${result.success}`)
  }
