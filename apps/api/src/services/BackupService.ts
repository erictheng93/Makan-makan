...
export class BackupService {
  ...
  private async executeBackup(backupId: string): Promise<void> {
    try {
      ...
      const storagePath = this.generateStoragePath(backup.restaurant_id, backupId)
      ...
      await this.createAuditLog({
        ...
        details: { backup_id: backupId, tables_count: backup.tables_included.length },
  ...
  async listBackups(query: ListBackupsQuery): Promise<{ backups: BackupRecord[], total: number }> {
    ...
    try {
      ...
      const countResult = await this.db.prepare(countSql).bind(restaurant_id).first()
   ...
   * Restore from backup
   */
  async restoreBackup(request: RestoreBackupRequest, userId: string): Promise<{ operation_id: string }> {
  ...
  async createOrUpdateConfiguration(configInput: any, userId: string): Promise<BackupConfiguration> {
  ...
  async getRestaurantMetrics(restaurantId: string, timeframe?: string): Promise<any> {
    console.log('Getting metrics for restaurant:', restaurantId, 'timeframe:', timeframe)
    return {}
  }

  async getRestaurantAlerts(restaurantId: string, filters?: any): Promise<BackupAlert[]> {
    console.log('Getting alerts for restaurant:', restaurantId, 'filters:', filters)
    return []
  }

  async createAlertPublicPublic(alert: Partial<BackupAlert>, context?: any): Promise<void> {
  ...
  private async updateBackupStatus(backupId: string, status: BackupStatus): Promise<void> {
    await this.db.prepare(
      `UPDATE backups SET status = ?, updated_at = ? WHERE id = ?`
