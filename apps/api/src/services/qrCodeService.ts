  ...
  async generateQRCode(options: QRCodeOptions): Promise<{
    ...
    try {
      ...
      if (this.needsAdvancedGeneration(style)) {
        finalUrl = await this.generateAdvancedQRCode(
          options.content,
          style,
          options.format,
          qrId,
    ...
    try {
      ...
      for (const table of options.tables) {
        ...
        const qrCodeData: CreateQRCodeData = {
          ...
          format: (options.format === "zip" ? "png" : options.format) as
            | "png"
            | "svg"
            | "pdf"
            | "jpeg",
  ...
  async createTemplate(
    ...
    try {
      ...
      const newTemplate: QRTemplate = {
        ...
        previewUrl,
        createdAt:
          typeof createdTemplate.createdAt === "string"
            ? createdTemplate.createdAt
            : new Date(createdTemplate.createdAt).toISOString(),
        updatedAt:
          typeof createdTemplate.updatedAt === "string"
            ? createdTemplate.updatedAt
            : new Date(createdTemplate.updatedAt).toISOString(),
      };
