...
describe('RealtimeBroadcastService', () => {
  ...
  describe('broadcastEvent', () => {
    it('應該成功廣播事件到 Durable Object', async () => {
      ...
      const result = await service.broadcastEvent('restaurant', 'rest_1', mockEvent)

      expect(result.success).toBe(true)
      expect(result.eventId).toBe('evt_123')
      expect(result.recipientCount).toBe(5)
      expect(mockDurableObjectNamespace.idFromName).toHaveBeenCalledWith('restaurant:rest_1')
  ...
  describe('broadcastNewOrder', () => {
    it('應該廣播新訂單事件到餐廳房間', async () => {
      ...
      mockDurableObjectStub.fetch.mockResolvedValue(
        new Response(JSON.stringify({ success: true, eventId: 'evt_new_123', recipientCount: 3 }))
      )

      const result = await service.broadcastNewOrder(newOrderEvent)

      expect(result.success).toBe(true)
      expect(mockDurableObjectNamespace.idFromName).toHaveBeenCalledWith('restaurant:rest_123')
    })
  })
  ...
  describe('broadcastOrderStatusUpdate', () => {
    it('應該廣播訂單狀態更新事件', async () => {
      ...
      mockDurableObjectStub.fetch.mockResolvedValue(
        new Response(JSON.stringify({ success: true, eventId: 'evt_update_456', recipientCount: 4 }))
      )

      const result = await service.broadcastOrderStatusUpdate(updateEvent)

      expect(result.success).toBe(true)
      expect(mockDurableObjectNamespace.idFromName).toHaveBeenCalledWith('restaurant:rest_456')
    })
  })
  ...
  describe('broadcastOrderItemStatusUpdate', () => {
    it('應該廣播訂單項目狀態更新', async () => {
      ...
      mockDurableObjectStub.fetch.mockResolvedValue(
        new Response(JSON.stringify({ success: true, eventId: 'evt_item_789', recipientCount: 2 }))
      )

      const result = await service.broadcastOrderItemStatusUpdate(itemUpdateEvent as any)
  ...
  describe('broadcastKitchenItemStatus', () => {
    it('應該廣播廚房項目狀態', async () => {
      ...
      mockDurableObjectStub.fetch.mockResolvedValue(
        new Response(JSON.stringify({ success: true, eventId: 'evt_kitchen_101', recipientCount: 1 }))
      )

      const result = await service.broadcastKitchenItemStatus(kitchenEvent as any)
  ...
  describe('broadcastMenuAvailabilityUpdate', () => {
    it('應該廣播菜單可用性更新', async () => {
      ...
      mockDurableObjectStub.fetch.mockResolvedValue(
        new Response(JSON.stringify({ success: true, eventId: 'evt_menu_202', recipientCount: 10 }))
      )

      const result = await service.broadcastMenuAvailabilityUpdate(menuEvent as any)
