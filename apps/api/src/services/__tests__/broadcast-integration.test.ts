 ...
 * 測試訊息廣播完整流程（API → Durable Object → WebSocket）
 */
...
describe('Broadcast Integration Tests', () => {
  ...
  describe('新訂單廣播', () => {
    it('應該成功廣播新訂單事件', async () => {
      ...
      // Simulate broadcast
      const durableObjectId = mockEnv.REALTIME_SESSION.idFromName('restaurant:rest_1')
      const durableObjectStub = mockEnv.REALTIME_SESSION.get(durableObjectId)

      const response = await durableObjectStub.fetch('http://localhost/broadcast', {
        method: 'POST',
      ...
      const events = [
        {
          ...
          timestamp: Date.now(),
          restaurantId: 'rest_1',
          data: { orderId: 1, orderNumber: 'ORD-001', items: [], totalAmount: 10 }
        },
        {
          type: RealtimeEventType.NEW_ORDER,
          eventId: 'evt_2',
          timestamp: Date.now(),
          restaurantId: 'rest_1',
          data: { orderId: 2, orderNumber: 'ORD-002', items: [], totalAmount: 20 }
        },
        {
          type: RealtimeEventType.NEW_ORDER,
          eventId: 'evt_3',
          timestamp: Date.now(),
          restaurantId: 'rest_1',
          data: { orderId: 3, orderNumber: 'ORD-003', items: [], totalAmount: 30 }
        }
      ]
