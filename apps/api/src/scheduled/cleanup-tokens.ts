...
export async function cleanupExpiredTokens(env: Env): Promise<CleanupResult> {
  ...
  try {
    ...
    if (env.SLACK_WEBHOOK_URL || env.ALERT_EMAIL) {
...
async function getCleanupStats(db: D1Database): Promise<{
  ...
  try {
    ...
    const [passwordResetResult, emailVerificationResult, phoneVerificationResult] =
      await Promise.all([
        db.prepare('SELECT COUNT(*) as count FROM password_reset_tokens WHERE expires_at < ?').bind(now).first<{ count: number }>(),
        db.prepare('SELECT COUNT(*) as count FROM email_verification_tokens WHERE expires_at < ?').bind(now).first<{ count: number }>(),
        db.prepare('SELECT COUNT(*) as count FROM phone_verification_tokens WHERE expires_at < ?').bind(now).first<{ count: number }>(),
      ])
 ...
 */
export async function cleanupOldLogs(env: CloudflareEnv): Promise<void> {
  try {
    const ninetyDaysAgo = Math.floor(Date.now() / 1000) - 90 * 24 * 60 * 60

    await env.DB.prepare('DELETE FROM password_change_logs WHERE created_at < ?')
      .bind(ninetyDaysAgo)
      .run()
