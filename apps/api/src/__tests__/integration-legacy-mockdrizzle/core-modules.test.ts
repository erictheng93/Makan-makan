...
describe("Core Modules Integration Tests", () => {
  ...
  describe("Menu and Order Integration", () => {
    it("should create menu items and process orders end-to-end", async () => {
      ...
      const categoryResponse = await app.request(
        `/api/v1/menu/${testRestaurantId}/categories`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      // 2. Create menu item
      const menuItemResponse = await app.request(
        `/api/v1/menu/${testRestaurantId}/items`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const kitchenResponse = await app.request(
        `/api/v1/kitchen/${testRestaurantId}/orders`,
        {
  ...
  describe("Queue and Table Integration", () => {
    it("should integrate queue system with table management", async () => {
      ...
      if (mockDrizzle) {
        ...
        console.log(
          "[TEST] Enhanced MockDrizzle with TableStore for this test",
        );
      }
      ...
      const testRootResponse = await app.request(
        "/api/v1/tables/test-root-no-auth",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      // 0c. Test with auth
      const testAuthResponse = await app.request(
        "/api/v1/tables/test-with-auth",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const testGetResponse = await app.request(
        "/api/v1/tables/?restaurantId=1",
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${authToken}`,
      ...
      const callResponse = await app.request(
        `/api/v1/queue/${testRestaurantId}/call-next`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const tableCheckBeforeSeat = await app.request(
      ...
      console.log(
        "[TEST] Table check BEFORE seat, status:",
        tableCheckBeforeSeat.status,
      );
      ...
      const tableStatusResponse = await app.request(
        `/api/v1/tables/${tableId}`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        },
      ...
      console.log(
        "[TEST] Table isOccupied status:",
        updatedTableData.data?.isOccupied,
      );
    });
  });
  ...
  describe("Analytics and Reporting Integration", () => {
    it("should generate analytics from order and queue data", async () => {
      ...
      const reportsResponse = await app.request(
        `/api/v1/analytics/revenue?restaurantId=${testRestaurantId}&period=today`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        },
      );
  ...
  describe("Error Handling Integration", () => {
    it("should handle cross-module errors gracefully", async () => {
      ...
      if (mockDrizzle) {
        ...
        console.log(
          "[TEST] Enhanced MockDrizzle with RestaurantStore for this test",
        );
      }
  ...
  describe("Data Consistency Integration", () => {
    it("should maintain data consistency across module operations", async () => {
      ...
      console.log(
        "[TEST] Initial kitchen orders:",
        kitchenOrders.map((o: any) => ({ id: o.id, status: o.status })),
      );

      // 3. Cancel order and verify consistency
      const cancelResponse = await app.request(
        `/api/v1/orders/${orderData.orderId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${authToken}` },
        },
