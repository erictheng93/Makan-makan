...
export function enhanceMockDrizzle(
  ...
  if (tableStore && originalQuery?.tables?.findFirst) {
    ...
    originalQuery.tables.findFirst = vi.fn(async (options: any) => {
      console.log('[EnhancedMockDrizzle] tables.findFirst() called with:', options)
  ...
  if (restaurantStore && originalQuery?.restaurants?.findFirst) {
    ...
    originalQuery.restaurants.findFirst = vi.fn(async (options: any) => {
      console.log('[EnhancedMockDrizzle] restaurants.findFirst() called with:', options)
      ...
      if (options?.where) {
        ...
        if (idMatch) {
          ...
          console.log('[EnhancedMockDrizzle] Extracted restaurant ID:', restaurantId)

          if (!restaurantStore.hasRestaurant(restaurantId)) {
            console.log('[EnhancedMockDrizzle] Restaurant not found, returning null')
            return null
          }
        ...
        whereChain.where = vi.fn((condition: any) => {
          ...
          getChain.get = vi.fn(async () => {
            if (tableName.includes('tables')) {
              ...
              if (idMatch) {
                const tableId = parseInt(idMatch[1], 10)
                console.log('[EnhancedMockDrizzle] select().where().get() for table ID:', tableId)
                const table = tableStore.getTable(tableId)
                if (table) {
                  console.log('[EnhancedMockDrizzle] Returning table from store:', !!table)
                  return table
                }
              }
            }
  ...
  if (tableStore && mockDrizzle.update) {
    ...
    mockDrizzle.update = vi.fn((table: any) => {
      const tableName = String(table)
      console.log('[EnhancedMockDrizzle] update() called for table:', tableName)
      ...
      setChain.set = (data: any) => {
        ...
        whereChain.where = (condition: any) => {
          console.log('[EnhancedMockDrizzle] where() called with condition:', String(condition))
          ...
          if (tableName.includes('tables')) {
            ...
            if (idMatch) {
              const tableId = parseInt(idMatch[1], 10)
              console.log('[EnhancedMockDrizzle] Extracted table ID for update:', tableId)
              ...
              if (existingTable) {
                ...
                console.log('[EnhancedMockDrizzle] Updated table in store:', tableId)
              }
            }
          }
