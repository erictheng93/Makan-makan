...
function parseInsertValues(sql: string, params: any[]): any {
  ...
  console.log(
    "[parseInsertValues] Params count:",
    params.length,
    "values:",
    ...
    prepare: (sql: string) => {
      ...
      return {
        bind: (...params: any[]) => {
          ...
          console.log(
            "[MockDB] bind() call stack:",
            new Error().stack?.split("\n").slice(2, 5).join("\n"),
          );
          const boundStatement = {
            run: async () => {
              console.log(
                "[MockDB] bind().run() called for INSERT:",
                isInsert,
                "RETURNING:",
                isReturning,
              );
            ...
            all: async () => {
              console.log("[MockDB] bind().all() called for", {
                isInsert,
                isReturning,
                isSelect,
              });
              ...
              if (
                isSelect &&
                (sql.includes("count(") || sql.includes("sum("))
              ) {
                const tableData = tables.get(tableName);
            ...
            raw: async () => {
              console.log("[MockDB] bind().raw() called for", {
                isInsert,
                isReturning,
                isSelect,
              });
              ...
              if (!tableData || tableData.size === 0) {
                ...
                if (
                  isSelect &&
                  (sql.includes("sum(") || sql.includes("count("))
                ) {
              ...
              return Array.from(tableData.values()).map((row) =>
                Object.values(row),
              );
            },
          };
          ...
          return new Proxy(boundStatement, {
            get(target: any, prop: string) {
              console.log(
                `[MockDB] Accessing property on bound statement: ${String(prop)}`,
              );
              if (prop in target) {
                return target[prop];
              }
              console.log(
                `[MockDB] WARNING: Unexpected property/method: ${String(prop)}`,
              );
              return async () => {
                console.log(
                  `[MockDB] Executing unexpected method: ${String(prop)}`,
                );
