...
export class SystemHealthMonitor {
   ...
   */
  async performHealthCheck(includeDetailed: boolean = false): Promise<HealthStatus> {
  ...
  private async checkSystem(): Promise<{ status: 'pass' | 'fail'; message?: string; responseTime?: number }> {
  ...
  private async checkDatabase(): Promise<{ status: 'pass' | 'fail'; message?: string; responseTime?: number }> {
    const start = Date.now()
  ...
  private async checkCache(): Promise<{ status: 'pass' | 'fail'; message?: string; responseTime?: number }> {
    const start = Date.now()
  ...
  private async checkStorage(): Promise<{ status: 'pass' | 'fail'; message?: string; responseTime?: number }> {
    const start = Date.now()
  ...
  private async checkNotifications(): Promise<{ status: 'pass' | 'fail'; message?: string; responseTime?: number }> {
    const start = Date.now()
    ...
    try {
      ...
      return {
        status: 'fail',
        message: error instanceof Error ? error.message : 'Notification service check failed',
        responseTime: Date.now() - start
      }
    }
  }

  private async checkFeatureModules(): Promise<{ status: 'pass' | 'fail'; message?: string; details?: any }> {
    try {
      const featureStatus = {
      ...
      return {
        status: 'fail',
        message: error instanceof Error ? error.message : 'Feature modules check failed'
      }
    }
  }
