...
describe.skip("POS API Integration Tests", () => {
  ...
  describe("GET /api/v1/pos/stats/shifts - 獲取班次統計", () => {
    it("應該返回班次統計（管理員）", async () => {
      const res = await app.request("/api/v1/pos/stats/shifts?restaurantId=1", {
        method: "GET",
        headers: {
          Authorization: `Bearer ${await createTestToken({ role: 0 })}`,
        },
...
const TEST_JWT_SECRET =
  "test-jwt-secret-for-pos-integration-tests-minimum-32-chars";
    ...
    prepare: (query: string) => ({
      bind: (...args: any[]) => ({
        ...
        all: async () => ({
          results: Array.from(
            mockData[
              extractTableName(query) as keyof typeof mockData
            ]?.values() || [],
          ),
        }),
