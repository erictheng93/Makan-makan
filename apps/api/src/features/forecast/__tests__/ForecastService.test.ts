...
describe("ForecastService", () => {
  ...
  describe("generateForecast", () => {
    ...
    it("should re-normalize weights when only 2 weeks of data exists", async () => {
      ...
      const item = results[0].items[0];
      // 2 weeks: weights 0.4, 0.3 → normalized by (0.4+0.3)=0.7
      // WMA base = (60*0.4 + 40*0.3) / 0.7 = (24+12)/0.7 = 51.43
      // Trend: recentAvg=60, olderAvg=40 (only 1 in older), trendPercent=50%
      // adjusted = 51.43 * (1 + 0.5 * 0.5) = 51.43 * 1.25 = 64.29
    ...
    it("should fall back to stale cache when generation fails", async () => {
      ...
      mockDb.prepare.mockImplementation(() => {
        callCount++;
        // First call: getHistoricalSales → throw error
        if (callCount === 1) {
          return {
            bind: vi.fn().mockReturnValue({
        ...
        // Second call: getStaleCache → return stale D1 record
        return {
          bind: vi.fn().mockReturnValue({
    ...
    it("should persist forecast to D1 via saveForecastToDb", async () => {
      ...
      // Should have called prepare for INSERT OR REPLACE
  ...
  describe("getForecast", () => {
    ...
    it("should skip expired D1 record and trigger on-demand generation", async () => {
      ...
      mockDb.prepare.mockImplementation(() => {
        callCount++;
        if (callCount === 1) {
          // First call: getForecast D1 lookup → expired record
          return {
            bind: vi.fn().mockReturnValue({
              first: vi.fn().mockResolvedValue({
                data: JSON.stringify({
                ...
                generated_by: "statistical",
                expires_at_ms: Date.now() - 86400000, // expired yesterday
              }),
            }),
          };
        }
        // Subsequent calls: on-demand generateForecast
        return {
          bind: vi.fn().mockReturnValue({
            all: vi.fn().mockResolvedValue({ results: [] }),
            run: vi.fn().mockResolvedValue({ success: true }),
            ...
            bind: vi.fn().mockReturnValue({
              all: vi.fn().mockResolvedValue({
                results: [
                  {
                    forecast_date: "2026-03-10",
                    data: JSON.stringify({ "1": { predicted: 0 } }),
                  },
                ],
              }),
            }),
            ...
            bind: vi.fn().mockReturnValue({
              all: vi.fn().mockResolvedValue({
                results: [
                  {
                    forecast_date: "2026-03-10",
                    data: JSON.stringify({ "1": { predicted: 50 } }),
                  },
                ],
              }),
            }),
            ...
            bind: vi.fn().mockReturnValue({
              all: vi.fn().mockResolvedValue({
                results: [
                  {
                    forecast_date: "2026-03-10",
                    data: JSON.stringify({ "999": { predicted: 50 } }),
                  },
                ],
              }),
            }),
  ...
  describe("getAlerts", () => {
    ...
    it("should generate high_demand alert when predicted > 30 and confidence >= 0.7", async () => {
      ...
      mockDb.prepare.mockReturnValue({
        bind: vi.fn().mockReturnValue({
          all: vi.fn().mockResolvedValue({
            results: [{ id: 1, name: "雞排", inventory_count: 100 }],
          }),
        }),
      });
    ...
    it("should generate low_stock alert when predicted exceeds inventory", async () => {
      ...
      mockDb.prepare.mockReturnValue({
        bind: vi.fn().mockReturnValue({
          all: vi.fn().mockResolvedValue({
            results: [{ id: 1, name: "雞排", inventory_count: 10 }],
          }),
        }),
      });
    ...
    it("should generate unusual_spike alert when predicted > 1.5x historicalAvg", async () => {
      ...
      mockDb.prepare.mockReturnValue({
        bind: vi.fn().mockReturnValue({
          all: vi.fn().mockResolvedValue({
            results: [{ id: 1, name: "雞排", inventory_count: 200 }],
          }),
        }),
      });
    ...
    it("should sort alerts by severity: critical > warning > info", async () => {
      ...
      mockDb.prepare.mockReturnValue({
        bind: vi.fn().mockReturnValue({
          all: vi.fn().mockResolvedValue({
            results: [
              { id: 1, name: "雞排", inventory_count: 5 }, // low_stock critical (60 > 5*2)
              { id: 2, name: "滷肉飯", inventory_count: 200 }, // high_demand warning (50 > 50? no, ==50)
            ],
          }),
        }),
      });
  ...
  describe("KV cache failure fallback", () => {
    it("getForecast propagates error when KV.get throws (no silent fallback)", async () => {
      // The service does NOT catch KV errors internally — they propagate to the caller
    ...
    it("generateForecast falls back to stale cache when KV.put throws", async () => {
      // KV.put fails → error caught → getStaleCache called → returns stale result
      ...
      mockDb.prepare.mockImplementation(() => {
        dbCallCount++;
        if (dbCallCount === 1) {
          // getHistoricalSales → no data (empty results for .all())
          return {
            bind: vi.fn().mockReturnValue({
              all: vi.fn().mockResolvedValue({ results: [] }),
            }),
      ...
      // At least one result returned (stale fallback) — even if the flow
      // produces multiple from both the normal path and the stale path,
      // the stale flag must be present on at least one result
  ...
  describe("getAccuracy DB chunking", () => {
    it("should chunk IDs when there are exactly 90 items (single chunk)", async () => {
      ...
      // 90 items → 1 chunk query (call #2) + 1 actuals query (call #3) = callCount=3 total
      // callCount - 1 (forecast) - 1 (actuals) = 1 name-chunk call
      expect(callCount).toBe(3); // forecast + 1 name chunk + actuals
    ...
    it("should make 2 chunk queries when there are 91 items (crosses chunk boundary)", async () => {
      ...
      // 91 items → 2 chunk queries (90 + 1) + 1 actuals query = callCount=4 total
      // 1 forecast query + 2 name chunk queries + 1 actuals query
        ...
        bind: vi.fn().mockReturnValue({
          all: vi.fn().mockResolvedValue({
            results: [
              {
                forecast_date: "2026-03-10",
                data: JSON.stringify({}), // empty data dict
              },
            ],
          }),
        }),
            ...
            bind: vi.fn().mockReturnValue({
              all: vi.fn().mockResolvedValue({
                results: [
                  {
                    forecast_date: "2026-03-10",
                    data: JSON.stringify({ "42": { predicted: 20 } }),
                  },
                ],
              }),
            }),
