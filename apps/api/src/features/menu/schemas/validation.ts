...
export const menuImportSchema = z.object({
  categories: z.array(createCategorySchema.omit({ sortOrder: true }).extend({
...
export const menuExportQuerySchema = z.object({
  ...
  includeImages: z.string().transform(val => val === 'true').optional().default('false'),
...
export const validateMenuItemAvailability = (item: any) => {
  ...
  if (item.availableHours) {
    ...
    if (item.availableHours.start && item.availableHours.end) {
      const [startHour, startMinute] = item.availableHours.start.split(':').map(Number)
      const [endHour, endMinute] = item.availableHours.end.split(':').map(Number)
    ...
    if (item.availableHours.days && !item.availableHours.days.includes(now.getDay())) {
      return false
    }
  }
  return true
...
export const validatePriceConsistency = (price: number, originalPrice?: number) => {
...
export const validateCustomizationOptions = (options: any) => {
  ...
  if (options.customizations) {
    for (const customization of options.customizations) {
      if (customization.required && customization.type === 'single') {
        const hasDefault = customization.choices.some((choice: any) => choice.isDefault)
        if (!hasDefault) {
          throw new Error(`Required customization "${customization.name}" must have a default choice`)
        }
      }
    }
  }
  ...
  if (options.sizes && options.sizes.length > 1) {
    ...
    if (defaultSizes.length !== 1) {
      throw new Error('Exactly one size must be marked as default when multiple sizes are available')
    }
  }
...
export const validateCompleteMenuItem = createMenuItemSchema.refine(
  (data) => validatePriceConsistency(data.price, data.originalPrice),
  { message: "Price validation failed" }
).refine(
  (data) => validateCustomizationOptions(data.options),
