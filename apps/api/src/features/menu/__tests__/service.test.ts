...
vi.mock('@makanmakan/database', () => ({
  MenuService: vi.fn(() => mockDbService)
}))
...
describe('MenuService', () => {
  ...
  describe('getMenu', () => {
    ...
    it('should handle cache store errors gracefully', async () => {
      ;(mockEnv.CACHE_KV.get as any).mockResolvedValue(null)
      ;(mockEnv.CACHE_KV.put as any).mockRejectedValue(new Error('Cache store error'))
      mockDbService.getMenu.mockResolvedValue(mockMenuStructure)
  ...
  describe('createMenuItem', () => {
    ...
    it('should validate category access', async () => {
      ...
      await expect(menuService.createMenuItem(createData))
  ...
  describe('updateMenuItem', () => {
    ...
    beforeEach(() => {
      mockDbService.getMenuItem.mockResolvedValue(mockMenuItem)
      mockDbService.updateMenuItem.mockResolvedValue({ ...mockMenuItem, ...updateData })
    })
    ...
    it('should throw when item not found', async () => {
      ...
      await expect(menuService.updateMenuItem(999, updateData))
    ...
    it('should validate new category when changing', async () => {
      ...
      await expect(menuService.updateMenuItem(1, { categoryId: 2 }))
        .rejects.toThrow('Category does not belong to the specified restaurant')
    })
  ...
  describe('deleteMenuItem', () => {
    beforeEach(() => {
      mockDbService.getMenuItem.mockResolvedValue(mockMenuItem)
      mockDbService.updateMenuItem.mockResolvedValue({ ...mockMenuItem, isAvailable: false })
    })
  ...
  describe('updateCategory', () => {
    ...
    it('should throw when category not found', async () => {
      ...
      await expect(menuService.updateCategory(999, updateData))
        .rejects.toThrow('Category not found')
    })
  ...
  describe('deleteCategory', () => {
    ...
    it('should delete empty category', async () => {
      mockDbService.searchMenuItems.mockResolvedValue({ items: [], pagination: {} })
    ...
    it('should throw when category has items', async () => {
      ...
      await expect(menuService.deleteCategory(1))
        .rejects.toThrow('Cannot delete category that contains menu items')
    })
  })
  ...
  describe('batchUpdateAvailability', () => {
    it('should batch update availability', async () => {
      ...
      expect(mockDbService.batchUpdateAvailability).toHaveBeenCalledWith('1', updates)
    })
  ...
  describe('batchMoveItems', () => {
    ...
    it('should throw when category not found', async () => {
      ...
      await expect(menuService.batchMoveItems(1, [{ id: 1, categoryId: 99 }]))
        .rejects.toThrow('Category not found')
    })
  ...
  describe('getMenuAnalytics', () => {
    beforeEach(() => {
      ...
      mockDbService.getMenu.mockResolvedValue({
        ...
        menuItems: [
          ...
          { ...mockMenuItem, id: 3, dietaryInfo: { vegetarian: true, vegan: true } }
        ]
      })
    })
    ...
    it('should throw when menu not found', async () => {
      ...
      await expect(menuService.getMenuAnalytics(999))
        .rejects.toThrow('Menu not found for restaurant')
    })
  })
  ...
  describe('cache invalidation', () => {
    it('should handle cache deletion error gracefully', async () => {
      ;(mockEnv.CACHE_KV.delete as any).mockRejectedValue(new Error('Cache error'))
      mockDbService.getMenuItem.mockResolvedValue(mockMenuItem)
      mockDbService.updateMenuItem.mockResolvedValue(mockMenuItem)
    ...
    it('should skip cache operations when cache service unavailable', async () => {
      const serviceWithoutCache = new MenuService({ ...mockEnv, CACHE_KV: undefined } as any)
