...
export interface ISystemService {
  createErrorReport(data: ErrorReportRequest, userId: number, restaurantId: number | null, userAgent?: string): Promise<ErrorReportResponse>
  getSystemHealth(): Promise<SystemHealthResponse>
  getErrorStats(restaurantId?: number): Promise<ErrorStats>
  cleanupOldErrorReports(daysOld?: number): Promise<CleanupResponse>
  sendCriticalErrorNotification(errors: ErrorReportItem[], user: any, slackWebhookUrl?: string): Promise<void>
}

// Event types for system operations
export type SystemEvent =
  | { type: 'ERROR_REPORT_CREATED'; payload: { reportId: string; errorCount: number; userId: number } }
  | { type: 'CRITICAL_ERROR_DETECTED'; payload: { errors: ErrorReportItem[]; userId: number } }
