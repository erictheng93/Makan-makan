    ...
    try {
      ...
      return c.json({
        ...
        error: {
          ...
          message: error instanceof Error ? error.message : 'Failed to submit error report'
        }
      }, 500)
    }
  }
...
routes.get('/error-stats',
  ...
  async (c) => {
    try {
      ...
      const restaurantId = user.role === 1 ? user.restaurantId : query.restaurantId
      ...
      return c.json({
        ...
        error: {
          ...
          message: error instanceof Error ? error.message : 'Failed to get error statistics'
        }
      }, 500)
    }
  }
 ...
 */
routes.delete('/error-reports/cleanup',
  authMiddleware,
  ...
  async (c) => {
    try {
      ...
      return c.json({
        ...
        error: {
          code: 'CLEANUP_FAILED',
          message: error instanceof Error ? error.message : 'Failed to cleanup error reports'
        }
      }, 500)
    }
  }
...
routes.get('/health/detailed',
  ...
  async (c) => {
    try {
      ...
      // 執行基本健康檢查
      const basicHealthResponse = await fetch(`${c.req.url.split('/health/detailed')[0]}/health`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
      })
      const basicData = await basicHealthResponse.json() as { status: string }
      ...
      const { createDatabase, count, gte, orders, users, restaurants, auditLogs, sql } = await import('@makanmakan/database')
      const db = createDatabase(c.env.DB)
...
routes.get('/health/metrics',
  ...
  async (c) => {
    try {
      ...
      const { createDatabase, count, gte, orders, sql } = await import('@makanmakan/database')
      const db = createDatabase(c.env.DB)
