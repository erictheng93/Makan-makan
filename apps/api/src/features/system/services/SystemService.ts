...
import { ErrorReportingService, createDatabase, sql, type CreateErrorReportData } from '@makanmakan/database'
...
export class SystemService implements ISystemService {
  ...
  async createErrorReport(
    ...
    try {
      ...
      const significantErrors = data.errors.filter(error =>
  ...
  async getSystemHealth(): Promise<SystemHealthResponse> {
    try {
      ...
      const healthChecks = await Promise.allSettled([
        ...
        // KV health check
        this.env.CACHE_KV ? this.env.CACHE_KV.get('health-check') : Promise.resolve(null)
      ])
   ...
   */
  async getErrorStats(restaurantId?: number): Promise<ErrorStats> {
    try {
      ...
      this.logger.error('Failed to get error statistics', error as Error, { restaurantId })
  ...
  async cleanupOldErrorReports(daysOld: number = 30): Promise<CleanupResponse> {
    try {
      ...
      const deletedCount = await this.errorReportingService.cleanupOldErrorReports(daysOld)
      ...
      this.logger.error('Failed to cleanup error reports', error as Error, { daysOld })
  ...
  async sendCriticalErrorNotification(
    ...
    try {
      ...
      this.logger.error('Failed to send critical error notification', error as Error, {
        errorCount: errors.length,
        userId: user.id
      })
