...
describe('SystemService', () => {
  ...
  describe('Create Error Report', () => {
    ...
    it('應該處理用戶代理信息', async () => {
      ...
      const result = await service.createErrorReport(request, 1, 1, 'Chrome/91.0')
    ...
    it('應該在錯誤報告服務失敗時拋出錯誤', async () => {
      ...
      await expect(service.createErrorReport(request, 1, 1)).rejects.toThrow('Failed to submit error report')
    })
  })
  ...
  describe('System Health', () => {
    ...
    it('應該在健康檢查完全失敗時返回失敗狀態', async () => {
      ...
      mockEnv.CACHE_KV.get = vi.fn().mockRejectedValue(new Error('Cache failure'))
  ...
  describe('Error Statistics', () => {
    ...
    it('應該在統計服務失敗時拋出錯誤', async () => {
      ...
      await expect(service.getErrorStats()).rejects.toThrow('Failed to get error statistics')
    })
  })
  ...
  describe('Cleanup Old Error Reports', () => {
    ...
    it('應該在清理失敗時拋出錯誤', async () => {
      ...
      await expect(service.cleanupOldErrorReports(30)).rejects.toThrow('Failed to cleanup error reports')
    })
  })
  ...
  describe('Critical Error Notification', () => {
    it('應該發送 Slack 通知', async () => {
      ...
      await service.sendCriticalErrorNotification(errors, user, 'https://hooks.slack.com/test')
    ...
    it('應該處理多個錯誤', async () => {
      ...
      await service.sendCriticalErrorNotification(errors, { id: 1 }, 'https://hooks.slack.com/test')
    ...
    it('應該在通知失敗時不拋出錯誤', async () => {
      ...
      // Should not throw
      await expect(
        service.sendCriticalErrorNotification(errors, { id: 1 }, 'https://hooks.slack.com/test')
  ...
  describe('Event Emission', () => {
    it('應該記錄錯誤報告創建事件', async () => {
      ...
      expect(debugLogs.some(log =>
        log.args[0] === 'Emitting system event'
      )).toBe(true)
    })
    ...
    it('應該在重大錯誤時發送額外事件', async () => {
      ...
      const debugLogs = mockLogger.logs.filter(log => log.level === 'debug')
      const eventLogs = debugLogs.filter(log => log.args[0] === 'Emitting system event')
