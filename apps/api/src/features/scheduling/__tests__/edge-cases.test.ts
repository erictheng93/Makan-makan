...
vi.mock('../../../shared/middleware', () => ({
  authMiddleware: vi.fn((c, next) => {
    c.set('user', { id: mockUserId, role: mockUserRole, restaurantId: mockRestaurantId })
    return next()
  }),
...
describe('Scheduling Edge Cases Tests', () => {
  ...
  describe('Time Conflict Detection', () => {
    it('應該檢測同一員工同一天的重疊班次', async () => {
      mockSchedulingService.createSchedule.mockRejectedValue(
        new Error('Schedule conflict: Employee already has a shift during this time')
      )
    ...
    it('應該檢測跨日班次的衝突', async () => {
      mockSchedulingService.createSchedule.mockRejectedValue(
        new Error('Schedule conflict: Overnight shift overlaps with next day schedule')
      )
  ...
  describe('Bulk Operations', () => {
    ...
    it('應該處理批量創建中的部分失敗', async () => {
      mockSchedulingService.bulkCreateSchedules.mockRejectedValue(
        new Error('Partial failure: 3 of 5 schedules created, 2 conflicts detected')
      )
    ...
    it('應該驗證批量操作的數量限制', async () => {
      mockSchedulingService.bulkCreateSchedules.mockRejectedValue(
        new Error('Bulk operation limit exceeded: maximum 100 schedules per request')
      )

      const schedules = Array(150).fill(null).map((_, i) => ({
        employeeId: `emp_${i}`,
        templateId: 1,
        date: '2024-01-15',
      }))
    ...
    it('應該在批量操作中檢測所有衝突', async () => {
      mockSchedulingService.bulkCreateSchedules.mockRejectedValue(
        new Error('Conflicts detected: emp_1 on 2024-01-15, emp_2 on 2024-01-16')
      )
  ...
  describe('Statistics Endpoints', () => {
    it('應該成功獲取每日統計', async () => {
      ...
      const req = new Request('http://localhost/scheduling/1/stats/daily?date=2024-01-15')
      const res = await app.fetch(req, { DB: {} })
      const data = await res.json()
    ...
    it('應該成功獲取每週摘要', async () => {
      ...
      const req = new Request('http://localhost/scheduling/1/stats/weekly?weekStartDate=2024-01-15')
      const res = await app.fetch(req, { DB: {} })
      const data = await res.json()
    ...
    it('應該處理統計查詢錯誤', async () => {
      mockSchedulingService.getDailyStats.mockRejectedValue(new Error('Database error'))
  ...
  describe('Conflict Resolution', () => {
    it('應該成功獲取單一衝突詳情', async () => {
      mockSchedulingService.getConflict.mockResolvedValue({
        ...
        employeeId: 'emp_1',
        employeeName: '張三',
        schedule1: { id: 1, date: '2024-01-15', startTime: '08:00', endTime: '16:00' },
        schedule2: { id: 2, date: '2024-01-15', startTime: '14:00', endTime: '22:00' },
        status: 'unresolved',
      })
    ...
    it('應該成功解決衝突', async () => {
      ...
      const req = new Request('http://localhost/scheduling/conflicts/1/resolve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: 'user_123',
    ...
    it('應該支援按類型過濾衝突', async () => {
      mockSchedulingService.getConflicts.mockResolvedValue({
        items: [
          { id: 1, type: 'leave_conflict', status: 'unresolved' },
        ],
        total: 1,
      })

      const req = new Request('http://localhost/scheduling/1/conflicts?type=leave_conflict&page=1&limit=20')
      const res = await app.fetch(req, { DB: {} })
      const data = await res.json()
  ...
  describe('Swap Request Edge Cases', () => {
    ...
    it('應該處理換班請求的取消', async () => {
      ...
      const req = new Request('http://localhost/scheduling/swap-requests/1/cancel', {
        method: 'POST',
      })
