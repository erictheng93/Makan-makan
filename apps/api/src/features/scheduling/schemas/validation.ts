...
const timeString = z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Time must be in HH:MM format')
...
export const createSchedulingRuleSchema = z.object({
  ...
  name: nonEmptyString.max(100),
  description: z.string().max(500).optional().nullable(),
  ruleType: z.enum(['max_hours_per_day', 'max_hours_per_week', 'min_rest_period', 'max_consecutive_days', 'skill_requirement', 'custom']),
...
export const updateSchedulingRuleSchema = z.object({
  name: nonEmptyString.max(100).optional(),
  description: z.string().max(500).optional().nullable(),
  ruleType: z.enum(['max_hours_per_day', 'max_hours_per_week', 'min_rest_period', 'max_consecutive_days', 'skill_requirement', 'custom']).optional(),
...
export const createAvailabilitySchema = z.object({
  ...
  (data) => {
    ...
    if (data.availabilityType === 'specific_date') {
      return data.startDate !== undefined && data.startDate !== null &&
...
export const conflictFiltersSchema = z.object({
  restaurantId: z.string().regex(/^\d+$/).transform(Number).optional(),
  conflictType: z.enum(['overlapping_shifts', 'insufficient_rest', 'max_hours_exceeded', 'consecutive_days_exceeded', 'skill_mismatch', 'leave_conflict', 'availability_conflict']).optional(),
  severity: z.enum(['error', 'warning', 'info']).optional(),
...
export const swapRequestFiltersSchema = z.object({
  ...
  status: z.enum(['pending', 'accepted', 'approved', 'rejected', 'cancelled', 'expired']).optional(),
