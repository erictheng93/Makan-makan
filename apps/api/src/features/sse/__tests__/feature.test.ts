...
vi.mock('../services/SSEService', () => ({
  SSEService: vi.fn().mockImplementation(() => mockSSEService),
}))
...
describe('SSE Feature Tests', () => {
  ...
  beforeEach(async () => {
    ...
    mockSSEService.getConnectionsByRestaurant.mockReturnValue([
      { id: 'conn1', restaurantId: 1, userId: 1, role: 0, lastHeartbeat: Date.now() },
      { id: 'conn2', restaurantId: 1, userId: 2, role: 1, lastHeartbeat: Date.now() },
    ])
  ...
  describe('POST /broadcast/menu-update', () => {
    ...
    it('應該支援不同的更新類型', async () => {
      ...
      const updateTypes = ['created', 'updated', 'deleted', 'availability_changed']
  ...
  describe('POST /test', () => {
    ...
    it('應該處理測試廣播失敗', async () => {
      mockSSEService.broadcastTest.mockRejectedValue(new Error('Test broadcast failed'))
