...
export class SSEService implements ISSEService {
  ...
  async broadcast(event: BroadcastEvent): Promise<void> {
    ...
    if (event.targetRoles && event.targetRoles.length > 0) {
      targetConnections = targetConnections.filter((conn) =>
        event.targetRoles!.includes(conn.role),
      );
    }
  ...
  getConnectionStatus(): ConnectionStatus {
    ...
    for (const conn of connections) {
      connectionsByRestaurant[conn.restaurantId] =
        (connectionsByRestaurant[conn.restaurantId] || 0) + 1;
  ...
  cleanupExpiredConnections(): void {
    ...
    for (const [connectionId, connection] of connectionEntries) {
      if (now - connection.lastHeartbeat > timeout) {
        this.logger.info("Cleaning up expired SSE connection", {
          connectionId,
        });
