  ...
  try {
    ...
    if ('customerName' in requestData) {
      ...
      joinRequest = {
        ...
        specialRequests: requestData.special_requests || requestData.specialRequests
      }
    }
    ...
    if (!result.success) {
      return c.json(createErrorResponse(result.error || 'Failed to join queue'), 400)
    }
...
app.get('/:restaurantId/status', async (c) => {
  try {
    ...
    if (!result.success) {
      return c.json(createErrorResponse(result.error || 'Failed to get queue status'), 500)
    }
    ...
    return c.json(
      createErrorResponse('Failed to get queue status'),
      500
    )
  }
})
...
app.post('/:restaurantId/call-next', authMiddleware, async (c) => {
  try {
    ...
    if (!result.success) {
      return c.json(createErrorResponse(result.error || 'Failed to call next customer'), 400)
    }
...
app.post('/:restaurantId/migrate', authMiddleware, async (c) => {
  try {
    ...
    return c.json(
      createErrorResponse('Failed to migrate queue'),
      500
    )
  }
})
