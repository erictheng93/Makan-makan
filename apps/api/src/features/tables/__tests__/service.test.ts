...
describe('TablesService', () => {
  ...
  describe('getRestaurantTables', () => {
    ...
    it('should return hasPrev when on later pages', async () => {
      ...
      const result = await tablesService.getRestaurantTables(1, { page: 2, limit: 20 })
    ...
    it('should handle database errors gracefully', async () => {
      mockTableService.getRestaurantTables.mockRejectedValue(new Error('Database error'))

      await expect(tablesService.getRestaurantTables(1, {})).rejects.toThrow('Failed to fetch restaurant tables')
    })
    ...
    it('should pass filters to database service', async () => {
      ...
      expect(mockTableService.getRestaurantTables).toHaveBeenCalledWith('1', filters)
    })
  })
  ...
  describe('getTableById', () => {
    ...
    it('should handle errors and throw appropriate message', async () => {
      mockTableService.getTableById.mockRejectedValue(new Error('Database error'))

      await expect(tablesService.getTableById(1)).rejects.toThrow('Failed to fetch table')
    })
  })
  ...
  describe('createTable', () => {
    ...
    it('should handle creation errors', async () => {
      ...
      mockTableService.createTable.mockRejectedValue(new Error('Duplicate table number'))

      await expect(tablesService.createTable(tableData)).rejects.toThrow('Failed to create table')
    })
  ...
  describe('updateTable', () => {
    it('should update table successfully', async () => {
      ...
      mockTableService.updateTable.mockResolvedValue({ ...mockTable, ...updateData })
    ...
    it('should handle update errors', async () => {
      mockTableService.updateTable.mockRejectedValue(new Error('Update failed'))

      await expect(tablesService.updateTable(1, { capacity: 10 })).rejects.toThrow('Failed to update table')
    })
    ...
    it('should update table features', async () => {
      ...
      mockTableService.updateTable.mockResolvedValue({ ...mockTable, features: updateData.features })
  ...
  describe('deleteTable', () => {
    ...
    it('should handle deletion errors', async () => {
      mockTableService.deleteTable.mockRejectedValue(new Error('Deletion error'))

      await expect(tablesService.deleteTable(1)).rejects.toThrow('Failed to delete table')
    })
  })
  ...
  describe('occupyTable', () => {
    it('should occupy table successfully with all parameters', async () => {
      ...
      expect(result).toBe(true)
      expect(mockTableService.occupyTable).toHaveBeenCalledWith(1, 100, 'John Doe', 60)
    })
    ...
    it('should occupy table with minimal parameters', async () => {
      ...
      expect(result).toBe(true)
      expect(mockTableService.occupyTable).toHaveBeenCalledWith(1, 100, undefined, undefined)
    })
    ...
    it('should handle occupation errors', async () => {
      mockTableService.occupyTable.mockRejectedValue(new Error('Occupation error'))

      await expect(tablesService.occupyTable(1, 100)).rejects.toThrow('Failed to occupy table')
    })
  })
  ...
  describe('releaseTable', () => {
    ...
    it('should handle release errors', async () => {
      mockTableService.releaseTable.mockRejectedValue(new Error('Release error'))

      await expect(tablesService.releaseTable(1)).rejects.toThrow('Failed to release table')
    })
  })
  ...
  describe('markTableCleaned', () => {
    it('should mark table as cleaned successfully', async () => {
      ...
      expect(result).toBe(true)
      expect(mockTableService.markTableCleaned).toHaveBeenCalledWith(1, 'Deep cleaned')
    })
    ...
    it('should mark table as cleaned without notes', async () => {
      ...
      expect(result).toBe(true)
      expect(mockTableService.markTableCleaned).toHaveBeenCalledWith(1, undefined)
    })
    ...
    it('should handle marking errors', async () => {
      mockTableService.markTableCleaned.mockRejectedValue(new Error('Marking error'))

      await expect(tablesService.markTableCleaned(1)).rejects.toThrow('Failed to mark table as cleaned')
    })
  })
  ...
  describe('regenerateQRCode', () => {
    ...
    it('should regenerate QR code with custom data', async () => {
      ...
      expect(result.success).toBe(true)
      expect(mockTableService.regenerateQRCode).toHaveBeenCalledWith(1, customData)
    })
    ...
    it('should handle regeneration errors and return error response', async () => {
      mockTableService.regenerateQRCode.mockRejectedValue(new Error('QR service error'))
  ...
  describe('generateBulkQRCodes', () => {
    ...
    it('should generate bulk QR codes with options', async () => {
      ...
      const result = await tablesService.generateBulkQRCodes(1, tableIds, options)
    ...
    it('should handle bulk generation errors', async () => {
      mockTableService.generateBulkQRCodes.mockRejectedValue(new Error('Bulk service error'))
  ...
  describe('getAvailableTables', () => {
    it('should fetch available tables successfully', async () => {
      ...
      // Service converts restaurantId to string for database layer
      expect(mockTableService.getAvailableTables).toHaveBeenCalledWith('1', undefined)
    })
    ...
    it('should handle errors', async () => {
      mockTableService.getAvailableTables.mockRejectedValue(new Error('Database error'))

      await expect(tablesService.getAvailableTables(1)).rejects.toThrow('Failed to fetch available tables')
    })
  })
  ...
  describe('getTableStats', () => {
    ...
    it('should handle errors', async () => {
      mockTableService.getTableStats.mockRejectedValue(new Error('Stats error'))

      await expect(tablesService.getTableStats(1)).rejects.toThrow('Failed to fetch table statistics')
    })
  })
  ...
  describe('getTableByQRCode', () => {
    it('should fetch table by QR code successfully', async () => {
      ...
      expect(result).toEqual(mockTable)
      expect(mockTableService.getTableByQRCode).toHaveBeenCalledWith('qr-code-12345')
    })
    ...
    it('should handle errors', async () => {
      mockTableService.getTableByQRCode.mockRejectedValue(new Error('QR lookup error'))

      await expect(tablesService.getTableByQRCode('qr-code')).rejects.toThrow('Failed to fetch table by QR code')
    })
  })
  ...
  describe('validateTableAccess', () => {
    ...
    it('should grant access to same restaurant', () => {
      const table = { ...mockTable, restaurantId: 1 }
      const result = tablesService.validateTableAccess(table as Table, 1, false)
      expect(result).toBe(true)
    })

    it('should deny access to different restaurant for non-admin', () => {
      const table = { ...mockTable, restaurantId: 1 }
      const result = tablesService.validateTableAccess(table as Table, 2, false)
      expect(result).toBe(false)
    })
  })
  ...
  describe('getPublicTableInfo', () => {
    it('should return only public fields', () => {
      ...
      const result = tablesService.getPublicTableInfo(table as unknown as Table)
