...
import { OrderStatus, OrderPaymentStatus, OrderPaymentMethod, UserRole } from '@makanmakan/shared-types'
...
function convertStatusArray(status: string | string[] | undefined): OrderStatus[] | undefined {
...
function stringToPaymentStatus(status: string): OrderPaymentStatus {
  ...
  return statusMap[status as keyof typeof statusMap] ?? OrderPaymentStatus.PENDING
}

// Helper function to convert payment status array
function convertPaymentStatusArray(status: string | string[] | undefined): OrderPaymentStatus[] | undefined {
  if (!status) return undefined
  if (typeof status === 'string') {
...
function convertPaymentMethodArray(method: string | string[] | undefined): OrderPaymentMethod[] | undefined {
...
async function broadcastOrderUpdate(
  ...
  try {
    ...
    logger.debug('Order update broadcasted successfully', { orderId, targetRoles })
  } catch (error) {
    logger.error('Failed to broadcast order update via SSE', error instanceof Error ? error : undefined, {})
...
app.post('/preview-coupon',
  ...
  async (c) => {
    try {
      ...
      logger.info('Previewing coupon', { couponCode: data.couponCode, userId: user.id })
      ...
      logger.error('Coupon preview error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.post('/',
  ...
  async (c) => {
    try {
      ...
      logger.info('Creating new order', { restaurantId: data.restaurantId, userId: user.id })
...
app.get('/',
  ...
  async (c) => {
    try {
      ...
      const result = await ordersService.getOrders(filters, user.id, user.role as UserRole)
      ...
      logger.error('Get orders error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.get('/:id',
  ...
  async (c) => {
    try {
      ...
      logger.error('Get order error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.put('/:id/status',
  ...
  async (c) => {
    try {
      ...
      logger.info('Updating order status', { orderId: id, newStatus: data.status, userId: user.id })
      ...
      const statusTransitions: Record<number, string[]> = {
        0: ['pending', 'confirmed', 'preparing', 'ready', 'delivered', 'cancelled'], // Admin
      ...
      logger.error('Update order status error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.get('/stats',
  ...
  async (c) => {
    try {
      ...
      if (user.role === 1) { // Owner only sees their restaurant
      ...
      logger.error('Get order stats error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.get('/analytics',
  ...
  async (c) => {
    try {
      ...
      logger.error('Get order analytics error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.post('/bulk',
  ...
  async (c) => {
    try {
      ...
      logger.info('Executing bulk order operation', { action: data.action, orderCount: data.orderIds.length })
      ...
      logger.error('Bulk order operation error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.post('/export',
  ...
  async (c) => {
    try {
      ...
      logger.error('Export orders error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.get('/:id/receipt',
  ...
  async (c) => {
    try {
      ...
      logger.error('Generate receipt error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
...
app.get('/active',
  ...
  async (c) => {
    try {
      ...
      const restaurantId = user.role === 0 
      ...
      logger.error('Get active orders error', error instanceof Error ? error : undefined, {})
      return c.json({
        success: false,
