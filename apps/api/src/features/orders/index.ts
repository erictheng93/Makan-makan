...
class OrdersModule implements FeatureModule {
  ...
  getHealthStatus(env?: Env) {
    ...
    if (env) {
      try {
        ...
        healthData.dependencies.orderService = service
          ? "healthy"
          : "unhealthy";
      } catch (error) {
        this.logger.error(
          "Health check failed for OrdersService",
          error instanceof Error ? error : undefined,
          {},
        );
  ...
  validateConfiguration(env: Env): {
    ...
    if (!env.INTERNAL_API_TOKEN) {
      this.logger.warn(
        "INTERNAL_API_TOKEN not set, internal API calls may fail",
      );
    }
  ...
  async initialize(env: Env): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        "Failed to initialize orders feature",
        error instanceof Error ? error : undefined,
        {},
      );
      throw error;
...
export default {
  ...
  validateConfiguration: (env: Env) =>
    createOrdersModule().validateConfiguration(env),
