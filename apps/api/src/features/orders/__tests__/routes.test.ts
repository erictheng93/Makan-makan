  ...
  describe('Route Schema Validation', () => {
    ...
    describe('Bulk Operation Endpoint Schema', () => {
      ...
      it('should reject bulk operation with too many orders', () => {
        const request = {
          action: 'update_status',
          orderIds: Array(101).fill(0).map((_, i) => i + 1)
        }
  ...
  describe('Status Transition Validation', () => {
    describe('Valid Transitions', () => {
      ...
      it('should allow preparing -> cancelled', () => {
        expect(validateOrderStatusTransition('preparing', 'cancelled')).toBe(true)
      })
    ...
    describe('Invalid Transitions', () => {
      ...
      it('should not allow confirmed -> delivered (skip ready)', () => {
        expect(validateOrderStatusTransition('confirmed', 'delivered')).toBe(false)
      })

      it('should not allow preparing -> delivered (skip ready)', () => {
        expect(validateOrderStatusTransition('preparing', 'delivered')).toBe(false)
      })
      ...
      it('should not allow cancelled -> any status', () => {
        ...
        expect(validateOrderStatusTransition('cancelled', 'preparing')).toBe(false)
      })

      it('should not allow backward transitions', () => {
        expect(validateOrderStatusTransition('confirmed', 'pending')).toBe(false)
        expect(validateOrderStatusTransition('preparing', 'confirmed')).toBe(false)
    ...
    describe('Edge Cases', () => {
      ...
      it('should handle same status transition', () => {
        ...
        expect(validateOrderStatusTransition('confirmed', 'confirmed')).toBe(false)
      })
    })
  })
