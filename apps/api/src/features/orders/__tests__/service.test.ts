...
import { OrderStatus, OrderPaymentStatus, OrderPaymentMethod } from '@makanmakan/shared-types'
...
vi.mock('../../../services/RealtimeBroadcastService', () => ({
  RealtimeBroadcastService: vi.fn().mockImplementation(() => ({
    broadcastNewOrder: vi.fn().mockResolvedValue({ success: true, eventId: 'evt-1', recipientCount: 1 }),
    broadcastOrderStatusUpdate: vi.fn().mockResolvedValue({ success: true, eventId: 'evt-2', recipientCount: 1 }),
...
describe('OrdersService', () => {
  ...
  beforeEach(async () => {
    ...
    // Get mocked services
    const { OrderService, CouponService } = await import('@makanmakan/database')
    ...
    (OrderService as any).mockImplementation(() => mockBaseOrderService);
    (CouponService as any).mockImplementation(() => mockCouponService)
  ...
  describe('Order Creation', () => {
    describe('createOrder', () => {
      ...
      it('should reject order with invalid restaurant ID', async () => {
        const invalidData = { ...validOrderData, restaurantId: 0 }

        await expect(service.createOrder(invalidData, 100))
      ...
      it('should reject order with too many items', async () => {
        ...
        await expect(service.createOrder(invalidData, 100))
          .rejects.toThrow('Order cannot exceed 100 items')
      })
      ...
      it('should reject order with quantity exceeding limit', async () => {
        ...
        await expect(service.createOrder(invalidData, 100))
          .rejects.toThrow('Invalid item quantity: cannot exceed 999')
      })
      ...
      it('should reject order with notes exceeding limit', async () => {
        ...
        await expect(service.createOrder(invalidData, 100))
          .rejects.toThrow('Order notes cannot exceed 1000 characters')
      })
  ...
  describe('Order Updates', () => {
    describe('updateOrder', () => {
      it('should update order successfully', async () => {
        ...
        const result = await service.updateOrder(1, { status: OrderStatus.CONFIRMED }, 100)
      ...
      it('should return null for non-existent order', async () => {
        ...
        const result = await service.updateOrder(999, { status: OrderStatus.CONFIRMED }, 100)
    ...
    describe('updateOrderStatus', () => {
      it('should update status and broadcast', async () => {
        const existingOrder = { id: 1, status: OrderStatus.PENDING, restaurantId: 1 }
        const updatedOrder = { id: 1, status: OrderStatus.CONFIRMED, restaurantId: 1 }
    ...
    describe('deleteOrder', () => {
      ...
      it('should reject deletion of non-pending order', async () => {
        ...
        await expect(service.deleteOrder(1, 100))
          .rejects.toThrow('Only pending orders can be deleted')
      })
    ...
    describe('validateCoupon', () => {
      it('should validate coupon successfully', async () => {
        mockCouponService.validateCoupon.mockResolvedValue({
          valid: true,
          coupon: { code: 'SAVE10', name: 'Save 10%', discountType: 'percentage', discountValue: 10 },
          discountAmount: 500,
          finalAmount: 4500
        })
      ...
      it('should handle validation errors', async () => {
        mockCouponService.validateCoupon.mockRejectedValue(new Error('Service unavailable'))
  ...
  describe('Analytics and Statistics', () => {
    describe('getOrderAnalytics', () => {
      ...
      it('should throw error if restaurant ID not provided', async () => {
        await expect(service.getOrderAnalytics({}))
  ...
  describe('Bulk Operations', () => {
    describe('bulkUpdateOrders', () => {
      it('should update multiple orders', async () => {
        const updatedOrder = { id: 1, status: OrderStatus.CONFIRMED }
        mockEnv.CACHE_KV.get.mockResolvedValue({ id: 1, status: OrderStatus.PENDING, restaurantId: 1 })
        mockBaseOrderService.updateOrderStatus.mockResolvedValue(updatedOrder)
      ...
      it('should cancel multiple orders', async () => {
        mockBaseOrderService.cancelOrder.mockResolvedValue({ id: 1, status: OrderStatus.CANCELLED })
      ...
      it('should handle partial failures', async () => {
        mockEnv.CACHE_KV.get
          .mockResolvedValueOnce({ id: 1, status: OrderStatus.PENDING, restaurantId: 1 })
          .mockResolvedValueOnce(null) // Order not found
        mockBaseOrderService.updateOrderStatus.mockResolvedValue({ id: 1, status: OrderStatus.CONFIRMED })
    ...
    describe('generateReceipt', () => {
      it('should generate receipt for order', async () => {
        const order = {
          ...
          items: [
            { id: 1, menuItem: { name: 'Nasi Lemak' }, quantity: 2, unitPrice: 1500, totalPrice: 3000 }
          ],
          subtotal: 3000,
          taxAmount: 180,
          serviceCharge: 300,
      ...
      it('should throw error for non-existent order', async () => {
        ...
        await expect(service.generateReceipt(999))
