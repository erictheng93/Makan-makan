...
describe("Orders Analytics", () => {
  ...
  describe("getOrderAnalytics", () => {
    it("should return analytics from cache if available", async () => {
      ...
      const result = await ordersService.getOrderAnalytics({
        restaurantId: "1",
      });
    ...
    it("should throw error when restaurantId is missing", async () => {
      await expect(ordersService.getOrderAnalytics({})).rejects.toThrow(
  ...
  describe("getDailyStats", () => {
    ...
    it("should accept specific date parameter", async () => {
      ...
      expect(mockOrderServiceInstance.getDailyOrderStats).toHaveBeenCalledWith(
        "1",
        specificDate,
      );
    ...
    it("should handle database errors gracefully", async () => {
      mockOrderServiceInstance.getDailyOrderStats.mockRejectedValue(
        new Error("Database error"),
      );

      await expect(ordersService.getDailyStats("1")).rejects.toThrow(
        "Database error",
      );
    });
  });
