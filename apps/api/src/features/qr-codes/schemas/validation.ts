...
const idParam = z.object({
  id: z.string().transform(Number).refine(val => Number.isInteger(val) && val > 0, {
...
const bulkQRSchema = z.object({
  ...
  })).min(1, 'At least one table is required').max(100, 'Maximum 100 tables allowed'),
...
const listTemplatesQuery = z.object({
  ...
  limit: z.string().transform(Number).refine(val => Number.isInteger(val) && val > 0 && val <= 100, {
    message: 'Limit must be a positive integer up to 100'
  }).optional(),
  category: z.enum(['modern', 'classic', 'colorful', 'minimalist', 'branded']).optional(),
...
const statsQuery = z.object({
  restaurantId: z.string().transform(Number).refine(val => Number.isInteger(val) && val > 0, {
