...
const passwordSchema = z.string()
  ...
  .refine(
    (password) => password.length >= 8 ? PASSWORD_STRENGTH_REGEX.test(password) : true,
    {
      message: 'Password must contain at least one uppercase letter, one lowercase letter, one number, and one special character when 8+ characters'
    }
  )
...
export type SecurityEventsQuerySchema = z.infer<typeof securityEventsQuerySchema>