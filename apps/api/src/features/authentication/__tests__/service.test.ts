...
describe('AuthService Extended Tests', () => {
  ...
  describe('Register Edge Cases', () => {
    ...
    it('should handle registration error', async () => {
      ...
      expect(mockPerformance.recordMetric).toHaveBeenCalledWith('auth.register.error', 1)
    })
  })
  ...
  describe('Token Refresh Edge Cases', () => {
    ...
    it('should handle refresh token error', async () => {
      mockDbAuthService.refreshToken.mockRejectedValue(new Error('Token service unavailable'))

      await expect(authService.refreshToken('some-token')).rejects.toThrow('Token service unavailable')
      expect(mockPerformance.recordMetric).toHaveBeenCalledWith('auth.refreshToken.error', 1)
    })
  })
  ...
  describe('Logout Edge Cases', () => {
    ...
    it('should handle logout error gracefully', async () => {
      mockDbAuthService.logout.mockRejectedValue(new Error('Logout service error'))
  ...
  describe('Token Validation Edge Cases', () => {
    it('should handle validation error gracefully', async () => {
      ...
      mockDbAuthService.validateToken.mockRejectedValue(new Error('Validation service error'))

      const result = await authService.validateToken('error-token')

      expect(result.valid).toBe(false)
      expect(result.error).toBe('Token validation failed')
      expect(mockPerformance.recordMetric).toHaveBeenCalledWith('auth.validateToken.error', 1)
    })
  ...
  describe('User Profile Methods', () => {
    it('should get user profile from cache', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith('User profile retrieved from cache', { userId: 1 })
    })
    ...
    it('should update user profile and clear cache', async () => {
      ...
      const result = await authService.updateUserProfile(1, { fullName: 'Updated Name' })
  ...
  describe('Password Change Edge Cases', () => {
    it('should handle password change error', async () => {
      mockDbAuthService.changePassword.mockRejectedValue(new Error('Password service error'))

      const result = await authService.changePassword(1, 'old', 'new')

      expect(result.success).toBe(false)
      expect(result.error).toBe('Failed to change password')
      expect(mockPerformance.recordMetric).toHaveBeenCalledWith('auth.changePassword.error', 1)
    })
  })
  ...
  describe('Session Management', () => {
    ...
    it('should terminate all sessions', async () => {
      ...
      expect(result).toBe(true)
      expect(mockPerformance.recordMetric).toHaveBeenCalledWith('auth.terminateAllSessions.success', 1)
    })
