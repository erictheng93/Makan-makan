...
export interface SecurityEvent {
  type: 'LOGIN' | 'LOGIN_FAILED' | 'LOGOUT' | 'PASSWORD_CHANGED' |
...
export interface IAuthService {
  ...
  logout(userId: number, token?: string, allSessions?: boolean): Promise<boolean>
  validateToken(token: string): Promise<TokenValidation>

  // User management
  getUserProfile(userId: number): Promise<UserProfile | null>
  updateUserProfile(userId: number, data: Partial<AuthUser>): Promise<AuthUser | null>
  ...
  setupTwoFactor(userId: number, password: string): Promise<{ secret: string; qrCode: string; backupCodes: string[] }>
  verifyTwoFactor(userId: number, token: string, backupCode?: string): Promise<{ success: boolean; error?: string }>
  disableTwoFactor(userId: number, password: string, token?: string): Promise<{ success: boolean; error?: string }>
  ...
  // Email verification
  requestEmailVerification(userId: number): Promise<{ success: boolean; error?: string }>
...
export type AuthEvent =
  | { type: 'USER_LOGGED_IN'; payload: { userId: number; deviceInfo?: DeviceInfo } }
