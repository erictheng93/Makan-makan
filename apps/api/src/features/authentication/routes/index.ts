 ...
 */
export type AuthMiddleware = (c: Context<{ Bindings: Env }>, next: Next) => Promise<Response | void>
export type RequireRoleFactory = (roles: number[]) => AuthMiddleware
export type BlacklistTokenFn = (c: Context<{ Bindings: Env }>, token: string, expiryTime?: number) => Promise<void>
 ...
 */
export function createAuthRoutes(deps: AuthRouteDependencies = {}): Hono<{ Bindings: Env }> {
  ...
  const AuthService = deps.AuthService ?? ((env: Env) => new DefaultAuthService(env))
