...
export class ReceiptService extends BaseService {
  ...
  async getReceipts(
    ...
    try {
      const { startDate, endDate, receiptType, page = 1, limit = 20 } = options || {}
   ...
   * 生成收據內容
   */
  private async generateReceiptContent(order: any, templateName: string): Promise<any> {
    // 獲取訂單項目
    const orderItems = await this.d1.prepare(`
      SELECT
        oi.*,
