 ...
 */
app.post('/:registerId/deactivate',
  authMiddleware,
  requireRole([0, 1]), // Admin or Owner
  validateParams(registerParamsSchema),
  async (c) => {
    try {
      ...
      const registerService = new RegisterService(c.env.DB as any)
      const result = await registerService.toggleRegisterStatus(registerId, false)
