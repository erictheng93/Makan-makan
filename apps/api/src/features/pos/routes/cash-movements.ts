...
app.post('/shifts/:shiftId/cash-movements',
  ...
  async (c) => {
    try {
      ...
      const result = await cashMovementService.processCashMovement(shiftId, data, user.id)
 ...
 */
app.get('/registers/:registerId/cash-count',
  authMiddleware,
  requireRole([0, 1, 4]), // Admin, Owner, Cashier
 ...
 */
app.post('/cash-movements/:movementId/approve',
  authMiddleware,
  ...
  async (c) => {
    try {
      ...
      const cashMovementService = new CashMovementService(c.env.DB as any)
      const result = await cashMovementService.approveCashMovement(movementId, user.id)
 ...
 */
app.post('/cash-movements/:movementId/reject',
  authMiddleware,
  requireRole([0, 1]), // Admin or Owner only
  validateParams(z.object({
    movementId: z.string().uuid()
  ...
  async (c) => {
    try {
      ...
      const cashMovementService = new CashMovementService(c.env.DB as any)
      const result = await cashMovementService.rejectCashMovement(movementId, user.id, reason)
