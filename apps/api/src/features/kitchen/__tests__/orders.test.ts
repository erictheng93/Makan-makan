      ...
      orders: [
        {
          ...
          items: [
            { id: 1, menuItemId: 101, quantity: 2, status: 0, menuItem: { name: '炒飯' }, notes: '不要蔥' }
          ],
        ...
        },
        {
          ...
          createdAt: new Date().toISOString(),
          items: [
            { id: 2, menuItemId: 102, quantity: 1, status: 1, menuItem: { name: '炒麵' } }
          ]
        }
      ],
...
describe('KitchenService Orders', () => {
  ...
  describe('getKitchenOrders', () => {
    ...
    it('should transform order items correctly', async () => {
      ...
      const allOrders = [...result.pending, ...result.preparing, ...result.ready]
    ...
    it('should handle empty orders gracefully', async () => {
      ...
      const { OrdersService } = await import('../../orders/services/OrdersService')
      vi.mocked(OrdersService).mockImplementationOnce(() => ({
        getOrders: vi.fn().mockResolvedValue({
          orders: [],
  ...
  describe('updateOrderItemStatus', () => {
    ...
    it('should broadcast status update to kitchen connections', async () => {
      ...
      const result = await kitchenService.updateOrderItemStatus(1, 1, 1, statusUpdate, 100)
