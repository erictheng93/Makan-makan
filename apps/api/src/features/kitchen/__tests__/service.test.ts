  ...
  describe('Kitchen Operations', () => {
    describe('getKitchenOrders', () => {
      it('should fetch and transform kitchen orders', async () => {
          ...
          orders: [
            {
              ...
              status: 1, // CONFIRMED
              items: [
                { id: 1, menuItem: { name: 'Nasi Lemak' }, quantity: 2, status: 0, notes: '' }
              ],
        ...
        (OrdersService as any).mockImplementation(() => ({
          getOrders: mockGetOrders
        }))

        // Create new service instance with mocked OrdersService
      ...
      it('should handle empty orders', async () => {
        // Reset mock to return empty orders
        const { OrdersService } = await import('../../orders/services/OrdersService');
        (OrdersService as any).mockImplementation(() => ({
          getOrders: vi.fn().mockResolvedValue({
            orders: [],
