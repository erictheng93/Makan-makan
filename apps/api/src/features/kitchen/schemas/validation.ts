...
export const orderItemStatusUpdateSchema = z.object({
  status: z.enum(['pending', 'preparing', 'ready', 'completed'], {
    invalid_type_error: 'Status must be one of: pending, preparing, ready, completed'
  }),
...
export const broadcastTestEventSchema = z.object({
  type: z.enum(['NEW_ORDER', 'ORDER_STATUS_UPDATE', 'ORDER_CANCELLED', 'PRIORITY_UPDATE']).optional(),
...
export const kitchenSSEEventSchema = z.object({
  ...
  data: z.object({
    type: z.enum(['NEW_ORDER', 'ORDER_STATUS_UPDATE', 'ORDER_CANCELLED', 'PRIORITY_UPDATE', 'HEARTBEAT']),
