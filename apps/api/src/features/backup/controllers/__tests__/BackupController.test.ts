...
describe('BackupController', () => {
  ...
  describe('createBackup - 建立備份', () => {
    it('should create backup and return 201', async () => {
      ...
      expect(mockValidationService.validateCreateBackupRequest).toHaveBeenCalledOnce()
      expect(mockValidationService.verifyRestaurantAccess).toHaveBeenCalledWith(c, VALID_UUID)
      expect(mockBackupService.createBackup).toHaveBeenCalledWith(
        expect.objectContaining({ restaurant_id: VALID_UUID }),
        '1'
      )
      expect(c.json).toHaveBeenCalledWith(
        expect.objectContaining({ success: true, data: { backup_id: 'new-id' } }),
        201
      )
    })
  ...
  describe('listBackups - 列出備份', () => {
    it('should return backup list', async () => {
      ...
      expect(mockValidationService.verifyRestaurantAccess).toHaveBeenCalledOnce()
      expect(c.json).toHaveBeenCalledWith(
    ...
    it('should return 400 on error', async () => {
      mockBackupService.listBackups.mockRejectedValueOnce(new Error('Query failed'))
  ...
  describe('getBackup - 取得單一備份', () => {
    it('should return backup details', async () => {
      const backup = { id: VALID_UUID, restaurant_id: VALID_UUID, name: 'Test' }
  ...
  describe('downloadBackup - 下載備份', () => {
    it('should return download response for completed backup', async () => {
      const backup = { id: VALID_UUID, restaurant_id: VALID_UUID, status: 'completed' }
      mockBackupService.getBackupById.mockResolvedValue(backup)
    ...
    it('should return 400 when backup is not completed', async () => {
      mockBackupService.getBackupById.mockResolvedValue({
        id: VALID_UUID, restaurant_id: VALID_UUID, status: 'in_progress'
      })
  ...
  describe('restoreBackup - 還原備份', () => {
    it('should initiate restore and return 201', async () => {
      ...
      expect(mockValidationService.validateRestoreRequest).toHaveBeenCalledOnce()
  ...
  describe('deleteBackup - 刪除備份', () => {
    it('should delete backup and return success', async () => {
      mockBackupService.getBackupById.mockResolvedValue({
        id: VALID_UUID, restaurant_id: VALID_UUID
      })
      ...
      expect(mockBackupService.deleteBackup).toHaveBeenCalledWith(VALID_UUID, '1')
      expect(c.json).toHaveBeenCalledWith(
  ...
  describe('getConfigurations - 取得設定列表', () => {
    it('should return configurations for valid restaurant', async () => {
      ...
      expect(mockConfigService.getConfigurations).toHaveBeenCalledWith(VALID_UUID)
      expect(c.json).toHaveBeenCalledWith(
  ...
  describe('saveConfiguration - 儲存設定', () => {
    it('should save configuration and return 201', async () => {
      ...
      mockConfigService.createOrUpdateConfiguration.mockResolvedValue(savedConfig)

      const c = createMockContext({ body: config, user: { id: 1, role: 0 } })
      await controller.saveConfiguration(c)

      expect(mockValidationService.validateConfigurationRequest).toHaveBeenCalledOnce()
      expect(mockConfigService.createOrUpdateConfiguration).toHaveBeenCalledWith(
        expect.objectContaining({ restaurant_id: VALID_UUID }),
        '1'
      )
      expect(c.json).toHaveBeenCalledWith(
    ...
    it('should return 400 when validation fails', async () => {
      ...
      expect(c.json).toHaveBeenCalledWith(
        expect.objectContaining({ success: false, error: 'Configuration name is required' }),
        400
      )
    })
  })
  ...
  describe('getSystemHealth - 系統健康狀態（管理員專用）', () => {
    ...
    it('should return 500 on service error', async () => {
      mockBackupService.getSystemHealth.mockRejectedValueOnce(new Error('DB down'))
  ...
  describe('getRestaurantMetrics - 餐廳備份指標', () => {
    it('should return metrics with default period', async () => {
      ...
      expect(mockBackupService.getRestaurantMetrics).toHaveBeenCalledWith(VALID_UUID, 'week')
      expect(c.json).toHaveBeenCalledWith(
    ...
    it('should pass custom period', async () => {
      ...
      expect(mockBackupService.getRestaurantMetrics).toHaveBeenCalledWith(VALID_UUID, 'month')
    })
  ...
  describe('getRestaurantAlerts - 餐廳備份警報', () => {
    it('should return all alerts by default', async () => {
      ...
      expect(mockBackupService.getRestaurantAlerts).toHaveBeenCalledWith(VALID_UUID, false)
      expect(c.json).toHaveBeenCalledWith(
    ...
    it('should filter unresolved alerts when requested', async () => {
      ...
      expect(mockBackupService.getRestaurantAlerts).toHaveBeenCalledWith(VALID_UUID, true)
    })
