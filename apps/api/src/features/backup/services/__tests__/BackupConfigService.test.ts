...
const { mockSelectChain, mockInsertChain, mockUpdateChain, mockDeleteChain, mockDrizzleDb } = vi.hoisted(() => {
  ...
  return { mockSelectChain, mockInsertChain, mockUpdateChain, mockDeleteChain, mockDrizzleDb }
})
...
const buildConfig = (overrides: Partial<BackupConfiguration> = {}): BackupConfiguration => ({
  id: VALID_UUID,
...
describe('BackupConfigService', () => {
  ...
  describe('getConfigurations - 取得餐廳設定列表', () => {
    it('should return configurations for a restaurant', async () => {
      selectResults = [buildDbRow(), buildDbRow({ id: 'id-2', name: 'Config 2' })]
    ...
    it('should throw a user-safe error on DB failure', async () => {
      ...
      await expect(service.getConfigurations(VALID_UUID))
        .rejects.toThrow('Failed to fetch backup configurations')
    })
  })
  ...
  describe('createConfiguration - 建立設定', () => {
    ...
    it('should throw on insert failure', async () => {
      ...
      await expect(service.createConfiguration(buildConfig()))
        .rejects.toThrow('Failed to create backup configuration')
  ...
  describe('updateConfiguration - 更新設定', () => {
    it('should update an existing configuration', async () => {
      ...
      const result = await service.updateConfiguration(VALID_UUID, { name: 'Updated Name' })
    ...
    it('should throw when configuration not found', async () => {
      ...
      await expect(service.updateConfiguration('missing-id', { name: 'New' }))
        .rejects.toThrow()
    })
  })
  ...
  describe('deleteConfiguration - 刪除設定', () => {
    it('should delete a configuration not in use', async () => {
      ...
      await expect(service.deleteConfiguration(VALID_UUID)).resolves.toBeUndefined()
    ...
    it('should prevent deletion when configuration is in use', async () => {
      ...
      await expect(service.deleteConfiguration(VALID_UUID))
        .rejects.toThrow('Failed to delete backup configuration')
    })
  })
  ...
  describe('getScheduledConfigurations - 取得排程設定', () => {
    it('should return configs with active schedules', async () => {
      const scheduled = buildDbRow({ scheduleEnabled: true, scheduleCron: '0 2 * * *' })
      mockSelectChain.then = (resolve: any) =>
  ...
  describe('validateConfigurationCompatibility - 設定相容性驗證', () => {
    it('should reject external storage provider', async () => {
      ...
      await expect(service.validateConfigurationCompatibility(config))
    ...
    it('should reject notifications enabled without channels', async () => {
      ...
      await expect(service.validateConfigurationCompatibility(config))
        .rejects.toThrow('Notifications are enabled but no notification channels specified')
    })
  ...
  describe('cloneConfiguration - 複製設定', () => {
    it('should clone a configuration to another restaurant', async () => {
      ...
      const result = await service.cloneConfiguration(VALID_UUID, targetRestaurant, 'user-2')
    ...
    it('should throw when source configuration not found', async () => {
      ...
      await expect(service.cloneConfiguration('missing', 'target', 'user'))
