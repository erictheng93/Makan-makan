...
export interface IGroupOrderService {
  // Core operations
  createGroupOrder(data: CreateGroupOrderRequest, hostId: number): Promise<{ success: boolean; data?: CreateGroupOrderResponse; error?: string }>
  joinGroup(shareCode: string, memberData: JoinGroupRequest): Promise<{ success: boolean; data?: JoinGroupResponse; error?: string }>
  getGroupOrder(groupOrderId: string): Promise<GroupOrderSummary | null>

  // Cart operations
  addCartItem(groupOrderId: string, itemData: AddCartItemRequest): Promise<{ success: boolean; data?: GroupOrderCartItem; error?: string }>
  updateCartItem(groupOrderId: string, itemId: string, updateData: UpdateCartItemRequest): Promise<{ success: boolean; data?: GroupOrderCartItem; error?: string }>
  removeCartItem(groupOrderId: string, itemId: string, memberId: string): Promise<{ success: boolean; error?: string }>

  // Bill and payment operations
  splitBill(groupOrderId: string, splitData: SplitBillRequest): Promise<{ success: boolean; data?: any; error?: string }>
  processPayment(groupOrderId: string, memberId: string, paymentData: ProcessPaymentRequest): Promise<{ success: boolean; data?: any; error?: string }>

  // Group management
  leaveGroup(groupOrderId: string, memberId: string): Promise<{ success: boolean; error?: string }>
...
export type GroupOrderErrorCode = typeof GROUP_ORDER_ERROR_CODES[keyof typeof GROUP_ORDER_ERROR_CODES]

// Events for real-time updates
export type GroupOrderEvent =
  | { type: 'GROUP_CREATED'; payload: { groupOrderId: string; shareCode: string } }
  | { type: 'MEMBER_JOINED'; payload: { groupOrderId: string; member: GroupOrderMember } }
  | { type: 'MEMBER_LEFT'; payload: { groupOrderId: string; memberId: string } }
  | { type: 'CART_UPDATED'; payload: { groupOrderId: string; item: GroupOrderCartItem; action: 'added' | 'updated' | 'removed' } }
  | { type: 'BILL_SPLIT'; payload: { groupOrderId: string; splitData: any } }
  | { type: 'PAYMENT_MADE'; payload: { groupOrderId: string; memberId: string; amount: number } }
  | { type: 'ORDER_FINALIZED'; payload: { groupOrderId: string; totalAmount: number } }
  | { type: 'ORDER_CANCELLED'; payload: { groupOrderId: string; reason: string } }