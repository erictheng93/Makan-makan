...
export interface LeaveType {
  id: number
  restaurantId: number | null  // null for system-level leave types
  code: string
  name: string
...
export interface LeaveCalendarEvent {
  id: number
  restaurantId: number | null  // null for system-wide/Taiwan public holidays
...
export interface ILeaveService {
  ...
  getLeaveBalance(employeeId: number, leaveTypeId: number, year: number): Promise<EmployeeLeaveBalance | null>
  adjustLeaveBalance(adjustment: LeaveBalanceAdjustment): Promise<EmployeeLeaveBalance>
  accrueLeaveBalances(restaurantId: number, year: number): Promise<number>  // Returns count of accrued balances

  // Leave Requests
  getLeaveRequests(filters: LeaveRequestFilters): Promise<{ items: LeaveRequestWithRelations[]; total: number }>
  ...
  // Leave Calendar
  getHolidays(restaurantId: number | null, year: number): Promise<LeaveCalendarEvent[]>
  isWorkingDay(restaurantId: number, date: string): Promise<boolean>

  // Statistics & Analytics
  getLeaveStatistics(restaurantId: number, startDate: string, endDate: string): Promise<LeaveStatistics>
  getUpcomingLeaves(restaurantId: number, days: number): Promise<LeaveRequestWithRelations[]>
  getExpiringBalances(restaurantId: number, months: number): Promise<LeaveBalanceWithType[]>
}
