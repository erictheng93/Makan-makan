...
vi.mock('../../../shared/middleware', () => ({
  authMiddleware: vi.fn((c: Context, next: Next) => {
    c.set('user', { id: mockUserId, role: mockUserRole, restaurantId: 1, username: 'testuser' })
    return next()
  }),
...
describe('Leaves Edge Cases Tests', () => {
  ...
  describe('Leave Request Schema Validation', () => {
    it('should reject end date before start date', async () => {
      const { createLeaveRequestSchema } = await import('../schemas/validation')
  ...
  describe('Balance Adjustment Schema Validation', () => {
    it('should reject adjustment exceeding range', async () => {
      const { adjustLeaveBalanceSchema } = await import('../schemas/validation')
  ...
  describe('Helper Functions', () => {
    it('should calculate single day leave correctly', async () => {
      ...
      const days = calculateLeaveDays('2025-01-20', '2025-01-20', 'full', 'full')
      expect(days).toBe(1)
    })

    it('should calculate multi-day leave correctly', async () => {
      const { calculateLeaveDays } = await import('../schemas/validation')
      const days = calculateLeaveDays('2025-01-20', '2025-01-24', 'full', 'full')
      expect(days).toBe(5)
    })
    ...
    it('should handle cross-year leave', async () => {
      const { calculateLeaveDays } = await import('../schemas/validation')
      const days = calculateLeaveDays('2024-12-30', '2025-01-02', 'full', 'full')
      expect(days).toBe(4)
    })

    it('should handle leap year correctly', async () => {
      const { calculateLeaveDays } = await import('../schemas/validation')
      const days = calculateLeaveDays('2024-02-28', '2024-03-01', 'full', 'full')
      expect(days).toBe(3)
    })

    it('should throw error for invalid date range', async () => {
      const { validateLeaveRequestDates } = await import('../schemas/validation')
      expect(() => {
  ...
  describe('API Edge Cases', () => {
    ...
    it('should handle non-existent employee ID', async () => {
      ...
      const req = new Request('http://localhost/leaves/balances?employeeId=99999&year=2025')
      const res = await app.fetch(req, { DB: {} })
    ...
    it('should handle concurrent requests', async () => {
      ...
      const requests = Array(3).fill(null).map((_, i) =>
        app.fetch(new Request('http://localhost/leaves/R-001/requests', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            employeeId: 1,
