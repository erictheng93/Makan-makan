...
vi.mock('../../../shared/middleware', () => ({
  authMiddleware: vi.fn((c, next) => {
    c.set('user', { id: mockUserId, role: mockUserRole, restaurantId: 'R-001' })
    return next()
  }),
  ...
  describe('Approval Chain', () => {
    ...
    it('應該在第一級審批後進入第二級', async () => {
      mockLeaveService.approveLeaveRequest.mockResolvedValue({
        ...
        approvalHistory: [
          { level: 1, approverId: 2, approvedAt: Date.now(), status: 'approved' },
        ],
      })
    ...
    it('應該記錄完整的審批歷史', async () => {
      mockLeaveService.getLeaveRequest.mockResolvedValue({
        ...
        status: 'approved',
        approvalHistory: [
          { level: 1, approverId: 2, approvedAt: Date.now() - 86400000, status: 'approved', comments: 'OK' },
          { level: 2, approverId: 1, approvedAt: Date.now(), status: 'approved', comments: 'Final OK' },
        ],
      })
  ...
  describe('Schedule Conflicts', () => {
    it('應該檢測請假與排班的衝突', async () => {
      ...
      mockLeaveService.createLeaveRequest.mockRejectedValue(
        new Error('Conflict: Employee has scheduled shifts on 2025-01-20, 2025-01-21')
      )
    ...
    it('應該在取消請假後恢復排班', async () => {
      ...
      mockLeaveService.cancelLeaveRequest.mockResolvedValue({
        ...
        status: 'cancelled',
        restoredSchedules: [
          { id: 101, date: '2025-01-20', restored: true },
        ],
      })
