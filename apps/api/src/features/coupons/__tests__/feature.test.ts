...
vi.mock('../services/CouponsService', () => ({
  CouponsService: vi.fn().mockImplementation(() => mockCouponsService),
}))
...
describe('Coupons Feature Module', () => {
  ...
  describe('POST /coupons/validate', () => {
    it('should validate coupon successfully with business rules', async () => {
      ...
      mockCouponsService.validateCouponWithBusinessRules.mockResolvedValue(validationResult)
      ...
      expect(mockCouponsService.validateCouponWithBusinessRules).toHaveBeenCalledWith(
        'VALID10', '1', 100, undefined, undefined
      )
    })
  ...
  describe('GET /coupons/available/:restaurantId', () => {
    it('should return available coupons for restaurant', async () => {
      ...
      mockCouponsService.getAvailableCoupons.mockResolvedValue(availableCoupons)
  ...
  describe('POST /coupons', () => {
    it('should create coupon with validation', async () => {
      ...
      mockCouponsService.createCouponWithValidation.mockResolvedValue(createdCoupon)

      const testApp = new Hono()
      testApp.use('*', (c, next) => {
        (c as any).testBody = couponData
        c.set('user', { id: 1, username: 'testuser', role: 1, restaurantId: 1 })
        c.env = { DB: {} }
        return next()
      })
      testApp.route('/', couponsRoutes)
  ...
  describe('GET /coupons', () => {
    it('should return paginated coupons with enhanced filters', async () => {
      ...
      mockCouponsService.getCouponsWithEnhancedFilters.mockResolvedValue(couponsResult)
  ...
  describe('POST /coupons/bulk', () => {
    ...
    it('should deny access to coupons from different restaurant', async () => {
      mockCouponsService.getCoupon.mockResolvedValueOnce({ id: 1, restaurantId: 2 })
