 ...
 */
routes.get('/available/:restaurantId',
...
routes.post('/use',
  ...
  async (c) => {
    try {
      ...
      return c.json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to record coupon usage'
      }, 500)
    }
  }
)
...
routes.get('/analytics/trends',
  ...
  async (c) => {
    try {
      ...
      return c.json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch coupon trends'
      }, 500)
    }
  }
)
