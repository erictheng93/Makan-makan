...
describe('Users Routes Unit Tests', () => {
  ...
  describe('GET /users - Get users list', () => {
    ...
    it('店主應該只能獲取自己餐廳的用戶', async () => {
      ...
      const result = await mockUsersService.getUsers(mockOwnerUser, { restaurantId: 1 })
    ...
    it('應該支持角色過濾', async () => {
      ...
      expect(mockUsersService.getUsers).toHaveBeenCalledWith(mockAdminUser, { role: USER_ROLES.CHEF })
    })
  ...
  describe('POST /users - Create user', () => {
    it('管理員應該能創建任何角色的用戶', async () => {
      ...
      const result = await mockUsersService.createUser(mockAdminUser, createData)
    ...
    it('店主應該只能創建員工角色', async () => {
      ...
      const result = await mockUsersService.createUser(mockOwnerUser, createData)
  ...
  describe('PUT /users/:id - Update user', () => {
    it('應該成功更新用戶資料', async () => {
      ...
      const result = await mockUsersService.updateUser(mockAdminUser, 3, updateData)
  ...
  describe('PATCH /users/:id/status - Update user status', () => {
    it('應該成功啟用用戶', async () => {
      ...
      const result = await mockUsersService.updateUserStatus(mockAdminUser, 3, true)
    ...
    it('應該成功停用用戶', async () => {
      ...
      const result = await mockUsersService.updateUserStatus(mockAdminUser, 3, false)
    ...
    it('不能停用自己', async () => {
      ...
      const result = await mockUsersService.updateUserStatus(mockAdminUser, 1, false)
    ...
    it('店主不能停用其他餐廳的用戶', async () => {
      ...
      const result = await mockUsersService.updateUserStatus(mockOwnerUser, 100, false)
  ...
  describe('POST /users/:id/reset-password - Reset password', () => {
    it('管理員應該能重設用戶密碼', async () => {
      ...
      const result = await mockUsersService.resetPassword(mockAdminUser, 3, 'newPassword123')
    ...
    it('店主應該能重設員工密碼', async () => {
      ...
      const result = await mockUsersService.resetPassword(mockOwnerUser, 3, 'newPassword123')
    ...
    it('應該處理用戶不存在', async () => {
      ...
      const result = await mockUsersService.resetPassword(mockAdminUser, 999, 'newPassword')
  ...
  describe('GET /users/stats - Get user statistics', () => {
    it('應該返回用戶統計數據', async () => {
      ...
      const result = await mockUsersService.getUserStats(mockAdminUser, undefined)
  ...
  describe('GET /users/search - Search users', () => {
    it('應該支持關鍵字搜索', async () => {
      ...
      const result = await mockUsersService.searchUsers(mockAdminUser, 'john', undefined, 10)
    ...
    it('應該支持餐廳過濾', async () => {
      ...
      expect(mockUsersService.searchUsers).toHaveBeenCalledWith(mockAdminUser, 'test', 1, 10)
    })
    ...
    it('應該支持限制結果數量', async () => {
      ...
      expect(mockUsersService.searchUsers).toHaveBeenCalledWith(mockAdminUser, 'test', undefined, 5)
    })
  })
  ...
  describe('Permission Validation', () => {
    describe('canManageUser', () => {
      it('管理員可以管理所有用戶', () => {
        ...
        expect(mockUsersService.canManageUser(mockAdminUser, USER_ROLES.OWNER, 2)).toBe(true)
      })

      it('店主只能管理自己餐廳的員工', () => {
        mockUsersService.canManageUser.mockReturnValue(true)

        expect(mockUsersService.canManageUser(mockOwnerUser, USER_ROLES.CHEF, 1)).toBe(true)
      })

      it('店主不能管理其他餐廳的員工', () => {
        mockUsersService.canManageUser.mockReturnValue(false)

        expect(mockUsersService.canManageUser(mockOwnerUser, USER_ROLES.CHEF, 2)).toBe(false)
      })

      it('店主不能管理管理員', () => {
        mockUsersService.canManageUser.mockReturnValue(false)

        expect(mockUsersService.canManageUser(mockOwnerUser, USER_ROLES.ADMIN, 1)).toBe(false)
      })
    })
    ...
    describe('canViewUser', () => {
      it('管理員可以查看所有用戶', () => {
        ...
        expect(mockUsersService.canViewUser(mockAdminUser, { id: 100 })).toBe(true)
      })

      it('用戶可以查看自己', () => {
        mockUsersService.canViewUser.mockReturnValue(true)

        expect(mockUsersService.canViewUser(mockStaffUser, { id: 3 })).toBe(true)
      })

      it('店主可以查看同餐廳員工', () => {
        mockUsersService.canViewUser.mockReturnValue(true)

        expect(mockUsersService.canViewUser(mockOwnerUser, { id: 3, restaurantId: 1 })).toBe(true)
      })
    })
  })
  ...
  describe('Error Handling', () => {
    it('應該處理數據庫錯誤', async () => {
      mockUsersService.getUsers.mockRejectedValue(new Error('Database connection failed'))

      await expect(mockUsersService.getUsers(mockAdminUser, {})).rejects.toThrow()
    })
