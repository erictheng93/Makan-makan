...
describe('UsersService', () => {
  ...
  describe('canManageUser', () => {
    it('admin can manage all users', () => {
      expect(usersService.canManageUser(mockAdminUser, USER_ROLES.OWNER, 2)).toBe(true)
      expect(usersService.canManageUser(mockAdminUser, USER_ROLES.ADMIN, 1)).toBe(true)
      expect(usersService.canManageUser(mockAdminUser, USER_ROLES.CHEF, 1)).toBe(true)
      expect(usersService.canManageUser(mockAdminUser, USER_ROLES.CUSTOMER, 1)).toBe(true)
    })

    it('owner can manage staff in same restaurant', () => {
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.CHEF, 1)).toBe(true)
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.SERVICE, 1)).toBe(true)
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.CASHIER, 1)).toBe(true)
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.CUSTOMER, 1)).toBe(true)
    })

    it('owner cannot manage admin or other owners', () => {
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.ADMIN, 1)).toBe(false)
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.OWNER, 1)).toBe(false)
    })

    it('owner cannot manage staff in different restaurant', () => {
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.CHEF, 2)).toBe(false)
      expect(usersService.canManageUser(mockOwnerUser, USER_ROLES.SERVICE, 2)).toBe(false)
    })

    it('other roles cannot manage users', () => {
      expect(usersService.canManageUser(mockChefUser, USER_ROLES.CHEF, 1)).toBe(false)
      expect(usersService.canManageUser(mockChefUser, USER_ROLES.CUSTOMER, 1)).toBe(false)
    })
  })
  ...
  describe('canUpdateUser', () => {
    it('admin can update all users', () => {
      expect(usersService.canUpdateUser(mockAdminUser, { id: 200, role: USER_ROLES.OWNER, restaurantId: 5 })).toBe(true)
    })

    it('user can update themselves', () => {
      expect(usersService.canUpdateUser(mockChefUser, { id: 102, role: USER_ROLES.CHEF, restaurantId: 1 })).toBe(true)
    })

    it('owner can update manageable users', () => {
      expect(usersService.canUpdateUser(mockOwnerUser, { id: 200, role: USER_ROLES.CHEF, restaurantId: 1 })).toBe(true)
    })

    it('owner cannot update non-manageable users', () => {
      expect(usersService.canUpdateUser(mockOwnerUser, { id: 200, role: USER_ROLES.ADMIN, restaurantId: 1 })).toBe(false)
    })
  })
  ...
  describe('getUsers', () => {
    ...
    it('owner should get only restaurant users', async () => {
      ...
      expect(mockUserService.getRestaurantUsers).toHaveBeenCalledWith('1', expect.any(Object))
    })
  ...
  describe('createUser', () => {
    ...
    it('owner should not create admin', async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe('Insufficient permissions to create this type of user')
      expect(result.status).toBe(403)
    })
  ...
  describe('updateUser', () => {
    it('should update user successfully', async () => {
      mockUserService.getUserById.mockResolvedValue(mockUser)
      mockUserService.updateUser.mockResolvedValue({ ...mockUser, fullName: 'Updated Name' })

      const result = await usersService.updateUser(mockAdminUser, 1, { fullName: 'Updated Name' })
    ...
    it('should return 404 when user not found', async () => {
      ...
      const result = await usersService.updateUser(mockAdminUser, 999, { fullName: 'Updated' })
    ...
    it('should return 403 when access denied', async () => {
      ...
      const result = await usersService.updateUser(mockOwnerUser, 1, { fullName: 'Updated' })
    ...
    it('user can update themselves', async () => {
      ...
      mockUserService.getUserById.mockResolvedValue(selfUser)
      mockUserService.updateUser.mockResolvedValue({ ...selfUser, fullName: 'Self Updated' })

      const result = await usersService.updateUser(mockChefUser, 102, { fullName: 'Self Updated' })
  ...
  describe('changePassword', () => {
    it('user can change own password', async () => {
      ...
      const result = await usersService.changePassword(mockChefUser, 102, 'oldPass', 'newPass')
    ...
    it('admin can change any password', async () => {
      ...
      const result = await usersService.changePassword(mockAdminUser, 1, 'oldPass', 'newPass')

      expect(result.success).toBe(true)
    })

    it('should deny non-admin changing other passwords', async () => {
      const result = await usersService.changePassword(mockChefUser, 200, 'old', 'new')
    ...
    it('should handle incorrect current password', async () => {
      mockAuthService.changePassword.mockResolvedValue({ success: false, error: 'Incorrect password' })

      const result = await usersService.changePassword(mockChefUser, 102, 'wrong', 'new')
  ...
  describe('updateUserStatus', () => {
    it('should activate user successfully', async () => {
      mockUserService.getUserById.mockResolvedValue({ ...mockUser, role: USER_ROLES.CHEF })
      mockUserService.updateUser.mockResolvedValue({ ...mockUser, isActive: true })

      const result = await usersService.updateUserStatus(mockAdminUser, 1, true)
    ...
    it('should deactivate user successfully', async () => {
      mockUserService.getUserById.mockResolvedValue({ ...mockUser, role: USER_ROLES.CHEF })
      mockUserService.updateUser.mockResolvedValue({ ...mockUser, isActive: false })

      const result = await usersService.updateUserStatus(mockAdminUser, 1, false)
    ...
    it('should return 404 when user not found', async () => {
      ...
      const result = await usersService.updateUserStatus(mockAdminUser, 999, false)
    ...
    it('should not allow deactivating self', async () => {
      mockUserService.getUserById.mockResolvedValue({ id: 100, role: USER_ROLES.ADMIN })

      const result = await usersService.updateUserStatus(mockAdminUser, 100, false)
    ...
    it('should deny insufficient permissions', async () => {
      mockUserService.getUserById.mockResolvedValue({ ...mockUser, role: USER_ROLES.ADMIN })

      const result = await usersService.updateUserStatus(mockOwnerUser, 1, false)
  ...
  describe('resetPassword', () => {
    it('admin should reset password successfully', async () => {
      ...
      const result = await usersService.resetPassword(mockAdminUser, 1, 'newPassword123')
    ...
    it('owner should reset staff password', async () => {
      mockUserService.getUserById.mockResolvedValue({ ...mockUser, role: USER_ROLES.CHEF, restaurantId: 1 })
      mockUserService.resetPassword.mockResolvedValue(true)

      const result = await usersService.resetPassword(mockOwnerUser, 1, 'newPassword123')
    ...
    it('should return 404 when user not found', async () => {
      ...
      const result = await usersService.resetPassword(mockAdminUser, 999, 'newPassword')
    ...
    it('should deny insufficient permissions', async () => {
      ...
      const result = await usersService.resetPassword(mockOwnerUser, 1, 'newPassword')
    ...
    it('should handle reset failure', async () => {
      ...
      const result = await usersService.resetPassword(mockAdminUser, 1, 'newPassword')
  ...
  describe('searchUsers', () => {
    ...
    it('owner should search only restaurant users', async () => {
      ...
      // Service converts restaurantId to string for database layer
      expect(mockUserService.searchUsers).toHaveBeenCalledWith('test', '1', undefined)
    })
    ...
    it('should apply restaurant filter for admin', async () => {
      ...
      // Service converts restaurantId to string for database layer
      expect(mockUserService.searchUsers).toHaveBeenCalledWith('test', '5', undefined)
    })
    ...
    it('should apply limit', async () => {
      ...
      expect(mockUserService.searchUsers).toHaveBeenCalledWith('test', undefined, 10)
    })
