...
import { TokenBlacklistService, type RevokeReason } from '../services/TokenBlacklistService'
...
describe('TokenBlacklistService', () => {
  ...
  const testToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyb29tVHlwZSI6ImN1c3RvbWVyIiwicm9vbUlkIjoicm9vbV8xMjMiLCJyZXN0YXVyYW50SWQiOiJyZXN0XzEiLCJyb2xlIjoiY3VzdG9tZXIiLCJleHAiOjE3MzM0MjQwMDAsImlhdCI6MTczMzQyMzcwMH0.abc123'
  ...
  describe('revokeToken', () => {
    ...
    it('應該在 KV 錯誤時拋出異常', async () => {
      ...
      await expect(service.revokeToken(testToken, 'logout')).rejects.toThrow('KV error')
    })
  })
  ...
  describe('revokeUserTokens', () => {
    it('應該撤銷用戶的所有 token', async () => {
      ...
      const result = await service.revokeUserTokens('user_1', 'password_change', 'admin_1')
    ...
    it('應該在 KV 錯誤時拋出異常', async () => {
      ...
      await expect(service.revokeUserTokens('user_1', 'logout')).rejects.toThrow('KV error')
    })
  })
  ...
  describe('trackUserToken', () => {
    ...
    it('應該在 KV 錯誤時靜默失敗', async () => {
      ...
      // 不應該拋出異常
      await expect(service.trackUserToken('user_1', testToken)).resolves.toBeUndefined()
    })
  })
