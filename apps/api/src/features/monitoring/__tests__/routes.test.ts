...
vi.mock('../services/MonitoringService', async () => {
  ...
  const mockService = {
    getHealthStatus: vi.fn().mockResolvedValue({
      overall: 'healthy',
      components: {
        api: { status: 'healthy', latency: 50, errorRate: 0, lastCheck: Date.now(), issues: [] },
        database: { status: 'healthy', latency: 25, errorRate: 0, lastCheck: Date.now(), issues: [] },
        cache: { status: 'healthy', lastCheck: Date.now(), issues: [], metrics: { hitRate: 0.9, totalKeys: 100 } },
    ...
    getMetrics: vi.fn().mockResolvedValue({
      timestamp: Date.now(),
      apiMetrics: { totalRequests: 100, errorRate: 0.02, averageResponseTime: 250, p95ResponseTime: 400, p99ResponseTime: 500, slowRequestCount: 2, requestsPerSecond: 10 },
      databaseMetrics: { queryCount: 200, averageQueryTime: 25, slowQueryCount: 1, connectionPoolUsage: 0.5, errorCount: 0 },
      cacheMetrics: { hitRate: 0.85, totalKeys: 100, totalSize: 2048, expiringKeysCount: 5, invalidationCount: 2 },
      resourceMetrics: { memoryUsage: 0.6, cpuUsage: 0.3, activeConnections: 25, queueLength: 0 },
      errorMetrics: { totalErrors: 1, criticalErrors: 0, warningCount: 1, errorsByType: {} }
    }),
    ...
    updateAlertRule: vi.fn().mockImplementation(async (id: string, updates: any) => {
  ...
  return {
    ...
    MonitoringService: vi.fn().mockImplementation(() => mockService),
    DEFAULT_ALERT_RULES: [
      { name: 'High API Error Rate', condition: 'apiMetrics.errorRate > 0.1', metric: 'apiMetrics.errorRate', operator: '>', threshold: 0.1, duration: 300, config: { type: 'slack', severity: 'critical', enabled: true, interval: 15 } },
      { name: 'Slow API Response Time', condition: 'apiMetrics.averageResponseTime > 1000', metric: 'apiMetrics.averageResponseTime', operator: '>', threshold: 1000, duration: 300, config: { type: 'slack', severity: 'warning', enabled: true, interval: 30 } }
    ]
  }
})
...
vi.mock('../../../middleware/auth', () => ({
  ...
  requireRole: (roles: number[]) => vi.fn().mockImplementation(async (c: any, next: any) => {
...
vi.mock('../../../middleware/validation', () => ({
  validateBody: () => vi.fn().mockImplementation(async (c: any, next: any) => {
    c.set('validatedBody', await c.req.json())
    await next()
  }),
  validateQuery: () => vi.fn().mockImplementation(async (c: any, next: any) => {
...
describe('Monitoring Routes', () => {
  ...
  describe('GET /monitoring/health', () => {
    it('應該返回系統健康狀態', async () => {
      const response = await app.request('/monitoring/health', {
        method: 'GET'
      }, mockEnv as Env)
  ...
  describe('GET /monitoring/metrics', () => {
    it('應該返回系統指標（需要管理員權限）', async () => {
      ...
      const response = await app.request('/monitoring/metrics', {
        method: 'GET'
      }, mockEnv as Env)
    ...
    it('應該支持時間段查詢參數', async () => {
      const response = await app.request('/monitoring/metrics?period=1h&granularity=5m', {
        method: 'GET'
      }, mockEnv as Env)
  ...
  describe('POST /monitoring/errors', () => {
    it('應該手動記錄錯誤', async () => {
      const response = await app.request('/monitoring/errors', {
        method: 'POST',
  ...
  describe('警報規則管理', () => {
    describe('GET /monitoring/alerts/rules', () => {
      it('應該返回警報規則列表', async () => {
        ...
        const response = await app.request('/monitoring/alerts/rules', {
          method: 'GET'
        }, mockEnv as Env)
      ...
      it('應該支持分頁', async () => {
        const response = await app.request('/monitoring/alerts/rules?page=1&limit=10', {
          method: 'GET'
        }, mockEnv as Env)
    ...
    describe('PUT /monitoring/alerts/rules/:id', () => {
      it('應該更新現有的警報規則', async () => {
        // First create a rule
        const createResponse = await app.request('/monitoring/alerts/rules', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: 'Rule to Update',
        ...
        // Now update it
        const response = await app.request(`/monitoring/alerts/rules/${ruleId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            threshold: 0.15,
      ...
      it('應該返回 404 當規則不存在', async () => {
        const response = await app.request('/monitoring/alerts/rules/non_existent_rule_id', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  describe('GET /monitoring/alerts/defaults', () => {
    it('應該返回預設警報規則', async () => {
      const response = await app.request('/monitoring/alerts/defaults', {
        method: 'GET'
      }, mockEnv as Env)
  ...
  describe('POST /monitoring/alerts/test', () => {
    it('應該發送測試警報', async () => {
      const response = await app.request('/monitoring/alerts/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: 'slack',
  ...
  describe('GET /monitoring/overview', () => {
    it('應該返回監控總覽', async () => {
      const response = await app.request('/monitoring/overview', {
        method: 'GET'
      }, mockEnv as Env)
  ...
  describe('GET /monitoring/reports/performance', () => {
    it('應該生成效能報告', async () => {
      const response = await app.request('/monitoring/reports/performance?days=7', {
        method: 'GET'
      }, mockEnv as Env)
    ...
    it('報告應該包含具體的效能指標', async () => {
      const response = await app.request('/monitoring/reports/performance', {
        method: 'GET'
      }, mockEnv as Env)
