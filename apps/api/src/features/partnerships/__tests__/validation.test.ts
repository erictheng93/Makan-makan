...
describe("Partnership Validation Schemas", () => {
  describe("createPartnershipSchema", () => {
    ...
    it("should accept all partner types", () => {
      ...
      types.forEach((type) => {
        const result = createPartnershipSchema.safeParse({
          ...validPartnership,
          partnerType: type,
        });
        expect(result.success).toBe(true);
...
describe("Member Validation Schemas", () => {
  describe("memberVerificationSchema", () => {
    ...
    it("should accept all member types", () => {
      ...
      types.forEach((type) => {
        const result = memberVerificationSchema.safeParse({
          ...validVerification,
          memberType: type,
        });
        expect(result.success).toBe(true);
...
describe("Usage Log Validation Schemas", () => {
  describe("logUsageSchema", () => {
    ...
    it("should reject negative amounts", () => {
      expect(
        logUsageSchema.safeParse({ ...validUsage, discountAmount: -10 })
          .success,
      ).toBe(false);
      expect(
        logUsageSchema.safeParse({ ...validUsage, originalAmount: -100 })
          .success,
      ).toBe(false);
      expect(
        logUsageSchema.safeParse({ ...validUsage, finalAmount: -50 }).success,
      ).toBe(false);
    });
  });
