...
describe('Notifications Feature Tests', () => {
  ...
  describe('POST /test', () => {
    ...
    it('應該支援 SMS 類型', async () => {
      ...
      const res = await app.fetch(req, { DB: {}, TWILIO_ACCOUNT_SID: 'test', TWILIO_AUTH_TOKEN: 'test' })
      const data = await res.json()
    ...
    it('應該處理服務異常', async () => {
      mockNotificationService.sendTestNotification.mockRejectedValue(new Error('Service unavailable'))
  ...
  describe('Notification Categories', () => {
    it('應該支援請假相關通知', async () => {
      mockNotificationService.sendNotification.mockResolvedValue({ success: true, errors: [] })
