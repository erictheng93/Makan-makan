...
test.describe("性能與可存取性測試", () => {
  ...
  test.describe("性能測試", () => {
    ...
    test("JavaScript 包大小檢查", async ({ page }) => {
      ...
      // 檢查主要 JavaScript 包大小
      const totalJSSize = jsRequests.reduce(
        (total, req) => total + req.size,
        0,
      );
    ...
    test("圖片載入優化", async ({ page }) => {
      ...
      const modernFormatRatio =
        modernFormats.length / Math.max(imageRequests.length, 1);
  ...
  test.describe("可存取性測試", () => {
    ...
    test("螢幕閱讀器支援", async ({ page }) => {
      ...
      const ariaAttributes = await page.evaluate(() => {
        ...
        return {
          ...
          labels: document.querySelectorAll("[aria-label], [aria-labelledby]")
            .length,
    ...
    test("焦點管理", async ({ page }) => {
      ...
      // 等待模態框出現
      await page
        .waitForSelector('[role="dialog"]', { timeout: 2000 })
        .catch(() => {
          // 如果沒有模態框，跳過這個測試
          return null;
        });
