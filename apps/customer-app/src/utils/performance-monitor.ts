...
export class PWAPerformanceMonitor {
    ...
    if ('PerformanceObserver' in window) {
      ...
      const fidObserver = new PerformanceObserver((list) => {
        ...
        entries.forEach((entry: any) => {
          this.metrics.timeToInteractive = entry.processingStart - entry.startTime
        })
      })
   ...
   * 記錄數據庫操作性能
   */
  recordDatabaseOperation(operationType: string, duration: number, success: boolean): void {
    this.metrics.dbQueryCount++

    // 更新平均操作時間
    this.metrics.dbOperationTime =
      (this.metrics.dbOperationTime * (this.metrics.dbQueryCount - 1) + duration) / this.metrics.dbQueryCount

    // 更新錯誤率
    if (!success) {
      const errorCount = this.metrics.dbErrorRate * (this.metrics.dbQueryCount - 1) + 1
      this.metrics.dbErrorRate = errorCount / this.metrics.dbQueryCount
    } else {
      this.metrics.dbErrorRate =
        (this.metrics.dbErrorRate * (this.metrics.dbQueryCount - 1)) / this.metrics.dbQueryCount
    }

    console.log(`💾 DB Operation: ${operationType} (${duration}ms, ${success ? 'success' : 'error'})`)
  }

  /**
   * 記錄同步操作性能
   */
  recordSyncOperation(batchSize: number, duration: number, success: boolean): void {
    // 更新批次大小
    ...
    const successCount = (this.metrics.syncSuccessRate * previousCount) + (success ? 1 : 0)
    this.metrics.syncSuccessRate = successCount / newCount

    console.log(`🔄 Sync Operation: ${batchSize} items (${duration}ms, ${success ? 'success' : 'error'})`)
  }
  ...
  private analyzeMetrics(): PerformanceAlert[] {
    ...
    } else if (this.metrics.cacheHitRate < this.thresholds.cacheHitRate.warning) {
      alerts.push({
        type: 'warning',
        metric: 'cacheHitRate',
        currentValue: this.metrics.cacheHitRate,
    ...
    // 檢查響應時間
    if (this.metrics.averageResponseTime > this.thresholds.averageResponseTime.critical) {
      alerts.push({
        type: 'critical',
    ...
    } else if (this.metrics.averageResponseTime > this.thresholds.averageResponseTime.warning) {
      alerts.push({
        type: 'warning',
        metric: 'averageResponseTime',
        currentValue: this.metrics.averageResponseTime,
    ...
    // 檢查數據庫性能
    if (this.metrics.dbOperationTime > this.thresholds.dbOperationTime.critical) {
      alerts.push({
        type: 'critical',
    ...
    // 檢查同步成功率
    if (this.metrics.syncSuccessRate < this.thresholds.syncSuccessRate.critical) {
      alerts.push({
        type: 'critical',
