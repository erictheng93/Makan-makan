...
export class OptimizedBackgroundSync {
  ...
  private requeueFailedItems(batch: SyncBatch): void {
    ...
    for (const item of batch.items) {
      ...
      if (item.retryCount < item.maxRetries) {
        ...
        console.log(`🔄 Requeued item ${item.id} (retry ${item.retryCount}/${item.maxRetries})`)
      } else {
  ...
  private updatePerformanceMetrics(
    ...
    const totalBatches = this.performanceMetrics.totalSynced / this.performanceMetrics.averageBatchSize || 1
    this.performanceMetrics.successRate = (oldSuccessRate * (totalBatches - 1) + (success ? 1 : 0)) / totalBatches

    // 更新平均批次大小
    this.performanceMetrics.averageBatchSize =
      (this.performanceMetrics.averageBatchSize * (totalBatches - 1) + itemCount) / totalBatches

    // 更新平均處理時間
    if (success && processingTime > 0) {
      this.performanceMetrics.averageProcessingTime =
        (this.performanceMetrics.averageProcessingTime * (totalBatches - 1) + processingTime) / totalBatches
    }
