...
class OptimizedOfflineStorageManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('optimizedOrders')) {
          const ordersStore = db.createObjectStore('optimizedOrders', { keyPath: 'id' })
          ordersStore.createIndex('restaurant_date', ['restaurant_id', 'created_at'], { unique: false })
          ordersStore.createIndex('synced_timestamp', ['synced', 'created_at'], { unique: false })
          ordersStore.createIndex('status_priority', ['status', 'total_amount'], { unique: false })
        }

        // 🚀 Compressed restaurants cache
        if (!db.objectStoreNames.contains('compressedRestaurants')) {
          const restaurantsStore = db.createObjectStore('compressedRestaurants', { keyPath: 'id' })
          restaurantsStore.createIndex('cached_at', 'cached_at', { unique: false })
          restaurantsStore.createIndex('size_index', 'compressedSize', { unique: false })
        }

        // 🚀 Optimized menu items with smart indexing
        if (!db.objectStoreNames.contains('optimizedMenuItems')) {
          const menuStore = db.createObjectStore('optimizedMenuItems', { keyPath: 'id' })
          menuStore.createIndex('restaurant_category', ['restaurant_id', 'category_id'], { unique: false })
          menuStore.createIndex('price_available', ['price', 'available'], { unique: false })
        ...
        if (!db.objectStoreNames.contains('userFavorites')) {
          const favoritesStore = db.createObjectStore('userFavorites', { keyPath: 'id' })
  ...
  async batchSaveOrders(orders: OfflineOrder[]): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < orders.length; i += this.BATCH_SIZE) {
        ...
        await Promise.all(
          batch.map(order => new Promise<void>((resolve, reject) => {
      ...
      console.log(`[Storage-OPT] ✅ Batch saved ${orders.length} orders in ${duration.toFixed(2)}ms`)
  ...
  async getOrdersOptimized(restaurantId: string, limit: number = 50): Promise<OfflineOrder[]> {
    const start = performance.now()
    ...
    try {
      ...
      const range = IDBKeyRange.bound([restaurantId, 0], [restaurantId, Date.now()])
      ...
      console.error('[Storage-OPT] ❌ Optimized order retrieval failed:', error)
      throw error
    }
  }
  ...
  async cacheRestaurantOptimized(restaurant: CachedRestaurant): Promise<void> {
    ...
    try {
      ...
      console.log(`[Storage-OPT] ✅ Cached restaurant ${restaurant.id} (${restaurantWithMeta.compressedSize} bytes)`)
  ...
  async cacheMenuItemsOptimized(menuItems: CachedMenuItem[]): Promise<void> {
    ...
    try {
      ...
      const duration = performance.now() - start
      this.logPerformance('cache_menu_items_optimized', duration, uniqueItems.length)

      console.log(`[Storage-OPT] ✅ Cached ${uniqueItems.length} menu items (${menuItems.length - uniqueItems.length} duplicates removed)`)
  ...
  async queryMenuItemsOptimized(
    ...
    try {
      ...
      if (categoryId) {
        ...
        if (priceRange) {
          results = results.filter(item => item.price >= priceRange.min && item.price <= priceRange.max)
        }
      ...
      const duration = performance.now() - start
      this.logPerformance('query_menu_items_optimized', duration, results.length)

      // Update cache metadata
      await this.updateCacheMetadata(`menu_${restaurantId}_${categoryId || 'all'}`)
  ...
  async smartCacheCleanup(): Promise<void> {
    ...
    try {
      ...
      const sortedByAccess = allMetadata.sort((a, b) => a.accessCount - b.accessCount)
      const toCleanup = sortedByAccess.slice(0, Math.floor(sortedByAccess.length * 0.1)) // Clean 10%
      ...
      console.log(`[Storage-OPT] ✅ Smart cleanup completed: ${cleanedCount} items cleaned`)
  ...
  getPerformanceReport(): any {
    ...
    const operations = {} as Record<string, { count: number; totalDuration: number; totalRecords: number }>

    // Analyze performance data
    for (const log of this.performanceLog) {
      if (!operations[log.operation]) {
        operations[log.operation] = { count: 0, totalDuration: 0, totalRecords: 0 }
      }
    ...
    for (const [operation, stats] of Object.entries(operations)) {
      ...
      if (avgDuration > 100) {
        report.recommendations.push(`Consider optimizing ${operation} (avg: ${avgDuration.toFixed(2)}ms)`)
      }
    }
  ...
  private decompressIfNeeded(data: any): any {
    if (data._compressed) {
      const { _compressed, _originalSize, _compressedData, ...originalData } = data
  ...
  private logPerformance(operation: string, duration: number, recordCount: number): void {
    ...
    console.log(`[Storage-OPT] 📊 ${operation}: ${duration.toFixed(2)}ms (${recordCount} records)`)
  }
  ...
  async getOfflineOrders(): Promise<OfflineOrder[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const results = request.result.map(item => this.decompressIfNeeded(item))
        resolve(results)
      }
      request.onerror = () => reject(request.error)
    })
...
optimizedOfflineStorage.initialize().catch(error => {
  console.error('[Storage-OPT] ❌ Failed to initialize optimized offline storage:', error)
})
