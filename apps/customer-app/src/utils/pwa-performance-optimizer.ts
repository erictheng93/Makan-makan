...
import { optimizedOfflineStorage } from './offline-storage-optimized'
...
export class ServiceWorkerOptimizer {
  ...
  // 記錄性能指標
  recordPerformance(request: Request, responseTime: number, fromCache: boolean, success: boolean): void {
    ...
    if (fromCache) {
      ...
      current.avgNetworkTime = (current.avgNetworkTime * (current.totalRequests - 1) + responseTime) / current.totalRequests
      if (success) {
    ...
    current.networkSuccessRate = current.networkSuccesses / (current.totalRequests - current.cacheHits)
...
export class IndexedDBOptimizer {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('optimizedOrders')) {
          ...
          ordersStore.createIndex('status_priority', ['status', 'priority'], { unique: false })
          ordersStore.createIndex('synced_timestamp', ['synced', 'timestamp'], { unique: false })
        }

        // 壓縮數據存儲
        if (!db.objectStoreNames.contains('compressedCache')) {
          const cacheStore = db.createObjectStore('compressedCache', { keyPath: 'key' })
  ...
  private decompressData(data: any): any {
    if (data._compressed) {
      const { _compressed, _originalSize, compressedSize, ...original } = data
      return original
    }
    return data
  }
  ...
  private logPerformance(operation: string, duration: number, recordCount?: number): void {
    ...
    console.log(`📊 IndexedDB ${operation}: ${duration.toFixed(2)}ms${recordCount ? ` (${recordCount} records)` : ''}`)
  }
  ...
  getPerformanceReport(): any {
    ...
    const operations = {} as Record<string, { count: number; totalDuration: number }>
...
export class BackgroundSyncOptimizer {
  ...
  // 智慧批次處理
  async queueForBatchSync(type: string, data: any, priority: 'low' | 'normal' | 'high' | 'critical' = 'normal'): Promise<void> {
  ...
  getBatchStatus(): Record<string, any> {
    ...
    for (const [type, queue] of this.batchQueue.entries()) {
      status[type] = {
        ...
        oldestItem: queue.length > 0 ? Date.now() - queue[queue.length - 1].timestamp : 0
      }
    }
