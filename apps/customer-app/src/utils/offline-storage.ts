...
class OfflineStorageManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('cachedRestaurants')) {
          const restaurantsStore = db.createObjectStore('cachedRestaurants', { keyPath: 'id' })
          restaurantsStore.createIndex('cached_at', 'cached_at', { unique: false })
        }

        // Menu items cache
        if (!db.objectStoreNames.contains('cachedMenuItems')) {
          const menuStore = db.createObjectStore('cachedMenuItems', { keyPath: 'id' })
  ...
  async getCachedRestaurant(restaurantId: string): Promise<CachedRestaurant | null> {
  ...
  async saveFavorite(favorite: { id: string; type: 'restaurant' | 'menu_item'; data: any }): Promise<void> {
  ...
  async clearAllCache(): Promise<void> {
    const storeNames = ['cachedRestaurants', 'cachedMenuItems', 'userFavorites']
