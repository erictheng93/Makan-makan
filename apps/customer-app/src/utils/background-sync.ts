...
export interface SyncEvent {
  id: string
  type: 'order_submission' | 'favorite_sync' | 'settings_sync' | 'feedback_sync'
  data: any
  timestamp: string
  retryCount: number
  maxRetries: number
...
class CustomerBackgroundSyncService {
  ...
  private saveSyncQueue(): void {
    try {
      localStorage.setItem('customer_sync_queue', JSON.stringify(this.syncQueue))
    } catch (error) {
  ...
  private async syncSingleOrder(order: OfflineOrder): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[Background Sync] Order ${order.id} synced successfully as ${result.order_id}`)
      ...
      console.error(`[Background Sync] Failed to sync order ${order.id}:`, error)
  ...
  async queueOrderSync(order: OfflineOrder, reason: 'new' | 'retry' = 'new'): Promise<void> {
    const syncEvent: SyncEvent = {
      id: `order_${order.id}_${Date.now()}`,
      type: 'order_submission',
      data: order,
      timestamp: new Date().toISOString(),
      retryCount: reason === 'retry' ? (this.getExistingRetryCount(order.id) + 1) : 0,
      maxRetries: 5,
      priority: 'high'
    }
    ...
    try {
      ...
      for (const event of sortedQueue) {
        if (event.retryCount >= event.maxRetries) {
          console.warn(`[Background Sync] Max retries reached for event ${event.id}`)
        ...
        try {
          ...
          console.error(`[Background Sync] Failed to process event ${event.id}:`, error)
  ...
  private async processSyncEvent(event: SyncEvent): Promise<void> {
    switch (event.type) {
      ...
      case 'feedback_sync':
        ...
        break
      default:
        console.warn(`[Background Sync] Unknown sync event type: ${event.type}`)
    }
  }
  ...
  private async registerBackgroundSync(tag: string): Promise<void> {
    if ('serviceWorker' in navigator && 'sync' in (window as any).ServiceWorkerRegistration.prototype) {
      try {
        ...
        console.error(`[Background Sync] Failed to register sync: ${tag}`, error)
      }
    }
  }
  ...
  private async handleServiceWorkerSync(tag: string): Promise<void> {
    ...
    switch (tag) {
      ...
      case 'feedback-sync':
        ...
        const feedbackEvents = this.syncQueue.filter(e => e.type === 'feedback_sync')
  ...
  private addOrUpdateSyncEvent(event: SyncEvent): void {
    const existingIndex = this.syncQueue.findIndex(e => e.type === event.type)
    if (existingIndex >= 0) {
  ...
  private getExistingRetryCount(orderId: string): number {
    const existing = this.syncQueue.find(e =>
      e.type === 'order_submission' && e.data.id === orderId
    )
  ...
  private notifyOrderSynced(offlineOrderId: string, onlineOrderId: string): void {
