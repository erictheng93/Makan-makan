...
watch(() => props.members, (newMembers) => {
  if (props.modelValue.mode === 'custom' && Object.keys(customShares.value).length === 0) {
...
const totalSharePercentage = computed(() => {
  return Object.values(customShares.value).reduce((sum, share) => sum + share, 0)
})
      ...
      >
        <div class="flex items-center space-x-3">
          <div
            ...
            :class="modelValue.mode === mode.id ? 'bg-indigo-500 text-white' : 'bg-gray-100 text-gray-500'"
          >
    ...
    <div v-if="modelValue.mode === 'equal'" class="bg-gray-50 rounded-xl p-4">
      ...
      <div class="space-y-2">
        ...
        >
          ...
          <span class="font-medium text-gray-900">{{ formatPrice(perPersonAmount) }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="modelValue.mode === 'custom'" class="bg-gray-50 rounded-xl p-4">
      <div class="mb-3 flex items-center justify-between">
        ...
        <span
          ...
          :class="isValidCustomShares ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'"
        >
        ...
        >
          ...
          <div class="flex items-center space-x-3">
            <input
              ...
              @input="updateCustomShare(member.id, Number(($event.target as HTMLInputElement).value))"
            />
      ...
      >
        ...
        <span>{{ t('group.sharesMustEqual100', '比例總和必須等於 100%') }}</span>
      </div>
    </div>

    <!-- Single Payer Selection -->
    <div v-if="modelValue.mode === 'single_payer'" class="bg-gray-50 rounded-xl p-4">
        ...
        >
          <div class="flex items-center space-x-3">
            <div
              ...
              :class="selectedPayer === member.id ? 'bg-indigo-500 text-white' : 'bg-gray-100 text-gray-500'"
            >
              <svg v-if="selectedPayer === member.id" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              <span v-else class="text-sm font-medium">{{ member.name.charAt(0) }}</span>
            </div>
            <span class="font-medium text-gray-900">
              {{ member.id === currentUserId ? t('group.me', '我') : member.name }}
            </span>
