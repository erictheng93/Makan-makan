...
import type { GroupCartItem, GroupMember, SplitBillConfig } from '@/composables/useGroupOrder'
...
<template>
  <div class="group-cart-panel bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <!-- Header -->
    <div class="px-4 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2">
          ...
          <span class="font-semibold">{{ t('group.sharedCart', '群組購物車') }}</span>
        </div>
    ...
    <div class="divide-y divide-gray-100">
          ...
          <div class="space-y-3">
            ...
            >
              <div class="flex-1 min-w-0">
                ...
                <div v-if="item.notes" class="text-xs text-gray-400 italic mt-0.5">
                  {{ item.notes }}
                </div>
              </div>
              ...
              <div class="flex items-center space-x-3 ml-4">
                ...
                <div v-if="canModifyItem(item)" class="flex items-center space-x-1">
                  <!-- Quantity Controls -->
                  <button
      ...
      <div v-if="cartItems.length === 0" class="p-8 text-center text-gray-500">
        ...
        <p class="text-sm mt-1">{{ t('group.startAdding', '開始添加餐點吧！') }}</p>
      </div>
    </div>
    ...
    <div class="px-4 py-4 bg-gray-50 border-t border-gray-100">
      ...
      <div v-if="isHost" class="mb-4">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button
            v-for="mode in ['equal', 'by_item', 'custom', 'single_payer'] as const"
            :key="mode"
            class="px-2 py-1.5 text-xs font-medium rounded-lg transition-colors"
            :class="splitBillConfig.mode === mode
      ...
      <div class="space-y-2">
        ...
        <div class="flex justify-between text-sm">
          <span class="text-gray-600">{{ t('group.myShare', '我的份額') }}</span>
          <span class="font-bold text-indigo-600">{{ formatPrice(myShare) }}</span>
        </div>
      </div>
    </div>
  </div>
