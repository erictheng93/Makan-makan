...
export function useGroupOrder(options: {
  ...
  async function createGroupOrder(): Promise<string | null> {
    ...
    try {
      const response = await apiClient.post<{ groupOrderId: string }>('/group-orders', {
        restaurantId,
        tableId,
        hostId: userId,
        hostName: userName
  ...
  async function joinGroupOrder(groupOrderId: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await apiClient.get<GroupOrder>(`/group-orders/${groupOrderId}`)
  ...
  async function connectToGroupOrder(groupOrderId: string): Promise<void> {
    try {
      // Get WebSocket token
      const tokenResponse = await apiClient.post<{ token: string }>('/realtime/auth/token', {
        roomType: 'customer',
        roomId: groupOrderId,
        restaurantId,
        tableId
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'member_status_updated':
          handleMemberStatusUpdated(data.data as { memberId: string; isOnline: boolean })
          break
  ...
  function handleCartItemUpdated(updatedItem: GroupCartItem): void {
    if (!groupOrder.value) return
    const index = groupOrder.value.cartItems.findIndex(i => i.id === updatedItem.id)
    if (index !== -1) {
  ...
  function handleCartItemRemoved(data: { itemId: string }): void {
    if (!groupOrder.value) return
    groupOrder.value.cartItems = groupOrder.value.cartItems.filter(i => i.id !== data.itemId)
    groupOrder.value.updatedAt = Date.now()
  }
  ...
  function handleMemberLeft(data: { memberId: string }): void {
    if (!groupOrder.value) return
    groupOrder.value.members = groupOrder.value.members.filter(m => m.id !== data.memberId)
  }

  function handleMemberStatusUpdated(data: { memberId: string; isOnline: boolean }): void {
    if (!groupOrder.value) return
  ...
  // Cart operations
  function addToCart(item: Omit<GroupCartItem, 'id' | 'addedBy' | 'addedByName' | 'addedAt'>): void {
    ...
    // Sync with server
    apiClient.post(`/group-orders/${groupOrder.value?.id}/cart`, cartItem).catch((err: unknown) => {
  ...
  function updateCartItem(itemId: string, updates: Partial<GroupCartItem>): void {
    if (!groupOrder.value) return
  ...
  async function submitOrder(): Promise<boolean> {
    ...
    try {
      const response = await apiClient.post(`/group-orders/${groupOrder.value.id}/submit`)
