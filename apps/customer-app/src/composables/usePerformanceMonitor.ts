      ...
      if ('indexedDB' in window) {
        ...
        for (const report of reportsToSend) {
          try {
            ...
            await deleteStore.delete(report.timestamp)
          } catch (e) {
            console.error('[PerformanceMonitor] Failed to send queued report:', e)
            break // Stop processing if network fails
          }
        }
      }
    } catch (e) {
      console.error('[PerformanceMonitor] Failed to process queued reports:', e)
    }
  }
