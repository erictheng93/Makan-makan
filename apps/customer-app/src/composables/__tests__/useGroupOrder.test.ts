...
// Mock environment variable for VITE_REALTIME_URL
// This must be done before importing the module that uses it
vi.stubEnv("VITE_REALTIME_URL", "wss://test-realtime.example.com");
...
describe("useGroupOrder", () => {
  ...
  describe("分帳模式設置", () => {
    it("setSplitBillMode 應該更新模式（僅限 host）", () => {
      const { groupOrder, setSplitBillMode, isHost } =
        useGroupOrder(mockOptions);
