  ...
  <div class="min-h-screen bg-gray-50">
      ...
      <div class="max-w-md mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex-1 text-center">
            <h1 class="text-lg font-semibold text-gray-900">{{ t('cart.title') }}</h1>
            <p class="text-sm text-gray-500">{{ restaurant?.name }} · {{ t('order.details.table') }} {{ tableId }}</p>
          </div>
    ...
    <main class="max-w-md mx-auto">
      ...
      <div v-if="cartStore.isEmpty" class="px-4 py-16 text-center">
        ...
        <p class="text-gray-600 mb-8">{{ t('cart.emptyDesc') }}</p>
        <button
      ...
      <div v-else class="px-4 py-6 space-y-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('order.title') }}</h3>
          ...
          <div class="space-y-3">
            ...
            <!-- 服務費 -->
            <div v-if="serviceCharge > 0" class="flex justify-between text-gray-600">
        ...
        <!-- 備註欄位 -->
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <label for="order-notes" class="block text-sm font-medium text-gray-700 mb-3">
        ...
        <!-- 顧客資訊 -->
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('order.details.customerInfo') }}</h3>
          
          <div class="space-y-4">
            <div>
              <label for="customer-name" class="block text-sm font-medium text-gray-700 mb-2">
                {{ t('order.details.customerName') }} ({{ t('menuItem.optional') }})
              </label>
              <input
                id="customer-name"
    ...
    <!-- 底部確認按鈕 -->
    <div v-if="!cartStore.isEmpty" class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4">
      <div class="max-w-md mx-auto">
        ...
        <div class="mt-3 text-center">
          <p class="text-sm text-gray-500">
            {{ t('common.confirm') }}
            <router-link to="/terms" class="text-indigo-600 hover:text-indigo-500">{{ t('terms.title') }}</router-link>
            {{ t('common.next') }}
            <router-link to="/privacy" class="text-indigo-600 hover:text-indigo-500">{{ t('privacy.title') }}</router-link>
          </p>
        </div>
      </div>
    </div>
...
const { mutate: createOrder } = useMutation({
  ...
  onSuccess: (order) => {
    ...
    cartStore.clearCart()
    router.push(`/restaurant/${props.restaurantId}/table/${props.tableId}/order/${order.id}`)
  },
  onError: (error: any) => {
...
const saveCustomerInfo = () => {
  try {
    localStorage.setItem('makanmakan_customer_info', JSON.stringify(customerInfo.value))
  } catch (error) {
