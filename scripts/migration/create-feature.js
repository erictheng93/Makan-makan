...
if (!options.name) {
  ...
  console.log('Usage: node scripts/migration/create-feature.js --name=my-feature --type=simple')
  process.exit(1)
}
...
if (!/^[a-z-]+$/.test(featureName)) {
  console.error('❌ Error: Feature name must be lowercase with hyphens only (e.g., "my-feature")')
  process.exit(1)
}
...
// Paths
const templatePath = path.join(__dirname, '../../apps/api/src/shared/templates/feature-template')
...
function copyTemplate(src, dest) {
  ...
  items.forEach(item => {
    ...
    if (fs.statSync(srcPath).isDirectory()) {
      ...
      Object.entries(transformations).forEach(([placeholder, replacement]) => {
        finalDestPath = finalDestPath.replace(new RegExp(placeholder, 'g'), replacement)
      })
...
try {
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    packageJson.scripts[`test:${featureName}`] = `cd apps/api && npm run test -- src/features/${featureName}`
    packageJson.scripts[`build:${featureName}`] = `cd apps/api && npm run build`
