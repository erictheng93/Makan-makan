...
if (!options.from || !options.to) {
  ...
  console.log('Usage: node scripts/migration/update-imports.js --from="routes/" --to="features/"')
  process.exit(1)
}
...
tsFiles.forEach(filePath => {
  try {
    ...
    const importRegex = new RegExp(`import\\s+.*?from\\s+['"\`]([^'"\`]*${fromPattern.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&')}[^'"\`]*)['"\`]`, 'gm')
    ...
    while ((match = importRegex.exec(content)) !== null) {
      ...
      const newImportPath = importPath.replace(new RegExp(fromPattern.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\$&'), 'g'), toPattern)
...
if (isDryRun) {
  ...
  if (process.argv.includes('--check-types')) {
    ...
    try {
      ...
      console.error('❌ TypeScript check failed. You may need to fix import paths manually.')
    }
  }
}
