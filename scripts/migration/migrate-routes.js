...
const schemaMatches = sourceContent.match(
  /const\s+\w+Schema\s*=[\s\S]*?(?=const|\n\n|$)/g,
);
console.log(
  `📋 Validation schemas found: ${schemaMatches ? schemaMatches.length : 0}`,
);

// Extract helper functions
const helperFunctionMatches = sourceContent.match(
  /^(async\s+)?function\s+\w+[\s\S]*?\n\}/gm,
);
console.log(
  `⚙️ Helper functions found: ${helperFunctionMatches ? helperFunctionMatches.length : 0}`,
);
