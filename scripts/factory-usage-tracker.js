...
const CONFIG = {
  ...
  // Factory 模式
  factoryPatterns: {
    userFactory: /userFactory\.(build|buildList|buildAdmin|buildShopOwner|buildChef|buildServiceCrew|buildCashier|buildCustomer|buildRestaurantTeam)/g,
    restaurantFactory: /restaurantFactory\.(build|buildList|buildWithShopMode|buildFastFood|buildFineDining|buildCafe)/g,
    categoryFactory: /categoryFactory\.(build|buildList|buildRestaurantCategories)/g,
    menuItemFactory: /menuItemFactory\.(build|buildList|buildForCategory|buildPopular|buildOnSale|buildVegetarian)/g,
    orderFactory: /orderFactory\.(build|buildList|buildPending|buildInProgress|buildCompleted|buildTakeaway|buildDelivery)/g,
    orderItemFactory: /orderItemFactory\.(build|buildList|buildForOrder|buildPrepared|buildServed)/g,
...
function generateReport(fileStats) {
  ...
  fileStats.forEach(stats => {
    ...
    Object.entries(stats.factoryUsage).forEach(([factory, count]) => {
      report.factoryUsage[factory] = (report.factoryUsage[factory] || 0) + count
    })
  })

  // 計算採用率
  report.summary.adoptionRate = (
    (report.summary.filesUsingFactory / report.summary.totalFiles) * 100
  ).toFixed(2)
...
function generateMarkdownReport(report) {
  ...
  if (report.files.missingReset.length > 0) {
    lines.push(`- [ ] 為 ${report.files.missingReset.length} 個文件添加 \`resetAllFactories()\``)
  }
