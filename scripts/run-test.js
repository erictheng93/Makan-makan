...
// Configuration
const MIGRATIONS_DIR = path.join(__dirname, '..', 'packages', 'database', 'migrations_v2');
...
function executeMigrations(db) {
  ...
  MIGRATIONS.forEach((filename, index) => {
    ...
    log(`\n[${migrationNum}/${MIGRATIONS.length}] Executing ${filename}...`, 'blue');
  ...
  log(`  Success: ${testResults.migrations.success}/${MIGRATIONS.length}`, 'green');
...
function validateStructure(db) {
  ...
  try {
    ...
    log(`Indexes: ${indexCount} (expected: ${EXPECTED.indexes})`,
    ...
    log(`Views: ${viewCount} (expected: ${EXPECTED.views})`,
    ...
    log(`Triggers: ${triggerCount} (expected: ${EXPECTED.triggers})`,
...
function generateReport() {
  ...
  const reportPath = path.join(__dirname, '..', 'docs', 'migrations_v2',
...
function printSummary(reportPath) {
  ...
  log(`\nTest Duration: ${((Date.now() - testResults.startTime) / 1000).toFixed(2)}s`, 'cyan');
  log(`\nMigrations:`, 'bright');
  ...
  log(`  Tables:   ${testResults.structure.tables?.actual}/${EXPECTED.tables} ${testResults.structure.tables?.match ? '✅' : '⚠️'}`);
  log(`  Indexes:  ${testResults.structure.indexes?.actual}/${EXPECTED.indexes} ${testResults.structure.indexes?.match ? '✅' : '⚠️'}`);
  log(`  Views:    ${testResults.structure.views?.actual}/${EXPECTED.views} ${testResults.structure.views?.match ? '✅' : '⚠️'}`);
  log(`  Triggers: ${testResults.structure.triggers?.actual}/${EXPECTED.triggers} ${testResults.structure.triggers?.match ? '✅' : '⚠️'}`);

  log(`\nReports:`, 'bright');
  log(`  Test Report: ${reportPath}`, 'cyan');
  log(`  Test Log: ${LOG_FILE}`, 'cyan');

  const allPassed = testResults.migrations.success === 16 &&
