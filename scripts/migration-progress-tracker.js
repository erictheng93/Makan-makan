...
function generateMarkdownReport(report) {
          ...
          const statusIcon =
            module.status === 'completed'
              ...
              : module.status === 'in-progress'
                ? '🔄'
                : '⏳'
          const progressBar = '█'.repeat(Math.floor((module.progress || 0) / 5))
          lines.push(
  ...
  report.milestones.forEach(milestone => {
    ...
    const targetDate = new Date(milestone.targetDate).toLocaleDateString('zh-TW')
  ...
  switch (command) {
    ...
    case 'update':
      ...
      if (!moduleName || !statusValue) {
        console.error('❌ 使用方式: npm run migration:update <module> <status> [progress]')
    ...
    default:
      ...
      console.log('')
      console.log('使用方式:')
      console.log('  npm run migration:init                        # 初始化狀態文件')
      console.log('  npm run migration:update <module> <status> [progress]  # 更新模組狀態')
      console.log('  npm run migration:report                      # 生成進度報告')
      console.log('')
      console.log('範例:')
