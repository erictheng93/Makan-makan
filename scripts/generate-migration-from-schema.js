...
function mapSqliteType(type) {
  ...
  if (upperType.includes('TEXT') || upperType.includes('CHAR') || upperType.includes('CLOB')) return 'TEXT';
  if (upperType.includes('REAL') || upperType.includes('DOUBLE') || upperType.includes('FLOAT')) return 'REAL';
  if (upperType.includes('BLOB')) return 'BLOB';
  if (upperType.includes('BOOL')) return 'INTEGER';  // SQLite 使用 INTEGER 存儲 BOOLEAN
  if (upperType.includes('DATETIME') || upperType.includes('DATE') || upperType.includes('TIME')) return 'TEXT';  // DATETIME → TEXT
  return upperType;
}
...
function generateCreateTable(tableName, schema, isNew = false) {
  ...
  for (const col of schema) {
    ...
    // DEFAULT
    if (col.dflt_value && col.dflt_value !== 'null' && col.dflt_value !== 'NULL') {
      ...
      if (col.name === 'restaurant_id' && isNew) {
        ...
        if (col.dflt_value.includes('strftime') || col.dflt_value.includes('(')) {
...
`;
  ...
  writeFileSync(
    join('C:\\Users\\minim\\OneDrive\\文档\\Code\\platform\\makanmakan\\packages\\database\\migrations', '0040_comprehensive_restaurant_id_migration.sql'),
    migration0040
  );
  console.log('  ✅ Generated 0040_comprehensive_restaurant_id_migration.sql\n');
...
`;
  ...
  writeFileSync(
    join('C:\\Users\\minim\\OneDrive\\文档\\Code\\platform\\makanmakan\\packages\\database\\migrations', '0041_remaining_tables_structure.sql'),
    migration0041
  );
...
`;
  ...
  writeFileSync(
    join('C:\\Users\\minim\\OneDrive\\文档\\Code\\platform\\makanmakan\\packages\\database\\migrations', '0042_migrate_data_part1.sql'),
    migration0042
  );
...
`;
  ...
  writeFileSync(
    join('C:\\Users\\minim\\OneDrive\\文档\\Code\\platform\\makanmakan\\packages\\database\\migrations', '0043_migrate_data_part2.sql'),
    migration0043
  );
