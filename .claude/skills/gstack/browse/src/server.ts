...
if (BROWSE_PARENT_PID > 0) {
  setInterval(() => {
    try {
      ...
      console.log(
        `[browse] Parent process ${BROWSE_PARENT_PID} exited, shutting down`,
      );
      shutdown();
    }
  }, 15_000);
    ...
    if (READ_COMMANDS.has(command)) {
      ...
      if (isScoped && command === "text") {
        ...
        if (strippedDescs.length > 0) {
          console.warn(
            `[browse] Content security: stripped ${strippedDescs.length} hidden elements for ${tokenInfo.clientId}`,
          );
        }
        try {
...
async function start() {
  ...
  try {
    ...
    if (err.code !== "ENOENT")
      console.debug("[browse] Log cleanup console:", err.message);
  }
  try {
    fs.unlinkSync(NETWORK_LOG_PATH);
  } catch (err: any) {
    if (err.code !== "ENOENT")
      console.debug("[browse] Log cleanup network:", err.message);
  }
  try {
    fs.unlinkSync(DIALOG_LOG_PATH);
  } catch (err: any) {
    if (err.code !== "ENOENT")
      console.debug("[browse] Log cleanup dialog:", err.message);
  }
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === "/welcome") {
        const welcomePath = (() => {
          ...
          try {
            if (require("fs").existsSync(projectWelcome)) return projectWelcome;
          } catch (err: any) {
            console.warn(
              "[browse] Error checking project welcome page:",
              err.message,
            );
          }
          ...
          try {
            if (require("fs").existsSync(builtinWelcome)) return builtinWelcome;
          } catch (err: any) {
            console.warn(
              "[browse] Error checking builtin welcome page:",
              err.message,
            );
          }
          return null;
