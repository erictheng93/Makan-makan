...
class PWAOptimizationVerifier {
  ...
  async verifyServiceWorkerOptimization() {
    ...
    if ("serviceWorker" in navigator) {
      ...
      tests.optimizedSWRegistered = registrations.some(
        (reg) => reg.active && reg.active.scriptURL.includes("sw-optimized.js"),
      );
    ...
    if ("caches" in window) {
      ...
      tests.cacheOptimized = cacheNames.some(
        (name) => name.includes("optimized") || name.includes("preload"),
      );
    }
  ...
  async verifyIndexedDBOptimization() {
    ...
    if (window.pwaPerformanceManager) {
      const report =
        window.pwaPerformanceManager.databaseOptimizer.getPerformanceReport();
  ...
  async verifyBackgroundSyncOptimization() {
    ...
    // 檢查優化背景同步
    if (
      window.pwaPerformanceManager &&
      window.pwaPerformanceManager.optimizedBackgroundSync
    ) {
      ...
      try {
        ...
        // 測試優先級處理
        if (queueStatus.test) {
          tests.priorityHandling =
            queueStatus.test.priorities &&
  ...
  async verifyPerformanceMonitoring() {
    ...
    if (
      window.pwaPerformanceManager &&
      window.pwaPerformanceManager.performanceMonitor
    ) {
  ...
  async verifyCacheStrategy() {
    ...
    if (window.pwaPerformanceManager) {
      try {
        const report =
          await window.pwaPerformanceManager.getComprehensivePerformanceReport();
        ...
        if (report.performance && report.performance.metrics) {
          ...
          const avgResponseTime =
            report.performance.metrics.averageResponseTime;
  ...
  generateReport() {
    ...
    const overallScore =
      this.testResults.reduce((sum, r) => sum + (r.score || 0), 0) / totalTests;
    ...
    console.log(
      `總體評分: ${report.summary.overallScore}/100 (${report.summary.status})`,
    );
    console.log(
      `測試結果: ${passedTests} 通過, ${partialTests} 部分通過, ${failedTests} 失敗`,
    );
