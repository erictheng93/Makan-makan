...
/*  查詢結轉成果array             
$sql0="SELECT * FROM shop_table INNER JOIN shop_info ON shop_table.shop_ID=shop_info.shop_ID WHERE shop_table.shop_ID='$kedai' AND shop_table.st_tableNumber='$meja'";//搜索訂單資料指令 捉取數據庫裡Banner的資料
$result0=mysqli_query($link,$sql0);//執行指令,並把結果存到result
$number_result0=mysqli_num_rows($result0);//符合條件的查詢結果的筆數
	if($number_result0>0){
		$result0_value=mysqli_fetch_array($result0);
	}
*/
...
$_SESSION['shopOrder_sn']=$last_id;
$_SESSION['shopOrder_ID']=$shopOrder_ID;
$_SESSION['shop_ID']=$shop_ID;
$_SESSION['shopOrder_table']=$shopOrder_table;
$_SESSION['shopOrderMenu_ID']=$shopOrderMenu_ID;
...
// Ensure $ShopCat is not null and is a string
if (is_string($ShopCat)) {
    $ShopArray = explode(',', $ShopCat); // Split a comma-delimited string into an array
} else {
    $ShopArray = []; // Set $ShopArray to an empty array if $ShopCat is not a valid string
}
            ...
            <tr>
                <td>
					...
					<table width="100%" border="0"  >
					  <tbody>
						<tr>
						  <td><h5><?php echo $arr[0]['shop_name']?></h5></td>
						  <td width="9%" align="right"><a href="totalorder.php?a=<?php echo $last_id ?>" target="iframe2" rel="modal:open" onclick="document.getElementById('id01').style.display='block'" class="w3-button w3-blue w3-margin-left w3-tiny"><img src="images/food-cart.png" width="30" height="30"/>送出</a></td>
						</tr>
						<tr>
						  <td colspan="2">
							...
							<div class="projcard-innerbox">
								...
								<div class="projcard-textbox">
									<table width="200" border="0" cellspacing="0" cellpadding="0">
										<tr>
										  <td width="45%"><?php echo $arr2[$i]['menu_foodname']?></td>
										  <td width="55%"><a href="order.php?a=<?php echo $arr2[$i]['menu_sn']?>" target="iframe2" rel="modal:open" onclick="document.getElementById('id01').style.display='block'" class="w3-button w3-blue w3-margin-left w3-tiny">選取</a></td>
										</tr>
										<tr>
										  <td><?php echo $arr2[$i]['menu_price']?></td>
									    </tr>
										<tr>
										  <td colspan="2"><?php echo mb_substr($arr2[$i]['menu_describe'],0,12)?>..</td>
									    </tr>
									</table>				
							  </div>									
							</div>
