...
export class ConnectionManager {
  ...
  private async executeBatch(): Promise<void> {
    ...
    await Promise.allSettled(batch.map((item) => this.executeWithRetry(item)));
  }
  ...
  private async executeWithRetry<T>(queryItem: QueryBatchItem): Promise<T> {
    ...
    try {
      ...
      if (
        ...
        queryItem.retryCount < queryItem.maxRetries
      ) {
        ...
        // Exponential backoff
        const delay =
          this.config.retryDelay * Math.pow(2, queryItem.retryCount);
