...
export type PlanDiscountType = typeof PLAN_DISCOUNT_TYPES[keyof typeof PLAN_DISCOUNT_TYPES]
export type UsageLogStatus = typeof USAGE_LOG_STATUS[keyof typeof USAGE_LOG_STATUS]
...
export const partnerships = sqliteTable('partnerships', {
  ...
  contractStartDate: integer('contract_start_date', { mode: 'timestamp_ms' }).notNull(),
  contractEndDate: integer('contract_end_date', { mode: 'timestamp_ms' }).notNull(),
  contractDocumentUrl: text('contract_document_url'),

  // 認證設定
  verificationMethod: text('verification_method').notNull().default('manual').$type<VerificationMethod>(),
  verificationConfig: text('verification_config', { mode: 'json' }).$type<Record<string, any>>().default({}),
  allowedEmailDomains: text('allowed_email_domains', { mode: 'json' }).$type<string[]>().default([]),

  // 優惠設定
  defaultDiscountType: text('default_discount_type').$type<'percentage' | 'fixed'>(),
  ...
  // 狀態控制
  status: text('status').notNull().default('draft').$type<PartnershipStatus>(),
  ...
  statusIdx: index('idx_partnerships_status').on(table.status, table.isActive),
  contractDatesIdx: index('idx_partnerships_contract_dates').on(table.contractStartDate, table.contractEndDate),
}))
...
export const partnershipPlans = sqliteTable('partnership_plans', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID().replace(/-/g, '')),
  ...
  applicableMenuItems: text('applicable_menu_items', { mode: 'json' }).$type<string[]>().default([]),
  applicableCategories: text('applicable_categories', { mode: 'json' }).$type<string[]>().default([]),
  excludedMenuItems: text('excluded_menu_items', { mode: 'json' }).$type<string[]>().default([]),
  excludedCategories: text('excluded_categories', { mode: 'json' }).$type<string[]>().default([]),

  // 時間限制
  applicableDays: text('applicable_days', { mode: 'json' }).$type<number[]>().default([]), // 0-6, 0=Sunday
  applicableTimeSlots: text('applicable_time_slots', { mode: 'json' }).$type<Array<{ start: string; end: string }>>().default([]),
  ...
  canCombineWithCoupons: integer('can_combine_with_coupons', { mode: 'boolean' }).default(false),
  canCombineWithPromotions: integer('can_combine_with_promotions', { mode: 'boolean' }).default(false),
  ...
  partnershipIdx: index('idx_partnership_plans_partnership').on(table.partnershipId),
  restaurantIdx: index('idx_partnership_plans_restaurant').on(table.restaurantId),
  codeIdx: index('idx_partnership_plans_code').on(table.partnershipId, table.restaurantId, table.planCode),
  validPeriodIdx: index('idx_partnership_plans_valid_period').on(table.validFrom, table.validTo),
}))
...
export const verifiedMembers = sqliteTable('verified_members', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID().replace(/-/g, '')),
  ...
  verificationExpiry: integer('verification_expiry', { mode: 'timestamp_ms' }),
  ...
  partnershipIdx: index('idx_verified_members_partnership').on(table.partnershipId),
  customerIdx: index('idx_verified_members_customer').on(table.customerId),
  memberIdIdx: index('idx_verified_members_member_id').on(table.partnershipId, table.memberId),
...
export const partnershipUsageLogs = sqliteTable('partnership_usage_logs', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID().replace(/-/g, '')),
  ...
  orderItems: text('order_items', { mode: 'json' }).$type<any[]>().default([]),

  // 使用資訊
  usedAt: integer('used_at', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch('now') * 1000)`),
  ...
  // 狀態
  status: text('status').notNull().default('completed').$type<UsageLogStatus>(),
  ...
  partnershipIdx: index('idx_partnership_usage_logs_partnership').on(table.partnershipId),
  ...
  restaurantIdx: index('idx_partnership_usage_logs_restaurant').on(table.restaurantId),
...
export const partnershipsRelations = relations(partnerships, ({ many, one }) => ({
...
export const partnershipPlansRelations = relations(partnershipPlans, ({ one, many }) => ({
...
export const verifiedMembersRelations = relations(verifiedMembers, ({ one, many }) => ({
  partnership: one(partnerships, {
...
export const partnershipUsageLogsRelations = relations(partnershipUsageLogs, ({ one }) => ({
  partnership: one(partnerships, {
