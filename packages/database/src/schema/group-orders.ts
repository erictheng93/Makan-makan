...
export const groupOrders = sqliteTable(
  "group_orders",
  {
    ...
    // 訂單狀態
    status: text("status").notNull().default("active"), // active, checkout, completed, cancelled
    ...
    lockedAt: integer("locked_at_ms", { mode: "timestamp_ms" }),
...
export const groupMembers = sqliteTable(
  "group_members",
  {
    ...
    // 狀態與時間 (milliseconds)
    joinedAt: integer("joined_at_ms", { mode: "timestamp_ms" }).notNull(),
    lastActiveAt: integer("last_active_at_ms", {
      mode: "timestamp_ms",
    }).notNull(),
