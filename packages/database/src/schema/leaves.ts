...
export const leaveTypes = sqliteTable('leave_types', {
  ...
  // Accrual Rules (計算規則)
  accrualType: text('accrual_type', { enum: ['yearly', 'monthly', 'none'] }).notNull().default('yearly'),
  accrualAmount: real('accrual_amount').notNull().default(0),
  accrualBasedOnSeniority: integer('accrual_based_on_seniority', { mode: 'boolean' }).notNull().default(false),
  ...
  canCarryover: integer('can_carryover', { mode: 'boolean' }).notNull().default(false),
  carryoverMaxDays: real('carryover_max_days'),
  carryoverExpiryMonths: integer('carryover_expiry_months'),

  // Documentation & Payment (文件與給付)
  requiresDocumentation: integer('requires_documentation', { mode: 'boolean' }).notNull().default(false),
  documentationRequiredAfterDays: integer('documentation_required_after_days'),
  isPaid: integer('is_paid', { mode: 'boolean' }).notNull().default(true),
  paymentRate: real('payment_rate').notNull().default(1.0), // 0.0 to 1.0

  // Restrictions (限制條件)
  allowHalfDay: integer('allow_half_day', { mode: 'boolean' }).notNull().default(true),
...
export const leaveRequests = sqliteTable('leave_requests', {
  ...
  startPeriod: text('start_period', { enum: ['full', 'am', 'pm'] }).notNull().default('full'),
  endPeriod: text('end_period', { enum: ['full', 'am', 'pm'] }).notNull().default('full'),
  ...
  replacementNotified: integer('replacement_notified', { mode: 'boolean' }).notNull().default(false),
...
export const leaveApprovalRules = sqliteTable('leave_approval_rules', {
  ...
  // Approvers (審批人)
  approverType: text('approver_type', { enum: ['role', 'specific_user'] }).notNull(),
  approverRoleIds: text('approver_role_ids'), // JSON array
  approverUserIds: text('approver_user_ids'), // JSON array

  // Auto-approval (自動審批)
  enableAutoApproval: integer('enable_auto_approval', { mode: 'boolean' }).notNull().default(false),
  autoApprovalConditions: text('auto_approval_conditions'), // JSON object

  // Escalation (升級)
  enableAutoEscalation: integer('enable_auto_escalation', { mode: 'boolean' }).notNull().default(false),
  escalationTimeoutHours: integer('escalation_timeout_hours'),
  escalationToUserId: integer('escalation_to_user_id').references(() => users.id),
