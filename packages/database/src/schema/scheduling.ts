...
export const shiftTemplates = sqliteTable('shift_templates', {
  ...
  shiftType: text('shift_type', { enum: ['regular', 'split', 'overnight'] }).notNull().default('regular'),
  ...
  // Split Shift Support
  isSplitShift: integer('is_split_shift', { mode: 'boolean' }).notNull().default(false),
...
export const schedulingRules = sqliteTable('scheduling_rules', {
  ...
  severity: text('severity', { enum: ['error', 'warning', 'info'] }).notNull().default('warning'),

  // System Rule Flag
  isSystemRule: integer('is_system_rule', { mode: 'boolean' }).notNull().default(false),
  isActive: integer('is_active', { mode: 'boolean' }).notNull().default(true),
...
export const schedulingConflicts = sqliteTable('scheduling_conflicts', {
  ...
  ruleId: integer('rule_id').references(() => schedulingRules.id, { onDelete: 'set null' }),
  ...
  // Resolution
  status: text('status', { enum: ['unresolved', 'acknowledged', 'resolved', 'ignored'] }).notNull().default('unresolved'),
...
export const scheduleSwapRequests = sqliteTable('schedule_swap_requests', {
  ...
  requesterScheduleId: integer('requester_schedule_id').notNull().references(() => employeeSchedules.id, { onDelete: 'cascade' }),

  // Target Information
  targetEmployeeId: integer('target_employee_id').references(() => users.id, { onDelete: 'cascade' }),
  targetScheduleId: integer('target_schedule_id').references(() => employeeSchedules.id, { onDelete: 'cascade' }),

  // Request Type
  requestType: text('request_type', { enum: ['swap', 'cover', 'drop'] }).notNull(),
  ...
  isOpenRequest: integer('is_open_request', { mode: 'boolean' }).default(false),
...
export const employeeAvailability = sqliteTable('employee_availability', {
  ...
  availabilityType: text('availability_type', { enum: ['recurring', 'specific_date'] }).notNull(),
  ...
  // Preference
  preferenceType: text('preference_type', { enum: ['preferred', 'available', 'unavailable'] }).notNull(),
