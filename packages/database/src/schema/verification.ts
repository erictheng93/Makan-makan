...
export const passwordResetTokens = sqliteTable('password_reset_tokens', {
  ...
  tokenType: text('token_type', { enum: ['email', 'sms'] }).notNull().default('email'),
  ...
  usedAt: integer('used_at', { mode: 'timestamp' }), // Mark as used
  ...
  userExpiresIdx: index('idx_password_reset_user_expires').on(table.userId, table.expiresAt),
...
export const emailVerificationTokens = sqliteTable('email_verification_tokens', {
  ...
  expiresAt: integer('expires_at', { mode: 'timestamp' }).notNull(), // 24 hours
...
export const phoneVerificationTokens = sqliteTable('phone_verification_tokens', {
  ...
  // Verification tracking
  expiresAt: integer('expires_at', { mode: 'timestamp' }).notNull(), // 5 minutes
  verifiedAt: integer('verified_at', { mode: 'timestamp' }),
  ...
  userPhoneIdx: index('idx_phone_verification_user_phone').on(table.userId, table.phone),
  otpExpiresIdx: index('idx_phone_verification_otp_expires').on(table.otpCode, table.expiresAt),
}))

export const phoneVerificationTokenRelations = relations(phoneVerificationTokens, ({ one }) => ({
  user: one(users, {
...
// ============================================
export const passwordChangeLogs = sqliteTable('password_change_logs', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  userId: integer('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  userCreatedIdx: index('idx_password_change_user_created').on(table.userId, table.createdAt),
