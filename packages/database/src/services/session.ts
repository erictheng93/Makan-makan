...
export class SessionService extends BaseService {
  ...
  async getActiveSessions(filters: SessionFilters = {}) {
    try {
      ...
      if (platform) {
        conditions.push(
          sql`json_extract(${sessions.deviceInfo}, '$.platform') = ${platform}`,
        );
      }

      if (deviceType) {
        conditions.push(
          sql`json_extract(${sessions.deviceInfo}, '$.deviceType') = ${deviceType}`,
        );
      }
  ...
  async getSessionStats(): Promise<SessionStats> {
    try {
      ...
      const [{ activeSessions }] = await this.db
        ...
        .from(sessions)
        .where(
          and(eq(sessions.isActive, true), gt(sessions.expiresAt, new Date())),
        );
