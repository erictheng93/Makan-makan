...
import type { CouponValidationResult, CreateCouponData, UseCouponData } from '../coupon'
...
describe('CouponService', () => {
  ...
  describe('createCoupon', () => {
    ...
    it('should throw error when coupon code already exists', async () => {
      ...
      mockDb.query.coupons.findFirst.mockResolvedValue({ id: 1, code: 'EXISTING' })

      await expect(couponService.createCoupon(couponData)).rejects.toThrow('優惠券代碼已存在')
    })
  })
