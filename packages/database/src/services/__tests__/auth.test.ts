...
describe("AuthService", () => {
  ...
  describe("Logout", () => {
    ...
    it("should throw error on database failure", async () => {
      ...
      // Act & Assert
      // Note: handleError in BaseService throws the original error message
      await expect(authService.logout(1)).rejects.toThrow("Database error");
    });
  });
  ...
  describe("ChangePassword", () => {
    ...
    it("should successfully change password with valid old password", async () => {
      ...
      vi.mocked(bcrypt.compare).mockResolvedValue(true as never);
      vi.mocked(bcrypt.hash).mockResolvedValue(
        "$2a$10$new-hashed-password" as never,
      );
  ...
  describe("CreateSession", () => {
    it("should create new session with cleanup", async () => {
      ...
      const cleanupSpy = vi
        .spyOn(authService, "cleanupExpiredSessions")
        .mockResolvedValue(0);
