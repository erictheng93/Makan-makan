  ...
  const db: any = {
    insert: (table: any) => {
      ...
      if (!tableName) {
        ...
        if (
          tableKeys.includes("registerId") &&
          tableKeys.includes("operatorId")
        ) {
          tableName = "shifts";
        } else if (
          tableKeys.includes("shiftId") &&
          tableKeys.includes("type")
        ) {
          tableName = "movements";
        } else if (
          tableKeys.includes("orderId") &&
          tableKeys.includes("receiptNumber")
        ) {
          tableName = "receipts";
        } else if (
          tableKeys.includes("originalOrderId") &&
          tableKeys.includes("refundNumber")
        ) {
          tableName = "refunds";
        } else if (
          tableKeys.includes("orderNumber") &&
          tableKeys.includes("totalAmount")
        ) {
          tableName = "orders";
        } else if (
          tableKeys.includes("fullName") &&
          tableKeys.includes("passwordHash")
        ) {
          tableName = "users";
        } else if (
          tableKeys.includes("shiftId") &&
          tableKeys.includes("reportData")
        ) {
          tableName = "reports";
        } else {
          tableName = "registers";
    ...
    select: (fields?: any) => {
      ...
      const identifyTableName = (table: any): string => {
        ...
        if (
          tableKeys.includes("restaurantId") &&
          tableKeys.includes("name") &&
          tableKeys.includes("isActive")
        )
          return "registers";
          ...
          return {
            where: (condition: any) => {
              ...
              return {
                get: async () => {
                  const dataMap =
                    mockData[currentTable as keyof typeof mockData];
                  ...
                  return {
                    get: async () => {
                      ...
                      if (innerQueryId) {
                        ...
                        const user =
                          users.get(targetShift.operatorId) ||
                          (users.size > 0
                            ? Array.from(users.values())[0]
                            : null);
                        const register =
                          registers.get(targetShift.registerId) ||
                          (registers.size > 0
                            ? Array.from(registers.values())[0]
                            : null);
  ...
  describe("收銀機管理", () => {
    ...
    describe("getRegisters", () => {
      it("應該返回餐廳的所有收銀機", async () => {
        // 先創建幾個收銀機
        await service.createRegister(
          { name: "POS-001", restaurantId: "R-001" },
          1,
        );
        await service.createRegister(
          { name: "POS-002", restaurantId: "R-001" },
          1,
        );
      ...
      it("應該只返回指定餐廳的收銀機", async () => {
        await service.createRegister(
          { name: "POS-R1", restaurantId: "R-001" },
          1,
        );
        await service.createRegister(
          { name: "POS-R2", restaurantId: "R-002" },
          1,
        );
  ...
  describe("班次管理", () => {
    ...
    beforeEach(async () => {
      ...
      const registerResult = await service.createRegister(
        { name: "Test-Register", restaurantId: "R-001" },
        1,
      );
  ...
  describe("收據管理", () => {
    ...
    describe("printReceipt", () => {
      it("應該成功打印收據", async () => {
        ...
        const result = await service.printReceipt(receiptData, testRegisterId);
