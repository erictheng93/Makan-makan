...
describe('TableService', () => {
  ...
  describe('createTable', () => {
    ...
    it('should throw error when table number already exists', async () => {
      ...
      // Act & Assert
      await expect(
        tableService.createTable(validTableData)
      ).rejects.toThrow('Database operation failed: createTable')
    })
  ...
  describe('releaseTable', () => {
    it('should release table successfully', async () => {
      ...
      mockDb.update.mockReturnValue({
        set: vi.fn().mockReturnValue(createQueryChain([{ ...mockTable, isOccupied: false }]))
      })
  ...
  describe('markTableCleaned', () => {
    it('should mark table as cleaned', async () => {
      // Arrange
      mockDb.update.mockReturnValue({
        set: vi.fn().mockReturnValue(createQueryChain([{ ...mockTable, lastCleanedAt: new Date() }]))
      })

      // Act
      const result = await tableService.markTableCleaned(1, 'Cleaned and sanitized')
  ...
  describe('getTableStats', () => {
    it('should return table statistics', async () => {
      ...
      mockDb.select.mockReturnValueOnce(createQueryChain([{ totalTables: 10 }]))
      // Query 2: occupiedTables (uses alias)
      mockDb.select.mockReturnValueOnce(createQueryChain([{ occupiedTables: 3 }]))
      // Query 3: availableTables (uses alias)
      mockDb.select.mockReturnValueOnce(createQueryChain([{ availableTables: 7 }]))
      // Query 4: inactiveTables (uses alias)
      mockDb.select.mockReturnValueOnce(createQueryChain([{ inactiveTables: 0 }]))
      // Query 5: byFloor
    ...
    it('should return zero stats when no tables exist', async () => {
      ...
      mockDb.select.mockReturnValueOnce(createQueryChain([{ occupiedTables: 0 }]))
      mockDb.select.mockReturnValueOnce(createQueryChain([{ availableTables: 0 }]))
      mockDb.select.mockReturnValueOnce(createQueryChain([{ inactiveTables: 0 }]))
