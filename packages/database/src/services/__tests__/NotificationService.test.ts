...
describe('NotificationService', () => {
  ...
  describe('Email Notifications', () => {
    ...
    it('應該成功發送測試 email', async () => {
      const result = await service.sendTestNotification('email', 'test@example.com')

      expect(result.success).toBe(true)
      expect(mockEmailProvider.sentEmails).toHaveLength(1)
      expect(mockEmailProvider.sentEmails[0].to).toBe('test@example.com')
      expect(mockEmailProvider.sentEmails[0].subject).toContain('Test Notification')
    })
  })
  ...
  describe('Template Rendering', () => {
    it('應該替換所有模板變數', () => {
      const template = 'Hello {{name}}, your order {{orderId}} is ready at {{time}}.'
    ...
    it('應該處理條件判斷 (if 語句)', () => {
      const template = 'Order {{orderId}}{{#if notes}} - Note: {{notes}}{{/if}}'
    ...
    it('應該處理多個變數和重複變數', () => {
      const template = '{{name}} ordered {{item}} and {{item}} again. Total: {{total}}'
  ...
  describe('Bulk Notifications', () => {
    ...
    it('應該處理部分失敗情況', async () => {
      ...
      expect(result.errors[0].errors).toContain('Email failed: Invalid email address')
    })
  ...
  describe('Error Handling', () => {
    ...
    it('應該處理無效的測試通知類型', async () => {
      const result = await service.sendTestNotification('invalid' as any, 'test@test.com')
  ...
  describe('Provider Configuration', () => {
    it('應該正確初始化 email 提供者', () => {
      const provider = new ResendEmailProvider('test-api-key', 'test@example.com')
      expect(provider).toBeDefined()
    })

    it('應該正確初始化 SMS 提供者', () => {
      const provider = new TwilioSMSProvider('test-sid', 'test-token', '+1234567890')
      expect(provider).toBeDefined()
    })

    it('應該在沒有配置時不初始化提供者', () => {
      const emptyEnv = { JWT_SECRET: 'test' } as CloudflareEnv
  ...
  describe('Template Validation', () => {
    it('應該包含所有必需的通知類別模板', () => {
      ...
      for (const category of requiredCategories) {
        expect(notificationTemplates[category as keyof typeof notificationTemplates]).toBeDefined()
      }
    })

    it('每個模板都應該有必需的屬性', () => {
      for (const [category, template] of Object.entries(notificationTemplates)) {
