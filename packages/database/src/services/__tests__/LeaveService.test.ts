...
describe('LeaveService', () => {
  ...
  describe('Leave Type Management', () => {
    ...
    it('should not delete system-defined leave types', async () => {
      ...
      await expect(service.deleteLeaveType(1)).rejects.toThrow('Cannot delete system-defined leave type')
    })
  ...
  describe('Leave Request Management', () => {
    ...
    it('should handle multi-level approval workflow', async () => {
      ...
      mockDb.update.mockReturnValue({
        set: vi.fn().mockReturnValue(createQueryChain([partiallyApprovedRequest])),
      })
    ...
    it('should fail to reject already approved request', async () => {
      ...
      await expect(service.rejectLeaveRequest(1, 1, 'Too late')).rejects.toThrow('Leave request is not in pending status')
    })
    ...
    it('should fail to cancel already rejected request', async () => {
      ...
      await expect(service.cancelLeaveRequest(1, 1, 'Want to cancel')).rejects.toThrow('Leave request cannot be cancelled')
    })
  })
  ...
  describe('Leave Accrual', () => {
    it('should accrue yearly leave balances for all employees', async () => {
      ...
      const result = await service.accrueLeaveBalances(1, 2025)
