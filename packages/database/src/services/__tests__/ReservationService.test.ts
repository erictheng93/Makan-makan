  ...
  return {
    run: async (query: any) => {
      ...
      if (sqlStr.includes('INSERT INTO reservations')) {
        ...
        // Handle slot updates
        if (sqlStr.includes('current_reservations = current_reservations + 1')) {
          Array.from(slots.values()).forEach(slot => {
            slot.current_reservations += 1
          })
        } else if (sqlStr.includes('MAX(0, current_reservations - 1)')) {
          Array.from(slots.values()).forEach(slot => {
            slot.current_reservations = Math.max(0, slot.current_reservations - 1)
          })
        }
    ...
    get: async (query: any) => {
      ...
      if (sqlStr.includes('SELECT * FROM reservation_slots') && sqlStr.includes('restaurant_id')) {
        const restaurantId = params[0]
        const date = params[1]
        const timeSlot = params[2]

        const slot = Array.from(slots.values()).find(s =>
      ...
      if (sqlStr.includes('SELECT * FROM reservations') && (sqlStr.includes('WHERE r.id') || sqlStr.includes('WHERE id'))) {
        const id = params[0]
        const reservation = reservations.get(id)
      ...
      } else if (sqlStr.includes('SELECT * FROM reservations') && sqlStr.includes('confirmation_code')) {
        const code = params[0]
        const reservation = Array.from(reservations.values()).find(r => r.confirmation_code === code)
      ...
      } else if (sqlStr.includes('SELECT') && sqlStr.includes('FROM reservations')) {
        // Stats query
        const reservationList = Array.from(reservations.values())
        return {
          total_reservations: reservationList.length,
          confirmed_count: reservationList.filter(r => r.status === 'confirmed').length,
          completed_count: reservationList.filter(r => r.status === 'completed').length,
          no_show_count: reservationList.filter(r => r.status === 'no_show').length,
          cancelled_count: reservationList.filter(r => r.status === 'cancelled').length,
          total_guests: reservationList.reduce((sum, r) => sum + (r.party_size || 0), 0),
          no_show_rate: 0,
          avg_party_size: reservationList.length > 0
            ? reservationList.reduce((sum, r) => sum + (r.party_size || 0), 0) / reservationList.length
            : 0
        }
      }
    ...
    all: async (query: any) => {
      ...
      if (sqlStr.includes('SELECT * FROM reservations') || sqlStr.includes('SELECT r.*')) {
      ...
      } else if (sqlStr.includes('FROM tables') || sqlStr.includes('SELECT t.*')) {
        // Return available tables with proper filtering
        const partySize = params.find((p: any) => typeof p === 'number' && p > 0 && p <= 20) || 1
        return Array.from(tables.values())
          .filter(t => t.current_status === 'available' && t.is_active === 1 && t.capacity >= partySize)
          .map(t => ({
            ...t,
            turnover_count: 0
          }))
...
describe('ReservationService', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(service as any, 'getSlotById').mockImplementation(async (...args: unknown[]) => {
    ...
    vi.spyOn(service, 'assignTable').mockImplementation(async (request: any) => {
      const tables = mockDB.getTables()
      const availableTables = Array.from(tables.values())
        .filter((t: any) => t.current_status === 'available' && t.is_active === 1 && t.capacity >= request.partySize) as any[]
      ...
      for (const table of availableTables) {
        if ((table as any).capacity >= request.partySize && (table as any).capacity < bestTable.capacity) {
          bestTable = table
        }
    ...
    vi.spyOn(service, 'getReservationById').mockImplementation(async (id: string): Promise<any> => {
      const reservations = mockDB.getReservations()
      const reservation = reservations.get(id) as any
      if (!reservation) return null
      return {
        ...reservation,
        table: { id: reservation.table_id, number: `T${reservation.table_id}`, capacity: 4 }
      }
    })

    // Mock getReservationByCode
    vi.spyOn(service, 'getReservationByCode').mockImplementation(async (code: string) => {
      const reservations = mockDB.getReservations()
      const reservation = Array.from(reservations.values()).find((r: any) => r.confirmation_code === code) as any
      if (!reservation) return null
      return {
        ...reservation,
        table: { id: reservation.table_id, number: `T${reservation.table_id}`, capacity: 4 }
    ...
    vi.spyOn(service, 'createReservation').mockImplementation(async (data: any) => {
      ...
      const tables = mockDB.getTables()
      const availableTables = Array.from(tables.values())
        .filter((t: any) => t.current_status === 'available' && t.is_active === 1 && t.capacity >= data.partySize)
      const tableId = availableTables.length > 0 ? (availableTables[0] as any).id : 1
    ...
    vi.spyOn(service, 'createSlot').mockImplementation(async (data: any) => {
      const id = 'slot_' + Date.now() + '_' + Math.random().toString(36).substring(2, 9)
      const now = Date.now()
    ...
    vi.spyOn(service, 'updateReservation').mockImplementation(async (id: string, data: any) => {
      const reservations = mockDB.getReservations()
      const reservation = reservations.get(id)
      if (!reservation) {
      ...
      // Update fields
      if (data.customerName) reservation.customerName = reservation.customer_name = data.customerName
      if (data.customerPhone) reservation.customerPhone = reservation.customer_phone = data.customerPhone
      if (data.customerEmail !== undefined) reservation.customerEmail = reservation.customer_email = data.customerEmail
      if (data.partySize) reservation.partySize = reservation.party_size = data.partySize
      if (data.reservationDate) reservation.reservationDate = reservation.reservation_date = data.reservationDate
      if (data.reservationTime) reservation.reservationTime = reservation.reservation_time = data.reservationTime
      if (data.durationMinutes) reservation.durationMinutes = reservation.duration_minutes = data.durationMinutes
      if (data.tableId !== undefined) reservation.tableId = reservation.table_id = data.tableId
      if (data.specialRequests !== undefined) reservation.specialRequests = reservation.special_requests = data.specialRequests
    ...
    vi.spyOn(service, 'completeReservation').mockImplementation(async (id: string) => {
      const reservations = mockDB.getReservations()
      const reservation = reservations.get(id)
      if (!reservation) throw new Error('訂位不存在')
    ...
    vi.spyOn(service, 'cancelReservation').mockImplementation(async (id: string, reason?: string) => {
      const reservations = mockDB.getReservations()
      const reservation = reservations.get(id)
      if (!reservation) throw new Error('訂位不存在')
    ...
    vi.spyOn(service, 'listReservations').mockImplementation(async (filters: any) => {
      const reservations = mockDB.getReservations()
      let data = Array.from(reservations.values()) as any[]

      // Apply filters
      if (filters.restaurantId) {
        data = data.filter((r: any) => r.restaurant_id === filters.restaurantId || r.restaurantId === filters.restaurantId)
      }
      if (filters.status) {
      ...
      if (filters.reservationDate) {
        data = data.filter((r: any) => r.reservation_date === filters.reservationDate || r.reservationDate === filters.reservationDate)
      }
    ...
    vi.spyOn(service, 'getReservationStats').mockImplementation(async (restaurantId: string, date?: string) => {
      const reservations = mockDB.getReservations()
      let data = Array.from(reservations.values()) as any[]

      // Filter by restaurant
      data = data.filter((r: any) => r.restaurant_id === restaurantId || r.restaurantId === restaurantId)

      // Filter by date if provided
      if (date) {
        data = data.filter((r: any) => r.reservation_date === date || r.reservationDate === date)
      }
      ...
      const noShowCount = data.filter((r: any) => r.status === 'no_show').length
      const cancelledCount = data.filter((r: any) => r.status === 'cancelled').length
      const totalGuests = data.reduce((sum: number, r: any) => sum + (r.party_size || r.partySize || 0), 0)
      ...
      return {
        ...
        averagePartySize: totalReservations > 0 ? totalGuests / totalReservations : 0
      }
    })
  })
  ...
  describe('Create Reservation', () => {
    ...
    it('應該驗證顧客姓名', async () => {
      ...
      await expect(service.createReservation(request)).rejects.toThrow('顧客姓名為必填')
    })
    ...
    it('應該驗證手機號碼格式', async () => {
      ...
      await expect(service.createReservation(request)).rejects.toThrow('請提供有效的手機號碼')
    })
    ...
    it('應該驗證用餐人數範圍', async () => {
      ...
      await expect(service.createReservation(request)).rejects.toThrow('用餐人數必須在 1-20 人之間')
    })
    ...
    it('應該驗證日期格式', async () => {
      ...
      await expect(service.createReservation(request)).rejects.toThrow('日期格式錯誤')
    })
    ...
    it('應該驗證時間格式', async () => {
      ...
      await expect(service.createReservation(request)).rejects.toThrow('時間格式錯誤')
    })
    ...
    it('應該拒絕過去的時間訂位', async () => {
      ...
      await expect(service.createReservation(request)).rejects.toThrow('無法訂位過去的時間')
    })
  ...
  describe('List Reservations', () => {
    ...
    it('應該支持多個狀態篩選', async () => {
      ...
      result.data.forEach(reservation => {
        expect(['confirmed', 'pending'].includes(reservation.status)).toBe(true)
      })
    })
  ...
  describe('Update Reservation', () => {
    ...
    it('應該在訂位不存在時拋出錯誤', async () => {
      ...
      await expect(service.updateReservation('non-existent-id', update)).rejects.toThrow('訂位不存在')
    })
  })
  ...
  describe('Reservation Status', () => {
    ...
    it('應該取消訂位', async () => {
      const result = await service.cancelReservation(reservationId, '顧客臨時取消')
  ...
  describe('Reservation Stats', () => {
    beforeEach(async () => {
      ...
      const requests = [
        { name: '顧客1', status: 'confirmed', partySize: 2, phone: '0912345670' },
        { name: '顧客2', status: 'confirmed', partySize: 4, phone: '0912345671' },
        { name: '顧客3', status: 'completed', partySize: 3, phone: '0912345672' },
        { name: '顧客4', status: 'no_show', partySize: 2, phone: '0912345673' },
        { name: '顧客5', status: 'cancelled', partySize: 4, phone: '0912345674' }
      ]
  ...
  describe('Error Handling', () => {
    it('應該處理資料庫錯誤', async () => {
      ...
      vi.spyOn(service, 'createReservation').mockRejectedValue(new Error('Database error'))
      ...
      await expect(service.createReservation(request)).rejects.toThrow('Database error')
    })
