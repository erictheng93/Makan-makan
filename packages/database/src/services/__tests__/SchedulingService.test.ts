...
describe("SchedulingService", () => {
  ...
  describe("Swap Request Management", () => {
    ...
    it("should accept swap request and update status", async () => {
      ...
      // Mock select for checking existing request - Use createQueryChain for leftJoin support
      const selectChain = createQueryChain([existingRequest]);
      selectChain.limit = vi.fn().mockResolvedValue([existingRequest]);
      mockDb.select.mockReturnValue(selectChain);

      // Mock update
      mockDb.update.mockReturnValue({
        set: vi.fn().mockReturnValue(createQueryChain([updatedRequest])),
      });
    ...
    it("should reject swap request with reason", async () => {
      ...
      // Use createQueryChain for proper leftJoin support
      const selectChain = createQueryChain([existingRequest]);
      selectChain.limit = vi.fn().mockResolvedValue([existingRequest]);
      mockDb.select.mockReturnValue(selectChain);
    ...
    it("should throw error when accepting non-pending request", async () => {
      ...
      const chain1 = createQueryChain([existingRequest]);
      chain1.limit = vi.fn().mockResolvedValue([existingRequest]);
      mockDb.select.mockReturnValue(chain1);
    ...
    it("should cancel swap request by requester", async () => {
      ...
      const chain2 = createQueryChain([existingRequest]);
      chain2.limit = vi.fn().mockResolvedValue([existingRequest]);
      mockDb.select.mockReturnValue(chain2);
    ...
    it("should throw error when non-requester tries to cancel", async () => {
      ...
      const chain3 = createQueryChain([existingRequest]);
      chain3.limit = vi.fn().mockResolvedValue([existingRequest]);
      mockDb.select.mockReturnValue(chain3);
  ...
  describe("Error Handling", () => {
    ...
    it("should throw error when rejecting already rejected request", async () => {
      ...
      const chain4 = createQueryChain([existingRequest]);
      chain4.limit = vi.fn().mockResolvedValue([existingRequest]);
      mockDb.select.mockReturnValue(chain4);
  ...
  describe("Integration Scenarios", () => {
    it("should handle complete swap request workflow", async () => {
      ...
      // 2. Accept request - Use createQueryChain for proper leftJoin support
      const acceptChain = createQueryChain([created]);
      acceptChain.limit = vi.fn().mockResolvedValue([created]);
      mockDb.select.mockReturnValue(acceptChain);
      ...
      mockDb.update.mockReturnValue({
        set: vi.fn().mockReturnValue(createQueryChain([acceptedMock])),
      });

      const accepted = await service.acceptSwapRequest(1, 2);
      expect(accepted.status).toBe("accepted");

      // 3. Approve request - Use createQueryChain for proper leftJoin support
      const approveChain = createQueryChain([accepted]);
      approveChain.limit = vi.fn().mockResolvedValue([accepted]);
      mockDb.select.mockReturnValue(approveChain);
      ...
      mockDb.update.mockReturnValue({
        set: vi.fn().mockReturnValue(createQueryChain([approvedMock])),
      });
