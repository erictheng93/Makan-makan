...
export class IntelligentD1Service extends BaseService {
  ...
  async executeWithOptimization<T>(
    ...
    if (result && options.cacheKey && this.cacheManager) {
      const cacheStrategy = this.determineCacheStrategy(querySignature, executionTime, options)
  ...
  async getPaginatedWithPrefetch<T>(
    ...
    if (pagination.hasNext && prefetchPages > 0) {
      ...
      if (page > 1 && page <= 3) { // Users often go back in first few pages
  ...
  async getOrderAnalytics(
    ...
    return this.executeWithOptimization(query.sql, query.params, {
      cacheKey,
      cacheTtl,
      tags: [`restaurant:${restaurantId}`, 'analytics', `timerange:${timeRange}`],
      priority,
  ...
  private recordQueryError(
    ...
    console.error('Query execution error:', { querySignature, error, executionTime })
  }

  private getQueryTimeout(priority: 'high' | 'normal' | 'low'): number {
    switch (priority) {
      case 'high': return 10000 // 10 seconds for critical queries
      case 'normal': return 5000 // 5 seconds for normal queries
      case 'low': return 2000 // 2 seconds for background queries
    }
  }
  ...
  private triggerPredictivePreloading(querySignature: string, options: any): void {
    // Analyze query patterns and preload related queries
    const patterns = this.predictRelatedQueries(querySignature)
    
    patterns.forEach(pattern => {
      if (pattern.predictedNextExecution < Date.now() + 300000) { // Next 5 minutes
  ...
  private predictRelatedQueries(querySignature: string): QueryPattern[] {
    ...
    return Array.from(this.queryPatterns.values())
      .filter(pattern => this.isRelatedPattern(querySignature, pattern.pattern))
  ...
  private learnQueryPattern(
    ...
    const pattern: QueryPattern = {
      ...
      averageTime: existing 
        ? (existing.averageTime * existing.frequency + executionTime) / (existing.frequency + 1)
        : executionTime,
  ...
  private predictNextExecution(
    ...
    const avgFrequencyMs = existing.frequency > 1 
  ...
  private getAnalyticsCacheTtl(timeRange: string): number {
    switch (timeRange) {
      case '1h': return 60 // 1 minute for real-time
  ...
  private buildAnalyticsQuery(restaurantId: number, timeRange: string, metrics: string[]): { sql: string; params: any[] } {
    ...
    return {
      ...
      params: [restaurantId, new Date(Date.now() - this.getTimeThreshold(timeRange))]
    }
  }
}