...
export class VerificationService extends BaseService {
  ...
  async requestPasswordReset(params: PasswordResetRequestParams): Promise<{
    ...
    try {
      ...
      // Send notification
      const appBaseUrl = this.env.API_BASE_URL?.replace('/api', '') || 'http://localhost:5173'
      ...
      if (method === 'email' && user.email) {
        await this.notificationService.sendNotification({
          ...
          data: {
            ...
            requestTime: new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' }),
          },
        })
  ...
  async resetPassword(params: ResetPasswordParams): Promise<{
    ...
    try {
      ...
      await this.db.insert(passwordChangeLogs).values(logEntry).run()
      ...
      if (user?.email) {
        ...
        await this.notificationService.sendNotification({
          ...
          data: {
            userName: user.fullName || user.username,
            changeTime: new Date().toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' }),
  ...
  async sendEmailVerification(params: SendEmailVerificationParams): Promise<{
    ...
    try {
      ...
      await this.db.insert(emailVerificationTokens).values(verificationToken).run()
  ...
  async verifyEmail(params: VerifyEmailParams): Promise<{
    ...
    try {
      ...
      if (verificationToken.verifiedAt) {
        return { success: true, message: 'Email 已驗證', userId: verificationToken.userId }
      }
  ...
  async sendPhoneVerification(params: SendPhoneVerificationParams): Promise<{
    ...
    try {
      ...
      await this.db.insert(phoneVerificationTokens).values(verificationToken).run()
  ...
  async verifyPhone(params: VerifyPhoneParams): Promise<{
    ...
    try {
      ...
      if (verificationToken.attemptCount >= 3) {
        return { success: false, message: '驗證碼已失效，請重新發送', attemptsLeft: 0 }
      }
