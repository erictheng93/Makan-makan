...
export class PrinterService {
  ...
  private async processJob(job: PrintJob): Promise<void> {
    ...
    try {
      ...
      // 生成打印機命令
      const commands = await this.generatePrintCommands(job.content, job.deviceId)
  ...
  private async generatePrintContent(request: PrintRequest): Promise<PrintContent> {
    // 這裡會根據請求數據和地區設定生成收據內容
    // 實作將在 ReceiptFormatter 中完成
    throw new Error('generatePrintContent not implemented yet')
  }

  private async generatePrintCommands(content: PrintContent, deviceId: string): Promise<Buffer> {
    // 這裡會根據打印機品牌生成對應的命令集
    // 實作將在各品牌的 Driver 中完成
  ...
  getStatistics(): PrintStatistics {
    ...
    return {
      ...
      deviceUptime: devices.filter(d => d.status === 'online').length / devices.length * 100,
  ...
  async healthCheck(): Promise<{
    ...
    const serviceStatus = onlineDevices === 0 ? 'unhealthy' : 
                         onlineDevices < devices.length ? 'degraded' : 'healthy'
