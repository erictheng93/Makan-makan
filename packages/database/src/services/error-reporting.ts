...
export class ErrorReportingService extends BaseService {
      ...
      const [report] = await this.db
        ...
        .values({
          ...
          errorContext: data.errorContext
            ? JSON.stringify(data.errorContext)
            : null,
          originalError: data.originalError
            ? JSON.stringify(data.originalError)
            : null,
  ...
  // 批量創建錯誤報告
  async createBulkErrorReports(
    reports: CreateErrorReportData[],
  ): Promise<ErrorReport[]> {
    try {
    ...
    try {
      ...
      return {
        reports: reports.map((report) => ({
          ...
          errorContext: report.error_reports.errorContext
            ? JSON.parse(report.error_reports.errorContext)
            : null,
          originalError: report.error_reports.originalError
            ? JSON.parse(report.error_reports.originalError)
            : null,
        })),
        pagination: {
          page,
  ...
  async getErrorStats(
    ...
    try {
      ...
      const errorsBySeverity = severityStats.reduce(
        (acc, item) => {
  ...
  // 獲取最常見的錯誤
  async getCommonErrors(restaurantId?: string, limit: number = 10) {
    try {
      const conditions = [];
      ...
      return await this.db
        ...
        .where(whereClause)
        .groupBy(
          errorReports.errorMessage,
          errorReports.errorType,
          errorReports.severity,
        )
