...
export class VietnamReceiptFormatter extends RegionReceiptFormatter {
  ...
  formatSummary(order: any, payment?: any): PrintContent["summary"] {
    // 越南 VAT 通常是含稅價格
    const totalWithoutVAT = order.total / 1.1;
...
export class ReceiptFormattingService {
  ...
  formatReceipt(request: PrintRequest): PrintContent {
    ...
    if (!region) {
      throw new Error(
        `Region configuration not found for country: ${request.country}`,
      );
    }

    const template =
      this.templates.get(`${request.country}_${request.type}`) ||
      this.templates.get(`default_${request.type}`) ||
      this.templates.get("default_receipt");

    if (!template) {
      throw new Error(
        `Template not found for country: ${request.country}, type: ${request.type}`,
      );
    }
