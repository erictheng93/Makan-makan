...
export class EpsonDriver extends PrinterDriver {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      console.error(`Failed to connect to Epson printer ${this.device.id}:`, error)
  ...
  async getStatus(): Promise<PrinterDevice['status']> {
    ...
    try {
      ...
      console.error(`Status check failed for Epson printer ${this.device.id}:`, error)
      return 'error'
    }
  }
  ...
  async openDrawer(): Promise<boolean> {
    ...
    try {
      ...
      console.error(`Failed to open drawer on Epson printer ${this.device.id}:`, error)
      return false
    }
  }
  ...
  async cutPaper(): Promise<boolean> {
    ...
    try {
      ...
      console.error(`Failed to cut paper on Epson printer ${this.device.id}:`, error)
      return false
    }
  }
  ...
  async buzzer(times = 1): Promise<boolean> {
    ...
    try {
      ...
      console.error(`Failed to buzz on Epson printer ${this.device.id}:`, error)
      return false
    }
  }
  ...
  private generateHeader(header: PrintContent['header']): Buffer {
    ...
    commands.push(ESCPOSCommands.textColumns('Receipt No:', transaction.receiptNumber, width))
    commands.push(ESCPOSCommands.textColumns('Order ID:', transaction.orderId, width))
    
    if (transaction.tableNumber) {
      commands.push(ESCPOSCommands.textColumns('Table:', transaction.tableNumber, width))
    }

    if (transaction.customerName) {
      commands.push(ESCPOSCommands.textColumns('Customer:', transaction.customerName, width))
    }

    commands.push(ESCPOSCommands.textColumns('Cashier:', transaction.cashier, width))
    ...
    for (const item of items) {
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        for (const modifier of item.modifiers) {
          ...
          commands.push(ESCPOSCommands.textColumns(modifierLine, modifierPriceStr, width))
        }
      }
    }
  ...
  private generateSummary(summary: PrintContent['summary']): Buffer {
    ...
    // 小計
    commands.push(ESCPOSCommands.textColumns('Subtotal:', this.formatPrice(summary.subtotal), width))

    // 稅項
    if (summary.tax && summary.tax.length > 0) {
      for (const tax of summary.tax) {
        commands.push(ESCPOSCommands.textColumns(`${tax.name}:`, this.formatPrice(tax.amount), width))
      }
    }

    // 服務費
    if (summary.serviceCharge) {
      commands.push(ESCPOSCommands.textColumns(`${summary.serviceCharge.name}:`, this.formatPrice(summary.serviceCharge.amount), width))
    }

    // 小費
    if (summary.tip && summary.tip > 0) {
      commands.push(ESCPOSCommands.textColumns('Tip:', this.formatPrice(summary.tip), width))
    }

    // 折扣
    if (summary.discount && summary.discount.amount > 0) {
      commands.push(ESCPOSCommands.textColumns(`${summary.discount.name}:`, `-${this.formatPrice(summary.discount.amount)}`, width))
    }

    // 總計
    commands.push(ESCPOSCommands.separator('-', width))
    commands.push(ESCPOSCommands.printTotal('TOTAL:', this.formatPrice(summary.total), width))

    // 支付資訊
    commands.push(ESCPOSCommands.lineFeed())
    for (const payment of summary.payment) {
      commands.push(ESCPOSCommands.textColumns(`${payment.method}:`, this.formatPrice(payment.amount), width))
    ...
    if (summary.change && summary.change > 0) {
      commands.push(ESCPOSCommands.textColumns('Change:', this.formatPrice(summary.change), width))
    }
  ...
  private generateFooter(footer: PrintContent['footer']): Buffer {
    ...
    if (footer.barcode) {
      commands.push(ESCPOSCommands.lineFeed())
      commands.push(ESCPOSCommands.barcode128(footer.barcode.data, 50, 2, true))
    ...
    if (footer.contactInfo) {
      ...
      if (footer.contactInfo.supportPhone) {
        commands.push(ESCPOSCommands.textLine(`Support: ${footer.contactInfo.supportPhone}`))
      }
  ...
  async printTestPage(): Promise<boolean> {
    try {
      ...
      console.error(`Test print failed on Epson printer ${this.device.id}:`, error)
      return false
    }
  }
