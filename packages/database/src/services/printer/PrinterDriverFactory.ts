...
export class DefaultPrinterDriverFactory implements PrinterDriverFactory {
  ...
  private async createEpsonDriver(device: PrinterDevice, config?: EpsonDriverConfig): Promise<EpsonDriver> {
  ...
  private async createStarDriver(device: PrinterDevice, config?: StarDriverConfig): Promise<StarDriver> {
  ...
  private async createCitizenDriver(device: PrinterDevice, config?: CitizenDriverConfig): Promise<CitizenDriver> {
  ...
  private async createGenericDriver(device: PrinterDevice, config?: EpsonDriverConfig): Promise<EpsonDriver> {
  ...
  async detectPrinter(connectionInfo: {
    ...
    console.log(`Detecting printer at ${connectionInfo.type}:${connectionInfo.address}`)
  ...
  private async queryDeviceInfo(connectionInfo: any): Promise<string | null> {
    ...
    // 根據地址模式猜測品牌
    if (connectionInfo.address.includes('epson') || connectionInfo.address.includes('tm-')) {
      return 'EPSON TM-T88VI'
    } else if (connectionInfo.address.includes('star') || connectionInfo.address.includes('tsp')) {
      return 'STAR TSP143III'
    } else if (connectionInfo.address.includes('citizen') || connectionInfo.address.includes('ct-')) {
  ...
  async scanForPrinters(connectionTypes: PrinterConnection[] = ['usb', 'network']): Promise<PrinterDevice[]> {
  ...
  private async scanConnectionType(connectionType: PrinterConnection): Promise<PrinterDevice[]> {
  ...
  private initializeBrandDetectors(): void {
    ...
    this.brandDetectors.set('epson', (deviceInfo: string) => {
      return deviceInfo.toLowerCase().includes('epson') || deviceInfo.includes('TM-')
    })

    this.brandDetectors.set('star', (deviceInfo: string) => {
      return deviceInfo.toLowerCase().includes('star') || deviceInfo.includes('TSP')
    })

    this.brandDetectors.set('citizen', (deviceInfo: string) => {
      return deviceInfo.toLowerCase().includes('citizen') || deviceInfo.includes('CT-')
    })
  }
  ...
  private determineStarEmulation(model: string): 'StarPRNT' | 'StarLine' | 'StarGraphic' {
    const modelLower = model.toLowerCase()
  ...
  private determineCitizenSeries(model: string): 'CT-S' | 'CT-D' | 'CT-E' | 'PPU' {
  ...
  async getDeviceCapabilities(device: PrinterDevice): Promise<PrinterCapabilities> {
    try {
      const driver = await this.createDriver(device)
  ...
  private async queryActualCapabilities(driver: any): Promise<PrinterCapabilities> {
    // 實際查詢打印機能力的實作
    // 這裡返回基本能力
