...
class StarPRNTCommands {
  ...
  static qrCode(data: string, size = 3): Buffer {
    ...
    commands.push(Buffer.from([dataBytes.length & 0xFF, (dataBytes.length >> 8) & 0xFF]))
    commands.push(dataBytes)
  ...
  static textColumns(left: string, right: string, totalWidth = 32): Buffer {
    ...
    const spacesNeeded = Math.max(0, totalWidth - leftBytes.length - rightBytes.length)
...
export class StarDriver extends PrinterDriver {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      console.error(`Failed to connect to Star printer ${this.device.id}:`, error)
  ...
  async getStatus(): Promise<PrinterDevice['status']> {
    ...
    try {
      ...
      console.error(`Status check failed for Star printer ${this.device.id}:`, error)
      return 'error'
    }
  }
  ...
  async openDrawer(): Promise<boolean> {
    ...
    try {
      ...
      console.error(`Failed to open drawer on Star printer ${this.device.id}:`, error)
      return false
    }
  }
  ...
  async cutPaper(): Promise<boolean> {
    ...
    try {
      ...
      console.error(`Failed to cut paper on Star printer ${this.device.id}:`, error)
      return false
    }
  }
  ...
  private generateHeader(header: PrintContent['header'], width: number): Buffer {
    ...
    if (restaurant.taxNumber) {
      commands.push(StarPRNTCommands.textLine(`Tax No: ${restaurant.taxNumber}`))
    }
    ...
    commands.push(StarPRNTCommands.textColumns('Receipt:', transaction.receiptNumber, width))
    commands.push(StarPRNTCommands.textColumns('Order:', transaction.orderId, width))
    
    if (transaction.tableNumber) {
      commands.push(StarPRNTCommands.textColumns('Table:', transaction.tableNumber, width))
    }

    commands.push(StarPRNTCommands.textColumns('Cashier:', transaction.cashier, width))
    commands.push(StarPRNTCommands.textLine(transaction.timestamp.toLocaleString()))
    ...
    for (const item of items) {
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        for (const modifier of item.modifiers) {
          ...
          commands.push(StarPRNTCommands.textColumns(modifierLine, modifierPriceStr, width))
        }
      }
    }
  ...
  private generateSummary(summary: PrintContent['summary'], width: number): Buffer {
    ...
    // 小計
    commands.push(StarPRNTCommands.textColumns('Subtotal:', this.formatPrice(summary.subtotal), width))

    // 稅項
    if (summary.tax && summary.tax.length > 0) {
      for (const tax of summary.tax) {
        commands.push(StarPRNTCommands.textColumns(`${tax.name}:`, this.formatPrice(tax.amount), width))
      }
    }
    ...
    commands.push(StarPRNTCommands.textColumns('TOTAL:', this.formatPrice(summary.total), width))
    commands.push(StarPRNTCommands.setTextSize(1, 1))
    commands.push(StarPRNTCommands.setBold(false))
    ...
    for (const payment of summary.payment) {
      commands.push(StarPRNTCommands.textColumns(payment.method, this.formatPrice(payment.amount), width))
    }

    if (summary.change && summary.change > 0) {
      commands.push(StarPRNTCommands.textColumns('Change:', this.formatPrice(summary.change), width))
    }
  ...
  private generateFooter(footer: PrintContent['footer']): Buffer {
    ...
    if (footer.barcode) {
      commands.push(StarPRNTCommands.lineFeed())
      commands.push(StarPRNTCommands.barcode(footer.barcode.data, footer.barcode.format as any))
    }
  ...
  async printTestPage(): Promise<boolean> {
    try {
      ...
      commands.push(StarPRNTCommands.textLine(`Model: ${this.driverConfig.printer.model}`))
      commands.push(StarPRNTCommands.textLine(`Emulation: ${this.driverConfig.printer.emulation}`))
      commands.push(StarPRNTCommands.textLine(`Paper Width: ${this.driverConfig.printer.paperWidth}mm`))
      commands.push(StarPRNTCommands.separator('=', this.getPaperWidth()))
