...
class CitizenCommands extends ESCPOSCommands {
  ...
  // Citizen 蜂鳴器 (增強版)
  static buzzerCitizen(
    pattern: "short" | "long" | "double" | "triple" = "short",
  ): Buffer {
    switch (pattern) {
      case "short":
  ...
  static downloadLogo(logoId: number, logoData: Buffer): Buffer {
    ...
    commands.push(
      Buffer.from([
        ...
        0x2a,
        logoId,
        logoData.length & 0xff,
        (logoData.length >> 8) & 0xff,
      ]),
    );
...
export class CitizenDriver extends PrinterDriver {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      console.error(
        `Failed to connect to Citizen printer ${this.device.id}:`,
        error,
      );
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      console.error(
        `Error disconnecting Citizen printer ${this.device.id}:`,
        error,
      );
    } finally {
  ...
  async print(commands: Buffer): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `Print failed on Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  async getStatus(): Promise<PrinterDevice["status"]> {
    ...
    try {
      ...
      console.error(
        `Status check failed for Citizen printer ${this.device.id}:`,
        error,
      );
      return "error";
    }
  ...
  async openDrawer(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `Failed to open drawer on Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  async cutPaper(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `Failed to cut paper on Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  async buzzer(times = 1): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `Failed to buzz on Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  async ejectPaper(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `Failed to eject paper on Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  async retractPaper(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        `Failed to retract paper on Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  async downloadLogo(logoId: number, logoData: Buffer): Promise<boolean> {
    try {
      ...
      console.error(
        `Failed to download logo to Citizen printer ${this.device.id}:`,
        error,
      );
      return false;
    }
  ...
  private generateHeader(
    ...
    if (restaurant.taxNumber) {
      commands.push(
        CitizenCommands.textLine(`Tax No: ${restaurant.taxNumber}`),
      );
    }
    ...
    commands.push(
      CitizenCommands.textColumns("Order ID:", transaction.orderId, width),
    );
    commands.push(
      CitizenCommands.textColumns("Cashier:", transaction.cashier, width),
    );
    commands.push(
      CitizenCommands.textLine(transaction.timestamp.toLocaleString()),
    );
    ...
    for (const item of items) {
      ...
      if (item.modifiers && item.modifiers.length > 0) {
        for (const modifier of item.modifiers) {
          ...
          commands.push(
            CitizenCommands.textColumns(modifierLine, modifierPriceStr, width),
          );
        }
      }
    }
  ...
  async printTestPage(): Promise<boolean> {
    try {
      ...
      commands.push(
        CitizenCommands.textLine(`Model: ${this.driverConfig.printer.model}`),
      );
      commands.push(
        CitizenCommands.textLine(`Series: ${this.driverConfig.printer.series}`),
      );
      commands.push(
        CitizenCommands.textLine(
          `Paper: ${this.driverConfig.printer.paperWidth}mm`,
        ),
      );
