...
export class ESCPOSCommands {
  ...
  static textLine(text: string, encoding = "utf8"): Buffer {
    return Buffer.concat([this.text(text, encoding), this.lineFeed()]);
  }
  ...
  static qrCode(
    ...
    // QR Code 模型
    commands.push(
      Buffer.from([this.GS, 0x28, 0x6b, 0x04, 0x00, 0x31, 0x41, 0x32, 0x00]),
    );

    // QR Code 大小
    commands.push(
      Buffer.from([this.GS, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x43, size]),
    );

    // 錯誤修正等級
    const ecLevel = { L: 0x30, M: 0x31, Q: 0x32, H: 0x33 }[errorCorrection];
    commands.push(
      Buffer.from([this.GS, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x45, ecLevel]),
    );
    ...
    // 列印
    commands.push(
      Buffer.from([this.GS, 0x28, 0x6b, 0x03, 0x00, 0x31, 0x51, 0x30]),
    );
  ...
  static barcode128(
    ...
    commands.push(
      Buffer.from([this.GS, 0x6b, 0x49, dataBytes.length, ...dataBytes]),
    );
