-- Canonical backup-system schema (relocated from the orphan
-- packages/database/src/migrations/005_backup_system.sql).
--
-- Two issues in the original 005:
--   1. It lived under src/migrations/ so `wrangler d1 migrations apply`
--      never picked it up, leaving backup_records uncreated in every
--      fresh D1 — blocking A6 at `POST /api/v1/backup/create`.
--   2. It declared `CREATE INDEX idx_backup_records_created_at ON
--      backup_records(created_at)` even though the table has no
--      created_at column (only started_at / completed_at / expires_at).
--      Applying 005 directly therefore failed with
--      "no such column: created_at at offset 75: SQLITE_ERROR".
--
-- This migration is the repaired copy: same table shape, the broken
-- index removed, and everything lands in migrations_fresh/ so the
-- normal `pnpm db:migrate:local` / `wrangler d1 migrations apply`
