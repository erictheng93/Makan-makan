-- Canonical backup-system schema, mirrored from the Drizzle source of
-- truth in packages/database/src/schema/backup.ts. The earlier
-- packages/database/src/migrations/005_backup_system.sql was an orphan
-- (never picked up by wrangler d1 migrations apply because it lived
-- under src/migrations/) AND it drifted from the live schema:
--   - backup_records in 005 had a bogus CREATE INDEX on a non-existent
--     `created_at` column → SQLITE_ERROR when applied directly.
--   - 005's column lists (updated_at, expires_at, retry_count, etc.)
--     no longer match backup.ts, so the BackupService's Drizzle INSERT
--     failed with "no such column: updated_at" in CI.
--
-- This migration lands in migrations_fresh/ so the normal
-- `pnpm db:migrate:local` / `wrangler d1 migrations apply` pipeline
-- picks it up, and its column set is a byte-for-byte projection of
-- backup.ts so the ORM INSERTs issued by BackupService succeed.
