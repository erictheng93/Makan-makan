...
export class CategoryFactory extends BaseFactory<CategoryTestData> {
  build(options?: FactoryOptions<CategoryTestData>): CategoryTestData {
    ...
    const categoryName = options?.overrides?.name ?? randomChoice([...CommonCategories])
...
export class MenuItemFactory extends BaseFactory<MenuItemTestData> {
  build(options?: FactoryOptions<MenuItemTestData>): MenuItemTestData {
    ...
    // 根據分類選擇菜名
    const dishNames = DishNames[categoryName as keyof typeof DishNames] || DishNames.主菜
    ...
    const baseData: MenuItemTestData = {
      ...
      price: basePrice,
      originalPrice: randomBoolean(0.3) ? basePrice + randomNumber(20, 100) : null,
      ...
      allergens: randomBoolean(0.3)
        ...
        : [],
      options: categoryName === '飲料' || categoryName === '套餐'
        ? [
            {
              name: '大小',
              choices: [
   ...
   * 生成素食菜品
   */
  buildVegetarian(options?: FactoryOptions<MenuItemTestData>): MenuItemTestData {
    return this.build({
      ...options,
      overrides: {
        dietaryInfo: {
