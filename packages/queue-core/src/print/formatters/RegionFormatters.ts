...
export class RegionFormatterFactory {
  private static formatters = new Map<
    CountryCode,
    new (region: RegionConfig, countryCode: CountryCode) => IRegionFormatter
  >();
  ...
  static createFormatter(
    ...
    if (!FormatterClass) {
      throw new Error(
        `No region formatter found for country code: ${countryCode}`,
      );
    }
...
export const formatTaxNumberByCountry = (
  taxNumber: string,
  countryCode: CountryCode,
  region: RegionConfig,
): string => {
