...
export class ReceiptFormattingService {
  ...
  async formatReceipt(request: PrintRequest): Promise<PrintContent> {
    try {
      ...
      throw new PrintFormattingError(
        `Failed to format receipt: ${error}`,
        { request, error }
      )
    }
  }
  ...
  async formatBatch(requests: PrintRequest[]): Promise<PrintContent[]> {
    ...
    if (errors.length > 0) {
      console.warn(`Batch formatting completed with ${errors.length} errors:`, errors)
    }
  ...
  async previewReceipt(
    ...
    try {
      ...
      return {
        ...
        preview,
        metadata: {
          templateUsed: options?.templateId || `${request.country}_${request.type}`,
  ...
  private validateRegionConfig(config: RegionConfig): void {
    ...
    if (!config.numberFormat || !config.tax || !config.receipt) {
      throw new PrintFormattingError('Region config missing formatting sections')
    }
  }
  ...
  private generateMockRequest(country: CountryCode, type: PrintJobType): PrintRequest {
    return {
      country,
      type,
      restaurantId: '1',
  ...
  private generateTextPreview(content: PrintContent): string {
    ...
    if (content.header.transactionInfo) {
      ...
      if (transactionInfo.timestamp) {
        lines.push(`Date: ${new Date(transactionInfo.timestamp).toLocaleString()}`)
      }
    }
    lines.push('-'.repeat(width))
    ...
    for (const item of content.items) {
      ...
      const priceLine = this.rightAlign(`$${item.totalPrice.toFixed(2)}`, width)
      lines.push(priceLine)
    }
    lines.push('-'.repeat(width))

    // Summary
    lines.push(this.formatSummaryLine('Subtotal', content.summary.subtotal, width))
    ...
    lines.push('='.repeat(width))
    lines.push(this.formatSummaryLine('TOTAL', content.summary.total, width, true))
  ...
  private formatSummaryLine(label: string, amount: number, width: number, bold = false): string {
