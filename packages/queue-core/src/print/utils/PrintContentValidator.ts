...
export class PrintContentValidator {
   ...
   */
  static validateForPrinter(content: PrintContent, printerType: string): ValidationResult {
    ...
    switch (printerType.toLowerCase()) {
      ...
      case 'inkjet':
        if (content.footer.qrCode && content.footer.barcode) {
          baseResult.warnings.push('Both QR code and barcode may slow down inkjet printing')
        }
        break
      case 'dot_matrix':
        if (content.header.logo) {
          baseResult.warnings.push('Logo printing not supported on dot matrix printers')
        }
        break
    }
