...
export class PrinterService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // 啟動健康監控
      this.healthMonitor.on('device_status_changed', this.handleDeviceStatusChange.bind(this))
    ...
    try {
      ...
      await Promise.all(
        Array.from(this.drivers.values()).map(driver =>
          driver.disconnect().catch((error: any) =>
  ...
  async registerPrinter(deviceConfig: {
    ...
    try {
      ...
      if (!connected) {
        throw new PrinterConnectionError(`Failed to connect to printer: ${deviceConfig.id}`)
      }
  ...
  getDevices(): PrinterDevice[] {
    return Array.from(this.drivers.values()).map(driver => driver.getDevice())
  }
  ...
  async print(request: PrintRequest): Promise<PrintResponse> {
    ...
    try {
      ...
      // 記錄統計
      this.statisticsCollector.recordJobCreated({ deviceId: job.deviceId, jobId: job.id })
  ...
  private setupJobManagerCallbacks(): void {
    ...
    this.jobManager.on('job_failed', (data: { job: PrintJob; error: Error }) => {
  ...
  private handleDeviceError(data: {
    deviceId: string
    error: any
  }): void {
