...
import type { PrintContent, PrintRequest, OrderData } from '@makanmakan/shared-types'
...
describe('PrintJobManager', () => {
  ...
  function createTestPrintRequest(content: PrintContent, overrides: Partial<PrintRequest> = {}): PrintRequest & { deviceId: string; content: PrintContent } {
  ...
  describe('Job Creation', () => {
    ...
    it('should reject job when queue is full', async () => {
      ...
      const promises = []
      for (let i = 0; i < 10; i++) {
        promises.push(jobManager.createJob(createTestPrintRequest(mockContent, {
          deviceId: `device-${i}`
        })))
      }
      ...
      await expect(jobManager.createJob(createTestPrintRequest(mockContent, {
  ...
  describe('Job Status Management', () => {
    it('should retrieve job by ID', async () => {
      ...
      const job = await jobManager.createJob(createTestPrintRequest(mockContent))
    ...
    it('should filter jobs by status', async () => {
      ...
      const job1 = await jobManager.createJob(createTestPrintRequest(mockContent, {
        deviceId: 'device-1'
      }))

      const job2 = await jobManager.createJob(createTestPrintRequest(mockContent, {
        deviceId: 'device-2'
      }))
  ...
  describe('Device Job Management', () => {
    it('should get jobs by device', async () => {
      ...
      await jobManager.createJob(createTestPrintRequest(mockContent, {
        deviceId: 'device-1'
      }))
