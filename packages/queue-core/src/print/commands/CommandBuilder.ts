...
export class CommandBuilder {
   ...
   * Add barcode command
   */
  addBarcode(data: string, type: 'CODE128' | 'CODE39' | 'EAN13' = 'CODE128'): CommandBuilder {
    this.commands.push({
      type: 'barcode',
      data,
      options: { type }
      ...
      switch (command.type) {
        case 'text':
          if (command.options?.alignment) {
            commands.push(ESCPOSCommands.setAlignment(command.options.alignment))
          }
          ...
          if (command.options?.size) {
            commands.push(ESCPOSCommands.setTextSize(
        ...
        case 'barcode':
          commands.push(ESCPOSCommands.printBarcode(command.data, command.options?.type))
          break

        case 'qr':
          commands.push(ESCPOSCommands.printQRCode(command.data, command.options?.size))
          break
  ...
  static fromPrintContent(content: PrintContent): CommandBuilder {
    ...
    if (content.header?.restaurantInfo?.name) {
      builder.addText(content.header.restaurantInfo.name, { bold: true, alignment: 'center' })
    }

    if (content.header?.restaurantInfo?.address) {
      builder.addText(content.header.restaurantInfo.address, { alignment: 'center' })
    }
    ...
    if (content.header?.transactionInfo) {
      ...
      builder.addText(`Time: ${content.header.transactionInfo.timestamp.toLocaleString()}`)
    }
    ...
    for (const item of content.items) {
      ...
      builder.addText(`  $${item.totalPrice.toFixed(2)}`, { alignment: 'right' })
    }

    builder.addFeed(1)

    // Add summary
    builder.addText(`Subtotal: $${content.summary.subtotal.toFixed(2)}`, { alignment: 'right' })
    for (const tax of content.summary.tax) {
      builder.addText(`${tax.name}: $${tax.amount.toFixed(2)}`, { alignment: 'right' })
    }
    builder.addText(`Total: $${content.summary.total.toFixed(2)}`, { bold: true, alignment: 'right' })
    ...
    if (content.footer?.qrCode) {
      const size = content.footer.qrCode.size === 'small' ? 2 :
                   content.footer.qrCode.size === 'medium' ? 3 : 4
      builder.addQRCode(content.footer.qrCode.data, size)
    }

    if (content.footer?.barcode) {
      builder.addBarcode(content.footer.barcode.data, content.footer.barcode.format as any)
    }
