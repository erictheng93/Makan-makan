...
export class ESCPOSCommands {
  ...
  static setAlignment(alignment: 'left' | 'center' | 'right'): string {
    ...
    return ESCPOSCommands.ESC + 'a' + String.fromCharCode(alignmentCodes[alignment])
  }
   ...
   * Print barcode
   */
  static printBarcode(data: string, type: 'CODE128' | 'CODE39' | 'EAN13' = 'CODE128'): string {
    const barcodeTypes = {
      CODE128: 73,
      CODE39: 69,
      EAN13: 67
  ...
  static printQRCode(data: string, size: number = 3): string {
    // QR code model
    const modelCommand = ESCPOSCommands.GS + '(k' + String.fromCharCode(4, 0, 49, 65, 50, 0)

    // QR code size
    const sizeCommand = ESCPOSCommands.GS + '(k' + String.fromCharCode(3, 0, 49, 67, size)

    // QR code data
    const dataLength = data.length + 3
    const dataCommand = ESCPOSCommands.GS + '(k' +
                       String.fromCharCode(dataLength % 256, Math.floor(dataLength / 256), 49, 80, 48) +
                       data

    // Print QR code
    const printCommand = ESCPOSCommands.GS + '(k' + String.fromCharCode(3, 0, 49, 81, 48)
