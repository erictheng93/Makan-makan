...
export class PrintTimeoutError extends PrintError {
  constructor(message: string, timeout: number, context?: any) {
    super(`${message} (timeout: ${timeout}ms)`, 'PRINT_TIMEOUT_ERROR', { timeout, ...context })
...
export const createPrintError = (
  type: 'connection' | 'job' | 'formatting' | 'driver' | 'config' | 'timeout' | 'validation',
  message: string,
  context?: any
): PrintError => {
  switch (type) {
    ...
    case 'validation':
      return new PrintValidationError(message, context?.validationErrors || [], context)
    default:
...
export const handlePrintError = (error: any): PrintError => {
  ...
  if (error instanceof Error) {
    return new PrintError(error.message, 'UNKNOWN_ERROR', { originalError: error })
  }

  return new PrintError(String(error), 'UNKNOWN_ERROR', { originalValue: error })
}

// 錯誤分類器
export const categorizePrintError = (error: PrintError): {
  category: 'hardware' | 'software' | 'network' | 'configuration' | 'data' | 'unknown'
  ...
  // 硬體相關錯誤
  if (code.includes('CONNECTION') || code.includes('DRIVER') || code.includes('TIMEOUT')) {
    return {
      category: 'hardware',
      severity: 'high',
      recoverable: true
