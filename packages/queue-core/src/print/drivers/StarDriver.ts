...
export class StarDriver extends PrinterDriver {
    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          message: error instanceof Error ? error.message : 'Star print job failed'
        }
      }
    }
  }
  ...
  private buildStarSpecificCommands(content: PrintContent): string {
    ...
    if (content.header?.transactionInfo) {
      ...
      commands.push(`Time: ${content.header.transactionInfo.timestamp.toLocaleString()}\n`)
    }
    ...
    commands.push(`\x1B\x45Total: $${content.summary.total.toFixed(2)}\x1B\x46\n`) // Bold

    commands.push('\n')

    // Add footer
    if (content.footer?.thankYouMessage) {
      commands.push(`\x1B\x61\x01${content.footer.thankYouMessage}\x1B\x61\x00\n`) // Center align
    }
  ...
  async playMelody(melodyNumber: number = 1): Promise<boolean> {
    ...
    try {
      ...
      await this.sendStarCommands(`\x1B\x42\x0A${String.fromCharCode(melodyNumber)}`)
      return true
    } catch {
      return false
    }
  ...
  async printLogo(logoNumber: number = 1): Promise<boolean> {
    ...
    try {
      ...
      await this.sendStarCommands(`\x1B\x1C\x70${String.fromCharCode(logoNumber)}\x00`)
      return true
    } catch {
      return false
    }
