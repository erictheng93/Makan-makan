...
export class PrinterDriverFactory {
  ...
  static getInstance(config?: DriverFactoryConfig): PrinterDriverFactory {
    if (!PrinterDriverFactory.instance) {
      if (!config) {
        throw new PrinterDriverError('PrinterDriverFactory requires config for first initialization')
      }
  ...
  static async createDriver(
    ...
    const device: PrinterDevice = {
      ...
      address: this.formatAddress(deviceConfig.connectionType, deviceConfig.connectionParams),
      status: 'offline',
      capabilities: deviceConfig.capabilities || this.getDefaultCapabilities(brand),
      lastSeen: new Date(),
      isDefault: false
    }
  ...
  static async createDriverForDevice(device: PrinterDevice, config?: any): Promise<PrinterDriver> {
    try {
      switch (device.brand) {
        case 'epson':
        ...
        default:
          throw new PrinterDriverError(`Unsupported printer brand: ${device.brand}`)
      }
    } catch (error) {
  ...
  private static formatAddress(connectionType: PrinterConnection, params: any): string {
    switch (connectionType) {
      case 'network':
        return params.host && params.port ? `${params.host}:${params.port}` : params.address || ''
      case 'usb':
      case 'serial':
  ...
  private generateDeviceId(connectionInfo: any, model: string): string {
    ...
    const address = this.formatConnectionAddress(connectionInfo).replace(/[^a-zA-Z0-9]/g, '_')
  ...
  private async scanNetworkPrinters(ranges?: string[]): Promise<PrinterDevice[]> {
  ...
  private async queryDeviceCapabilities(driver: PrinterDriver): Promise<PrinterCapabilities> {
    // 實際查詢打印機能力
    try {
  ...
  private static determineStarEmulation(model: string): 'StarPRNT' | 'StarLine' | 'StarGraphic' {
  ...
  private static determineCitizenSeries(model: string): 'CT-S' | 'CT-D' | 'CT-E' | 'PPU' {
