...
describe("RequestDeduplicator", () => {
  ...
  describe("Cache Size Management", () => {
    it("should enforce maximum cache size", async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        promises.push(
          smallCache.dedupe(`key${i}`, () => requestFn(`value${i}`)),
        );
      }
  ...
  describe("Error Handling", () => {
    it("should not cache failed requests", async () => {
      ...
      await expect(deduplicator.dedupe("test-key", failingFn)).rejects.toThrow(
        "Attempt 1 failed",
      );
...
describe("withDeduplication HOF", () => {
  ...
  it("should wrap function with deduplication", async () => {
    ...
    // Act
    const [result1, result2] = await Promise.all([wrappedFn(1), wrappedFn(1)]);
    // Assert
