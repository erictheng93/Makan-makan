...
import { RequestDeduplicator, getDeduplicator, resetDeduplicator, withDeduplication, batchDedupe } from '../request-deduplication';
describe('RequestDeduplicator', () => {
    ...
    describe('Basic Deduplication', () => {
        ...
        it('should not deduplicate requests with different keys', async () => {
            // Arrange
            const requestFn = vi.fn(async (value) => ({ data: value }));
            // Act
    ...
    describe('TTL and Custom Options', () => {
        ...
        it('should use custom key generator', async () => {
            // Arrange
            const customKeyGen = vi.fn((...args) => `custom-${args[0]}`);
            const dedup = new RequestDeduplicator({ keyGenerator: customKeyGen });
            const requestFn = vi.fn(async (id) => ({ id }));
            // Act
    ...
    describe('Cache Size Management', () => {
        it('should enforce maximum cache size', async () => {
            ...
            const requestFn = vi.fn(async (key) => ({ data: key }));
    ...
    describe('Error Handling', () => {
        it('should not cache failed requests', async () => {
            ...
            await expect(deduplicator.dedupe('test-key', failingFn)).rejects.toThrow('Attempt 1 failed');
            // Second attempt should retry (not use cached error)
            await expect(deduplicator.dedupe('test-key', failingFn)).rejects.toThrow('Attempt 2 failed');
    ...
    describe('dedupeByArgs', () => {
        it('should auto-generate key from arguments', async () => {
            // Arrange
            const requestFn = vi.fn(async (id, name) => ({
                id,
                name
            }));
            // Act
...
describe('withDeduplication HOF', () => {
    ...
    it('should wrap function with deduplication', async () => {
        // Arrange
        const originalFn = vi.fn(async (id) => ({ id, data: 'test' }));
    ...
    it('should preserve function signature', async () => {
        const originalFn = async (id, name) => ({ id, name });
        const wrappedFn = withDeduplication(originalFn);
        // Act
...
describe('batchDedupe', () => {
    ...
    it('should deduplicate identical requests in batch', async () => {
        // Arrange
        const requestFn = vi.fn(async (id) => ({ id, data: `user${id}` }));
        const requests = [
            () => requestFn(1),
...
describe('Debug Mode', () => {
    ...
    it('should log errors in debug mode', async () => {
        ...
        // Assert
        expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Error for key'), expect.any(Error));
        // Cleanup
        dedup.clear();
    });
});
