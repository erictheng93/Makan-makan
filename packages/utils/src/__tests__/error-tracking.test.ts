...
describe("ErrorTracker", () => {
  ...
  describe("Statistics", () => {
    it("should calculate error statistics", () => {
      // Arrange
      tracker.captureError(new Error("Critical failure"), {
        severity: "critical",
      });
      tracker.captureError(new Error("Network error"), {
        severity: "high",
        category: "network",
      });
      tracker.captureError(new Error("Validation error"), {
        severity: "low",
        category: "validation",
      });
  ...
  describe("Hooks and Options", () => {
    it("should call beforeSend hook", () => {
      ...
      const beforeSend = vi.fn(
        (error: TrackedError): TrackedError => ({
          ...error,
