...
export class SSEConnectionPool {
  ...
  async connect(
    ...
    if (this.connections.size >= this.maxConnections) {
      throw new Error(
        `Maximum connections limit (${this.maxConnections}) reached`,
      );
    }
      ...
      try {
        ...
        eventSource.onerror = (event) => {
          ...
          if (
            ...
            connection.reconnectAttempts < (options.retry?.maxAttempts || 5)
          ) {
            ...
            reject(
              new Error(
                `Failed to establish SSE connection to ${connection.url}`,
              ),
            );
          }
        };
        ...
        connection.status = "error";
        connection.statistics.lastError =
  ...
  private setupHeartbeat(connection: SSEConnection): void {
    ...
    connection.heartbeatInterval = setInterval(() => {
      ...
      if (timeSinceLastActivity > (heartbeat.timeoutAfter || 60000)) {
        console.warn(
          `Connection ${connection.id} heartbeat timeout, reconnecting...`,
        );
  ...
  private scheduleReconnection(connection: SSEConnection): void {
    ...
    console.log(
      `Reconnecting ${connection.id} in ${delay}ms (attempt ${connection.reconnectAttempts})`,
    );
  ...
  private async reconnectConnection(connection: SSEConnection): Promise<void> {
    ...
    try {
      ...
      if (
        connection.reconnectAttempts <
        (connection.options.retry?.maxAttempts || 5)
      ) {
        this.scheduleReconnection(connection);
      } else {
        connection.status = "error";
    ...
    this.connections.forEach((connection) => {
      if (connection.status === "connected") {
        ...
        if (timeSinceActivity > timeout) {
          console.warn(
            `Connection ${connection.id} appears stale, checking...`,
          );
