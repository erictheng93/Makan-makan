...
export class QueueService implements IQueueService {
  ...
  async joinQueue(request: JoinQueueRequest): Promise<ApiResponse<JoinQueueResponse>> {
    try {
      const validatedData = validateJoinQueue(request)

      // Check if queue system is enabled
      const settings = await this.settingsRepository.findByRestaurant(validatedData.restaurantId)
      if (!settings?.isEnabled) {
        throw new QueueDisabledError(validatedData.restaurantId)
      }

      // Check if queue is full
      const currentQueueSize = await this.queueRepository.getQueueSize(validatedData.restaurantId)
      if (currentQueueSize >= settings.maxQueueSize) {
        throw new QueueFullError(validatedData.restaurantId, settings.maxQueueSize)
      }
      ...
      // Generate queue data
      const queueNumber = await this.queueRepository.getMaxQueueNumber(validatedData.restaurantId) + 1
      const checkInCode = this.generateCheckInCode()
      const priority = this.metricsService.calculatePriority(validatedData, settings.priorityRules)
      const estimatedWait = await this.metricsService.calculateEstimatedWaitTime(
      ...
      const queueData: Omit<WaitingQueue, 'id'> = {
        ...
        notificationMethods: validatedData.notificationMethods || [NotificationType.SMS],
      ...
      const currentPosition = await this.queueRepository.getQueuePosition(queue.id)
      ...
      return {
        success: false,
        error: error instanceof QueueError ? error.message : 'Failed to join queue'
      }
    }
  }

  async getQueueStatus(restaurantId: number): Promise<ApiResponse<QueueStatusResponse>> {
    try {
      const stats = await this.queueRepository.getQueueStatistics(restaurantId)
      const realtimeMetrics = await this.metricsService.getRealtimeMetrics(restaurantId)
  ...
  async getQueuePosition(queueId: string): Promise<ApiResponse<QueuePositionResponse>> {
    try {
      ...
      return {
        success: false,
        error: error instanceof QueueError ? error.message : 'Failed to get queue position'
      }
    }
  }

  async callNext(request: CallNextRequest, operatorId: number): Promise<ApiResponse<WaitingQueue>> {
    try {
      ...
      if (validatedData.specificQueueId) {
        nextInQueue = await this.queueRepository.findById(validatedData.specificQueueId)
        if (!nextInQueue || nextInQueue.status !== QueueStatus.WAITING) {
          throw new QueueNotFoundError(validatedData.specificQueueId)
        }
      } else {
        nextInQueue = await this.queueRepository.findNextInQueue(validatedData.restaurantId)
        if (!nextInQueue) {
          return {
            success: false,
      ...
      return {
        success: false,
        error: error instanceof QueueError ? error.message : 'Failed to call next customer'
      }
    }
  }
  ...
  async seatCustomer(request: SeatCustomerRequest): Promise<ApiResponse<void>> {
    try {
      ...
      if (!['waiting', 'called'].includes(queue.status)) {
        throw new InvalidQueueStatusError(validatedData.queueId, queue.status, ['waiting', 'called'])
      }
      ...
      await this.notificationService.sendNotification(validatedData.queueId, 'seated')
      ...
      return {
        success: false,
        error: error instanceof QueueError ? error.message : 'Failed to seat customer'
      }
    }
  }
  ...
  async cancelQueue(request: CancelQueueRequest): Promise<ApiResponse<void>> {
    try {
      ...
      if (queue.status !== QueueStatus.WAITING) {
        throw new InvalidQueueStatusError(validatedData.queueId, queue.status, ['waiting'])
      }
      ...
      await this.notificationService.sendNotification(validatedData.queueId, 'cancelled')
      ...
      return {
        success: false,
        error: error instanceof QueueError ? error.message : 'Failed to cancel queue'
      }
    }
  }

  async getCurrentQueue(request: GetCurrentQueueRequest): Promise<ApiResponse<CurrentQueueResponse>> {
    try {
      const filters = {
        status: request.status ? [request.status] : [QueueStatus.WAITING, QueueStatus.CALLED],
        limit: request.limit || 50
      }

      const queues = await this.queueRepository.findByRestaurant(request.restaurantId, filters)
  ...
  async getQueueHistory(request: GetQueueHistoryRequest): Promise<ApiResponse<QueueHistoryResponse>> {
    try {
      const filters = {
        status: request.status ? [request.status] : [QueueStatus.SEATED, QueueStatus.CANCELLED, QueueStatus.NO_SHOW],
  ...
  async getQueueStatistics(request: GetQueueStatisticsRequest): Promise<ApiResponse<QueueStatisticsResponse>> {
    try {
      const dateRange = request.dateFrom && request.dateTo ? {
        from: request.dateFrom,
        to: request.dateTo
      } : undefined

      const stats = await this.queueRepository.getQueueStatistics(request.restaurantId, dateRange)
      const hourlyBreakdown = await this.queueRepository.getHourlyBreakdown(request.restaurantId, dateRange)
  ...
  async getQueueSettings(restaurantId: number): Promise<ApiResponse<QueueSettings>> {
    try {
      let settings = await this.settingsRepository.findByRestaurant(restaurantId)
  ...
  async updateQueueSettings(restaurantId: number, updates: UpdateQueueSettingsRequest): Promise<ApiResponse<void>> {
    try {
  ...
  private isWithinBusinessHours(_businessHours: Record<string, unknown>): boolean {
  ...
  private createDefaultSettings(restaurantId: number): Omit<QueueSettings, 'createdAt' | 'updatedAt'> {
    return {
      restaurantId,
      isEnabled: true,
      maxQueueSize: 50,
