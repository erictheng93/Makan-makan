...
class PWATestSuite {
      ...
      if (manifestLink) {
        ...
        if (response.ok) {
          ...
          if (manifest.icons) {
            ...
            manifestTest.hasLargeIcon = manifest.icons.some(
              (icon) => icon.sizes && icon.sizes.includes("512x512"),
            );
          }

          // 檢查快捷方式
          manifestTest.hasShortcuts = !!(
  ...
  async testOfflineCapabilities() {
    ...
    try {
      // 檢查網路狀態 API
      offlineTest.networkApiSupported =
  ...
  async testPushNotifications() {
    ...
    try {
      ...
      // 檢查 VAPID 支援
      pushTest.vapidSupported =
        pushTest.pushSupported &&
  ...
  async testInstallation() {
    ...
    try {
      ...
      installTest.beforeInstallPromptSupported =
  ...
  async testPerformance() {
    ...
    try {
      ...
      if (perfTest.performanceApiSupported) {
        ...
        perfTest.totalResourceSize = resources.reduce(
  ...
  async testCacheStrategies() {
    ...
    try {
      ...
      strategies.staleWhileRevalidate =
        await this.testStaleWhileRevalidate(testUrl);
    } catch (error) {
  ...
  checkCustomSyncImplementation() {
    // 檢查是否存在自定義同步實作
    const hasCustomerSync =
      typeof window.customerBackgroundSync !== "undefined";
    ...
    return {
      ...
      total: [hasCustomerSync, hasAdminSync, hasKitchenSync].filter(Boolean)
        .length,
    };
  }
