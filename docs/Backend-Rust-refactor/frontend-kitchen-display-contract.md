...
|---|---|---|---|
| `apps/api` (main REST API) | `/api/v1` (proxied) | `apiClient`/`api` (from `src/services/authApi.ts`, built via `@makanmasak/auth-client`) | Auth, kitchen order board reads, item-status writes, SSE token minting (unused path), error reporting, realtime WS auth-token minting |
