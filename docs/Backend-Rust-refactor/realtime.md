...
10. **Guest-scope requirement** (customer room only): `authPayload.guestFlag` must be set → else `403`. `verifyWebSocketToken` pins a guest payload's `roomId` to `order:{orderId}` or `customer:{tableId}`, so the flag is what proves the room was bound to something verified at mint time. A plain customer-role token carries no such binding (issue #96).
11. **Table/seat ownership** (customer room only): `validateTableAccess` — with no `tableId`, the only accepted case is an order-scoped guest token (`scope === "guest-realtime"` **and** `orderId` present); anything else → `403`. This previously failed open ("no `tableId` ⇒ assume shop mode ⇒ allow"), which let any customer token into any room. With a `tableId`, runs **two separate D1 `.prepare()` queries** (one against `tables`, one against `seats`) and compares `seat.table_id !== table.id` in JS — there is no SQL JOIN — to confirm the seat belongs to that table and both are active. DB error → `403`.
