...
```typescript
// 使用範例
const trafficDrivers = await productAnalysis.getTrafficDrivers(
  restaurantId,
  { range: "30d" },
  10,
);
...
```typescript
const bestsellers = await productAnalysis.getBestsellers(
  restaurantId,
  { range: "30d" },
  10,
);
...
```typescript
const profitLeaders = await productAnalysis.getProfitLeaders(
  restaurantId,
  { range: "30d" },
  10,
);
          ...
          <el-card v-for="product in trafficDrivers" :key="product.menuItemId">
            ...
            <div class="metrics">
              ...
              <div class="metric">
                ...
                <span class="value"
                  >{{ (product.conversionRate * 100).toFixed(1) }}%</span
                >
              </div>
              <div class="metric">
