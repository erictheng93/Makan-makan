...
<template>
  <div class="schedule-view">
    ...
    <div class="week-navigation">
      ...
      <el-button type="primary" @click="showCopyWeekDialog">複製上週班表</el-button>
    </div>
    ...
    <el-row :gutter="20" class="stats-row">
      ...
      <el-col :span="6">
        <el-statistic title="總工時" :value="weekStats.totalHours" suffix="小時" />
      </el-col>
      <el-col :span="6">
        <el-statistic title="排班員工" :value="weekStats.totalEmployees" suffix="人" />
      </el-col>
      <el-col :span="6">
...
const weekStats = computed(() => ({
  ...
  totalHours: weekSchedules.value.reduce((sum, s) => sum + s.durationMinutes / 60, 0),
    ...
    <table class="schedule-table">
      ...
      <tbody>
        <tr v-for="employee in employees" :key="employee.id">
          ...
          >
            <ScheduleCard
              v-for="schedule in getSchedulesForEmployeeDay(employee.id, day - 1)"
              :key="schedule.id"
              :schedule="schedule"
              draggable="true"
...
function getSchedulesForEmployeeDay(employeeId: number, dayOffset: number): Schedule[] {
...
<template>
  <el-dialog v-model="visible" title="班次模板管理" width="800px">
    ...
    <!-- Create/Edit Dialog -->
    <el-dialog v-model="formVisible" :title="formMode === 'create' ? '新增班次' : '編輯班次'">
...
function getAlertType(severity: string): 'success' | 'info' | 'warning' | 'error' {
...
export class ConflictDetectionService {
  ...
  async detectConflicts(
    ...
    const schedules = await this.getSchedulesInRange(restaurantId, dateFrom, dateTo)
  ...
  private async checkRestPeriod(
    ...
    // Group by employee
    const byEmployee = schedules.reduce((acc, s) => {
    ...
    for (const [employeeId, empSchedules] of Object.entries(byEmployee)) {
      ...
      for (let i = 0; i < sorted.length - 1; i++) {
        ...
        const restHours = (startTime.getTime() - endTime.getTime()) / (1000 * 60 * 60)
  ...
  private async checkMaxHours(
    ...
    const byEmployeeWeek = schedules.reduce((acc, s) => {
      const weekKey = getWeekKey(s.workDate)
      const key = `${s.employeeId}-${weekKey}`
      if (!acc[key]) acc[key] = { employeeId: s.employeeId, schedules: [], week: weekKey }
      acc[key].schedules.push(s)
      return acc
    }, {} as Record<string, { employeeId: number; schedules: Schedule[]; week: string }>)

    for (const [key, data] of Object.entries(byEmployeeWeek)) {
      const totalHours = data.schedules.reduce((sum, s) =>
        sum + (s.durationMinutes || 0) / 60, 0
      )
...
async function createScheduleWithValidation(data: CreateScheduleRequest) {
  ...
  if (potentialConflicts.some(c => c.severity === 'error')) {
    throw new ValidationError('Cannot create schedule due to blocking conflicts', {
...
const schedules = await scheduleService.getWeekSchedules(restaurantId, weekStart, {
  page: 1,
  limit: PAGE_SIZE
})
```
...
describe('Scheduling Workflow', () => {
  it('should complete full schedule creation workflow', async () => {
    ...
    expect(notifications.some(n => n.type === 'schedule_assigned')).toBe(true)
  })
})
```
...
const existing = await scheduleService.getWeekSchedules(restaurantId, targetWeekStart)
