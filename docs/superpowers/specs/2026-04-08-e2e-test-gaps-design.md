...
page.locator('button:has-text("Pay"), button:has-text("收款"), button:has-text("結帳"), [data-testid="pay-btn"]')
```
...
await page.route(/payments/, (route) => {
  callCount++;
  if (callCount === 1) route.fulfill({ status: 402, body: JSON.stringify({ success: false, error: { code: "CARD_DECLINED" } }) });
  else route.continue();
});
// ... click Pay ...
await expect(page.locator('[data-testid="payment-error"], [role="alert"]')).toBeVisible({ timeout: 5000 });
await expect(page.locator('[data-testid="pay-btn"], button:has-text("收款")')).toBeEnabled();
```
...
await page.route('**/api/v1/orders', (route) => {
  ...
  orderPostCount++;
  if (orderPostCount === 1) {
    route.fulfill({ status: 200, body: JSON.stringify({ success: true, data: createMockOrder() }) });
  } else {
    route.fulfill({ status: 409, body: JSON.stringify({ success: false, error: { code: 'OUT_OF_STOCK', itemId: 'item-1' } }) });
  }
});
```
...
await expect(page.locator('text=/已取消|Cancelled/i').first()).toBeVisible({ timeout: 8000 });
```
