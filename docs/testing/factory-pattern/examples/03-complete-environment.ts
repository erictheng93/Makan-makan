...
describe('範例 3: 完整測試環境', () => {
  ...
  it('應該提供所有員工的統一訪問', () => {
    ...
    expect(allEmployees).toEqual(
      expect.arrayContaining(testData.team.chefs)
    )
    expect(allEmployees).toEqual(
  ...
  it('應該驗證數據的完整性', () => {
    ...
    // 所有菜單項目都屬於該餐廳
    expect(testData.menuItems.every(item =>
      item.restaurantId === testData.restaurant.id
    )).toBe(true)

    // 所有員工都屬於該餐廳
    expect(testData.team.all.every(employee =>
      employee.restaurantId === testData.restaurant.id
    )).toBe(true)

    // 所有訂單都屬於該餐廳
    expect(testData.orders.every(order =>
      order.restaurantId === testData.restaurant.id
    )).toBe(true)
  })
})
