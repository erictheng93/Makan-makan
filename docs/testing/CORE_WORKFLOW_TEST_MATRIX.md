...
| `kitchen-display` | `tests/e2e/smoke/kitchen-display.spec.ts` verifies kitchen HTML liveness and, when credentials are configured, chef login plus kitchen queue API access. This credentialed layer is optional and gated on local credential availability. | order stores, order card, settings, service-worker helpers | `apps/kitchen-display/src/__tests__/integration/kitchen-display.real.integration.test.ts` covers module-gated kitchen API access, role/restaurant boundaries, confirmed-order queue visibility, and item status persistence from pending to preparing to ready | `tests/e2e/kitchen-display/kitchen-display.spec.ts` covers browser chef dashboard routing and visible item transitions; `tests/e2e/integration/real-workflows.spec.ts` includes confirmed-order queue visibility, browser-triggered item transition to preparing, SSE EventSource construction, new-order audio playback request via browser audio stub, offline banner behavior, offline queued action replay after reconnect, and audio toggle persistence when `WORKFLOW_KITCHEN_URL` and chef credentials are set | Low-medium: true cross-worker realtime broadcast binding still needs integration coverage |
...
| `onboarding-app` | none dedicated | API location, onboarding store, i18n | `apps/management-api/src/__tests__/integration/onboarding-workflow.real.integration.test.ts` covers onboarding public API flow | `tests/e2e/integration/real-workflows.spec.ts` includes application form submission; Cloudflare verification/completion is legacy-only and no longer part of the active onboarding flow | Medium-low: admin approval activation remains covered by management-api integration tests, while browser E2E coverage is application submission only |
...
- `tests/e2e/integration/real-workflows.spec.ts`: true customer browser
  ...
  tracking. The suite now also includes customer UI cart
  add/increase/decrease/remove/re-add, notes, checkout request payload
  verification, multi-vendor market checkout submission/tracking, and service
  booking create/verify/cancel,
  ...
  onboarding-app application form submission. The previous Cloudflare
  verify/complete browser flow is retained only as historical context and is no
  longer part of the active product flow.
