...
> Last updated: 2026-06-05. Source of truth for "how far are we from the
...
working backend. Remaining P0 is mostly onboarding geo and customer-app UI for
the new flows.
...
1. Customer-app UI for the new flows: 卷 entry at market checkout, and the service
   booking flow (availability → book → pay with 代幣). **(M)**
2. Onboarding → market geo auto-assignment. **(M)**
3. Service-booking `/slots` capacity-management routes + admin UI. **(S–M)**
