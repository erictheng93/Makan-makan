...
describe("Simple Component Logic", () => {
  ...
  class MenuItemLogic {
    ...
    canCustomize(): boolean {
      return !!(
        this.item.customization_options &&
  ...
  describe("基本功能測試", () => {
    ...
    it("正確判斷商品可用性", () => {
      const availableItem = new MenuItemLogic({
        ...mockMenuItem,
        available: true,
      });
      const unavailableItem = new MenuItemLogic({
        ...mockMenuItem,
        available: false,
      });
  ...
  describe("圖片處理邏輯", () => {
    it("處理圖片 URL", () => {
      ...
      expect(withImage.getCartData().image_url).toBe(
        "/images/beef-noodles.jpg",
      );
