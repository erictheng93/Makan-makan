...
describe("migration dual-track guard", () => {
  ...
  it("legacy cutover drops partnership views before removing partnership money columns", () => {
    ...
    for (const viewName of [
      ...
      const dropView = migration.indexOf(`DROP VIEW IF EXISTS \`${viewName}\``);
      expect(dropView).toBeGreaterThan(-1);
      expect(dropView).toBeLessThan(firstPartnershipMoneyDrop);
    }
  });

  // The fresh track was squashed into 0000_baseline_strict.sql, so it no
  // longer carries the cutover's ordering. What that ordering existed to
  // guarantee is an end state, and the baseline can be asserted on directly.
  it("fresh baseline carries the post-cutover partnership shape", () => {
    ...
    expect(baseline).toMatch(/`default_discount_value_cents`/);
    expect(baseline).not.toMatch(/`default_discount_value`[^_]/);
    for (const viewName of [
      "vw_active_partnership_plans",
      "vw_member_usage_summary",
      "vw_partnership_statistics",
    ]) {
      expect(baseline).not.toContain(viewName);
    }
  });
});
