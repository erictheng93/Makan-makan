...
const mockOrders = [
  {
    ...
    items: [
      ...
      { id: 2, name: '珍珠奶茶', quantity: 3, price: 45.50, status: 'pending' }
    ],
    customerName: '張小明',
...
test.describe('Admin Dashboard - 訂單管理', () => {
  ...
  test('應該顯示訂單列表', async ({ page }) => {
    // 等待訂單列表載入
    await page.waitForSelector('[data-testid="orders-list"], .orders-table, table')

    // 驗證訂單數量
    const orderRows = await page.locator('tbody tr, [data-testid="order-item"]').count()
    ...
    await expect(page.locator('text=285.50').or(page.locator('text=285.5')).first()).toBeVisible()
  })
  ...
  test('應該能夠篩選訂單狀態', async ({ page }) => {
    ...
    // 點擊狀態篩選器
    const filterButton = page.locator('[data-testid="status-filter"], select[name="status"], button:has-text("狀態")').first()
    await filterButton.click()

    // 選擇「準備中」狀態
    await page.click('text=準備中, [value="preparing"], [data-value="preparing"]')
  ...
  test('應該能夠查看訂單詳情', async ({ page }) => {
    ...
    // 點擊第一筆訂單
    await page.click('text=ORD-2025-001, [data-order-id="1"], tr:has-text("ORD-2025-001")')

    // 等待詳情模態框或頁面載入
    await page.waitForSelector('[data-testid="order-detail"], .order-detail-modal, .modal:visible', {
      timeout: 10000
    })
  ...
  test('應該能夠搜尋訂單', async ({ page }) => {
    ...
    // 驗證其他訂單被過濾
    const visibleOrders = await page.locator('tbody tr:visible, [data-testid="order-item"]:visible').count()
  ...
  test('應該能夠取消訂單', async ({ page }) => {
    ...
    const cancelButton = page.locator(
      'button:has-text("取消訂單"), [data-testid="cancel-order"]'
    ).first()
    ...
    if (await cancelButton.isVisible()) {
      ...
      // 確認取消操作（如果有確認對話框）
      const confirmDialog = page.locator('button:has-text("確定"), button:has-text("確認")').last()
  ...
  test('應該顯示訂單統計資訊', async ({ page }) => {
    // 查找統計卡片或儀表板
    const statsSection = page.locator('[data-testid="order-stats"], .stats-grid, .statistics-panel')

    if (await statsSection.isVisible({ timeout: 5000 })) {
      // 驗證統計資訊存在
      const statCards = await statsSection.locator('.stat-card, .card, [role="status"]').count()
  ...
  test('應該實時更新訂單狀態（WebSocket）', async ({ page }) => {
    ...
    const newOrderExists = await page.locator('text=ORD-2025-004').isVisible({ timeout: 2000 }).catch(() => false)
...
test.describe('Admin Dashboard - 訂單管理（錯誤處理）', () => {
  ...
  test('應該處理網路超時', async ({ page }) => {
    ...
    const loadingOrError = await Promise.race([
      page.waitForSelector('.loading, .spinner', { timeout: 5000 }).then(() => 'loading'),
      page.waitForSelector('.error-message, .alert-error', { timeout: 35000 }).then(() => 'error')
    ]).catch(() => 'timeout')
