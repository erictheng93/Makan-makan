...
async function login(page: Page, role: number = 1) {
  ...
  await page.fill(
    ...
    role === 2 ? "chef" : role === 4 ? "cashier" : "admin",
  );
...
const mockKitchenOrders = [
  ...
  },
  {
    ...
    tableName: "B-3",
    items: [{ id: 3, name: "湯麵", quantity: 3, status: "pending", notes: "" }],
    status: "pending",
...
test.describe("Admin Dashboard - 廚房顯示", () => {
  ...
  test("應該顯示待處理訂單列表", async ({ page }) => {
    await page.waitForSelector(
      '[data-testid="kitchen-orders"], .kitchen-orders-list, .order-card',
    );
  ...
  test("應該顯示訂單計時器", async ({ page }) => {
    ...
    const timer = page.locator(
      '[data-testid="order-timer"], .timer, .elapsed-time',
    );
  ...
  test("應該能夠按優先級排序訂單", async ({ page }) => {
    ...
    const sortButton = page
      .locator('button:has-text("排序"), select[name="sort"]')
      .first();
...
test.describe("Admin Dashboard - 隊列管理", () => {
  ...
  test("應該顯示排隊列表", async ({ page }) => {
    await page.waitForSelector(
      '[data-testid="queue-list"], .queue-list, .queue-table',
    );

    const customerCount = await page
      .locator('[data-testid="queue-item"], tbody tr')
      .count();
  ...
  test("應該能夠安排座位", async ({ page }) => {
    ...
    const tableOption = page
      .locator('button:has-text("A-1"), [data-table-id="1"]')
      .first();
  ...
  test("應該顯示排隊統計資訊", async ({ page }) => {
    ...
    const statsSection = page.locator(
      '[data-testid="queue-stats"], .stats-grid',
    );
...
test.describe("Admin Dashboard - POS 收銀", () => {
  ...
  test("應該顯示待付款訂單", async ({ page }) => {
    ...
    const orderCount = await page
      .locator('[data-testid="order-item"], .order-card')
      .count();
  ...
  test("應該能夠處理現金支付", async ({ page }) => {
    ...
    const orderItem = page
      .locator('[data-order-id="1"], tr:has-text("ORD-2025-001")')
      .first();
    if (await orderItem.isVisible({ timeout: 3000 })) {
      ...
      const cashButton = page
        .locator('button:has-text("現金"), [data-payment="cash"]')
        .first();
      if (await cashButton.isVisible({ timeout: 2000 })) {
        ...
        const amountInput = page.locator(
          'input[name="receivedAmount"], #receivedAmount',
        );
        ...
        // 確認支付
        const confirmButton = page
          .locator('button:has-text("確認"), button[type="submit"]')
          .last();
        await confirmButton.click();
  ...
  test("應該顯示交易記錄", async ({ page }) => {
    await page.waitForSelector(
      '[data-testid="transactions"], .transactions-list',
    );

    const transactionCount = await page
      .locator('[data-testid="transaction-item"], tbody tr')
      .count();
  ...
  test("應該顯示銷售摘要", async ({ page }) => {
    const summarySection = page.locator(
      '[data-testid="sales-summary"], .sales-summary',
    );

    if (await summarySection.isVisible({ timeout: 5000 })) {
      await expect(
        summarySection.locator("text=1500, text=1,500"),
      ).toBeVisible();
    }
  });
});
...
test.describe("Admin Dashboard - 廚房/隊列/POS（錯誤處理）", () => {
  ...
  test("應該處理廚房訂單更新失敗", async ({ page }) => {
    ...
    const prepareButton = page
      .locator('button:has-text("準備"), button:has-text("開始")')
      .first();
    if (await prepareButton.isVisible({ timeout: 3000 })) {
      await prepareButton.click();
  ...
  test("應該處理安排座位時無可用桌台", async ({ page }) => {
    await page.route("/api/v1/queue*", async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          data: { customers: mockQueueCustomers, statistics: {} },
        }),
      });
    });
    ...
    if (await seatButton.isVisible({ timeout: 3000 })) {
      ...
      // 驗證顯示無可用桌台訊息
      const noTablesMessage = await page
        .locator("text=無可用桌台, text=沒有可用, .empty-state")
