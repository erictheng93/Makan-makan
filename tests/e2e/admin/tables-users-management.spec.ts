...
test.describe('Admin Dashboard - 桌台管理', () => {
  ...
  test('應該顯示桌台列表', async ({ page }) => {
    await page.waitForSelector('[data-testid="tables-list"], .tables-grid, .tables-table')

    const tableCount = await page.locator('[data-testid="table-card"], .table-card, tbody tr').count()
  ...
  test('應該能夠新增桌台', async ({ page }) => {
    ...
    const addButton = page.locator('button:has-text("新增"), [data-testid="add-table"]').first()
  ...
  test('應該能夠篩選桌台狀態', async ({ page }) => {
    const filterButton = page.locator('select[name="status"], button:has-text("篩選")').first()
    ...
    if (await filterButton.isVisible({ timeout: 3000 })) {
      ...
      const visibleTables = await page.locator('[data-testid="table-card"]:visible, tbody tr:visible').count()
...
test.describe('Admin Dashboard - 用戶管理', () => {
  ...
  test('應該顯示員工列表', async ({ page }) => {
    await page.waitForSelector('[data-testid="users-list"], .users-table, table')

    const userCount = await page.locator('[data-testid="user-item"], tbody tr').count()
  ...
  test('應該能夠新增員工', async ({ page }) => {
    ...
    const addButton = page.locator('button:has-text("新增"), [data-testid="add-user"]').first()
    await addButton.click()
  ...
  test('應該能夠按角色篩選員工', async ({ page }) => {
    const filterSelect = page.locator('select[name="role"], [data-testid="role-filter"]').first()
  ...
  test('應該顯示員工統計資訊', async ({ page }) => {
    const statsSection = page.locator('[data-testid="users-stats"], .stats-grid')
  ...
  test('應該處理新增桌台時的重複名稱錯誤', async ({ page }) => {
    await page.route('/api/v1/tables*', async route => {
      if (route.request().method() === 'GET') {
        await route.fulfill({
          status: 200,
          body: JSON.stringify({ success: true, data: { tables: mockTables, pagination: {} } })
        })
  ...
  test('應該處理新增用戶時的驗證錯誤', async ({ page }) => {
    await page.route('/api/v1/users*', async route => {
      if (route.request().method() === 'GET') {
        await route.fulfill({
          status: 200,
          body: JSON.stringify({ success: true, data: { users: mockUsers, pagination: {} } })
        })
      } else if (route.request().method() === 'POST') {
        await route.fulfill({
          status: 400,
