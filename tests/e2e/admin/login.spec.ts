...
test.describe("Admin Dashboard Login Flow", () => {
  ...
  test("should show validation errors for empty form", async ({ page }) => {
    ...
    await expect(page.locator(".error-message, .text-red-500")).toHaveCount({
      min: 1,
    });
  });
  ...
  test("should handle successful login flow", async ({ page }) => {
    ...
    await expect(page.locator("h1, .dashboard-title")).toContainText(
      /儀表板|dashboard/i,
    );
  });
  ...
  test("should handle logout correctly", async ({ page }) => {
    ...
    await page.click(
      '[data-testid="logout-button"], .logout-button, button:has-text("登出"), button:has-text("Logout")',
    );
...
test.describe("Admin Dashboard Navigation", () => {
  ...
  test("should show correct navigation items for owner role", async ({
    page,
  }) => {
