...
export class OrderingHelper {
  ...
  async addItemToCart(itemId: string, quantity: number = 1) {
    for (let i = 0; i < quantity; i++) {
      await this.page
        .locator(`[data-testid="menu-item-${itemId}"] [data-testid="add-btn"]`)
        .click();
  ...
  async addCustomizedItemToCart(
    ...
    await expect(
      this.page.locator('[data-testid="customization-modal"]'),
    ).toBeVisible();

    // 選擇辣度
    if (customizations.spiceLevel) {
      await this.page
        .locator(`[data-testid="spice-level-${customizations.spiceLevel}"]`)
        .click();
    }
    ...
    if (customizations.notes) {
      await this.page
        .locator('[data-testid="special-notes"]')
        .fill(customizations.notes);
    }
  ...
  async updateItemQuantity(itemId: string, quantity: number) {
    const currentQuantity = await this.page
      .locator(`[data-testid="item-quantity-${itemId}"]`)
      .textContent();
    ...
    if (difference > 0) {
      for (let i = 0; i < difference; i++) {
        await this.page
          .locator(`[data-testid="quantity-increase-${itemId}"]`)
          .click();
      }
    } else if (difference < 0) {
      for (let i = 0; i < Math.abs(difference); i++) {
        await this.page
          .locator(`[data-testid="quantity-decrease-${itemId}"]`)
          .click();
      }
    }

    await expect(
      this.page.locator(`[data-testid="item-quantity-${itemId}"]`),
  ...
  async removeItemFromCart(itemId: string) {
    ...
    // 確認移除對話框
    await expect(
      this.page.locator('[data-testid="confirm-remove-modal"]'),
    ).toBeVisible();
    await this.page.locator('[data-testid="confirm-remove-btn"]').click();

    await expect(
      this.page.locator(`[data-testid="cart-item-${itemId}"]`),
    ).not.toBeVisible();
  }
  ...
  async fillCustomerInfo(customerInfo: {
    ...
    await this.page
      ...
      .fill(customerInfo.name);
    await this.page
      .locator('[data-testid="customer-phone"]')
      .fill(customerInfo.phone);
  ...
  async expectCartCount(count: number) {
    await expect(this.page.locator('[data-testid="cart-count"]')).toHaveText(
      count.toString(),
    );
  }
  ...
  async expectTotalPrice(price: number) {
    await expect(
      this.page.locator('[data-testid="total-price"]'),
    ).toContainText(`NT$${price}`);
  }
}
...
export class AdminHelper {
  ...
  async addMenuItem(item: {
    ...
    await this.page
      .locator('[data-testid="item-category"]')
      .selectOption(item.category);

    if (item.imagePath) {
      await this.page
        .locator('[data-testid="image-upload"]')
        .setInputFiles(item.imagePath);
    }
    ...
    // 確認新菜品出現在列表中
    await expect(
      this.page.locator(`[data-testid="menu-item"][data-name="${item.name}"]`),
    ).toBeVisible();
  }
  ...
  async updateOrderStatus(orderId: string, status: string) {
    await this.page
      .locator(
        `[data-testid="order-${orderId}"] [data-testid="status-dropdown"]`,
      )
      .selectOption(status);
    ...
    await expect(
      this.page.locator(
        `[data-testid="order-${orderId}"] [data-testid="order-status"]`,
      ),
    ).toContainText(this.getStatusText(status));
  }
  ...
  async addTable(tableInfo: { number: string; seats: string }) {
    ...
    await this.page
      .locator('[data-testid="table-number"]')
      .fill(tableInfo.number);
    await this.page
      .locator('[data-testid="table-seats"]')
      .fill(tableInfo.seats);

    await this.page.locator('[data-testid="save-table-btn"]').click();

    // 確認新桌號出現在列表中
    await expect(
      this.page.locator(`[data-testid="table-${tableInfo.number}"]`),
    ).toBeVisible();
  }
  ...
  async generateTableQRCode(tableNumber: string) {
    await this.page
      .locator(
        `[data-testid="table-${tableNumber}"] [data-testid="generate-qr-btn"]`,
      )
      .click();

    // 確認 QR Code 彈窗出現
    await expect(
      this.page.locator('[data-testid="qr-code-modal"]'),
    ).toBeVisible();
    await expect(
      this.page.locator('[data-testid="qr-code-image"]'),
    ).toBeVisible();
  }
...
export class OrderTrackingHelper {
  ...
  async expectOrderStatus(status: string) {
    ...
    await expect(
      this.page.locator('[data-testid="current-status"]'),
  ...
  async expectEstimatedTime(minutes: number) {
    await expect(
      this.page.locator('[data-testid="estimated-time"]'),
    ).toContainText(`${minutes} 分鐘`);
  }
