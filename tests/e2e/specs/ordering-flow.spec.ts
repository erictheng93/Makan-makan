...
test.describe("顧客點餐流程", () => {
  ...
  test("完整點餐流程 - 基本商品", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="menu-item"]')).toHaveCount(3, {
      timeout: 10000,
    });
    ...
    await expect(page.locator('[data-testid="item-quantity-2"]')).toHaveText(
      "1",
    );
  ...
  test("客製化商品點餐流程", async ({ page }) => {
    ...
    const customizationInfo = page.locator(
      '[data-testid="item-customization-1"]',
    );
    ...
    await expect(page.locator('[data-testid="item-notes-1"]')).toContainText(
      "少油少鹽，謝謝",
    );
  ...
  test("購物車管理功能", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="confirm-clear-modal"]'),
    ).toBeVisible();
  ...
  test("響應式設計 - 手機版", async ({ page }) => {
    ...
    // 1. 確認手機版菜單正確顯示
    await expect(
      page.locator('[data-testid="mobile-menu-grid"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="desktop-menu-grid"]'),
    ).not.toBeVisible();
    ...
    // 確認浮動購物車按鈕顯示
    await expect(
      page.locator('[data-testid="floating-cart-btn"]'),
    ).toBeVisible();
    ...
    // 確認購物車從底部滑出
    await expect(
      page.locator('[data-testid="mobile-cart-drawer"]'),
    ).toBeVisible();
  });
  ...
  test("商品庫存不足處理", async ({ page }) => {
    ...
    await expect(
      menuItem.locator('[data-testid="unavailable-badge"]'),
    ).toBeVisible();
    await expect(
      menuItem.locator('[data-testid="add-to-cart-btn"]'),
    ).toBeDisabled();

    // 4. 嘗試點擊不可用商品
    await menuItem.click();
    await expect(
      page.locator('[data-testid="unavailable-message"]'),
    ).toContainText("此商品暫時無法提供");
  });
});
