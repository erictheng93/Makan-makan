...
describe('Complete Ordering Flow - E2E Test', () => {
  ...
  afterAll(async () => {
    ...
    console.log(`   🔍 Scan to Menu: ${performanceMetrics.scanToMenu.toFixed(2)}ms`)
    console.log(`   🛒 Menu to Order: ${performanceMetrics.menuToOrder.toFixed(2)}ms`)
    console.log(`   🔔 Order to Kitchen: ${performanceMetrics.orderToKitchen.toFixed(2)}ms`)
    console.log(`   👨‍🍳 Kitchen to Complete: ${performanceMetrics.kitchenToComplete.toFixed(2)}ms`)
    console.log(`   💰 Complete to Payment: ${performanceMetrics.completeToPayment.toFixed(2)}ms`)
    console.log(`   ⏱️  Total Time: ${performanceMetrics.totalTime.toFixed(2)}ms`)
    console.log()
  })
  ...
  it('should complete full ordering flow from scan to payment', async () => {
    ...
    console.log(`   ✅ Menu loaded in ${performanceMetrics.scanToMenu.toFixed(2)}ms`)
    ...
    console.log(`   ✅ Kitchen notified in ${performanceMetrics.orderToKitchen.toFixed(2)}ms`)
    ...
    const customerNotificationPromise = helper.waitForWebSocketMessage(
      customerWs,
      (msg) => msg.type === 'order_status_update' && msg.data.status === 'confirmed',
      10000
    )
    ...
    const preparingNotificationPromise = helper.waitForWebSocketMessage(
      customerWs,
      (msg) => msg.type === 'order_status_update' && msg.data.status === 'preparing',
      10000
    )
    ...
    const readyNotificationPromise = helper.waitForWebSocketMessage(
      customerWs,
      (msg) => msg.type === 'order_status_update' && msg.data.status === 'ready',
      10000
    )
    ...
    performanceMetrics.kitchenToComplete = performance.now() - preparingStartTime
    console.log(`   ✅ Order ready in ${performanceMetrics.kitchenToComplete.toFixed(2)}ms`)
    ...
    const deliveredNotificationPromise = helper.waitForWebSocketMessage(
      customerWs,
      (msg) => msg.type === 'order_status_update' && msg.data.status === 'delivered',
      10000
    )
    ...
    console.log(`   ✅ Payment completed in ${performanceMetrics.completeToPayment.toFixed(2)}ms`)
  ...
  it('should handle errors gracefully throughout the flow', async () => {
    ...
    const invalidOrderResponse = await helper.apiRequest('POST', '/api/v1/orders', {
      body: {
        restaurantId: testRestaurant.id,
        tableId: testTable.id,
        items: [
  ...
  it('should meet performance standards', async () => {
    ...
    console.log(`   ✅ Order creation: ${orderTime.toFixed(2)}ms (budget: ${performanceBudget.orderCreation}ms)`)
    ...
    console.log(`   ✅ Menu fetch: ${menuTime.toFixed(2)}ms (budget: ${performanceBudget.menuFetch}ms)`)
