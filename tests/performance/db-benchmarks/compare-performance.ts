...
async function comparePerformance(config: RegressionConfig = DEFAULT_CONFIG) {
  ...
  if (!baseline) {
    ...
    console.log('✅ Baseline created. Run this script again to compare performance.')
    return { passed: true, warnings: 0, failures: 0 }
  }

  console.log(`📊 Baseline Version: ${baseline.version}`)
  console.log(`📅 Baseline Date: ${new Date(baseline.timestamp).toISOString()}`)
  console.log()
  ...
  for (const result of comparison) {
    ...
    console.log(`     Baseline: ${result.baselineAvg.toFixed(2)}ms → Current: ${result.currentAvg.toFixed(2)}ms`)
  ...
  if (failures > 0 || warnings > 0) {
    ...
    if (failures > 0) {
      ...
      comparison
        ...
        .forEach(r => {
          console.log(`     • ${r.queryName}: +${r.percentageChange.toFixed(1)}% (${r.baselineAvg.toFixed(2)}ms → ${r.currentAvg.toFixed(2)}ms)`)
        })
      console.log()
    }
    ...
    if (warnings > 0) {
      ...
      comparison
        .filter(r => r.percentageChange > config.warningThreshold && r.percentageChange <= config.failureThreshold)
        .forEach(r => {
          console.log(`     • ${r.queryName}: +${r.percentageChange.toFixed(1)}% (${r.baselineAvg.toFixed(2)}ms → ${r.currentAvg.toFixed(2)}ms)`)
        })
      console.log()
    ...
    console.log('     • Run `npm run db:analyze` to update query planner statistics')
    console.log()
  }

  // Fail if configured to do so
  if (!passed && config.failOnRegression) {
    console.log('❌ Failing due to performance regressions (--fail-on-regression)\n')
    process.exit(1)
  }
