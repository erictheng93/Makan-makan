...
describe('N+1 Query Detection', () => {
  ...
  describe('Order Listing Endpoint Simulation', () => {
    it('should detect N+1 problem in naive order listing', async () => {
      ...
      console.log(`   Has N+1 Problem: ${analysis.hasN1Problem ? '❌ YES' : '✅ NO'}`)
  ...
  describe('Batch Loading Detection', () => {
    it('should detect when batch loading would be beneficial', async () => {
      ...
      console.log(`   Total Queries: ${analysis.totalQueries}`)
      console.log(`   Should use batch loading: ${analysis.totalQueries > 5 ? '✅ YES' : '❌ NO'}`)
  ...
  describe('Complex Workflow Simulation', () => {
    it('should detect N+1 in complete order creation workflow', async () => {
      ...
      for (const itemId of menuItemIds) {
        await tester.measureQuery(`SELECT * FROM menu_items WHERE id = ? AND is_available = true`, [itemId])
      }
      ...
      for (const itemId of menuItemIds) {
        await tester.measureQuery(`INSERT INTO order_items (...) VALUES (...)`, [])
      }
  ...
  describe('Real-world Endpoint Simulations', () => {
    it('GET /api/v1/orders - should detect N+1 in naive implementation', async () => {
      ...
      const orders = await db.prepare(`SELECT * FROM orders WHERE restaurant_id = ? LIMIT 20`).bind(1).all()
      await tester.measureQuery(`SELECT * FROM orders WHERE restaurant_id = ? LIMIT 20`, [1])

      for (const order of orders.results) {
        await tester.measureQuery(`SELECT * FROM order_items WHERE order_id = ?`, [order.id])
        await tester.measureQuery(`SELECT * FROM tables WHERE id = ?`, [order.table_id])
      }
...
async function setupTestData(db: any): Promise<void> {
  ...
  await db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).bind(restaurantId, 'Test Restaurant', 'Casual', 'Restaurant', new Date().toISOString(), new Date().toISOString()).run()
  ...
  for (let i = 1; i <= 5; i++) {
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(i, restaurantId, `Category ${i}`, `Category ${i}`, i, 1, new Date().toISOString(), new Date().toISOString()).run()
  }
  ...
  for (let i = 1; i <= 30; i++) {
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      i, restaurantId, ((i % 5) + 1), `Item ${i}`, `Item ${i}`, 100, 1, i, new Date().toISOString(), new Date().toISOString()
    ).run()
  }
  ...
  for (let i = 1; i <= 10; i++) {
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(i, restaurantId, `T${i}`, 4, 1, new Date().toISOString(), new Date().toISOString()).run()
  }
  ...
  for (let i = 1; i <= 15; i++) {
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(
      i, restaurantId, ((i % 10) + 1), ['pending', 'confirmed', 'preparing'][i % 3], 200, new Date().toISOString(), new Date().toISOString()
    ).run()
    ...
    for (let j = 1; j <= 3; j++) {
      await db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).bind(i, ((i + j) % 30) + 1, j, 100, new Date().toISOString(), new Date().toISOString()).run()
    }
  }
  ...
  for (let i = 1; i <= 10; i++) {
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      i, restaurantId, `user${i}`, `user${i}@test.com`, `User ${i}`, 'hash', i % 5, 1, new Date().toISOString(), new Date().toISOString()
    ).run()
  }
}
