...
describe('Database Query Performance Benchmarks', () => {
  ...
  describe('Menu Item Queries', () => {
    ...
    it('should handle menu item search efficiently', async () => {
      ...
      const result = await tester.benchmarkQuery(query, [testRestaurantId, searchTerm, searchTerm], 10)
  ...
  describe('Order Queries', () => {
    ...
    it('should validate index usage for orders query', async () => {
      ...
      const result = await tester.validateIndexUsage(query, [testRestaurantId, 'pending'])
  ...
  describe('Concurrent Load Testing', () => {
    it('should handle 10 concurrent menu queries', async () => {
      ...
      const result = await tester.benchmarkConcurrent(query, [testRestaurantId], 10)

      console.log(`📊 Concurrent Load Test (10 queries):`)
      console.log(`   Total Time: ${result.totalTime.toFixed(2)}ms`)
      console.log(`   Avg Query Time: ${result.avgQueryTime.toFixed(2)}ms`)
      console.log(`   Success Rate: ${(result.successCount / result.concurrency * 100).toFixed(1)}%`)
    ...
    it('should handle 50 concurrent order queries', async () => {
      ...
      const result = await tester.benchmarkConcurrent(query, [testRestaurantId], 50)
  ...
  describe('Stress Testing', () => {
    it('should maintain performance under sustained load', async () => {
      ...
      expect(result.successfulQueries / result.totalQueries).toBeGreaterThan(0.95) // 95% success
      expect(result.avgTime).toBeLessThan(PERFORMANCE_THRESHOLD_MS)
    })
  })
})
...
async function setupTestData(db: any): Promise<number> {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    `).bind(
      ...
      new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(), // Last 30 days
      new Date().toISOString()
    ).run()
