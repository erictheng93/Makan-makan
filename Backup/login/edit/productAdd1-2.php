...
for($i=0;$i<$lenOriginArrayName;$i++){
	$key = array_search($OriginArrayID[$i], $ShopArray);
	
	if(strlen($key)>0 ){
		echo $OriginArrayID[$i];
		array_push($newArr, $OriginArrayName[$i]);
	}
}
//print_r($newArr);
?>
        ...
        <td>
			...
			<select name=category id=category>
				<option value='' selected>Select</option>
					...
					for($i=0;$i<$lenShopArray; $i++) {
					echo "<option value=$ShopArray[$i]>$newArr[$i]</option>";
					}
					?>
			</select>
		</td>
